...
public class Sweclips implements java.io.Serializable {
	...
	public static final int SMOD_ANNULAR = 32; /*
												 * count as penumbral for Lunar
												 */
	public static final int SMOD_ANNTOT = 64;
	public static final int SMOD_PARTIAL = 128;
	public static final int SMOD_ALL = (SMOD_TOTAL | SMOD_ANNULAR | SMOD_PARTIAL | SMOD_ANNTOT);
	...
	static final String infocmd2 = "        -eswe   swiss ephemeris\n"
	...
	public static final int ECL_LUN_PENUMBRAL = 1; /*
													 * eclipse types for hocal
													 * list
													 */
	...
	static final String zod_nam[] = { "ar", "ta", "ge", "cn", "le", "vi", "li", "sc", "sa", "cp", "aq", "pi" };

	/**
	 * This class is not be instantiated, it is supposed to be run via the main
	 * method.
	 */
	private Sweclips() {
	}
	...
	void main_start(String[] argv) {
		String saves;
		String s1, s2;
		StringBuffer serr = new StringBuffer(swed.AS_MAXCH), serr_save = new StringBuffer(swed.AS_MAXCH),
		...
		int iflag = 0;
		long eclflag;
		double geopos[] = new double[20], attr[] = new double[20], tret[] = new double[20];
		double a, b, c;
		...
		for (i = 0; i < argv.length; i++) {
			if (argv[i].startsWith("-head")) {
				...
				direction = -1;
				direction_flag = true;
				// JAVA: unklar, was geschehen soll, falls nur "-b" ohne weitere
				// Angabe steht.
		...
		if ((smod & (SMOD_LUNAR | SMOD_SOLAR | SMOD_OCCULT)) != 0) {
			if ((smod
					& SMOD_ALL) == 0) /*
										 ...
										 */
				smod |= SMOD_ALL;
			if ((smod & SMOD_TOTAL) != 0)
				search_flag |= SweConst.SE_ECL_TOTAL;
			if ((smod & SMOD_ANNULAR) != 0)
				search_flag |= SweConst.SE_ECL_ANNULAR | SweConst.SE_ECL_PENUMBRAL;
			if ((smod & SMOD_PARTIAL) != 0)
				search_flag |= SweConst.SE_ECL_PARTIAL;
			if ((smod & SMOD_ANNTOT) != 0)
				search_flag |= SweConst.SE_ECL_ANNULAR_TOTAL;
		}
		if (with_header) {
			for (i = 0; i < argv.length; i++) {
				do_printf(argv[i]);
		...
		if (sp.charAt(0) == 'j') { /* it's a day number */
			...
			try {
				...
				while (!Character.isDigit(sp.charAt(i)) && sp.charAt(i) != '-') {
					i++;
				}
		...
		for (ii = 0; ii < nstep; ii++) {
			...
			if ((smod & SMOD_OCCULT) != 0 && (smod & SMOD_LOCAL) != 0) {
				if ((eclflag = sw.swe_lun_occult_when_loc(t_ut, ipl, starname, whicheph, geopos, tret, attr,
						direction_flag ? -1 : 0, serr)) == SweConst.ERR) {
					do_printf(serr);
					System.exit(0);
				} else {
					boolean has_found = false;
			...
			if ((smod & SMOD_SOLAR) != 0 && (smod & SMOD_LOCAL) == 0) {
				...
				if ((eclflag & SweConst.SE_ECL_PARTIAL) == 0 && (eclflag & SweConst.SE_ECL_NONCENTRAL) == 0) {
					if ((eclflag = sw.swe_sol_eclipse_when_loc(t_ut - 10, whicheph, geopos, tret, attr, 0,
							serr)) == SweConst.ERR) {
						do_printf(serr);
						System.exit(0);
					}
			...
			if ((smod & SMOD_OCCULT) != 0 && (smod & SMOD_LOCAL) == 0) {
				/* * global search for eclipses */
				if ((eclflag = sw.swe_lun_occult_when_glob(t_ut, ipl, starname, whicheph, search_flag, tret,
						direction_flag ? -1 : 0, serr)) == SweConst.ERR) {
					do_printf(serr);
					System.exit(0);
				}
				...
				sout += "\t" + s1 + "\t" + s2;
				if ((eclflag & SweConst.SE_ECL_PARTIAL) == 0 && (eclflag & SweConst.SE_ECL_NONCENTRAL) == 0) {
					if ((eclflag = sw.swe_lun_occult_when_loc(t_ut - 10, ipl, starname, whicheph, geopos, tret, attr, 0,
							serr)) == SweConst.ERR) {
						do_printf(serr);
						System.exit(0);
					}
			...
			if ((smod & SMOD_RISE) != 0) {
				dtretx.val = tret[0];
				if (sw.swe_rise_trans(t_ut, ipl, sbstar, whicheph, SweConst.SE_CALC_RISE, geopos, 1013.25, 10., dtretx,
						serr) != SweConst.OK) {
					tret[0] = dtretx.val;
					do_printf(serr);
					System.exit(0);
				...
				tret[0] = dtretx.val;
				dtretx.val = tret[1];
				if (sw.swe_rise_trans(t_ut, ipl, sbstar, whicheph, SweConst.SE_CALC_SET, geopos, 1013.25, 10., dtretx,
						serr) != SweConst.OK) {
					tret[1] = dtretx.val;
					do_printf(serr);
					System.exit(0);
			...
			if ((smod & SMOD_METR) != 0) {
				dtretx.val = tret[0];
				if (sw.swe_rise_trans(t_ut, ipl, sbstar, whicheph, SweConst.SE_CALC_MTRANSIT, geopos, 1013.25, 10.,
				...
				tret[0] = dtretx.val;
				dtretx.val = tret[1];
				if (sw.swe_rise_trans(t_ut, ipl, sbstar, whicheph, SweConst.SE_CALC_ITRANSIT, geopos, 1013.25, 10.,
						dtretx, serr) != SweConst.OK) {
					tret[1] = dtretx.val;
					do_printf(serr);
					System.exit(0);
