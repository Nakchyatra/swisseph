...
public class Transits implements java.io.Serializable {
	...
	private void initHelpTexts() {
		infocmd2 = "\n" + "  Command line options:\n" + "    Main options:\n"
				...
				+ "                     4 Ushashashi              14 t0=Aldebaran, 15" + swed.ODEGREE_STRING
				+ " taurus\n" + "                     5 Krishnamurti            15 Hipparchos\n"
				+ "                     6 Djwhal Khul             16 Sassanian\n"
				+ "                     7 Sri Yukteshwar          17 Galactic center=0" + swed.ODEGREE_STRING
		...
		infoexamples = "\n" + "  Examples:\n\n" + "  Simple transits:\n"
				...
				+ swed.ODEGREE_STRING + " in sidereal zodiac) and Lahiri ayanamsh related to a\n"
	...
	public static void main(String argv[]) {
		...
		// sc.tzOffset = TimeZone.getDefault().getOffset(new
		// Date().getTime())/1000.0/3600.0/24.0; // Uses the CURRENT time zone
		// offset in all cases!
	...
	/**
	 * If you want to use this class in your own programs, you would just call
	 * this method. All output will go to stdout only (so far).
	 * 
	 * @param argv
	 *            array of Strings containing all parameters like on the command
	 *            line.
	...
	private int startCalculations(String[] argv, boolean withErrMsg) {
		...
		while (a.count > 0 || (a.enddate != null && !a.back && a.tjde1 < a.jdET2)
			...
			// None of the calculations ended normally, so we can stop
			// calculation here:
	...
	TransitResult calcNeighbouringTransits(TransitArguments a) {
		...
		boolean[] doCalc = new boolean[] { true, a.v.duplicateTransitPoints && a.v.rollover,
		...
		for (int i = 0; i < tValues.length; i++) {
			if (doCalc[i]) {
				try {
					// Skip calculation, if:
					// - ONLY the ORDER of the planets is different from
					// previous
					// calculations,
					// - AND:
					skip = a.v.tcIndex >= a.idxDuplicates &&
							// we don't care about a relative transit value of 0
							// in all cases:
							(tValues[i] == 0 ||
									// we don't care about a lon calculation
									// with 180 degrees as well
									(a.to.idxOffset == 0 && tValues[i] == 180));
					if (skip) {
						// check for isNaN, otherwise the final check for errCnt
						// may
						// not recognize, it didn't find a transit point. We can
						// "probably" remove the assignment all together, as
						// skipping
						// just means skipping, so one shouldn't care...
						// The problem with it arises on the final check for
						// errCnt,
						// which does not take in account the non-error case,
						// where
					...
					if ((swe.getType() & SwissephException.BEYOND_USER_TIME_LIMIT) != 0) {
						// beyond user time limit is normally(?) due to internal
						// limits in
						...
						// Hack, SwissEph does not yet return meaningful types
						// in all cases...
						...
						a.v.outOfTimeRange = true;
					} else if (swe.toString().indexOf(" outside ") > 0 && swe.toString().indexOf(" range ") > 0
		...
		if ((errCnt == 1 && !a.v.duplicateTransitPoints) || (errCnt == 3 && a.v.duplicateTransitPoints)) { // No
																											// more
																											// transit
																											// points
																											// possible
	...
	double calcTransit(TransitCalculator tc, double val, double jdStart, double jdEnd, boolean back)
		...
		for (int n1 = 0; n1 < planets1.length(); n1++) {
			...
			for (int n2 = 0; n2 < planets2.length(); n2++) {
				...
				if (!pl1.equals(pl2)) {
					...
					// Add only, if this planetary combination is not yet
					// inserted
					// (idx < 0 or the position of the idx in the String is
					// odd...)
					if ((idx < 0 || (idx >= 0 && (idx & 0x00000001) == 1))
							&& (idxR < 0 || (idxR >= 0 && (idxR & 0x00000001) == 1))) {
						// pls0 does not contain this planet combination in ANY
						// order...

						// Create planet combinations that most probably don't
						// start with
						// "02", "03" or similar, otherwise we might get "Out of
						// time
						// range", if the longitude value exceeds the maximum
						// distance
						// between Sun and Mercury, Sun and Venus or Venus and
						// Mercury.
						// Also moon should rather be calculated later, as moon
						// transits
						// require more iterations than others.
						if (comb.indexOf('0') >= 0 || comb.indexOf('1') >= 0 || "23".equals(comb)
								|| "32".equals(comb)) {
							pls0 += comb;
						} else {
							pls0 = comb + pls0;
		...
		if (s != null && s.length() > 0) {
			...
			for (int i = 0; i < num.length(); i++) {
				...
				if (!Character.isDigit(ch)) {
					// Has to be a decimal point, an exponent or a sign after an
					// exponent:
					if (ch == numIFracSeparator.charAt(0)) {
						...
						if (i != 0 || (lastChar == 'E' && dblString.length() == 0)) {
							// invalidValue(tv,idx);
							return invalidValue(par, s.indexOf('@'));
						}
	...
	String printFormatted(TransitArguments a, TransitResult tr) {
		...
		for (int n = 0; n < a.outputFormat.length(); n++) {
			...
			case (int) 'n': // output planet name(s):
				int len = 9;
				if (a.pls1.indexOf("g") > 0 || a.pls1.indexOf("c") > 0) {
					len += 4;
				}
				...
				if (tr.pl2 >= 0) {
					len += 12;
					if (a.pls2.indexOf("g") > 0 || a.pls2.indexOf("c") > 0) {
						len += 4;
					}
			...
			case (int) 'd': // output date, read optional decimal places:
				...
				for (int i = n + 1; i < a.outputFormat.length(); i++) {
					if (Character.isDigit(a.outputFormat.charAt(i))) {
						cnt = cnt * 10 + Character.getNumericValue(a.outputFormat.charAt(i));
						n++;
					} else {
						break;
					}
			...
			case (int) 'p': // read optional decimal places:
				...
				pflags &= ~(SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE
	...
	double parseHour(String s) {
		if (s.equals("now")) {
			...
			return cal.get(java.util.Calendar.HOUR_OF_DAY) + cal.get(java.util.Calendar.MINUTE) / 60.
	...
	int invalidValue(String arg, int idx, String locale) {
		if (arg.length() == idx) {
			...
			System.err.println("\nInvalid argument (" + arg.substring(idx) + ") to parameter '" + arg.substring(0, idx)
					+ "'.\n" + (locale != null
							? "Maybe the locale (" + Nlocale + ") requires a different format.\n" + "Also t" : "\nT")
	...
	String invalidArgument(String arg, int idx, String locale) {
		if (arg.length() == idx) {
			...
			return "Invalid argument (" + arg.substring(idx) + ") to parameter '" + arg.substring(0, idx) + "'.\n"
					+ (locale != null
							? "Maybe the " + "locale (" + Nlocale + ") requires a different format.\n" + "Also t"
							: "\nT")
					+ "ry option -h for a syntax description.";
	...
	/**
	 * Returns date + time in localized form from a JDET in a SweDate object
	 */
	String jdToDate(SweDate sdET, boolean printUT, boolean printLocalTime, int decPlaces) {
		...
		SweDate sx = new SweDate(sdET.getJulDay() - (printUT ? sdET.getDeltaT() : 0) + (printLocalTime ? tzOffset : 0)
		...
		if (sx.getYear() == 0) {
			...
			s = s.substring(0, idxy + 2) + s.substring(idxy, idxy + 1) + s.substring(idxy + 3);
	...
	String printJD(double jd, boolean printUT, boolean printLocalTime, int decPlaces) {
		...
		SweDate sx = new SweDate(jd - (printUT ? SweDate.getDeltaT(jd) : 0) + (printLocalTime ? tzOffset : 0)
	...
	String doubleToDMS(double d) {
		...
		return ((deg < 10 ? " " : "") + nnof.format(deg) + swed.ODEGREE_STRING + "" + (min < 10 ? nnof.format(0) : "")
				+ nnof.format(min) + "'" + (sec < 10 ? nnof.format(0) : "") + nnof.format(sec) + "\"");
	}
	...
	int letter_to_ipl(char letter) {
		...
		if (Character.isDigit(letter)) {
			try {
				return Integer.parseInt("" + letter) + SweConst.SE_SUN; // Allows
																		// for
																		// all
																		// kind
																		// of
	...
	TransitArguments parseArgs(String argv[]) throws IllegalArgumentException {
		...
		for (int i = 0; i < argv.length; i++) {
			if (argv[i].equals("-hdate")) {
				...
				if (argv[i].equals("-h") || argv[i].equals("--help") || argv[i].equals("-?") || argv[i].equals("/?")) {
				...
				sw.swe_close();
				return (null);
			} else if (argv[i].equals("-head") || argv[i].equals("-q")) {
				a.withHeader = false;
			} else if (argv[i].equals("-r")) {
				a.back = true;
			} else if (argv[i].startsWith("-lon") || argv[i].startsWith("-lat") || argv[i].startsWith("-dist")
					|| argv[i].startsWith("+lon") || argv[i].startsWith("+lat") || argv[i].startsWith("+dist")
					|| argv[i].startsWith("-partile") || argv[i].startsWith("-nonpartile")
				...
				if (argv[i].substring(1).startsWith("partile") || argv[i].startsWith("-nonpartile")) {
			...
			} else if (argv[i].equals("-hel") || argv[i].equals("-helio")) {
				a.helio = true;
				a.iflag |= SweConst.SEFLG_HELCTR;
				a.iflag |= SweConst.SEFLG_NOABERR;
				a.iflag |= SweConst.SEFLG_NOGDEFL;
			} else if (argv[i].equals("-equ") || argv[i].equals("-equatorial")) {
				a.equatorial = true;
			...
			} else if (argv[i].startsWith("-ut") || argv[i].startsWith("-et") || argv[i].startsWith("-lt")
					|| argv[i].startsWith("-t")) {
				...
				if (argv[i].length() > len) {
					...
					if (!a.sBeginhour.equals("now") && !a.sBeginhour.matches("[0-9][0-9]*:[0-9][0-9]*:[0-9][0-9]*")) {
				...
				a.calcSpeed = true;
			} else if (argv[i].startsWith("-b") || argv[i].startsWith("-j")) {
				if (argv[i].length() < 3) {
				...
				if (argv[i].length() < 3) {
					...
					boolean wob = (argv[i].startsWith("-J"));
					a.enddate = (wob ? "j" : "") + argv[i].substring(2);
					a.duplicateTransitPoints = true; // Well, only if
														// varyingTransitPoint
														// == true;
				}
				...
				if (idx24 >= 0) {
					force24hSystem = true;
					locale = locale.substring(0, idx24) + locale.substring(idx24 + 2);
				}
				...
				if (idx24 >= 0) {
					force24hSystem = true;
					Nlocale = Nlocale.substring(0, idx24) + Nlocale.substring(idx24 + 2);
				}
				Dlocale = Nlocale;
				...
				if (idx24 >= 0) {
					force24hSystem = true;
					Dlocale = Dlocale.substring(0, idx24) + Dlocale.substring(idx24 + 2);
				}
			...
			} else if (argv[i].startsWith("-rf") || argv[i].startsWith("-random_factors")) {
		...
		if (randomFactors != null) {
			...
			// TODO: should check for match against numIFracSeparator instead of
			// replacing numIFracSeparator with "."
	...
	TransitArguments initArgs(TransitArguments a) {
		...
		// Arabic numbers don't get used by default, see
		// http://bugs.sun.com/view_bug.do?bug_id=4336841
		...
		// ar_TN: 0
		// ar_YE: inherited
		boolean arabicNumbers = (nlocale.startsWith("ar") && !"ar_DZ".equals(nlocale) && !"ar_MA".equals(nlocale)
		...
		secondsIdx = el.getPatternLastIdx(dof.toPattern(), "s", dof); // No
																		// input
																		// with
																		// parts
																		// of
		...
		if (a.to == null) {
			throw new IllegalArgumentException("Specify a longitude, latitude or distance value to be "
		...
		// varyingTransitPoints is only meaningful, when we have changing
		// values:
			...
			try {
				...
				for (int k = 0; k < a.topoS.length(); k++) {
					if (!Character.isDigit(a.topoS.charAt(k))) {
						...
						if (ch != '-' && ch != '+') {
							if (!numIFracSeparator.equals("" + ch)) { // Well,
																		// can
																		// be a
																		// string,
																		// probably????
				...
				if (cnt != 2) {
					throw new IllegalArgumentException(invalidArgument("-topo" + a.topoS, 5, Nlocale));
				}
				// Read field values:
				String ts = a.topoS;
		...
		if (a.sde2 != null && ((!a.back && a.sde1.getJulDay() > a.sde2.getJulDay())
		...
		if (a.pls1 != null) {
			for (int n = 0; n < a.pls1.length(); n++) {
				...
				if (ipl_check == -1) {
					throw new IllegalArgumentException(
							"Unsupported planet " + (Character.isDigit(p.charAt(n)) ? "number" : "character") + ": '"
									+ p + "'.\nCheck for valid planets with the '-h' option.");
				} else if (ipl_check == -2) {
					throw new IllegalArgumentException("Syntax error on -p option: check for balanced /.../ "
		...
		if (a.mp1 || a.mp2) {
			if (a.outputFormat.indexOf("n") < 0 && (!a.outputFormatIsSet || appendOutputFormat)) {
		...
		if (a.objects != null) {
			...
			while (tk.hasMoreTokens()) {
				...
				if (!isValidHouseObject(obj)) {
					throw new IllegalArgumentException("Unknown house or ascendent object '" + obj + "' in '-o"
			...
			if (a.hsys != 'B' && a.hsys != 'C' && a.hsys != 'E' &&
					/* a.hsys != 'G' && */ a.hsys != 'H' && a.hsys != 'K' && a.hsys != 'M' && a.hsys != 'O'
					&& a.hsys != 'P' && a.hsys != 'R' && a.hsys != 'T' && a.hsys != 'U' && a.hsys != 'V'
					&& a.hsys != 'W' && a.hsys != 'X') {
				throw new IllegalArgumentException("Invalid house system '-hsys" + a.hsys + "'.\n"
						+ "Check for valid house systems with the '-h' option.");
			}
		}
		...
		if (a.equatorial && (a.transitValString.startsWith("-dist") || a.transitValString.startsWith("+dist"))) {
			a.equatorial = false;
			a.iflag &= ~SweConst.SEFLG_EQUATORIAL;
		}
	...
	boolean isValidHouseObject(String obj) {
		...
		return ("asc".equals(obj) || "mc".equals(obj) || "armc".equals(obj) || "vertex".equals(obj)
				|| "equasc".equals(obj) || "coasc1".equals(obj) || "coasc2".equals(obj) || "polasc".equals(obj));
	}
	...
	void writeHeader(TransitArguments a) {
		...
		if (a.sidmode >= 0) {
			...
			if (a.enddate == null) {
				System.out.print(":         " + (a.mp1 ? " " : "") + doubleToDMS(sw.swe_get_ayanamsa(a.tjde1)));
			} else {
				System.out.println(
						"\n on starting date: " + (a.mp1 ? " " : "") + doubleToDMS(sw.swe_get_ayanamsa(a.tjde1)));
				System.out.println(
						" on end date:      " + (a.mp1 ? " " : "") + doubleToDMS(sw.swe_get_ayanamsa(a.tjde2)));
			}
		}
		System.out.println();
		if (a.isLt) {
		...
		System.out.println("Starting date:     " + (a.mp1 ? " " : "") + jdToDate(a.sde1, a.isUt, a.isLt, 0)
				+ (a.isLt ? " LT" : (a.isUT ? " UT" : " ET")));
		if (a.enddate != null) {
			System.out.println("End date:          " + (a.mp1 ? " " : "") + jdToDate(a.sde2, a.isUT, a.isLT, 0)
					+ (a.isLT ? " LT" : (a.isUT ? " UT" : " ET")));
		}
		if (!a.yogaTransit && !a.partileStart && !a.partileEnd) {
			if (a.pls1 != null) {
				System.out.println("Transiting planet" + (a.mp1 ? "s" : "") + ": "
						+ limitLineLength(getPlanetNames(a.pls1, " or "), 70, "                    "));
			} else if (a.objects != null) {
				System.out.println("Transiting object" + (a.mpo ? "s" : "") + ": "
						+ limitLineLength(a.objects, 70, "                    "));
			}
		}
		System.out.print("Reference point:   " + (a.mp1 ? " " : ""));
		if (a.calcSpeed) {
			ObjFormatter dblf = new ObjFormatter(a.to.values, swed.ODEGREE_STRING + "/day", nnof);
			if (a.pls2 != null) {
				if (a.yogaTransit) {
					line = "combined " + (a.helio ? "heliocentric " : "") + " speed of "
							+ group(dblf, a.to.values.length, " or ") + " of " + (a.mp1 ? "the planets " : "planet ")
							+ getPlanetNames(a.pls1, " or ") + (a.mp1 ? " with " : " and ") + (a.mp2 ? "any of " : "")
							+ getPlanetNames(a.pls2, ", ");
				} else {
					line = (a.helio ? "heliocentric " : "") + "speed of " + group(dblf, a.to.values.length, " or ")
							+ " "
				...
				line += " in "
						+ (a.equatorial ? (a.to.idxOffset == 0 ? "right ascension"
								: "declination")
								: (a.to.idxOffset == 3 ? "longitudinal"
										: (a.to.idxOffset == 4 ? "latitudinal" : "distance")))
						+ " direction";
				if (a.varyingTransitPoints) {
					line += " with varying transit points";
				}
				System.out.println(limitLineLength(line, 70, "                   " + (a.mp1 ? " " : "")));
			} else {
				line = group(dblf, a.to.values.length, " or ") + " in " + (a.helio ? "heliocentric " : "")
						+ (a.equatorial ? (a.to.idxOffset == 0 ? "right ascension" : "declination")
								: (a.to.idxOffset == 3 ? "longitudinal"
										: (a.to.idxOffset == 4 ? "latitudinal" : "distance")))
			...
			if ((a.iflag & SweConst.SEFLG_TOPOCTR) != 0) {
				System.out.println("Topographic pos.:  " + (a.mp1 ? " " : "") + doubleToDMS(SMath.abs(a.top_long))
						+ (a.top_long < 0 ? "S" : "N") + "/" + doubleToDMS(SMath.abs(a.top_lat))
						+ (a.top_lat < 0 ? "W" : "E") + "/" + nnof.format(a.top_elev) + "m");
			}
		} else { // Transit over a lon / lat / dist position:
			ObjFormatter dblf = new ObjFormatter(a.to.values, (a.to.idxOffset == 2 ? " AU" : "" + swed.ODEGREE_STRING),
					nnof);
			if (a.pls2 != null) {
				if (a.yogaTransit) {
					line = "combined " + (a.helio ? "heliocentric " : "")
							+ (a.equatorial ? (a.to.idxOffset == 0 ? "right ascension" : "declination")
									: (a.to.idxOffset == 0 ? "longitudinal"
											: (a.to.idxOffset == 1 ? "latitudinal" : "distance")))
							+ " positions of " + (a.mp1 ? "the planets " : "planet ") + getPlanetNames(a.pls1, " or ")
							+ (a.mp1 ? " with " : " and ") + (a.mp2 ? "any of " : "") + getPlanetNames(a.pls2, ", ")
							+ " reach " + group(dblf, a.to.values.length, " or ")
							+ (a.sidmode >= 0 ? " in the sidereal zodiac" : "");
				} else if (a.partileStart) {
					if (a.partileEnd) {
						line = "starting and ending partile aspect of " + getPlanetNames(a.pls1, ", ") + " with "
								+ getPlanetNames(a.pls2, ", ");
					} else {
						line = "starting partile aspect of " + getPlanetNames(a.pls1, ", ") + " with "
								+ getPlanetNames(a.pls2, ", ");
					}
				} else if (a.partileEnd) {
					line = "ending partile aspect of " + getPlanetNames(a.pls1, ", ") + " with "
							+ getPlanetNames(a.pls2, ", ");
				} else {
				...
				line = group(dblf, a.to.values.length, " or ") + " " + (a.to.idxOffset == 2 ? "of " : "")
						+ (a.helio ? "heliocentric " : "")
						+ (a.equatorial ? (a.to.idxOffset == 0 ? "right ascension" : "declination")
								: (a.to.idxOffset == 0 ? "longitude" : (a.to.idxOffset == 1 ? "latitude" : "distance")))
						+ (a.sidmode >= 0 ? " in the sidereal zodiac" : "");
			}
			if (a.varyingTransitPoints) {
				line += " with varying transit points";
	...
	TransitCalculator[] initCalculators(TransitArguments a) throws IllegalArgumentException {
		// Init all required TransitCalculators:
		TransitCalculator tcs[] = null;
		a.idxDuplicates = Integer.MAX_VALUE;
		if (a.pls2 != null) { // relative or yoga or partile transits between
								// two planets
			String planetCombinations = getPlanetCombinations(a.pls1, a.pls2);
			if ("@".equals(planetCombinations)) {
				throw new IllegalArgumentException("Planets for relative, yoga or partile transits have to be "
						+ "different!\nUse the -h option for more information.");
			}
			...
			tcs = new TransitCalculator[tk.countTokens()]; // Nur dann
															// "Objekte", wenn
															// keine "Planeten"
															// angegeben sind?
			...
			if (index >= 0) {
				a.pls1 = a.pls1.substring(0, index) + a.pls1.substring(index + 1) + 'm';
			}
			...
			for (index = 0; index < a.pls1.length(); index++) {
				...
				if (ch == '1' && !ast) {
					a.pls1 = a.pls1.substring(0, index) + a.pls1.substring(index + 1) + '1';
					break;
				}
			}
			...
			for (int t = 0, ppos = 0; t < tcs.length; t++, ppos++) {
				...
				try {
					if (randomCount == 0) {
						...
						tcs[t] = new TCPlanet(sw, a.pl1, a.iflag, a.to.values[0].doubleValue(), randomCount,
								randomFactor);
					}
				} catch (SwissephException se) {
					if (se.getType() == SwissephException.FILE_NOT_FOUND) {
	...
	TransitResult calcNextTransit(TransitArguments a, TransitCalculator[] tcs) {
		...
		for (a.v.tcIndex = 0; a.v.tcIndex < tcs.length; a.v.tcIndex++) {
			...
			for (int tvn = 0; tvn < a.to.values.length; tvn++) {
				...
				// Select closest transit point of all current transit
				// calculations
				if ((a.back && tr2.jdET > tr.jdET) || (!a.back && tr2.jdET < tr.jdET)) {
...
class TransitValues implements java.io.Serializable {
	...
	boolean varyingTransitPoints = false; // e.g. -lon60+10 with -n, -N, -b -b,
											// -b -B
...
class TransitOffsets implements java.io.Serializable {
...
class ObjFormatter implements java.io.Serializable {
	Object[] arr = null;
	String postfix = "";
	NumberFormat nnof = null;
...
class TransitArguments implements java.io.Serializable {
	// CH-Zuerich:
...
class TransitResult implements java.io.Serializable {
