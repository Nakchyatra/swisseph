 ...
 * (probably) all possible options. See parameter -h for infos about all the
 * parameter switches.
 */
public class Swetest implements java.io.Serializable {
	...
	static final String infocmd2 = "     output format:\n"
	...
	static final String infocmd3 = "" + "        -geopos[long,lat,elev]	\n"
			...
			+ "        -nonut            no nutation \n" + "";
	static final String infocmd4 = "" + "        -speed            calculate high precision speed \n"
	...
	static final String infocmd5 = "" + "        -lunecl lunar eclipse\n" + "                output 1st line:\n"
			+ "                  eclipse date,\n" + "                  time of maximum (UT),\n"
			+ "		  Julian day number (6-digit fraction) of maximum\n" + "                output 2nd line:\n"
	...
	static final String infocmd6 = "" + "     specifications for risings and settings:\n"
			...
			+ "     backward search:\n" + "        -bwd\n";
	/*
	 * characters still available: bcgijklruvxy
	 */
	static final String infoplan = "\n" + "  Planet selection letters:\n" + "     planetary lists:\n"
			...
			+ "          \n";
	/*
	 * characters still available cgjv
	 */
	static final String infoform = "\n" + "  Output format SEQ letters:\n"
			...
			+ "	= magnitude\n" + "";
	static final String infoform2 = "" + "        v (reserved)\n" + "        V (reserved)\n" + "	";
	...
	static final String PLSEL_P = "0123456789mtA" + "BCcg" + "DEFGHI";
	...
	String se_pname;
	static final String[] zod_nam = new String[] { "ar", "ta", "ge", "cn", "le", "vi", "li", "sc", "sa", "cp", "aq",
			"pi" };
	...
	private double x[] = new double[6], x2[] = new double[6], xequ[] = new double[6], xcart[] = new double[6],
			xcartq[] = new double[6], xobl[] = new double[6], xaz[] = new double[6], xt[] = new double[6], hpos, hpos2,
			armc, xsv[] = new double[6];
	...
	private double attr[] = new double[20], tret[] = new double[20], datm[] = new double[4], dobs[] = new double[6];
	private int iflag = 0, iflag2; /* external flag: helio, geo... */
	private static final String hs_nam[] = { "undef", "Ascendant", "MC", "ARMC", "Vertex" };
	...
	/**
	 * This class is not to be instantiated, it is to be run via the main
	 * method.
	 */
	private Swetest() {
	}
	...
	private int main_start(String[] argv) {
		...
		for (i = 0; i < argv.length; i++) {
			if (argv[i].startsWith("-DSE_EPHE_PATH")) {
				...
				if (argv[i].length() > 3) {
					...
					// Extension to allow fraction of seconds; not
					// internationalized.
					double frac = 0;
					if (s1.indexOf('.') != s1.lastIndexOf('.')) {
						// HH:MM:SS.FFF
						frac = Double.valueOf(s1.substring(s1.lastIndexOf('.'))).doubleValue();
				...
				/*
				 * if (sid_mode > 0) sw.swe_set_sid_mode(sid_mode, 0, 0);
				 */
				...
				have_geopos = true;
			} else if (argv[i].startsWith("-topo") || argv[i].startsWith("-geopos")) {
				int plen = (argv[i].startsWith("-topo") ? 5 : 7);
				if (plen == 5) {
					iflag |= SweConst.SEFLG_TOPOCTR;
				}
				// sscanf(argv[i] + 5, "%lf,%lf,%lf", &top_long, &top_lat,
				// &top_elev);
				...
				have_geopos = true;
				if (argv[i].indexOf("A") >= 0 || argv[i].indexOf("V") >= 0)
					hel_using_AV = true;
			} else if (argv[i].startsWith("-at")) {
				// sscanf(argv[i]+3, "%lf,%lf,%lf,%lf", &(datm[0]), &(datm[1]),
				// &(datm[2]), &(datm[3]));
				...
				// sscanf(argv[i]+4, "%lf,%lf,%lf,%lf,%lf,%lf", &(dobs[0]),
				// &(dobs[1]), &(dobs[2]), &(dobs[3]), &(dobs[4]), &(dobs[5]));
				StringTokenizer d = new StringTokenizer(argv[i].substring(4), ",");
				try {
					dobs[0] = Double.parseDouble(d.nextToken());
					dobs[1] = Double.parseDouble(d.nextToken());
				...
				if (argv[i].length() > 2) {
					...
					// We need at least one char in Java, to be able to use
					// existing code
					plsel = " ";
				}
		...
		if (special_event == SP_OCCULTATION || special_event == SP_RISE_SET || special_event == SP_MERIDIAN_TRANSIT
				|| special_event == SP_HELIACAL) {
		...
		if (do_set_astro_models)
			sl.swe_set_astro_models(
					astro_models); /* secret test feature for dieter */
		...
		while (true) {
			...
			if (sp.length() > 0
					&& sp.charAt(0) == 'j') { /* it's a day number */
				if (sp.indexOf(',') >= 0)
				...
				sd.setJulDay(tjd);
				sd.setCalendarType(gregflag, SweDate.SE_KEEP_JD); // Keep
																	// JulDay!
				jyear = sd.getYear();
				jmon = sd.getMonth();
				jday = sd.getDay();
				...
				// if (sscanf (sp, "%d%*c%d%*c%d", &jday,&jmon,&jyear) < 1)
				// exit(1);
				jday = jmon = jyear = 0;
				boolean neg = false;
				i = 0;
				try {
					...
					while (i < sp.length() && !Character.isDigit(sp.charAt(i)) && sp.charAt(i) != '-') {
						i++;
					}
					neg = sp.charAt(i) == '-';
					if (neg) {
				...
				sd.setCalendarType(gregflag, SweDate.SE_KEEP_DATE); // Keep
																	// Date!
				tjd = sd.getJulDay();
				tjd += thour / 24.0;
				sd.setJulDay(tjd);
			...
			for (t = tjd, istep = 1; istep <= nstep; t += tstep, istep++) {
				...
				if (with_header) {
					...
					if (have_geopos) {
						System.out.print(String.format(Locale.US, "\ngeo. long %f, lat %f, alt %f", geopos[0],
								geopos[1], geopos[2]));
					}
					if (iflag_f >= 0) {
						iflag = iflag_f;
					...
					if (do_houses) {
						...
						if (!universal_time) {
							do_houses = false;
							System.out.print("option -house requires option -ut for " + "Universal Time\n");
						} else {
							s1 = dms(top_long, round_flag);
							s2 = dms(top_lat, round_flag);
							System.out.println(
									"Houses system " + (char) ihsy + " (" + shsy + ") for long=" + s1 + ", lat=" + s2);
						}
					}
				}
				...
				for (psp = plsel, p = 0; p < plsel.length(); p++) {
					if (psp.charAt(p) == ' ' || // These deals with plsel == ''
												// in C
							psp.charAt(p) == 'e') {
						continue;
					}
					...
					if (psp.charAt(p) == 'f') {
						...
						try {
							// Well, no: sastno may be appended a letter like
							// '10s'!!!
					...
					if ((iflag & SweConst.SEFLG_HELCTR) != 0) {
						if (ipl == SweConst.SE_SUN || ipl == SweConst.SE_MEAN_NODE || ipl == SweConst.SE_TRUE_NODE
					...
					if (ipl == SweConst.SE_FIXSTAR) {
						...
						/* phase, magnitude, etc. */
						if (iflgret != SweConst.ERR && (fmt.indexOf("+") >= 0 || fmt.indexOf("-") >= 0
								|| fmt.indexOf("*") >= 0 || fmt.indexOf("/") >= 0 || fmt.indexOf("=") >= 0)) {
					...
					if (iflgret < 0) {
						if (!serr.toString().equals(serr_save.toString())
								&& (ipl == SweConst.SE_SUN || ipl == SweConst.SE_MOON || ipl == SweConst.SE_MEAN_NODE
					...
					if (fmt.indexOf("a") >= 0 || fmt.indexOf("A") >= 0 || fmt.indexOf("D") >= 0 || fmt.indexOf("d") >= 0
							|| fmt.indexOf("Q") >= 0) {
					...
					if (fmt.indexOf("I") >= 0 || fmt.indexOf("i") >= 0 || fmt.indexOf("H") >= 0 || fmt.indexOf("h") >= 0
						 ...
						 * of 1013.25 mbar is assumed at 0 m above sea level. If
						 * the altitude of the observer is given (in geopos[2])
				...
				if (do_houses) {
					...
					if (iflgret < 0) {
						...
						for (ipl = 1; ipl < iofs + 4; ipl++) {
							...
							// if (strpbrk(fmt, "aADdQ") != NULL)
							if (fmt.indexOf('a') >= 0 || fmt.indexOf('A') >= 0 || fmt.indexOf('D') >= 0
									|| fmt.indexOf('d') >= 0 || fmt.indexOf('Q') >= 0) {
	...
	/*
	 * The string fmt contains a sequence of format specifiers; each character
	 * in fmt creates a column, the columns are sparated by the gap string.
	 */
	int print_line(int mode) {
		...
		for (sp = fmt; c < fmt.length(); c++) {
			...
			case 'L':
				...
				if (p >= psp.length() || (p < psp.length() && psp.charAt(p) != 'q' && psp.charAt(
						p) != 'y')) { /* != (delta t or time equation) */
					System.out.print(dms(x[0], round_flag));
					break;
				}
				// Fall through else...
			case 'l':
				if (is_label && sp.charAt(c) == 'l') {
					System.out.println("long");
					break;
				}
			...
			case 'S':
			case 's':
				if (fmt.indexOf("X") >= 0 || fmt.indexOf("U") >= 0 || fmt.indexOf("x") >= 0 || fmt.indexOf("u") >= 0
						|| (sp.length() > c + 1 && (sp.charAt(c + 1) == 'S' || sp.charAt(c + 1) == 's'))) {
					int c2 = 0;
					for (sp2 = fmt; c2 < fmt.length(); c2++) {
						...
						case 'A': /* speed! */
							if (is_label) {
								System.out.println("RA/day");
								break;
							}
							System.out.print(dms(xequ[3] / 15, round_flag | SweConst.SEFLG_EQUATORIAL));
							break;
						case 'a': /* speed! */
							if (is_label) {
								System.out.println("RA/day");
			...
			case 'A': /* right ascension */
				...
				System.out.print(dms(xequ[0] / 15, round_flag | SweConst.SEFLG_EQUATORIAL));
				break;
			case 'a': /* right ascension */
				if (is_label) {
					System.out.println("RA");
			...
			case 'n': {
				...
				int imeth = (Character.isLowerCase(sp.charAt(c))) ? SweConst.SE_NODBIT_MEAN : SweConst.SE_NODBIT_OSCU;
				iflgret = sw.swe_nod_aps(te, ipl, iflag, imeth, xasc, xdsc, null, null, serr);
				if (iflgret >= 0 && (ipl <= SweConst.SE_NEPTUNE || sp.charAt(c) == 'N')) {
					if (is_label) {
			...
			case 'f':
				if (!is_house) {
					...
					int imeth = (Character.isLowerCase(sp.charAt(c))) ? SweConst.SE_NODBIT_MEAN
							: SweConst.SE_NODBIT_OSCU;
					iflgret = sw.swe_nod_aps(te, ipl, iflag, imeth, null, null, xper, xaph, serr);
					if (iflgret >= 0 && (ipl <= SweConst.SE_NEPTUNE || sp.charAt(c) == 'F')) {
						if (is_label) {
			...
			case 'v': {
				...
				final int hexa[] = new int[] { 1, 43, 14, 34, 9, 5, 26, 11, 10, 58, 38, 54, 61, 60, 41, 19, 13, 49, 30,
						55, 37, 63, 22, 36, 25, 17, 21, 51, 42, 3, 27, 24, 2, 23, 8, 20, 16, 35, 45, 12, 15, 52, 39, 53,
	...
	private String dms_label_return_dms(int sgn, String s, int iflg) {
		...
		if ((iflg & BIT_LZEROES) != 0) {
			// while ((sp = strchr(s+2, ' ')) != NULL) *sp = '0'; // Replaces
			// all spaces by '0'es
	...
	private int letter_to_ipl(int letter) {
		...
		switch (letter) {
		case 'm':
			return SweConst.SE_MEAN_NODE;
		case 'c':
			return SweConst.SE_INTP_APOG;
		case 'g':
			return SweConst.SE_INTP_PERG;
		case 'n':
		case 'o':
			return SweConst.SE_ECL_NUT;
		case 't':
			return SweConst.SE_TRUE_NODE;
		case 'f':
			return SweConst.SE_FIXSTAR;
		case 'w':
			return SweConst.SE_WALDEMATH;
	...
	private int call_rise_set(double t_ut, int ipl, StringBuffer star, int whicheph, int special_mode, double[] geopos,
			StringBuffer serr) {
		int ii;
		int rsmi = 0;
		...
		DblObj tret_out = new DblObj(); // used as a temporary output double
										// value
		...
		for (ii = 0; ii < nstep; ii++, t_ut = tret[1] + 0.1) {
			sout = "";
			/*
			 * swetest -rise calculate and print rising and setting
			 */
			if (special_event == SP_RISE_SET) {
				/* rising */
				rsmi = SweConst.SE_CALC_RISE;
				if (norefrac)
					rsmi |= SweConst.SE_BIT_NO_REFRACTION;
				if (disccenter)
					rsmi |= SweConst.SE_BIT_DISC_CENTER;
				if (discbottom)
					rsmi |= SweConst.SE_BIT_DISC_BOTTOM;
				tret_out.val = tret[0];
				if (sw.swe_rise_trans(t_ut, ipl, star, whicheph, rsmi, geopos, 1013.25, 10, tret_out,
						serr) != SweConst.OK) {
					do_printf(serr);
					System.exit(0);
				}
				...
				if (discbottom)
					rsmi |= SweConst.SE_BIT_DISC_BOTTOM;
				tret_out.val = tret[1]; // temporary assignment to have an
										// output variable
				if (sw.swe_rise_trans(t_ut, ipl, star, whicheph, rsmi, geopos, 1013.25, 10, tret_out,
						serr) != SweConst.OK) {
					do_printf(serr);
				...
				tret[1] = tret_out.val;
				if ((time_flag & (BIT_TIME_LMT | BIT_TIME_LAT)) != 0) {
					double[] dtmp = new double[] { tret[0] }; // temporary
																// assignment to
																// have an
																// output
																// variable
					...
					tret[0] = dtmp[0];
					dtmp[0] = tret[1]; // temporary assignment to have an output
										// variable
				...
				if (tret[0] == 0 || tret[0] > tret[1]) {
					...
					t0 = tret[0];
					// swe_revjul(tret[0], gregflag, &jyear, &jmon, &jday,
					// &jut);
					sd.setJulDay(tret[0]);
					sd.setCalendarType(gregflag, SweDate.SE_KEEP_JD); // Keep
																		// JulDay!
				...
				if (tret[1] == 0) {
					...
					t1 = tret[1];
					// swe_revjul(tret[1], gregflag, &jyear, &jmon, &jday,
					// &jut);
					sd.setJulDay(tret[1]);
					sd.setCalendarType(gregflag, SweDate.SE_KEEP_JD); // Keep
																		// JulDay!
			...
			/*
			 * swetest -metr calculate and print transits over meridian
			 * (midheaven and lower midheaven
			 */
			if (special_event == SP_MERIDIAN_TRANSIT) {
				/* transit over midheaven */
				tret_out.val = tret[0];
				if (sw.swe_rise_trans(t_ut, ipl, star, whicheph, SweConst.SE_CALC_MTRANSIT, geopos, 1013.25, 10,
						tret_out, serr) != SweConst.OK) {
					do_printf(serr);
					return SweConst.ERR;
				}
				tret[0] = tret_out.val;
				...
				tret_out.val = tret[1];
				if (sw.swe_rise_trans(t_ut, ipl, star, whicheph, SweConst.SE_CALC_ITRANSIT, geopos, 1013.25, 10,
						tret_out, serr) != SweConst.OK) {
					do_printf(serr);
					return SweConst.ERR;
				}
	...
	private int call_lunar_eclipse(double t_ut, int whicheph, int special_mode, double[] geopos, StringBuffer serr) {
		...
		// int ihou[] = new int[1], imin[] = new int[1], isec[] = new int[1],
		// isgn[] = new int[1]; // int used as output parameters
		...
		for (ii = 0; ii < nstep; ii++, t_ut += direction) {
			sout = "";
			/*
			 * swetest -lunecl -how type of lunar eclipse and percentage for a
			 * given time:
			 */
			...
			/*
			 * swetest -lunecl find next lunar eclipse:
			 */
			/* locally visible lunar eclipse */
			if ((special_mode & SP_MODE_LOCAL) != 0) {
				if ((eclflag = sw.swe_lun_eclipse_when_loc(t_ut, whicheph, geopos, tret, attr, direction_flag ? 1 : 0,
						serr)) == SweConst.ERR) {
					do_printf(serr);
					return SweConst.ERR;
				}
				...
				jut = sd.getHour();
				/*
				 * if ((eclflag = swe_lun_eclipse_how(t_ut, whicheph, geopos,
				 * attr, serr)) == ERR) { do_printf(serr); return ERR; }
				 */
				...
				/*
				 * short output: date, time of day, umbral magnitude, umbral
				 * duration, saros series, member number
				 */
				// sprintf(sout_short,
				// "%s\t%2d.%2d.%4d\t%s\t%.3f\t%s\t%d\t%d\n", sout, jday, jmon,
				// jyear, hms(jut,0), attr[8],s1, (int) attr[9], (int)
				// attr[10]);
				// sprintf(sout + strlen(sout),
				// "%2d.%02d.%04d\t%s\t%.4f/%.4f\tsaros %d/%d\t%.6f\n", jday,
				// jmon, jyear, hms(jut,BIT_LZEROES), attr[0],attr[1], (int)
				// attr[9], (int) attr[10], t_ut);
				sout_short = String.format(Locale.US, "%s\t%2d.%2d.%4d\t%s\t%.3f\t%s\t%d\t%d\n", sout, jday, jmon,
						jyear, hms(jut, 0), attr[8], s1, (int) attr[9], (int) attr[10]);
				sout += String.format(Locale.US, "%2d.%02d.%04d\t%s\t%.4f/%.4f\tsaros %d/%d\t%.6f\n", jday, jmon, jyear,
						hms(jut, BIT_LZEROES), attr[0], attr[1], (int) attr[9], (int) attr[10], t_ut);
				/*
				 * second line: eclipse times, penumbral, partial, total begin
				 * and end
				 */
				...
				if ((eclflag = sw.swe_lun_eclipse_when(t_ut, whicheph, search_flag, tret, direction_flag ? 1 : 0,
						serr)) == SweConst.ERR) {
					do_printf(serr);
					return SweConst.ERR;
				}
				...
				// attr[10]);
				// sprintf(sout + strlen(sout),
				// "%2d.%02d.%04d\t%s\t%.4f/%.4f\tsaros %d/%d\t%.6f\tdt=%.2f\n",
				// jday, jmon, jyear, hms(jut,BIT_LZEROES), attr[0],attr[1],
				// (int) attr[9], (int) attr[10], t_ut, swe_deltat(t_ut) *
				// 86400);
				sout_short = String.format(Locale.US, "%s\t%2d.%2d.%4d\t%s\t%.3f\t%s\t%d\t%d\n", sout, jday, jmon,
						jyear, hms(jut, 0), attr[8], s1, (int) attr[9], (int) attr[10]);
				sout += String.format(Locale.US, "%2d.%02d.%04d\t%s\t%.4f/%.4f\tsaros %d/%d\t%.6f\tdt=%.2f\n", jday,
						jmon, jyear, hms(jut, BIT_LZEROES), attr[0], attr[1], (int) attr[9], (int) attr[10], t_ut,
						SweDate.getDeltaT(t_ut) * 86400);
				/*
				 * second line: eclipse times, penumbral, partial, total begin
				 * and end
				 */
	...
	private int call_solar_eclipse(double t_ut, int whicheph, int special_mode, double[] geopos, StringBuffer serr) {
		...
		for (ii = 0; ii < nstep; ii++, t_ut += direction) {
			sout = "";
			/*
			 * swetest -solecl -local -geopos... find next solar eclipse
			 * observable from a given geographic position
			 */
			if ((special_mode & SP_MODE_LOCAL) != 0) {
				if ((eclflag = sw.swe_sol_eclipse_when_loc(t_ut, whicheph, geopos, tret, attr, direction_flag ? 1 : 0,
						serr)) == SweConst.ERR) {
					do_printf(serr);
					return SweConst.ERR;
				} else {
					...
					if (!has_found) {
						...
						t_ut = tret[0];
						// swe_revjul(t_ut, gregflag, &jyear, &jmon, &jday,
						// &jut);
						sd.setJulDay(t_ut);
						sd.setCalendarType(gregflag, SweDate.SE_KEEP_JD); // Keep
																			// JulDay!
						...
						dt = (tret[3] - tret[2]) * 24 * 60;
						// sprintf(sout + strlen(sout),
						// "%2d.%02d.%04d\t%s\t%.4f/%.4f/%.4f\tsaros
						// %d/%d\t%.6f\n", jday, jmon, jyear,
						// hms(jut,BIT_LZEROES), attr[8], attr[0], attr[2],
						// (int) attr[9], (int) attr[10], t_ut);
						// sprintf(sout + strlen(sout), "\t%d min %4.2f sec\t",
						// (int) dt, fmod(dt, 1) * 60);
						sout += String.format(Locale.US, "%2d.%02d.%04d\t%s\t%.4f/%.4f/%.4f\tsaros %d/%d\t%.6f\n", jday,
								jmon, jyear, hms(jut, BIT_LZEROES), attr[8], attr[0], attr[2], (int) attr[9],
								(int) attr[10], t_ut);
			...
			/*
			 * swetest -solecl find next solar eclipse observable from anywhere
			 * on earth
			 */
			if ((special_mode & SP_MODE_LOCAL) == 0) {
				if ((eclflag = sw.swe_sol_eclipse_when_glob(t_ut, whicheph, search_flag, tret, direction_flag ? 1 : 0,
						serr)) == SweConst.ERR) {
					do_printf(serr);
					return SweConst.ERR;
				}
				...
				jday = sd.getDay();
				jut = sd.getHour();
				sout_short = String.format(Locale.US, "%s\t%2d.%2d.%4d\t%s\t%.3f", sout, jday, jmon, jyear, hms(jut, 0),
						attr[8]);
				sout += String.format(Locale.US, "%2d.%02d.%04d\t%s\t%f km\t%.4f/%.4f/%.4f\tsaros %d/%d\t%.6f\n", jday,
						jmon, jyear, hms(jut, 0), attr[3], attr[8], attr[0], attr[2], (int) attr[9], (int) attr[10],
						tret[0]);
				sout += "\t" + hms_from_tjd(tret[2]) + " ";
				if (tret[4] != 0) {
					sout += hms_from_tjd(tret[4]) + " ";
				...
				if ((eclflag & SweConst.SE_ECL_PARTIAL) == 0 && (eclflag & SweConst.SE_ECL_NONCENTRAL) == 0) {
					if ((eclflag = sw.swe_sol_eclipse_when_loc(t_ut - 10, whicheph, geopos_max, tret, attr, 0,
							serr)) == SweConst.ERR) {
						do_printf(serr);
						return SweConst.ERR;
					}
	...
	private int call_lunar_occultation(double t_ut, int ipl, StringBuffer star, int whicheph, int special_mode,
			double[] geopos, StringBuffer serr) {
		...
		for (ii = 0; ii < nstep; ii++) {
			...
			if ((special_mode & SP_MODE_LOCAL) != 0) {
				 ...
				 * (SE_ECL_ONE_TRY)
				 */
				if ((eclflag = sw.swe_lun_occult_when_loc(t_ut, ipl, star, whicheph, geopos, tret, attr,
						(direction_flag ? 1 : 0) | SweConst.SE_ECL_ONE_TRY, serr)) == SweConst.ERR) {
					do_printf(serr);
					return SweConst.ERR;
				} else
					if (eclflag == 0) { /*
					...
					t_ut = tret[0] + direction
							* 10; /*
									 * try again with start date increased by 10
									 */
					ii--;
				} else {
					t_ut = tret[0];
					...
					if (ipl != SweConst.SE_SUN) {
						if ((eclflag & SweConst.SE_ECL_OCC_BEG_DAYLIGHT) != 0
								&& (eclflag & SweConst.SE_ECL_OCC_END_DAYLIGHT) != 0)
							sout += "(daytime)"; /*
													 * occultation occurs during
													 * the day
													 */
			...
			if ((special_mode & SP_MODE_LOCAL) == 0) {
				 ...
				 * (SE_ECL_ONE_TRY)
				 */
				if ((eclflag = sw.swe_lun_occult_when_glob(t_ut, ipl, star, whicheph, search_flag, tret,
						(direction_flag ? 1 : 0) | SweConst.SE_ECL_ONE_TRY, serr)) == SweConst.ERR) {
					do_printf(serr);
					return SweConst.ERR;
				}
				if (eclflag == 0) { /*
									 * no occltation was found at next
									 * conjunction, try next conjunction
									 */
					t_ut = tret[0] + direction;
					ii--;
					continue;
				...
				sout += "\t" + s1 + "\t" + s2;
				if ((eclflag & SweConst.SE_ECL_PARTIAL) == 0 && (eclflag & SweConst.SE_ECL_NONCENTRAL) == 0) {
					if ((eclflag = sw.swe_lun_occult_when_loc(t_ut - 10, ipl, star, whicheph, geopos_max, tret, attr, 0,
							serr)) == SweConst.ERR) {
						do_printf(serr);
						return SweConst.ERR;
					}
	...
	private void do_print_heliacal(double[] dret, int event_type, String obj_name) {
		String sevtname[] = new String[] { "", "heliacal rising ", "heliacal setting", "evening first   ",
				"morning last    ", "evening rising  ", "morning setting ", };
		String stz = "UT";
		...
		if (event_type <= 4) {
			if (hel_using_AV) {
				...
				sout += String.format(Locale.US, "%s %s: %d/%02d/%02d %s %s (%.5f)\n", obj_name, sevtname[event_type],
						sd.getYear(), sd.getMonth(), sd.getDay(), stim0, stz, dret[0]);
			} else {
				...
				sout += String.format(Locale.US, "%s %s: %d/%02d/%02d %s %s (%.5f), opt %s, end %s, dur %.1f min\n",
						...
						stim1, stim2, (dret[2] - dret[0]) * 1440);
			}
		} else {
			stim0 = hms_from_tjd(dret[0]);
			stim0 = remove_whitespace(stim0);
	...
	private int call_heliacal_event(double t_ut, int ipl, StringBuffer star, int whicheph, int special_mode,
			double[] geopos, double[] datm, double[] dobs, StringBuffer serr) {
		...
		mercury_outer: for (ii = 0; ii < nstep; ii++, t_ut = dret[0] + 1) {
			...
			if (search_flag == 0) {
				if (ipl == SweConst.SE_VENUS || ipl == SweConst.SE_MERCURY) {
					/*
					 * we have heliacal rising (morning first), now find morning
					 * last
					 */
					event_type = SweConst.SE_MORNING_LAST;
					retflag = sh.swe_heliacal_ut(dret[0], geopos, datm, dobs, obj_name, event_type, helflag, dret,
							serr);
					if (retflag == SweConst.ERR) {
						do_printf(serr);
						return SweConst.ERR;
					...
					tsave1 = dret[0];
					/*
					 * mercury can have several evening appearances without any
					 * morning appearances in betweeen. We have to find out when
					 * the next morning appearance is and then find all evening
					 * appearances that take place before that
					 */
				...
				while (true) {
					if (ipl == SweConst.SE_MOON) {
						...
						if (false && ipl == SweConst.SE_MERCURY) {
							tsave1 = dret[0];
							// goto repeat_mercury;
							// quick hack by duplicating the code from the first
							// part of the repeat_mercury: section
							/* evening first */
							event_type = SweConst.SE_EVENING_FIRST;
							retflag = sh.swe_heliacal_ut(tsave1, geopos, datm, dobs, obj_name, event_type, helflag,
									dret, serr);
							if (retflag == SweConst.ERR) {
								do_printf(serr);
								return SweConst.ERR;
	...
	private int do_special_event(double tjd, int ipl, StringBuffer star, int special_event, int special_mode,
			double[] geopos, double[] datm, double[] dobs, StringBuffer serr) {
		int retc = 0;
	...
	/*
	 * make_ephemeris_path(). ephemeris path includes current working directory
	 * + program directory + default path from swephexp.h on current drive + on
	 * program drive + on drive C:
	 */
	private int make_ephemeris_path(int iflg, String argv0, StringBuffer opath) { // path
																					// for
																					// output
		int sp;
		String path = "";
