 ...
 * of Greenwich. America seems to often use a different notation!
 * <p>
 * <I><B>You will find the complete documentation for the original
 * SwissEphemeris package at
 * <A HREF="http://www.astro.ch/swisseph/sweph_g.htm"> http://www.astro.ch/
 * swisseph/sweph_g.htm</A>. By far most of the information there is directly
 * valid for this port to Java as well.</B></I>
 */
public class SwissEph implements java.io.Serializable {
	...
	/**
	 * Constructs a new SwissEph object with the specified search path for the
	 * Swiss Ephemeris data files. If you want to use this class in applets, you
	 * would have to specify the path as a valid http URL on the same www server
	 * from where your applet gets served, if the normal security restrictions
	 * apply.<br>
	 * ATTENTION: This constructor sets a global parameter used in calculation
	 * of delta T when parameter path is not null.
	 * 
	 * @param path
	 *            The search path for the Swiss Ephemeris and JPL data files. If
	 *            null or empty, a default path will be used. You will have to
	 *            quote ':', ';' and '\' characters, so a path like
	 *            <code>&quot;C:\swiss\ephe&quot;</code> has to be written as
	 *            <code>&quot;C\\:\\swiss\\ephe&quot;</code>, as any '\' will be
	 *            evaluated twice: the first time by the Java compiler, and the
	 *            second time by the program itself. You can specify multiple
	 *            path elements separated by the (unquoted) ':' or ';'
	 ...
	 * SwissEph constructor. The buffer size determines, how many bytes will get
	 * read on one single HTTP request. Increased buffer size will result in a
	 * reduced number of HTTP-requests, but it will increase the amount of data
	 * to be transferred. As the access to the data is <I>somehow</I> random, it
	 * does not make so much sense to increase the size arbitrarily.
	 * <P>
	 ...
	 * 
	 * @param size
	 *            The size of the buffer. It defaults to 300 bytes. Values less
	 *            than 100 bytes will be increased to 100 bytes, as you will
	 *            only increase the number of requests dramatically, but the
	 *            amount of bytes transferred will just be minimal less.
	 * @see SwissEph#swe_set_ephe_path(java.lang.String)
	 */
	...
	/*
	 * The routine called by the user. It checks whether a position for the same
	 * planet, the same t, and the same flag bits has already been computed. If
	 * yes, this position is returned. Otherwise it is computed. -> If the
	 * SEFLG_SPEED flag has been specified, the speed will be returned at offset
	 * 3 of position array x[]. Its precision is probably better than
	 * 0.002"/day. -> If the SEFLG_SPEED3 flag has been specified, the speed
	 * will be computed from three positions. This speed is less accurate than
	 * SEFLG_SPEED, i.e. better than 0.1"/day. And it is much slower. It is used
	 * for program tests only. -> If no speed flag has been specified, no speed
	 * will be returned.
	 */
	 ...
	 * nodes and apogees. See swe_calc(...) for more information.<br>
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @param tjd_ut
	 *            The Julian Day number in UT (Universal Time).
	 * @param ipl
	 *            The body to be calculated. See
	 *            <A HREF="SweConst.html">SweConst</A> for a list of bodies
	 * @param iflag
	 *            A flag that contains detailed specification on how the body is
	 *            to be computed. See <A HREF="SweConst.html">SweConst</A> for a
	 *            list of valid flags (SEFLG_*).
	 * @param xx
	 *            A double[6] in which the result is returned. See swe_calc()
	 *            for the description of this parameter
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return iflag or SweConst.ERR (-1); iflag MAY have changed from input
	 *         parameter!
	 * @see SwissEph#swe_calc(double, int, int, double[],
	 ...
	 * <P>
	 * The parameter xx is used as an output parameter containing the following
	 * info: <BLOCKQUOTE><CODE>xx[0]:&nbsp;&nbsp;&nbsp;longitude<BR>
	 * xx[1]:&nbsp;&nbsp;&nbsp;latitude<BR>
	 ...
	 * 
	 * @param tjd
	 *            The Julian Day number in ET (UT + deltaT).
	 * @param ipl
	 *            The body to be calculated. See
	 *            <A HREF="SweConst.html">SweConst</A> for a list of bodies
	 * @param iflag
	 ...
	 *            list of valid flags (SEFLG_*).
	 * @param xx
	 *            A double[6] in which the result is returned. See above for
	 *            more details.
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return iflag or SweConst.ERR (-1); iflag MAY have changed from input
	 *         parameter, when the calculation had used different flags, e.g.:
	 *         when specified SweConst.SEFLG_SWIEPH, but the ephemeris data
	 *         files wheren't available, the calculation automatically switches
	 *         to Moshier calculations (SweConst.SEFLG_MOSEPH).
	 * @see #swe_calc_ut(double, int, int, double[], java.lang.StringBuffer)
	 * @see #swe_fixstar_ut(java.lang.StringBuffer, double, int, double[],
	 *      java.lang.StringBuffer)
	 * @see #swe_fixstar(java.lang.StringBuffer, double, int, double[],
	 *      java.lang.StringBuffer)
	 */
	...
	private int _calc(double tjd, int ipl, int iflag, double xx[], StringBuffer serr) throws SwissephException {
		...
		/*
		 * function calls for Pluto with asteroid number 134340 are treated as
		 * calls for Pluto as main body SE_PLUTO. Reason: Our numerical
		 * integrator takes into account Pluto perturbation and therefore
		 * crashes with body 134340 Pluto.
		 */
		if (ipl == SweConst.SE_AST_OFFSET + 134340) {
			ipl = SweConst.SE_PLUTO;
		}
		/*
		 * if ephemeris flag != ephemeris flag of last call, we clear the save
		 * area, to prevent swecalc() using previously computed data for current
		...
		if (ipl < SweConst.SE_NPLANETS && ipl >= SweConst.SE_SUN) {
			...
			/*
			 * other bodies, e.g. asteroids called with ipl = SE_AST_OFFSET +
			 * MPC#
			 */
			sd = swed.savedat[SweConst.SE_NPLANETS];
		}
		/*
		 * if position is available in save area, it is returned. this is the
		 * case, if tjd = tsave and iflag = iflgsave. coordinate flags can be
		 * neglected, because save area provides all coordinate types. if ipl >
		 * SE_AST(EROID)_OFFSET, ipl must be checked, because all asteroids
		 * called by MPC number share the same save area.
		 */
		...
		try { // SwissephExceptions from swecalc
			if (sd.tsave != tjd || tjd == 0 || ipl != sd.ipl || ((sd.iflgsave & ~iflgcoor) != (iflag & ~iflgcoor))) {
				...
				if ((iflag & SweConst.SEFLG_SPEED3) == 0) {
					...
					/*
					 * with speed from three calls of swecalc(), slower and less
					 * accurate. (SLOW speed, for test only)
					 */
					sd.tsave = tjd;
					sd.ipl = ipl;
					switch (ipl) {
					...
					case SweConst.SE_TRUE_NODE:
						/*
						 * this is the optimum dt with Moshier ephemeris, but
						 * not with JPL ephemeris or SWISSEPH. To avoid
						 * completely false speed in case that JPL is wanted but
						 * the program returns Moshier, we use Moshier optimum.
						 * For precise speed, use JPL and FAST speed
						 * computation,
						 */
	...
	/*
	 * closes all open files, frees space of planetary data, deletes memory of
	 * all computed positions
	 */
	...
	/*
	 * sets ephemeris file path. also calls swe_close(). this makes sure that
	 * swe_calc() won't return planet positions previously computed from other
	 * ephemerides
	 */
	/**
	 ...
	 * elements are separated by a semikolon (;) or colon (:). Ephemeris path
	 * elements can be normal file system paths or http-URLs. If your elements
	 * contain colons or semikolons or spaces or backslashes, you have to escape
	 * them with a backslash (\), e.g.
	 * <CODE>&quot;./ephe:C\\:\\ephe:http\\://th-mack.de/datafiles&quot;</CODE>
	 * for a search path of: a) local subdirectory ephe, or, if something is not
	 * found here, b) in C:\ephe, or as a last resort c)
	 * http://th-mack.de/datafiles.
	 * <P>
	 * <B>Note: Opposed to the behaviour of the C version, the Java version does
	 * not evaluate environment variables. This is also true for the environment
	 * variable SE_EPHE_PATH!</B><BR>
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @param path
	 ...
	 *            <code>&quot;C\\:\\swiss\\ephe&quot;</code>, as any '\' will be
	 *            evaluated twice: the first time by the Java compiler, and the
	 *            second time by the program itself.
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public void swe_set_ephe_path(String path) {
		...
		// JAVA: Skipping this code in the Java version - it does not do
		// anything
		// meaningful anyway...
		swed.ephepath = s;
		/*
		 * try to open lunar ephemeris, in order to get DE number and set tidal
	...
	void load_dpsi_deps() {
		...
		// if ((swed.dpsi = (double *) calloc((size_t) SWE_DATA_DPSI_DEPS,
		// sizeof(double))) == NULL) {
		// swed.eop_dpsi_loaded = ERR;
		// return;
		// }
		swed.dpsi = new double[SwephData.SWE_DATA_DPSI_DEPS];
		// if ((swed.deps = (double *) calloc((size_t) SWE_DATA_DPSI_DEPS,
		// sizeof(double))) == NULL) {
		// swed.eop_dpsi_loaded = ERR;
		// return;
		// }
		...
		try {
			while ((s = fp.readLine()) != null) {
				 ...
				 * 2400000.5, swed.dpsi[n] * 1000, swed.deps[n] * 1000);exit(0);
				 */
				n++;
				mjdsv = mjd;
			}
			...
			fp.close();
			/*
			 * file finals.all may have some more data, and especially
			 * estimations for the near future
			 */
			try {
				fp = swi_fopen(-1, SwissLib.DPSI_DEPS_IAU1980_FILE_FINALS, swed.ephepath, null);
			} catch (SwissephException se) {
				return; /*
						 * return without error as existence of file is not
						 * mandatory
						 */
			}
			while ((s = fp.readLine()) != null) {
				...
				if (mjdsv > 0 && mjd - mjdsv != 1) {
					/*
					 * no error, as we do have data; however, if this file is
					 * usefull, then swed.eop_dpsi_loaded will be set to 2
					 */
				...
				/*
				 * fprintf(stderr, "tjd=%f, dpsi=%f, deps=%f\n", mjd +
				 * 2400000.5, swed.dpsi[n] * 1000, swed.deps[n] * 1000);
				 */
				n++;
				mjdsv = mjd;
			}
	...
	/*
	 * sets jpl file name. also calls swe_close(). this makes sure that
	 * swe_calc() won't return planet positions previously computed from other
	 * ephemerides
	 */
	/**
	 * This sets the name of the file that contains the ephemeris data for the
	 * use with the JPL ephemeris. It defaults to the string "de406.eph" defined
	 * in SweConst.SE_FNAME_DFT. If a path is given in fname, the path will be
	 * cut off, as the path is given by swe_set_ephe_path(...).
	 * 
	 * @param fname
	 *            Name of the JPL data file
	 ...
	 * 
	 * @param sid_mode
	 *            One of the above ayanamsha modes plus (optionally) one of the
	 *            non-standard sidereal calculation modes of
	 *            <CODE>SE_SIDBIT_ECL_T0</CODE> or
	 *            <CODE>SE_SIDBIT_SSY_PLANE</CODE>.
	...
	/**
	 * This sets a custom ayanamsha mode for sidereal planet calculations. Use
	 * SE_SIDM_USER optionally together with SE_SIDBIT_ECL_T0 or
	 * SE_SIDBIT_SSY_PLANE for custom modes.<br>
	 * You may want to use swe_set_sid_mode(int), if your are satisfied with the
	 * predefined ayanamsa modes.<br>
	 * This method is also for compatibility to the original C-source code. So
	 * you may also use any of the predefined sid_modes from
	 * swe_set_sid_mode(int), neglecting t0 and ayan_t0 parameters.<br>
	 * E.g., those two calls are identical:
	 * <blockquote> swe_set_sid_mode(SweConst.SE_SIDM_LAHIRI)<br>
	 * swe_set_sid_mode(SweConst.SE_SIDM_LAHIRI, 0, 0) </blockquote> Normally,
	 * you would use this method in the sense of:
	 * <blockquote> swe_set_sid_mode(SweConst.SE_SIDM_USER, 2450789.5,
	 * 23.454578)<br>
	 * </blockquote> If you don't set any ayanamsha mode via one of the
	 * swe_set_sid_mode() methods, it will default to Fagan/Bradley
	 * (SE_SIDM_FAGAN_BRADLEY).<br>
	 * 
	 * @param sid_mode
	 *            SweConst.SE_SIDM_USER plus (optionally) one of the
	 *            non-standard sidereal calculation modes of
	 *            <CODE>SE_SIDBIT_ECL_T0</CODE> or
	 *            <CODE>SE_SIDBIT_SSY_PLANE</CODE>. You may also use any of the
	 *            SE_SIDM_* parameters of swe_set_sid_mode(int). The parameters
	 *            t0 and ayan_t0 will be irrelevant in that case.
	 * @param t0
	 *            Reference date (Julian day), if sid_mode is SE_SIDM_USER
	 * @param ayan_t0
	 *            Initial ayanamsha at t0, if sid_mode is SE_SIDM_USER. This is
	 *            (tropical position - sidereal position) at date t0.
	...
	public void swe_set_sid_mode(int sid_mode, double t0, double ayan_t0) {
		...
		if (sid_mode == SweConst.SE_SIDM_J2000 || sid_mode == SweConst.SE_SIDM_J1900
	...
	/*
	 * the ayanamsa (precession in longitude) according to Newcomb's definition:
	 * 360 - longitude of the vernal point of t referred to the ecliptic of t0.
	 */
	/**
	 ...
	 * 
	 * @param tjd_et
	 *            The date as Julian Day in ET (Ephemeris Time or Dynamic Time)
	 ...
	 * 
	 * @param tjd_ut
	 *            The date as Julian Day in UT (Universal Time)
	 * @return The value of the ayanamsha
	 * @see #swe_set_sid_mode(int, double, double)
	...
	/**********************************************************
	 * get fixstar positions parameters: star name of star or line number in
	 * star file (start from 1, don't count comment). If no error occurs, the
	 * name of the star is returned in the format trad_name, nomeclat_name
	 *
	 * tjd absolute julian day iflag s. swecalc(); speed bit does not function x
	 * pointer for returning the ecliptic coordinates serr error return string
	 **********************************************************/
	/**
	...
	public int swe_fixstar(StringBuffer star, double tjd, int iflag, double xx[], StringBuffer serr) {
		int i;
		// int cmplen;
		// Missing parameters are in "boolean readFixstarParameters(...)" and
		// "int swe_fixstar_found(...)"!
		int epheflag, iflgsave;
		iflag |= SweConst.SEFLG_SPEED; /*
										 * we need this in order to work
										 * correctly
										 */
		iflgsave = iflag;
	...
	protected String[] readFixstarParameters(StringBuffer star, StringBuffer serr) {
		...
		sstar = star.toString().substring(0, SMath.min(star.length(), SweConst.SE_MAX_STNAME));
		if (sstar.length() > 0) {
			if (sstar.charAt(0) == ',') {
				isnomclat = true;
			} else if (Character.isDigit(sstar.charAt(0))) {
				// Use SwissLib.atoi(...) to allow for nonsense input data like
				// 27abc - necessary???
			...
			sstar = sstar.trim(); // trimming left side only in original source
									// code?
		}
		if (sstar.length() == 0) {
			if (serr != null) {
		...
		/******************************************************
		 * Star file close to the beginning, a few stars selected by
		 * Astrodienst. These can be accessed by giving their number instead of
		 * a name. All other stars can be accessed by name. Comment lines start
		...
		if (swed.fixfp == null) {
			...
			try {
				...
				try {
					// May throw SwissephException:
					swed.fixfp = swi_fopen(SwephData.SEI_FILE_FIXSTAR, SweConst.SE_STARFILE_OLD, swed.ephepath, null);
					...
					swed.is_old_starfile = false;
					/*
					 * no fixed star file available. If Spica is called, we
					 * provide it even without a star file, because Spica is
					 * required for the Ayanamsha SE_SIDM_TRUE_CITRA
					 */
		...
		try {
			while ((s = swed.fixfp.readLine()) != null) {
				...
				line++;
				// The name can be a line number, counted without(!!!) comment
				// lines:
	 ...
	 * of Ephemeris Time or Dynamical Time ET).
	 * <P>
	 ...
	 * 
	 * @param star
	 *            Actually, it is an input and an output parameter at the same
	 *            time. So it is not possible to define it as a String, but
	 *            rather as a StringBuffer. On input it defines the star to be
	 *            calculated and can be in three forms:<BR>
	 *            - as a positive integer number meaning the star in the file
	 *            fixstars.cat that is given on the line number of the given
	 *            number, without counting any comment lines beginning with #.
	 *            <BR>
	 *            - as a traditional name case insensitively compared to the
	 *            first name on every line in fixstars.cat.<BR>
	 *            - as a nomenclature prefixed by a comma. This name is compared
	 *            in a case preserving manner to the nomenclature name on every
	 *            line in fixstars.cat.<BR>
	 *            On Output it returns the complete name (traditional plus
	 *            nomenclature name), e.g. "<CODE>Alpheratz,alAnd</CODE>".<br>
	 *            <b>ATTENTION: This method possibly (re-)sets a global
	 *            parameter used in calculation of delta T. See
	 *            SweDate.setGlobalTidalAcc(double).</b>
	 * @param tjd_ut
	 *            The Julian Day in UT
	 * @param iflag
	 *            Any of the SweConst.SEFLG_* flags
	 * @param xx
	 *            A double[6] used as output parameter only. This returns
	 *            longitude, latitude and the distance (in AU) of the fixed
	 *            stars, but it does <B>not</B> return any speed values in xx[3]
	 *            to xx[5] as it does in swe_calc() / swe_calc_ut(), even if you
	 *            specify SweConst.SEFLG_SPEED in the flags parameter!
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return iflag or SweConst.ERR (-1); iflag MAY have changed from input
	...
	public int swe_fixstar_ut(StringBuffer star, double tjd_ut, int iflag, double[] xx, StringBuffer serr) {
		SweDate.swi_set_tid_acc(tjd_ut, iflag, 0);
		return swe_fixstar(star, tjd_ut + SweDate.getDeltaT(tjd_ut), iflag, xx, serr);
	}
	 ...
	 * seasnam.txt that should be more up to date and can be updated by the
	 * user. You can get a list of names from
	 * <A HREF="http://cfa-www.harvard.edu/iau/lists/MPNames.html">http://cfa-
	 * www.harvard.edu/iau/lists/MPNames.html</A>, which you would like to
	 * rename to seasnam.txt and move to your ephemeris directory.
	 * 
	 * @param ipl
	 *            The planet number
	...
	public String swe_get_planet_name(int ipl) {
		...
		/*
		 * function calls for Pluto with asteroid number 134340 are treated as
		 * calls for Pluto as main body SE_PLUTO
		 */
		if (ipl == SweConst.SE_AST_OFFSET + 134340) {
			ipl = SweConst.SE_PLUTO;
		}
		...
		default:
			...
			if (ipl > SweConst.SE_AST_OFFSET) {
				...
				if (ipl == swed.fidat[SwephData.SEI_FILE_ANY_AST].ipl[0]) {
					...
					retc = sweph(SwephData.J2000, ipl, SwephData.SEI_FILE_ANY_AST, 0, null, SwephData.NO_SAVE, xp,
							null);
				 ...
				 * the user. Some old ephemeris files return a '?' in the first
				 * position. There are still a couple of unnamed bodies that got
				 * their provisional designation before 1925, when the current
				 * method of provisional designations was introduced. They have
				 * an 'A' as the first character, e.g. A924 RC. The file
				 * seasnam.txt may contain comments starting with '#'. There
				 * must be at least two columns: 1. asteroid catalog number 2.
				 * asteroid name The asteroid number may or may not be in
				 * brackets
				 */
	...
	/**
	 * If you want to do calculations relative to the observer on some place on
	 * the earth rather than relative to the center of the earth, you will want
	 * to set the geographic location with this method.
	 * 
	 * @param geolon
	 ...
	 * 
	 * @param fname
	 *            filename of the JPL data file. Filenames are searched for in
	 *            the directories of SE_EPHE_PATH or set by SwissEph(String) or
	 ...
	 *            time and date in UT
	 * @param calc_flag
	 *            SweConst.SE_ECL2HOR (xin[0] contains ecliptic longitude,
	 *            xin[1] the ecliptic latitude) or SweConst.SE_EQU2HOR (xin[0] =
	 *            rectascension, xin[1] = declination)
	 * @param geopos
	 *            A double[3] containing the longitude, latitude and height of
	 *            the geographic position. Eastern longitude and northern
	 *            latitude is given by positive values, western longitude and
	 *            southern latitude by negative values.
	 * @param atpress
	 *            atmospheric pressure in mBar (hPa). If it is 0, the pressure
	 *            will be estimated from geopos[2] and attemp.
	 * @param attemp
	 *            atmospheric temperature in degrees Celsius.
	 * @param xin
	 *            double[3] with a content depending on parameter calc_flag. See
	 *            there. xin[3] does not need to be defined.
	 * @param xaz
	 *            Output parameter: a double[3] returning values as specified
	 *            above.
	 */
	public void swe_azalt(double tjd_ut, int calc_flag, double[] geopos, double atpress, double attemp, double[] xin,
			double[] xaz) {
		if (sc == null) {
	 ...
	 * calling swe_refrac.
	 * <P>
	 * xout is an output parameter containing the ecliptic or equatorial
	 * coordinates, depending on the value of the parameter calc_flag.
	 * 
	 * @param tjd_ut
	 *            time and date in UT
	 * @param calc_flag
	 *            SweConst.SE_HOR2ECL or SweConst.SE_HOR2EQU
	 * @param geopos
	 *            A double[3] containing the longitude, latitude and height of
	 *            the geographic position. Eastern longitude and northern
	 *            latitude is given by positive values, western longitude and
	 *            southern latitude by negative values.
	 * @param xin
	 *            double[2] with azimuth and true altitude of planet
	 * @param xout
	 *            Output parameter: a double[2] returning either ecliptic or
	 ...
	 * geographic longitude, latitude, and height. <BLOCKQUOTE>
	 * <P>
	 * <CODE>
	 ...
	 * </CODE></BLOCKQUOTE>
	 * <P>
	 * <B>Attention: attr must be a double[20]!</B><br>
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @param tjd_ut
	 ...
	 * @param ifl
	 *            To indicate, which ephemeris should be used (SEFLG_JPLEPH,
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH)
	 * @param geopos
	 *            A double[3] containing geographic longitude, latitude and
	 *            height in meters above sea level in this order. Eastern
	 *            longitude and northern latitude is given by positive values,
	 *            western longitude and southern latitude by negative values.
	 * @param attr
	 *            A double[20], on return containing the attributes of the
	 *            eclipse as above
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails
	 ...
	 * </CODE>
	 * <P>
	 * <B>Attention: tret must be a double[10]!</B><br>
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @param tjd_start
	 *            The Julian Day number in UT, from when to start searching
	 * @param ifl
	 *            To indicate, which ephemeris should be used (SEFLG_JPLEPH,
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH)
	 * @param ifltype
	 *            SweConst.SE_ECL_TOTAL for total eclipse or 0 for any eclipse
	 * @param tret
	 *            A double[10], on return containing the times of different
	 *            occasions of the eclipse as above
	 * @param backward
	 *            1, if search should be done backwards. If you want to have
	 *            only one conjunction of the moon with the body tested, add the
	 *            following flag: backward |= SE_ECL_ONE_TRY. If this flag is
	 *            not set, the function will search for an occultation until it
	 *            finds one. For bodies with ecliptical latitudes &gt; 5, the
	 *            function may search successlessly until it reaches the end of
	 *            the ephemeris. (Note: we do not add SE_ECL_ONE_TRY to ifl,
	 *            because ifl may contain SEFLG_TOPOCTR (=SE_ECL_ONE_TRY) from
	 ...
	 *         SweConst.SE_ECL_ANNULAR_TOTAL<BR>
	 ...
	 * swe_nod_aps_ut() with the one exception that the time has to be given in
	 * ET (Ephemeris Time or Dynamical Time). You would get ET by adding deltaT
	 * to the UT, e.g.,
	 * <CODE>tjd_et&nbsp;+&nbsp;SweDate.getDeltaT(tjd_et)</CODE>.
	 * <P>
	 * See swe_nod_aps_ut(...) for missing information.
	 * 
	 * @see SwissEph#swe_nod_aps_ut(double, int, int, int, double[], double[],
	 *      double[], double[], java.lang.StringBuffer)
	 */
	public int swe_nod_aps(double tjd_et, int ipl, int iflag, int method, double[] xnasc, double[] xndsc,
			double[] xperi, double[] xaphe, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
		return sc.swe_nod_aps(tjd_et, ipl, iflag, method, xnasc, xndsc, xperi, xaphe, serr);
	}

	/**
	 * Computes planetary nodes and apsides (perihelia, aphelia, second focal
	 * points of the orbital ellipses). This method is identical to
	 * swe_nod_aps() with the one exception that the time has to be given in UT
	 * (Universal Time) and not in ET (Ephemeris Time or Dynamical Time).<br>
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @param tjd_ut
	 ...
	 *            Planet number
	 * @param iflag
	 *            Any of the SEFLG_* flags
	 * @param method
	 *            Defines, what kind of calculation is wanted (SE_NODBIT_MEAN,
	 *            SE_NODBIT_OSCU, SE_NODBIT_OSCU_BAR, SE_NODBIT_FOPOINT)
	 * @param xnasc
	 *            Output parameter of double[6]. On return it contains six
	 ...
	 * @see SwissEph#swe_nod_aps(double, int, int, int, double[], double[],
	 *      double[], double[], java.lang.StringBuffer)
	 * @see SweConst#OK
	 * @see SweConst#ERR
	...
	public int swe_nod_aps_ut(double tjd_ut, int ipl, int iflag, int method, double[] xnasc, double[] xndsc,
		...
		return sc.swe_nod_aps_ut(tjd_ut, ipl, iflag, method, xnasc, xndsc, xperi, xaphe, serr);
	}
	 ...
	 * identical to swe_pheno_ut() with the one exception that the time has to
	 * be given in ET (Ephemeris Time or Dynamical Time). You would get ET by
	 * adding deltaT to the UT, e.g.,
	 * <CODE>tjd_et&nbsp;+&nbsp;SweDate.getDeltaT(tjd_et)</CODE>.
	 * <P>
	 * See swe_pheno_ut() for missing information.
	 * 
	 * @see SwissEph#swe_pheno_ut(double, int, int, double[],
	 *      java.lang.StringBuffer)
	 */
	 ...
	 *            The Julian Day number in UT (Universal Time).
	 * @param ipl
	 *            The body number to be calculated. See class
	 *            <A HREF="SweConst.html">SweConst</A> for a list of bodies
	 *            (SE_*)
	 * @param iflag
	 *            Which ephemeris is to be used (SEFLG_JPLEPH, SEFLG_SWIEPH,
	 *            SEFLG_MOSEPH). Also allowable flags: SEFLG_TRUEPOS,
	 *            SEFLG_HELCTR.
	 * @param attr
	 *            A double[20] in which the result is returned. See above for
	 *            more details.
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 ...
	 * 
	 * @param inalt
	 *            The true or apparent altitude to be converted
	 * @param atpress
	 *            Atmospheric pressure in mBar (hPa). If it is 0, the pressure
	 ...
	 *            The true or apparent altitude to be converted
	 * @param geoalt
	 *            altitude of observer above sea level in meters
	 * @param atpress
	 *            Atmospheric pressure in mBar (hPa). If it is 0, the pressure
	 *            will be estimated from attemp on sea level.
	 * @param attemp
	 ...
	 *            SweConst.SE_TRUE_TO_APP or SweConst.SE_APP_TO_TRUE
	 * @param dret
	 *            output parameter, use a double[4] as input.
	 * 
	 *            <pre>
	 ...
	 */
	public double swe_refrac_extended(double inalt, double geoalt, double atpress, double lapse_rate, double attemp,
			int calc_flag, double[] dret) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
	 ...
	 *            The Julian Day number in UT, from when to start searching
	 * @param ipl
	 *            Planet number, if times for planet or moon are to be
	 *            calculated.
	 * @param starname
	 *            The name of the star, if times for a star should be
	 *            calculated. It has to be null or the empty string otherwise!
	 * @param epheflag
	 *            To indicate, which ephemeris should be used (SEFLG_JPLEPH,
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH)
	 * @param rsmi
	 *            Specification, what type of calculation is wanted
	 *            (SE_CALC_RISE, SE_CALC_SET, SE_CALC_MTRANSIT,
	 *            SE_CALC_ITRANSIT). For SE_CALC_RISE or SE_CALC_SET you may add
	 *            SE_BIT_DISC_CENTER for rise or set of the center of the body,
	 *            SE_BIT_DISC_BOTTOM for the completely visible object. Add
	 *            SE_BIT_NO_REFRACTION for calculation without refraction
	 *            effects. Add SE_BIT_CIVIL_TWILIGHT or SE_BIT_NAUTIC_TWILIGHT
	 *            or SE_BIT_ASTRO_TWILIGHT for civil, nautical, or astronomical
	 *            twilight. Use SE_BIT_FIXED_DISC_SIZE to neglect the effect of
	 *            distance on disc size. The calculation method defaults to
	 *            SE_CALC_RISE.
	 * @param geopos
	 *            A double[3] containing the longitude, latitude and height of
	 *            the observer. Eastern longitude and northern latitude is given
	 *            by positive values, western longitude and southern latitude by
	 *            negative values.
	 * @param atpress
	 *            atmospheric pressure in mBar (hPa). If it is 0, the pressure
	 *            will be estimated from geopos[2] and attemp (1013.25 mbar for
	 *            sea level). When calculating MTRANSIT or ITRANSIT, this
	 *            parameter is not used.
	 * @param attemp
	 *            atmospheric temperature in degrees Celsius. When calculating
	 *            MTRANSIT or ITRANSIT, this parameter is not used.
	 * @param tret
	 *            Return value containing the time of rise or whatever was
	 *            requested. This is UT.
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails
	 * @return SweConst.OK (0) or SweConst.ERR (-1) or -2 if the body does not
	 *         rise or set
	 * @see SweConst#OK
	 * @see SweConst#ERR
	 * @see SweConst#SEFLG_JPLEPH
	 ...
	 * @see DblObj
	 */
	public int swe_rise_trans(double tjd_ut, int ipl, StringBuffer starname, int epheflag, int rsmi, double[] geopos,
			double atpress, double attemp, DblObj tret, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
		return sc.swe_rise_trans(tjd_ut, ipl, starname, epheflag, rsmi, geopos, atpress, attemp, tret, serr);
	}
	 ...
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @see #swe_rise_trans(double, int, StringBuffer, int, int, double[],
	 *      double, double, DblObj, StringBuffer)
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_rise_trans_true_hor(double tjd_ut, int ipl, StringBuffer starname, int epheflag, int rsmi,
			double[] geopos, double atpress, double attemp, double horhgt, DblObj tret, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
	 ...
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH)
	 * @param ifltype
	 *            SweConst.SE_ECL_TOTAL or any other SE_ECL_* constant or 0 for
	 *            any type of eclipse: <blockquote> SweConst.SE_ECL_TOTAL<BR>
	 *            SweConst.SE_ECL_ANNULAR<BR>
	 *            SweConst.SE_ECL_PARTIAL<BR>
	 *            SweConst.SE_ECL_ANNULAR_TOTAL<BR>
	 *            in combination with:<BR>
	 *            SweConst.SE_ECL_CENTRAL<BR>
	 *            SweConst.SE_ECL_NONCENTRAL </blockquote>
	 * @param tret
	 *            A double[10], on return containing the times of different
	 *            occasions of the eclipse as above
	 * @param backward
	 *            !=0, if search should be done backwards
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return -1 (SweConst.ERR), if the calculation failed<BR>
	...
	public int swe_sol_eclipse_when_glob(double tjd_start, int ifl, int ifltype, double tret[], int backward,
		...
		return sc.swe_sol_eclipse_when_glob(tjd_start, ifl, ifltype, tret, backward, serr);
	}

	/**
	 * Computes the next solar eclipse at a given geographical position. Note
	 * the uncertainty of Delta T for the remote past and the future.
	 * <P>
	 * tret is an output parameter with the following meaning:
	 * <P>
	 * <CODE>
	 ...
	 *            southern latitude by negative values.
	 * @param tret
	 *            A double[7], on return containing the times of different
	 *            occasions of the eclipse as specified above
	 * @param attr
	 *            A double[20], on return containing different attributes of the
	 *            eclipse. See above.
	 * @param backward
	 *            true, if search should be done backwards
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return -1 (SweConst.ERR), if the calculation failed<BR>
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_sol_eclipse_when_loc(double tjd_start, int ifl, double[] geopos, double[] tret, double[] attr,
			int backward, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
		return sc.swe_sol_eclipse_when_loc(tjd_start, ifl, geopos, tret, attr, backward, serr);
	}

	/**
	 * Computes the geographic location for a given time, where a solar eclipse
	 * is central (or maximum for a non-central eclipse).
	 * <P>
	 * Output parameters:
	 * <P>
	 * <BLOCKQUOTE><CODE>
	 ...
	 * </CODE></BLOCKQUOTE>
	 * <P>
	 * <B>ATTENTION: geopos must be a double[10], attr a double[20]!</B><br>
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 * 
	 * @param tjd_ut
	 ...
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH)
	 * @param geopos
	 *            A double[10], on return containing the geographic positions.
	 * @param attr
	 *            A double[20], on return containing the attributes of the
	 *            eclipse as above.
	 * @param serr
	...
	/*
	 * Same declaration as swe_sol_eclipse_when_loc(). In addition: int32 ipl
	 * planet number of occulted body char* starname name of occulted star. Must
	 * be NULL or "", if a planetary occultation is to be calculated. For the
	 * use of this field, also see swe_fixstar(). int32 ifl ephemeris flag. If
	 * you want to have only one conjunction of the moon with the body tested,
	 * add the following flag: ifl |= SE_ECL_ONE_TRY. If this flag is not set,
	 * the function will search for an occultation until it finds one. For
	 * bodies with ecliptical latitudes > 5, the function may search
	 * successlessly until it reaches the end of the ephemeris.
	 */
	/**
	 * Computes the next eclipse of any planet or fixstar at a given
	 * geographical position. Note the uncertainty of Delta T for the remote
	 * past and the future.
	 * <P>
	 * tret is an output parameter with the following meaning:
	 * <P>
	 ...
	 *            The Julian Day number in UT, from when to start searching
	 * @param ipl
	 *            Planet number of the occulted planet. See SE_SUN etc. for the
	 *            planet numbers.
	 * @param starname
	 *            The name of the fixstar, if looking for an occulted fixstar.
	 *            This has to be null or an empty StringBuffer, if you are
	 *            looking for a planet specified in parameter ipl. See routine
	 *            swe_fixstar() for this parameter.
	 * @param ifl
	 *            To indicate, which ephemeris should be used (SEFLG_JPLEPH,
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH) Additionally, you can specify
	 *            SE_ECL_ONE_TRY, to only search for one conjunction of the moon
	 *            with the planetary body. If this flag is not set, the function
	 *            will search for an occultation until it finds one. For bodies
	 *            with ecliptical latitudes &gt; 5, the function may search
	 *            successlessly until it reaches the end of the ephemeris.
	 * @param geopos
	 *            A double[3] containing the longitude, latitude and height of
	 *            the geographic position. Eastern longitude and northern
	 ...
	 *            eclipse. See above.
	 * @param backward
	 *            any value != 0 means, search should be done backwards
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return -1 (SweConst.ERR), if the calculation failed<BR>
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_lun_occult_when_loc(double tjd_start, int ipl, StringBuffer starname, int ifl, double[] geopos,
			double[] tret, double[] attr, int backward, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
	 ...
	 *
	 * tret[0] time of maximum eclipse tret[1] tret[2] time of partial phase
	 * begin (indices consistent with solar eclipses) tret[3] time of partial
	 * phase end tret[4] time of totality begin tret[5] time of totality end
	 * tret[6] time of penumbral phase begin tret[7] time of penumbral phase end
	 * tret[8] time of moonrise, if it occurs during the eclipse tret[9] time of
	 * moonset, if it occurs during the eclipse
	 *
	 * attr[0] umbral magnitude at tjd attr[1] penumbral magnitude attr[4]
	 * azimuth of moon at tjd attr[5] true altitude of moon above horizon at tjd
	 * attr[6] apparent altitude of moon above horizon at tjd attr[7] distance
	 * of moon from opposition in degrees attr[8] umbral magnitude at tjd (=
	 * attr[0]) attr[9] saros series number attr[10] saros series member number
	 * declare as attr[20] at least !
	 */
	public int swe_lun_eclipse_when_loc(double tjd_start, int ifl, double geopos[], double tret[], double attr[],
			int backward, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
	 ...
	 *            The Julian Day number in UT
	 * @param ipl
	 *            The planet, whose occultation by the moon should be searched.
	 * @param starname
	 *            The fixstar, whose occultation by the moon should be searched.
	 *            See swe_fixstar() for details. It has to be null or the empty
	 ...
	 *         SweConst.SE_ECL_ANNULAR | SweConst.SE_ECL_NONCENTRAL<br>
	 *         SweConst.SE_ECL_PARTIAL<br>
	 * @see #swe_sol_eclipse_where(double, int, double[], double[],
	 *      java.lang.StringBuffer)
	 * @see #swe_fixstar_ut(StringBuffer, double, int, double[], StringBuffer)
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_lun_occult_where(double tjd_ut, int ipl, StringBuffer starname, int ifl, double[] geopos,
	...
	/*
	 * When is the next lunar occultation anywhere on earth? This function also
	 * finds solar eclipses, but is less efficient than
	 * swe_sol_eclipse_when_glob().
	 *
	 * input parameters:
	 *
	 * tjd_start start time for search (UT) ipl planet number of occulted body
	 * starname name of occulted star. Must be NULL or "", if a planetary
	 * occultation is to be calculated. For the use of this field, also see
	 * swe_fixstar(). ifl ephemeris to be used (SEFLG_SWIEPH, etc.) ephemeris
	 * flag. If you want to have only one conjunction of the moon with the body
	 * tested, add the following flag: ifl |= SE_ECL_ONE_TRY. If this flag is
	 * not set, the function will search for an occultation until it finds one.
	 * For bodies with ecliptical latitudes > 5, the function may search
	 * successlessly until it reaches the end of the ephemeris.
	 *
	 * ifltype eclipse type to be searched (SE_ECL_TOTAL, etc.) 0, if any type
	 * of eclipse is wanted this functionality also works with occultations
	 *
	 * return values:
	 *
	 * retflag SE_ECL_TOTAL or SE_ECL_ANNULAR or SE_ECL_PARTIAL or
	 * SE_ECL_ANNULAR_TOTAL SE_ECL_CENTRAL SE_ECL_NONCENTRAL
	 *
	 * tret[0] time of maximum eclipse tret[1] time, when eclipse takes place at
	 * local apparent noon tret[2] time of eclipse begin tret[3] time of eclipse
	 * end tret[4] time of totality begin tret[5] time of totality end tret[6]
	 * time of center line begin tret[7] time of center line end tret[8] time
	 * when annular-total eclipse becomes total not implemented so far tret[9]
	 * time when annular-total eclipse becomes annular again not implemented so
	 * far declare as tret[10] at least!
	 *
	 */
	/**
	 * Computes the next lunar occultation anywhere on earth. This method also
	 * finds solar eclipses, but is less efficient than
	 * swe_sol_eclipse_when_glob().
	 * <P>
	 * tret is an output parameter with the following meaning:
	 ...
	 *            The Julian Day number in UT, from when to start searching
	 * @param ipl
	 *            planet number of occulted body
	 * @param starname
	 *            name of occulted star. Must be null or &quot;&quot;, if a
	 *            planetary occultation is to be calculated. For the use of this
	 *            field, also see swe_fixstar().
	 * @param ifl
	 *            To indicate, which ephemeris should be used (SEFLG_JPLEPH,
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH) If you like to have only one
	 *            conjunction of the moon with the body tested, add flag
	 *            SE_ECL_ONE_TRY. If this flag is not set, the function will
	 *            search for an occultation until it finds one. For bodies with
	 *            ecliptical latitudes &gt; 5, the function may search
	 *            successlessly until it reaches the end of the ephemeris.
	 * @param ifltype
	 *            eclipse type to be searched (SE_ECL_TOTAL, etc.). 0, if any
	 *            type of eclipse is wanted. This functionality also works with
	 *            occultations.
	 * @param tret
	 *            A double[10], on return containing the times of different
	 *            occasions of the eclipse as above
	 * @param backward
	 *            if != 0, search is done backwards
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return -1 (SweConst.ERR), if the calculation failed<BR>
	 ...
	 *         SweConst.SE_ECL_CENTRAL<BR>
	 *         SweConst.SE_ECL_NONCENTRAL
	 * @see #swe_sol_eclipse_when_glob(double, int, int, double[], int,
	 *      java.lang.StringBuffer)
	 * @see #swe_fixstar_ut(StringBuffer, double, int, double[], StringBuffer)
	 * @see SweConst#SE_ECL_ONE_TRY
	 * @see SweConst#SE_ECL_TOTAL
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_lun_occult_when_glob(double tjd_start, int ipl, StringBuffer starname, int ifl, int ifltype,
	 ...
	 * 
	 * if starname != NULL then a star is computed. iflag: use the flags
	 * SE_SWIEPH, SE_JPLEPH, SE_MOSEPH, SEFLG_TOPOCTR.
	 *
	 * imeth defines method: imeth = 0 sector from longitude and latitude imeth
	 * = 1 sector from longitude, with lat = 0 imeth = 2 sector from rise and
	 * set imeth = 3 sector from rise and set with refraction rise and set are
	 * defined as appearance and disappearance of disc center.
	 *
	 * geopos is an array of 3 doubles for geo. longitude, geo. latitude,
	 * elevation. atpress and attemp are only needed for imeth = 3. If imeth =
	 * 3, If imeth=3 and atpress not given (= 0), the programm assumes 1013.25
	 * mbar; if a non-zero height above sea is given in geopos, atpress is
	 * estimated. dgsect is return area (pointer to a double) serr is pointer to
	 ...
	 *            Planet number.
	 * @param starname
	 *            If starname != null and not an empty string, then a fixstar is
	 *            computed and not a planet specified in ipl. See swe_fixstar()
	 *            method on this.
	 * @param iflag
	 *            Use the flags SE_SWIEPH, SE_JPLEPH, SE_MOSEPH, SEFLG_TOPOCTR.
	 * @param imeth
	 *            defines the method.<br>
	 *            <blockquote> imeth = 0: sector from longitude and latitude<br>
	 ...
	 *            </blockquote> Rise and set are defined as appearance and
	 *            disappearance of disc center.
	 * @param geopos
	 *            An array of 3 doubles for geo. longitude, geo. latitude,
	 *            elevation in meter.
	 * @param atpress
	 *            Only needed for imeth = 3. If imeth=3 and atpress not given (=
	 *            0), the programm assumes 1013.25 mbar; if a non-zero height
	 *            above sea is given in geopos, atpress is estimated.
	 * @param attemp
	 *            Temperature. Only needed for imeth = 3.
	 * @param dgsect
	 *            Return value.
	 * @param serr
	 *            Pointer to error string, may be null.
	 ...
	 * @see SweConst#SEFLG_MOSEPH
	 */
	public int swe_gauquelin_sector(double t_ut, int ipl, StringBuffer starname, int iflag, int imeth, double[] geopos,
			double atpress, double attemp, DblObj dgsect, StringBuffer serr) {
		if (sc == null) {
			sc = new Swecl(this, sl, sm, swed);
		}
	 ...
	 * 
	 * @param hsys
	 *            House system character house systems are:
	 *            <P>
	 *            <BLOCKQUOTE><CODE>
	 * A&nbsp;&nbsp;equal<br>
	 * E&nbsp;&nbsp;equal<br>
	 ...
	 * which house a planet is and how far from its cusp it is. With Koch
	 * houses, the function sometimes returns 0, if the computation was not
	 * possible.
	 * 
	 * @param armc
	 *            The ARMC (= sidereal time)
	 * @param geolat
	 *            The latitude
	 * @param eps
	 *            The ecliptic obliquity (e.g. xx[0] of swe_calc(...))
	 * @param hsys
	 *            The house system. See swe_houses(...) for a list of all
	 *            houses.
	 * @param xpin
	 *            A double[2] containing the ecliptic longitude (xpin[0]) and
	 *            latitude (xpin[1]) of the planet in degrees. It is an input
	 *            parameter, describing tropical positions. Indeed, it needs a
	 *            double[6] as parameter with any value in the other doubles,
	 *            but the methods now accepts both a double[2] and a double[6].
	 * @param serr
	 *            StringBuffer to contain any error messages or warnings
	 * @return A value between 1.0 and 12.999999, indicating in which house a
	 *         planet is and how far from its cusp it is. Koch may return 0, if
	 *         the calculation was not possible.
	...
	/**
	 * Calculates the house positions and other vital points. You would use this
	 * method instead of swe_houses, if you do not have a date available, but
	 * just the ARMC (sidereal time).
	 * 
	 * @param armc
	 *            The ARMC (= sidereal time)
	 * @param geolat
	 *            The latitude on earth, for which the calculation has to be
	 *            done.
	 * @param eps
	 *            The ecliptic obliquity (e.g. xx[0] of swe_calc(...))
	 * @param hsys
	 *            The house system as a character given as an integer. See
	 *            swe_houses(...) for a list of all houses.
	 * @param cusp
	 *            The house cusps are returned here in cusp[1...12] for the
	 *            house 1 to 12.
	 * @param ascmc
	 *            The special points like ascendant etc. are returned here. See
	 *            swe_houses(...) for further info on this parameter.
	 * @see SwissEph#swe_houses(double, int, double, double, int, double[],
	 *      double[])
	 * @see SwissEph#swe_calc
	 * @return SweConst.OK (==0) or SweConst.ERR (==-1), if calculation was not
	 *         possible due to nearness to the polar circle in Koch or Placidus
	 *         house system or when requesting Gauquelin sectors. Calculation
	 *         automatically switched to Porphyry house calculation method in
	 *         this case, so that valid houses will be returned anyway, just in
	 *         a different house system than requested.
	 */
	...
	/**
	 * Calculates the house positions and other vital points. The possible house
	 * systems are:
	 * <P>
	 * <BLOCKQUOTE><CODE>
	 * (int)'A'&nbsp;&nbsp;equal<br>
	 ...
	 *  </CODE></BLOCKQUOTE> You can use the SE_ constants below from
	 * SweConst.java to access these values in ascmc[].
	 * <p>
	 * 
	 * @param tjd_ut
	 *            The Julian Day number in UT
	 * @param iflag
	 *            An additional flag for calculation. It must be 0 or
	 *            SEFLG_SIDEREAL and / or SEFLG_RADIANS.
	 * @param geolat
	 *            The latitude on earth, for which the calculation has to be
	 *            done.
	 * @param geolon
	 *            The longitude on earth, for which the calculation has to be
	 *            done. Eastern longitude and northern latitude is given by
	 *            positive values, western longitude and southern latitude by
	 *            negative values.
	 * @param hsys
	 *            The house system as a character given as an integer.
	 * @param cusp
	 *            (double[13]) The house cusps are returned here in cusp[1...12]
	 *            for the houses 1 to 12.
	 * @param ascmc
	 *            (double[10]) The special points like ascendant etc. are
	 ...
	 */
	public int swe_houses(double tjd_ut, int iflag, double geolat, double geolon, int hsys, double[] cusp,
			double[] ascmc) {
	...
	/**
	 * -- NOT YET IMPLEMENTED -- File to read from and write to the maximum and
	 * minimum speeds of planets and other objects.<br>
	 ...
	 * 
	 * @param fname
	 *            The filename to be used. It should be writable, so the extreme
	 *            speed values can be further improved.
	 * @param writeable
	 *            Say true here, if the file should be improved by newly
	 *            calculated values, false otherwise. It will be a good idea to
	 *            keep it writeable.
	 */
	 ...
	 * longitude, latitude, distance or speed value with geocentric or
	 * topocentric positions in a tropical or sidereal zodiac. Dates are
	 * interpreted as ET (=UT&nbsp;+&nbsp;deltaT).
	 * <p>
	 ...
	 *
	 * @param tc
	 *            The TransitCalculator that should be used here.
	 * @param jdET
	 *            The date (and time) in ET, from where to start searching.
	 * @param backwards
	 *            If backward search should be performed.
	...
	public double getTransitET(TransitCalculator tc, double jdET, boolean backwards)
		...
		return getTransitET(tc, jdET, backwards, (backwards ? -Double.MAX_VALUE : Double.MAX_VALUE));
	}
	 ...
	 *            If backward search should be performed.
	 * @param jdLimit
	 *            This is the date, when the search for transits should be
	 ...
	 *            The TransitCalculator that should be used here.
	 * @param jdUT
	 *            The date (and time) in UT, from where to start searching.
	 * @param backwards
	 *            If backward search should be performed.
	 * @return return A double containing the julian day number for the next /
	...
	public double getTransitUT(TransitCalculator tc, double jdUT, boolean backwards)
		...
		boolean calcUT = (tc instanceof TCHouses);
		double jdET = ext.getTransit(tc, jdUT + (calcUT ? 0 : SweDate.getDeltaT(jdUT)), backwards,
	 ...
	 *            This is the date, when the search for transits should be
	 *            stopped, even if no transit point had been found up to then.
	 *            It is interpreted as UT time as well.
	 * @return return A double containing the julian day number for the next /
	 *         previous transit as UT.
	 * @see swisseph.TCPlanet
	 * @see swisseph.TCPlanetPlanet
	...
	public double getTransitUT(TransitCalculator tc, double jdUT, boolean backwards, double jdLimit)
		...
		double jdET = ext.getTransit(tc, jdUT + SweDate.getDeltaT(jdUT), backwards,
	...
	private int swecalc(double tjd, int ipl, int iflag, double[] x, StringBuffer serr) throws SwissephException {
		...
		/******************************************
		 * which ephemeris is wanted, which is used? Three ephemerides are
		 * possible: MOSEPH, SWIEPH, JPLEPH. JPLEPH is best, SWIEPH is nearly as
		 * good, MOSEPH is least precise. The availability of the various
		 * ephemerides depends on the installed ephemeris files in the users
		 * ephemeris directory. This can change at any time. Swisseph should try
		 * to fulfil the wish of the user for a specific ephemeris, but use a
		 * less precise one if the desired ephemeris is not available for the
		 * given date and body. If internal ephemeris errors are detected (data
		 * error, file length error) an error is returned. If the time range is
		 * bad but another ephemeris can deliver this range, the other ephemeris
		 * is used. If no ephemeris is specified, DEFAULTEPH is assumed as
		 * desired. DEFAULTEPH is defined at compile time, usually as SWIEPH.
		...
		if (ipl == SweConst.SE_ECL_NUT) {
			...
			case SweConst.SEFLG_JPLEPH:
				...
				/*
				 * jpl ephemeris not on disk or date beyond ephemeris range or
				 * file corrupt
				 */
			...
			if ((retc = app_pos_etc_moon(iflag, serr)) != SweConst.OK) {
				return swecalc_error(x); // retc may be wrong with sidereal
											// calculation
			}
			/**********************************************
			...
			/*
			 * barycentric sun must be handled separately because of the
			 * following reasons: ordinary planetary computations use the
			 * function main_planet() and its subfunction jplplan(), see further
			 * below. now, these functions need the swisseph internal planetary
			 * indices, where SEI_EARTH = SEI_SUN = 0. therefore they don't know
			 * the difference between a barycentric sun and a barycentric earth
			 * and always return barycentric earth. to avoid this problem, many
			 * functions would have to be changed. as an alternative, we choose
			 * a more separate handling.
			 */
			 ...
			 * (pedp->xreturn = swed.pldat[SEI_EARTH].xreturn). in case a
			 * barycentric earth computation follows for the same date, the
			 * planetary functions will return the barycentric SUN unless we
			 * force a new computation of pedp->xreturn. this can be done by
			 * initializing the save of iflag.
			 */
			pedp.xflgs = -1;
			/******************************************
			 * mercury - pluto *
			 ******************************************/
				...
				|| ipl == SweConst.SE_MERCURY || ipl == SweConst.SE_VENUS || ipl == SweConst.SE_MARS
				|| ipl == SweConst.SE_JUPITER || ipl == SweConst.SE_SATURN || ipl == SweConst.SE_URANUS
				|| ipl == SweConst.SE_NEPTUNE || ipl == SweConst.SE_PLUTO || ipl == SweConst.SE_EARTH) {
			...
			/*
			 * to avoid infinitesimal deviations from latitude = 0 that result
			 * from conversions
			 */
			...
			/**********************************************
			 * mean lunar apogee ('dark moon', 'lilith') * for comment s.
			 * moshmoon.c, swi_mean_apog() *
			 **********************************************/
			...
			/*
			 * to avoid infinitesimal deviations from r-speed = 0 that result
			 * from conversions
			 */
			...
			if (tjd < SwephData.MOSHLUEPH_START || tjd > SwephData.MOSHLUEPH_END) {
				...
				if (serr != null) {
					serr.setLength(0);
					serr.append("Interpolated apsides are restricted to JD " + SwephData.MOSHLUEPH_START + " - JD "
			 ...
			 * minor planets *
			 ***********************************************/
		} else if (ipl == SweConst.SE_CHIRON || ipl == SweConst.SE_PHOLUS
				|| ipl == SweConst.SE_CERES /* Ceres - Vesta */
				|| ipl == SweConst.SE_PALLAS || ipl == SweConst.SE_JUNO || ipl == SweConst.SE_VESTA
			...
			if (ipli == SwephData.SEI_CHIRON && (tjd < SwephData.CHIRON_START || tjd > SwephData.CHIRON_END)) {
				if (serr != null) {
					serr.setLength(0);
					serr.append("Chiron's ephemeris is restricted to JD " + SwephData.CHIRON_START + " - JD "
			...
			if (ipli == SwephData.SEI_PHOLUS && (tjd < SwephData.PHOLUS_START || tjd > SwephData.PHOLUS_END)) {
				if (serr != null) {
					serr.setLength(0);
					serr.append("Pholus's ephemeris is restricted to JD " + SwephData.PHOLUS_START + " - JD "
							+ SwephData.PHOLUS_END);
				}
				return SweConst.ERR;
			}
			// do_asteroid:
			while (true) {
				 ...
				 * beyond ephemeris range. in this case redo with Moshier
				 */
			...
			while (true) {
				...
				if (smosh.swi_osc_el_plan(tjd, pdp.x, ipl - SweConst.SE_FICT_OFFSET, ipli, pedp.x, psdp.x,
						serr) != SweConst.OK) {
					return swecalc_error(x);
				}
				if (retc == SweConst.ERR) {
				...
				/*
				 * app_pos_etc_plan_osc() might have failed, if t(light-time) is
				 * beyond ephemeris range. in this case redo with Moshier
				 */
				if (retc == SwephData.NOT_AVAILABLE || retc == SwephData.BEYOND_EPH_LIMITS) {
					if (epheflag != SweConst.SEFLG_MOSEPH) {
	...
	int moshier_moon(double tjd, boolean do_save, double[] xpmret, StringBuffer serr) {
	...
	int sweph_sbar(double tjd, int iflag, PlanData psdp, PlanData pedp, StringBuffer serr) {
		int retc;
		/*
		 * sweplan() provides barycentric sun as a by-product in save area; it
		 * is saved in swed.pldat[SEI_SUNBARY].x
		 */
		retc = sweplan(tjd, SwephData.SEI_EARTH, SwephData.SEI_FILE_PLANET, iflag, SwephData.DO_SAVE, null, null, null,
				null, serr);
		if (retc == SweConst.ERR || retc == SwephData.NOT_AVAILABLE) {
			return SweConst.ERR;
		}
	...
	/*
	 * calculates obliquity of ecliptic and stores it together with its date,
	 * sine, and cosine
	 */
	...
	/*
	 * computes a main planet from any ephemeris, if it has not yet been
	 * computed for this date. since a geocentric position requires the earth,
	 * the earth's position will be computed as well. With SWISSEPH files the
	 * barycentric sun will be done as well. With Moshier, the moon will be done
	 * as well.
	 *
	 * tjd = julian day ipli = body number epheflag = which ephemeris? JPL,
	 * SWISSEPH, Moshier? iflag = other flags
	 *
	 * the geocentric apparent position of ipli (or whatever has been specified
	 * in iflag) will be saved in &swed.pldat[ipli].xreturn[];
	 *
	 * the barycentric (heliocentric with Moshier) position J2000 will be kept
	 * in &swed.pldat[ipli].x[];
	 */
	private int main_planet(double tjd, int ipli, int epheflag, int iflag, StringBuffer serr) throws SwissephException {
		int retc;
		...
		if (epheflag == SweConst.SEFLG_SWIEPH || calc_swieph) {
			...
			retc = sweplan(tjd, ipli, SwephData.SEI_FILE_PLANET, iflag, SwephData.DO_SAVE, null, null, null, null,
					serr);
			if (retc == SweConst.ERR) {
				return SweConst.ERR;
			}
	...
	/*
	 * Computes a main planet from any ephemeris or returns it again, if it has
	 * been computed before. In barycentric equatorial position of the J2000
	 * equinox. The earth's position is computed as well. With SWISSEPH and JPL
	 * ephemeris the barycentric sun is computed, too. With Moshier, the moon is
	 * returned, as well.
	 *
	 * tjd = julian day ipli = body number epheflag = which ephemeris? JPL,
	 * SWISSEPH, Moshier? iflag = other flags xp, xe, xs, and xm are the
	 * pointers, where the program either finds or stores (if not found) the
	 * barycentric (heliocentric with Moshier) positions of the following
	 * bodies: xp planet xe earth xs sun xm moon
	 *
	 * xm is used with Moshier only
	 */
	int main_planet_bary(double tjd, int ipli, int epheflag, int iflag, boolean do_save, double xp[], double xe[],
			double xs[], double xm[], StringBuffer serr) {
		int i;
		int retc;
		boolean calc_moshier = false;
		boolean calc_swieph = false;
		...
		if (epheflag == SweConst.SEFLG_SWIEPH || calc_swieph) {
			// sweph_planet:
			/* compute barycentric planet (+ earth, sun, moon) */
			retc = sweplan(tjd, ipli, SwephData.SEI_FILE_PLANET, iflag, do_save, xp, xe, xs, xm, serr);
			if (retc == SweConst.ERR || retc == SwephData.NOT_AVAILABLE) {
				return retc;
			}
		}
	...
	/*
	 * SWISSEPH this routine computes heliocentric cartesian equatorial
	 * coordinates of equinox 2000 of geocentric moon
	 *
	 * tjd julian date iflag flag do_save save J2000 position in save area
	 * pdp->x ? xp array of 6 doubles for lunar position and speed serr error
	 * string
	 */
	int swemoon(double tjd, int iflag, boolean do_save, double xpret[], StringBuffer serr) {
		...
		/*
		 * if planet has already been computed for this date, return if speed
		 * flag has been turned on, recompute planet
		 */
		speedf1 = pdp.xflgs & SweConst.SEFLG_SPEED;
		speedf2 = iflag & SweConst.SEFLG_SPEED;
		if (tjd == pdp.teval && pdp.iephe == SweConst.SEFLG_SWIEPH && ((speedf2 == 0) || (speedf1 != 0))) {
			xp = pdp.x;
		} else {
			/* call sweph for moon */
			retc = sweph(tjd, SwephData.SEI_MOON, SwephData.SEI_FILE_MOON, iflag, null, do_save, xp, serr);
			if (retc != SweConst.OK) {
				return (retc);
			}
			if (do_save) {
	...
	/*
	 * SWISSEPH this function computes 1. a barycentric planet plus, under
	 * certain conditions, 2. the barycentric sun, 3. the barycentric earth, and
	 * 4. the geocentric moon, in barycentric cartesian equatorial coordinates
	 * J2000.
	 *
	 * these are the data needed for calculation of light-time etc.
	 *
	 * tjd julian date ipli SEI_ planet number ifno ephemeris file number
	 * do_save write new positions in save area xp array of 6 doubles for
	 * planet's position and velocity xpe earth's xps sun's xpm moon's serr
	 * error string
	 *
	 * xp - xpm can be NULL. if do_save is TRUE, all of them can be NULL. the
	 * positions will be written into the save area (swed.pldat[ipli].x)
	 */
	int sweplan(double tjd, int ipli, int ifno, int iflag, boolean do_save, double xpret[], double xperet[],
			double xpsret[], double xpmret[], StringBuffer serr) {
		int i, retc;
		 ...
		 * file are heliocentric, other ones are barycentric. without xps, the
		 * heliocentric ones cannot be returned barycentrically.
		 */
		if (do_save || ipli == SwephData.SEI_SUNBARY || (pdp.iflg & SwephData.SEI_FLG_HELIO) != 0 || xpsret != null
		...
		if (do_save || ipli == SwephData.SEI_MOON || ipli == SwephData.SEI_EARTH || xperet != null || xpmret != null) {
			do_moon = true;
		}
		if (do_save) {
			xp = pdp.x;
		...
		if (do_sunbary) {
			...
			/*
			 * if planet has already been computed for this date, return if
			 * speed flag has been turned on, recompute planet
			 */
			if (tjd == psbdp.teval && psbdp.iephe == SweConst.SEFLG_SWIEPH && ((speedf2 == 0) || (speedf1 != 0))) {
				for (i = 0; i <= 5; i++) {
					xps[i] = psbdp.x[i];
				}
			} else {
				retc = sweph(tjd, SwephData.SEI_SUNBARY, SwephData.SEI_FILE_PLANET, iflag, null, do_save, xps,
						serr);/**/
				if (retc != SweConst.OK) {
					return (retc);
				}
		...
		if (do_moon) {
			...
			if (tjd == pmdp.teval && pmdp.iephe == SweConst.SEFLG_SWIEPH && ((speedf2 == 0) || (speedf1 != 0))) {
				for (i = 0; i <= 5; i++) {
					xpm[i] = pmdp.x[i];
				}
			} else {
				retc = sweph(tjd, SwephData.SEI_MOON, SwephData.SEI_FILE_MOON, iflag, null, do_save, xpm, serr);
				if (retc == SweConst.ERR) {
					return (retc);
				}
		...
		if (do_earth) {
			...
			if (tjd == pebdp.teval && pebdp.iephe == SweConst.SEFLG_SWIEPH && ((speedf2 == 0) || (speedf1 != 0))) {
				for (i = 0; i <= 5; i++) {
					xpe[i] = pebdp.x[i];
				}
			} else {
				retc = sweph(tjd, SwephData.SEI_EMB, SwephData.SEI_FILE_PLANET, iflag, null, do_save, xpe, serr);
				if (retc != SweConst.OK) {
					return (retc);
				}
				...
				embofs(xpe, 0, xpm, 0);
				/*
				 * speed is needed, if 1. true position is being computed before
				 * applying light-time etc. this is the position saved in
				 * pdp->x. in this case, speed is needed for light-time
				 * correction. 2. the speed flag has been specified.
				 */
		...
		if (ipli == SwephData.SEI_MOON) {
			...
			if (tjd == pdp.teval && pdp.iephe == SweConst.SEFLG_SWIEPH && ((speedf2 == 0) || (speedf1 != 0))) {
				...
				/*
				 * if planet is heliocentric, it must be transformed to
				 * barycentric
				 */
	...
	/*
	 * jpl ephemeris. this function computes 1. a barycentric planet position
	 * plus, under certain conditions, 2. the barycentric sun, 3. the
	 * barycentric earth, in barycentric cartesian equatorial coordinates J2000.
	 * 
	 * tjd julian day ipli sweph internal planet number do_save write new
	 * positions in save area xp array of 6 doubles for planet's position and
	 * speed vectors xpe earth's xps sun's serr pointer to error string
	 *
	 * xp - xps can be NULL. if do_save is TRUE, all of them can be NULL. the
	 * positions will be written into the save area (swed.pldat[ipli].x)
	 */
	int jplplan(double tjd, int ipli, int iflag, boolean do_save, double xpret[], double xperet[], double xpsret[],
		 ...
		 */
		/*
		 * double tjd_tdb, T; T = (tjd - 2451545.0)/36525.0; tjd_tdb = tjd +
		 * (0.001657 * sin(628.3076 * T + 6.2401) + 0.000022 * sin(575.3385 * T
		 * + 4.2970) + 0.000014 * sin(1256.6152 * T + 6.1969)) / 8640.0;
		 */
		if (do_save) {
			xp = pdp.x;
			xpe = pedp.x;
		...
		if (do_save || ipli == SwephData.SEI_EARTH || xperet != null || (ipli == SwephData.SEI_MOON)) {
			/*
		...
		if (do_save || ipli == SwephData.SEI_SUNBARY || xpsret != null || (ipli == SwephData.SEI_MOON)) {
		...
		if (!swed.jpl_file_is_open) {
			retc = open_jpl_file(ss, swed.jplfnam, swed.ephepath, serr);
			if (retc != SweConst.OK) {
				throw new SwissephException(tjd, SwissephException.FILE_OPEN_FAILED, retc, serr);
			}
		}
		if (do_earth) {
			/* barycentric earth */
	...
	/*
	 * this function looks for an ephemeris file, opens it, if not yet open,
	 * reads constants, if not yet read, computes a planet, if not yet computed
	 * attention: asteroids are heliocentric other planets barycentric
	 *
	 * tjd julian date ipli SEI_ planet number ifno ephemeris file number xsunb
	 * INPUT (!) array of 6 doubles containing barycentric sun (must be given
	 * with asteroids) do_save boolean: save result in save area xp return array
	 * of 6 doubles for planet's position serr error string
	 */
	int sweph(double tjd, int ipli, int ifno, int iflag, double xsunb[], boolean do_save, double xpret[],
			StringBuffer serr) {
		...
		if (fdp.fptr != null) {
			/*
			 * if tjd is beyond file range, close old file. if new asteroid,
			 * close old file.
			 */
			if (tjd < fdp.tfstart || tjd > fdp.tfend || (ipl == SwephData.SEI_ANYBODY && ipli != pdp.ibdy)) {
				try {
					fdp.fptr.close();
				} catch (java.io.IOException e) {
					// NBT
		...
		if (fdp.fptr == null) {
			...
			while (fdp.fptr == null) {
				...
				if (fdp.fptr == null) { // &&
					...
					/*
					 * if it is a numbered asteroid file, try also for short
					 * files (..s.se1) On the second try, the inserted 's' will
					 * be seen and not tried again.
					 */
					if (ipli > SweConst.SE_AST_OFFSET) {
						if (s.indexOf("s.") < 0) {
						...
						/*
						 * if we still have 'ast0' etc. in front of the
						 * filename, we remove it now, remove the 's' also, and
						 * try in the main ephemeris directory instead of the
						 * asteroid subdirectory.
						 */
						s = s.substring(0, s.indexOf("s.")) + s.substring(s.indexOf("s.") + 1);
						if (subdirlen > 0
								&& s.startsWith(subdirnam.substring(0, SMath.min(subdirnam.length(), subdirlen)))) {
		...
		/*
		 * if first ephemeris file (J-3000), it might start a mars period after
		 * -3000. if last ephemeris file (J3000), it might end a 4000-day-period
		 * before 3000.
		 */
		...
		if (pdp.segp == null || tjd < pdp.tseg0 || tjd > pdp.tseg1) {
			...
			/*
			 * rotate cheby coeffs back to equatorial system. if necessary, add
			 * reference orbit.
			 */
		...
		/*
		 * speed is needed, if 1. true position is being computed before
		 * applying light-time etc. this is the position saved in pdp->x. in
		 * this case, speed is needed for light-time correction. 2. the speed
		...
		/*
		 * if planet wanted is barycentric sun and must be computed from
		 * heliocentric earth and barycentric earth: the computation above gives
		 * heliocentric earth, therefore we have to compute barycentric earth
		 * and subtract heliocentric earth from it. this may be necessary with
		 * calls from sweplan() and from app_pos_etc_sun() (light-time).
		 */
		if (ipl == SwephData.SEI_SUNBARY && (pdp.iflg & SwephData.SEI_FLG_EMBHEL) != 0) {
			/*
			 * sweph() calls sweph() !!! for EMB. Attention: a new calculation
			 * must be forced in any case. Otherwise EARTH (instead of EMB) will
			 * possibly taken from save area. to force new computation, set
			 * pedp->teval = 0 and restore it after call of sweph(EMB).
			 */
			tsv = pedp.teval;
			pedp.teval = 0;
			retc = sweph(tjd, SwephData.SEI_EMB, ifno, iflag | SweConst.SEFLG_SPEED, null, SwephData.NO_SAVE, xemb,
					serr);
			if (retc != SweConst.OK) {
				return (retc);
			}
		...
		/*
		 * asteroids are heliocentric. if JPL or SWISSEPH, convert to
		 * barycentric
		 */
		if ((iflag & SweConst.SEFLG_JPLEPH) != 0 || (iflag & SweConst.SEFLG_SWIEPH) != 0) {
		...
		if (do_save) {
			...
			if (ifno == SwephData.SEI_FILE_PLANET || ifno == SwephData.SEI_FILE_MOON) {
				pdp.iephe = SweConst.SEFLG_SWIEPH;/**/
			} else {
	...
	FilePtr swi_fopen(int ifno, String fname, String ephepath, StringBuffer serr) throws SwissephException {
		//// #ifdef TRACE0
		// Trace.level++;
		// Trace.log("SwissEph.swi_fopen(int, String <" + fname + ">, String,
		//// StringBuffer)");
		//// #ifdef TRACE1
		// Trace.log(" ifno: " + ifno + "\n fname: " + fname + "\n ephepath: " +
		//// ephepath + "\n serr: " + serr);
		...
		for (i = 0; i < np; i++) {
			...
			if (s.length() + fname.length() < SwissData.AS_MAXCH) {
				...
				if (serr != null) {
					...
					throw new SwissephException(1. / 0., SwissephException.INVALID_FILE_NAME, SweConst.ERR, serr);
				}
			}
			fnamp = s;
			try {
		...
		// Trace.level--;
		//// #endif /* TRACE0 */
		throw new SwissephException(1. / 0., SwissephException.FILE_NOT_FOUND, SwephData.NOT_AVAILABLE, serr);
	}
	...
	private FilePtr tryFileAsURL(String fnamp, int ifno) {
		...
		try {
			...
			String sht = "HEAD " + fnamp + " HTTP/1.1\r\n" + "User-Agent: " + FilePtr.useragent + "\r\n" + "Host: "
					+ u.getHost() + ":" + (u.getPort() < 0 ? 80 : u.getPort()) + "\r\n\r\n";
	...
	/*
	 * converts planets from barycentric to geocentric, apparent positions
	 * precession and nutation according to flags ipli planet number iflag flags
	 * serr error string
	 */
	int app_pos_etc_plan(int ipli, int iflag, StringBuffer serr) {
		...
		} else if (ipli == SwephData.SEI_CHIRON || ipli == SwephData.SEI_PHOLUS || ipli == SwephData.SEI_CERES
				|| ipli == SwephData.SEI_PALLAS || ipli == SwephData.SEI_JUNO || ipli == SwephData.SEI_VESTA) {
			ifno = SwephData.SEI_FILE_MAIN_AST;
		...
		/*
		 * if the same conversions have already been done for the same date,
		 * then return
		 */
		...
		if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
			if (swed.topd.teval != pedp.teval || pedp.teval == 0) {
				if (swi_get_observer(pedp.teval, iflag | SweConst.SEFLG_NONUT, SwephData.DO_SAVE, xobs,
						serr) != SweConst.OK) {
					return SweConst.ERR;
				}
			} else {
		...
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0) {
			...
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				 ...
				 * arc second. To take this into account, we compute 1. true
				 * position - apparent position at time t - 1. 2. true position
				 * - apparent position at time t. 3. the difference between the
				 * two is the part of the daily motion that results from the
				 * change of dt.
				 */
				for (i = 0; i <= 2; i++) {
			...
			case SweConst.SEFLG_JPLEPH:
				...
				if ((iflag & SweConst.SEFLG_SPEED) != 0 && (iflag & SweConst.SEFLG_HELCTR) == 0
			...
			case SweConst.SEFLG_SWIEPH:
				if (ibody == SwephData.IS_PLANET) {
					retc = sweplan(t, ipli, ifno, iflag, SwephData.NO_SAVE, xx, xearth, xsun, null, serr);
				} else { /* asteroid */
					retc = sweplan(t, SwephData.SEI_EARTH, SwephData.SEI_FILE_PLANET, iflag, SwephData.NO_SAVE, xearth,
							null, xsun, null, serr);
					if (retc == SweConst.OK) {
						retc = sweph(t, ipli, ifno, iflag, xsun, SwephData.NO_SAVE, xx, serr);
					}
			...
			default:
				 ...
				 */
				/*
				 * if speed flag is true, we call swi_moshplan() for new t. this
				 * does not increase position precision, but speed precision,
				 * which becomes better than 0.01"/day. for precise speed, we
				 * need earth as well.
				 */
				if ((iflag & SweConst.SEFLG_SPEED) != 0
						&& (iflag & (SweConst.SEFLG_HELCTR | SweConst.SEFLG_BARYCTR)) == 0) {
					if (ibody == SwephData.IS_PLANET) {
						retc = smosh.swi_moshplan(t, ipli, SwephData.NO_SAVE, xxsv, xearth, serr);
					} else { /* if asteroid */
						retc = sweph(t, ipli, ifno, iflag, null, SwephData.NO_SAVE, xxsv, serr);
						if (retc == SweConst.OK) {
							retc = smosh.swi_moshplan(t, SwephData.SEI_EARTH, SwephData.NO_SAVE, xearth, xearth, serr);
						}
					}
					if (retc != SweConst.OK) {
						return (retc);
					 ...
					 * agree. The difference would be about 0.01", which is far
					 * below the intrinsic error of the moshier ephemeris.
					 */
					for (i = 3; i <= 5; i++) {
						xx[i] = xxsv[i];
					}
			...
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				...
				if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
					if (swi_get_observer(t, iflag | SweConst.SEFLG_NONUT, SwephData.NO_SAVE, xobs2,
							serr) != SweConst.OK) {
						return SweConst.ERR;
					}
					for (i = 0; i <= 5; i++) {
		...
		if ((iflag & SweConst.SEFLG_HELCTR) == 0 && (iflag & SweConst.SEFLG_BARYCTR) == 0) {
			...
			if ((iflag & SweConst.SEFLG_TRUEPOS) == 0) {
				/*
				 * Apparent speed is also influenced by the change of dt during
				 * motion. Neglect of this would result in an error of several
				 * 0.01"
				 */
				if ((iflag & SweConst.SEFLG_SPEED) != 0) {
					for (i = 3; i <= 5; i++) {
		 ...
		 ************************************/
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && (iflag & SweConst.SEFLG_NOGDEFL) == 0) {
		 ...
		 **********************************/
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && (iflag & SweConst.SEFLG_NOABERR) == 0) {
			/* SEFLG_NOABERR is on, if SEFLG_HELCTR or SEFLG_BARYCTR */
			swi_aberr_light(xx, xobs, iflag);
			/*
			 * Apparent speed is also influenced by the difference of speed of
			 * the earth between t and t-dt. Neglecting this would involve an
			 * error of several 0.1"
			 */
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				for (i = 3; i <= 5; i++) {
					xx[i] += xobs[i] - xobs2[i];
	...
	int app_pos_rest(PlanData pdp, int iflag, double[] xx, double[] x2000, Epsilon oe, StringBuffer serr) {
		...
		/************************************************
		 * transformation to ecliptic. * with sidereal calc. this will be
		...
		if ((iflag & SweConst.SEFLG_SIDEREAL) != 0) {
			...
			if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_ECL_T0) != 0) {
				if (swi_trop_ra2sid_lon(x2000, pdp.xreturn, 6, pdp.xreturn, 18, iflag, serr) != SweConst.OK) {
					return SweConst.ERR;
				}
				/* project onto solar system equator */
			} else if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_SSY_PLANE) != 0) {
				if (swi_trop_ra2sid_lon_sosy(x2000, pdp.xreturn, 6, pdp.xreturn, 18, iflag, serr) != SweConst.OK) {
					return SweConst.ERR;
				}
			} else {
	...
	/*
	 * input coordinates are J2000, cartesian. xout ecliptical sidereal position
	...
	int swi_trop_ra2sid_lon(double[] xin, double[] xout, int xoOffs, double[] xoutr, int xrOffs, int iflag,
			StringBuffer serr) {
	...
	int swi_trop_ra2sid_lon_sosy(double[] xin, double[] xout, int xoOffs, double[] xoutr, int xrOffs, int iflag,
			StringBuffer serr) {
	...
	int app_pos_etc_plan_osc(int ipl, int ipli, int iflag, StringBuffer serr) {
		...
		double xxsv[] = new double[6], xxsp[] = new double[] { 0, 0, 0 }, xobs[] = new double[6],
				xobs2[] = new double[6];
		double t;
		PlanData pdp = swed.pldat[ipli];
		PlanData pedp = swed.pldat[SwephData.SEI_EARTH];
		...
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0) {
			...
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				 ...
				 * position - apparent position at time t - 1. 2. true position
				 * - apparent position at time t. 3. the difference between the
				 * two is the daily motion resulting from the change of dt.
				 */
				for (i = 0; i <= 2; i++) {
					xxsv[i] = xxsp[i] = xx[i] - xx[i + 3];
				}
				for (j = 0; j <= niter; j++) {
					...
					for (i = 0; i <= 2; i++) {
						xxsp[i] = xxsv[i] - dt
								* pdp.x[i + 3];/* rough apparent position */
					}
				}
				/* true position - apparent position at time t-1 */
				for (i = 0; i <= 2; i++) {
			...
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				...
				retc = main_planet_bary(t, SwephData.SEI_EARTH, epheflag, iflag, SwephData.NO_SAVE, xearth, xearth,
						xsun, xmoon, serr);
				if (smosh.swi_osc_el_plan(t, xx, ipl - SweConst.SE_FICT_OFFSET, ipli, xearth, xsun,
						serr) != SweConst.OK) {
					return (SweConst.ERR);
				}
				if (retc != SweConst.OK) {
		...
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0) {
			/*
			 * Apparent speed is also influenced by the change of dt during
			 * motion. Neglect of this would result in an error of several 0.01"
			 */
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				for (i = 3; i <= 5; i++) {
					xx[i] -= xxsp[i - 3];
	...
	/*
	 * influence of precession on speed xx position and speed of planet in
	...
	void swi_precess_speed(double xx[], int xOffs, double t, int iflag, int direction) {
		...
		if (prec_model == 0)
			prec_model = SweConst.SEMOD_PREC_DEFAULT;
		...
		/*
		 * first correct rotation. this costs some sines and cosines, but
		 * neglect might involve an error > 1"/day
		 */
	...
	/*
	 * multiplies cartesian equatorial coordinates with previously calculated
	 * nutation matrix. also corrects speed.
	 */
	void swi_nutate(double xx[], int offs, int iflag, boolean backward) {
		...
		for (i = 0; i <= 2; i++) {
			if (backward) {
				x[i] = xx[0 + offs] * swed.nut.matrix[i][0] + xx[1 + offs] * swed.nut.matrix[i][1]
						+ xx[2 + offs] * swed.nut.matrix[i][2];
			} else {
				x[i] = xx[0 + offs] * swed.nut.matrix[0][i] + xx[1 + offs] * swed.nut.matrix[1][i]
		...
		if ((iflag & SweConst.SEFLG_SPEED) != 0) {
			...
			for (i = 0; i <= 2; i++) {
				if (backward) {
					x[i + 3] = xx[3 + offs] * swed.nut.matrix[i][0] + xx[4 + offs] * swed.nut.matrix[i][1]
							+ xx[5 + offs] * swed.nut.matrix[i][2];
				} else {
					x[i + 3] = xx[3 + offs] * swed.nut.matrix[0][i] + xx[4 + offs] * swed.nut.matrix[1][i]
							+ xx[5 + offs] * swed.nut.matrix[2][i];
				}
			}
			/*
			 * then apparent motion due to change of nutation during day. this
			 * makes a difference of 0.01"
			 */
			for (i = 0; i <= 2; i++) {
				if (backward) {
					xv[i] = xx[0 + offs] * swed.nutv.matrix[i][0] + xx[1 + offs] * swed.nutv.matrix[i][1]
							+ xx[2 + offs] * swed.nutv.matrix[i][2];
				} else {
					xv[i] = xx[0 + offs] * swed.nutv.matrix[0][i] + xx[1 + offs] * swed.nutv.matrix[1][i]
	...
	/*
	 * computes 'annual' aberration xx planet's position accounted for
	 * light-time and gravitational light deflection xe earth's position and
	 * speed
	 */
	...
	/*
	 * computes relativistic light deflection by the sun ipli sweph internal
	 * planet number xx planet's position accounted for light-time dt dt of
	 * light-time
	 */
	void swi_deflect_light(double xx[], int offs, double dt, int iflag) {
		 ...
		 * conjunction, the formula for the deflection angle as given in Expl.
		 * Suppl. p. 136 cannot be used. The deflection seems to increase
		 * rapidly towards infinity. The reason is that the formula considers
		 * the sun as a point mass. AA recommends to set deflection = 0 in such
		 * a case. However, to get a continous motion, we modify the formula for
		 * a non-point-mass, taking into account the mass distribution within
		 * the sun. For more info, s. meff().
		 */
		sina = SMath.sqrt(1 - ue * ue); /* sin(angle) between sun and planet */
		sin_sunr = SwephData.SUN_RADIUS
				/ re; /* sine of sun radius (= sun radius) */
		if (sina < sin_sunr) {
		...
		if ((iflag & SweConst.SEFLG_SPEED) != 0) {
			/*
			 * correction of speed influence of light deflection on a planet's
			 * apparent speed: for an outer planet at the solar limb with
			 * |v(planet) - v(sun)| = 1 degree, this makes a difference of
			 * 7"/day. if the planet is within the solar disc, the difference
			 * may increase to 30" or more. e.g. mercury at j2434871.45:
			 * distance from sun 45" 1. speed without deflection 2d10'10".4034
			 * 2. speed with deflection 2d10'42".8460 (-speed flag) 3. speed
			 * with deflection 2d10'43".4824 (< 3 positions/ -speed3 flag) 3. is
			 * not very precise. Smaller dt would give result closer to 2., but
			 * will probably never be as good as 2, unless long doubles are
			 * used. (try also j2434871.46!!) however, in such a case speed
			 * changes rapidly. before being passed by the sun, the planet
			 * accelerates, and after the sun has passed it slows down. some
			 * time later it regains 'normal' speed. to compute speed, we do the
			 * same calculation as above with slightly different u, e, q, and
			 * find out the difference in deflection.
			 */
			...
			qe = dot_prod(q, e);
			sina = SMath
					.sqrt(1 - ue * ue); /* sin(angle) between sun and planet */
			sin_sunr = SwephData.SUN_RADIUS
					/ re; /* sine of sun radius (= sun radius) */
			if (sina < sin_sunr) {
				meff_fact = meff(sina / sin_sunr);
	...
	/*
	 * converts the sun from barycentric to geocentric, the earth from
	 * barycentric to heliocentric computes apparent position, precession, and
	 * nutation according to flags iflag flags serr error string
	 */
	private int app_pos_etc_sun(int iflag, StringBuffer serr) {
		 ...
		 ***************************************/
		if (pedp.iephe == SweConst.SEFLG_MOSEPH || (iflag & SweConst.SEFLG_BARYCTR) != 0) {
			for (i = 0; i <= 5; i++) {
				xx[i] = xobs[i];
			}
		} else {
		...
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0) {
			/*
			 * number of iterations - 1 the following if() does the following:
			 * with jpl and swiss ephemeris: with geocentric computation of sun:
			 * light-time correction of barycentric sun position. with
			 * heliocentric or barycentric computation of earth: light-time
			 * correction of barycentric earth position. with moshier ephemeris
			 * (heliocentric!!!): with geocentric computation of sun: nothing!
			 * (aberration will be done later) with heliocentric or barycentric
			 * computation of earth: light-time correction of heliocentric earth
			 * position.
			 */
			if (pedp.iephe == SweConst.SEFLG_JPLEPH || pedp.iephe == SweConst.SEFLG_SWIEPH
					|| (iflag & SweConst.SEFLG_HELCTR) != 0 || (iflag & SweConst.SEFLG_BARYCTR) != 0) {
				for (i = 0; i <= 5; i++) {
					xearth[i] = xobs[i];
				...
				for (j = 0; j <= niter; j++) {
					...
					switch (pedp.iephe) {
					/*
					 * if geocentric sun, new sun at t' if heliocentric or
					 * barycentric earth, new earth at t'
					 */
					case SweConst.SEFLG_JPLEPH:
						try {
							if ((iflag & SweConst.SEFLG_HELCTR) != 0 || (iflag & SweConst.SEFLG_BARYCTR) != 0) {
					...
					case SweConst.SEFLG_SWIEPH:
						/*
						 * retc = sweph(t, SEI_SUN, SEI_FILE_PLANET, iflag,
						 * NULL, NO_SAVE, xearth, serr);
						 */
						if ((iflag & SweConst.SEFLG_HELCTR) != 0 || (iflag & SweConst.SEFLG_BARYCTR) != 0) {
							retc = sweplan(t, SwephData.SEI_EARTH, SwephData.SEI_FILE_PLANET, iflag, SwephData.NO_SAVE,
									xearth, null, xsun, null, serr);
						} else {
							retc = sweph(t, SwephData.SEI_SUNBARY, SwephData.SEI_FILE_PLANET, iflag, null,
	...
	/*
	 * transforms the position of the moon: heliocentric position barycentric
	 * position astrometric position apparent position precession and nutation
	 *
	 * note: for apparent positions, we consider the earth-moon system as
	 * independant. for astrometric positions (SEFLG_NOABERR), we consider the
	 * motions of the earth and the moon related to the solar system barycenter.
	 */
	int app_pos_etc_moon(int iflag, StringBuffer serr) {
		int i;
		int flg1, flg2;
		double xx[] = new double[6], xxsv[] = new double[6], xobs[] = new double[6], xxm[] = new double[6],
				xs[] = new double[6], xe[] = new double[6], xobs2[] = new double[6], dt;
		PlanData pedp = swed.pldat[SwephData.SEI_EARTH];
		PlanData psdp = swed.pldat[SwephData.SEI_SUNBARY];
		PlanData pdp = swed.pldat[SwephData.SEI_MOON];
		Epsilon oe = swed.oec;
		...
		if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
			if (swed.topd.teval != pdp.teval || swed.topd.teval == 0) {
				if (swi_get_observer(pdp.teval, iflag | SweConst.SEFLG_NONUT, SwephData.DO_SAVE, xobs,
						null) != SweConst.OK) {
					return SweConst.ERR;
				}
			} else {
		 ...
		 *******************************/
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0) {
			dt = SMath.sqrt(sl.square_sum(xxm)) * SweConst.AUNIT / SwephData.CLIGHT / 86400.0;
			t = pdp.teval - dt;
			switch (pdp.iephe) {
			case SweConst.SEFLG_JPLEPH:
				try {
				...
				break;
			case SweConst.SEFLG_SWIEPH:
				retc = sweplan(t, SwephData.SEI_MOON, SwephData.SEI_FILE_MOON, iflag, SwephData.NO_SAVE, xx, xe, xs,
						null, serr);
				if (retc != SweConst.OK) {
					return (retc);
				}
			...
			if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
				if (swi_get_observer(t, iflag | SweConst.SEFLG_NONUT, SwephData.NO_SAVE, xobs2, null) != SweConst.OK) {
					return SweConst.ERR;
				}
				for (i = 0; i <= 5; i++) {
					xobs2[i] += xe[i];
		...
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && (iflag & SweConst.SEFLG_NOABERR) == 0) {
			...
			/*
			 * Apparent speed is also influenced by the difference of speed of
			 * the earth between t and t-dt. Neglecting this would lead to an
			 * error of several 0.1"
			 */
			if ((iflag & SweConst.SEFLG_SPEED) != 0) {
				for (i = 3; i <= 5; i++) {
	...
	/*
	 * transforms the position of the barycentric sun: precession and nutation
	 * according to flags iflag flags serr error string
	 */
	...
	/*
	 * transforms position of mean lunar node or apogee: input is polar
	 * coordinates in mean ecliptic of date. output is, according to iflag:
	 * position accounted for light-time position referred to J2000 (i.e.
	 * precession subtracted) position with nutation equatorial coordinates
	 * cartesian coordinates heliocentric position is not allowed ??????????????
	 * DAS WAERE ZIEMLICH AUFWENDIG. SONNE UND ERDE MUESSTEN SCHON VORHANDEN
	 * SEIN! ipl bodynumber (SE_MEAN_NODE or SE_MEAN_APOG) iflag flags serr
	 * error string
	 */
	int app_pos_etc_mean(int ipl, int iflag, StringBuffer serr) {
		...
		if (((iflag & SweConst.SEFLG_SIDEREAL) != 0 && (swed.sidd.sid_mode & SweConst.SE_SIDBIT_ECL_T0) != 0)
	...
	/*
	 * SWISSEPH adds reference orbit to chebyshew series (if SEI_FLG_ELLIPSE),
	...
	void rot_back(int ipli) {
		 ...
		 * origin of longitudes and third axis along angular momentum this uses
		 * the standard formulas for equinoctal variables (see papers by broucke
		 * and by cefola).
		 */
	 ...
	 *
	 * xemb = hel./bar. position or velocity vectors of emb (input) earth
	 * (output) xmoon= geocentric position or velocity vector of moon
	 */
	...
	/*
	 * calculates the nutation matrix nu pointer to nutation data structure oe
	 * pointer to epsilon data structure
	 */
	...
	/*
	 * lunar osculating elements, i.e. osculating node ('true' node) and
	 * osculating apogee ('black moon', 'lilith'). tjd julian day ipl body
	 * number, i.e. SEI_TRUE_NODE or SEI_OSCU_APOG iflag flags (which ephemeris,
	 * nutation, etc.) serr error string
	 *
	 * definitions and remarks: the osculating node and the osculating apogee
	 * are defined as the orbital elements of the momentary lunar orbit. their
	 * advantage is that when the moon crosses the ecliptic, it is really at the
	 * osculating node, and when it passes its greatest distance from earth it
	 * is really at the osculating apogee. with the mean elements this is not
	 * the case. (some define the apogee as the second focus of the lunar
	 * ellipse. but, as seen from the geocenter, both points are in the same
	 * direction.) problems: the osculating apogee is given in the 'New
	 * International Ephemerides' (Editions St. Michel) as the 'True Lilith'.
	 * however, this name is misleading. this point is based on the idea that
	 * the lunar orbit can be approximated by an ellipse. arguments against
	 * this: 1. this procedure considers celestial motions as two body problems.
	 * this is quite good for planets, but not for the moon. the strong
	 * gravitational attraction of the sun destroys the idea of an ellipse. 2.
	 * the NIE 'True Lilith' has strong oscillations around the mean one with an
	 * amplitude of about 30 degrees. however, when the moon is in apogee, its
	 * distance from the mean apogee never exceeds 5 degrees. besides, the
	 * computation of NIE is INACCURATE. the mistake reaches 20 arc minutes.
	 * According to Santoni, the point was calculated using 'les 58 premiers
	 * termes correctifs au Perigee moyen' published by Chapront and
	 * Chapront-Touze. And he adds: "Nous constatons que meme en utilisant ces
	 * 58 termes CORRECTIFS, l'erreur peut atteindre 0,5d!" (p. 13) We avoid
	 * this error, computing the orbital elements directly from the position and
	 * the speed vector.
	 *
	 * how about the node? it is less problematic, because we we needn't derive
	 * it from an orbital ellipse. we can say: the axis of the osculating nodes
	 * is the intersection line of the actual orbital plane of the moon and the
	 * plane of the ecliptic. or: the osculating nodes are the intersections of
	 * the two great circles representing the momentary apparent orbit of the
	 * moon and the ecliptic. in this way they make some sense. then, the nodes
	 * are really an axis, and they have no geocentric distance. however, in
	 * this routine we give a distance derived from the osculating ellipse. the
	 * node could also be defined as the intersection axis of the lunar orbital
	 * plane and the solar orbital plane, which is not precisely identical to
	 * the ecliptic. this would make a difference of several arcseconds.
	 *
	 * is it possible to keep the idea of a continuously moving apogee that is
	 * exact at the moment when the moon passes its greatest distance from
	 * earth? to achieve this, we would probably have to interpolate between the
	 * actual apogees. the nodes could also be computed by interpolation. the
	 * resulting nodes would deviate from the so-called 'true node' by less than
	 * 30 arc minutes.
	 *
	 * sidereal and j2000 true node are first computed for the ecliptic of epoch
	 * and then precessed to ecliptic of t0(ayanamsa) or J2000. there is another
	 * procedure that computes the node for the ecliptic of t0(ayanamsa) or
	 * J2000. it is excluded by #ifdef SID_TNODE_FROM_ECL_T0
	 */
	private int lunar_osc_elem(double tjd, int ipl, int iflag, StringBuffer serr) {
		...
		double speed_intv = SwephData.NODE_CALC_INTV; /*
														 ...
														 */
		double a, b;
		double xpos[][] = new double[3][6], xx[][] = new double[3][6], xxa[][] = new double[3][6];
		double xp[];
		...
		ndp = swed.nddat[ipl];
		/*
		 * if elements have already been computed for this date, return if speed
		 * flag has been turned on, recompute
		 */
		flg1 = iflag & ~SweConst.SEFLG_EQUATORIAL & ~SweConst.SEFLG_XYZ;
		flg2 = ndp.xflgs & ~SweConst.SEFLG_EQUATORIAL & ~SweConst.SEFLG_XYZ;
		speedf1 = ndp.xflgs & SweConst.SEFLG_SPEED;
		speedf2 = iflag & SweConst.SEFLG_SPEED;
		if (tjd == ndp.teval && tjd != 0 && flg1 == flg2 && ((speedf2 == 0) || (speedf1 != 0))) {
			ndp.xflgs = iflag;
			ndp.iephe = iflag & SweConst.SEFLG_EPHMASK;
			return SweConst.OK;
		}
		/*
		 * the geocentric position vector and the speed vector of the moon make
		 * up the lunar orbital plane. the position vector of the node is along
		 * the intersection line of the orbital plane and the plane of the
		 * ecliptic. to calculate the osculating node, we need one lunar
		 * position with speed. to calculate the speed of the osculating node,
		 * we need three lunar positions and the speed of each of them. this is
		 * relatively cheap, if the jpl-moon or the swisseph moon is used. with
		 * the moshier moon this is much more expensive, because then we need 9
		 * lunar positions for three speeds. but one position and speed can
		 * normally be taken from swed.pldat[moon], which corresponds to three
		 * moshier moon calculations. the same is also true for the osculating
		 * apogee: we need three lunar positions and speeds.
		 */
		/*********************************************
		...
		/*
		 * there may be a moon of wrong ephemeris in save area force new
		 * computation:
		 */
		...
		do {
			...
			case SweConst.SEFLG_JPLEPH:
				...
				for (i = istart; i <= 2; i++) {
					...
					xp = xpos[i];
					try {
						retc = jplplan(t, ipli, iflag, SwephData.NO_SAVE, xp, null, null, serr);
					} catch (SwissephException swe) {
						retc = swe.getRC();
						/* read error or corrupt file */
						if (retc == SweConst.ERR) {
					...
					/*
					 * light-time-corrected moon for apparent node this makes a
					 * difference of several milliarcseconds with the node and
					 * 0.1" with the apogee. the simple formual 'x[j] -= dt *
					 * speed' should not be used here. the error would be
					 * greater than the advantage of computation speed.
					 */
					if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && retc >= SweConst.OK) {
						dt = SMath.sqrt(sl.square_sum(xpos[i])) * SweConst.AUNIT / SwephData.CLIGHT / 86400.0;
						try {
							retc = jplplan(t - dt, ipli, iflag, SwephData.NO_SAVE, xpos[i], null, null, serr); /**/
						} catch (SwissephException swe) {
							retc = swe.getRC();
							/* read error or corrupt file */
							if (retc == SweConst.ERR) {
					...
					retc = swi_plan_for_osc_elem(iflag | SweConst.SEFLG_SPEED, t,
							xpos[i]); /* retc is always ok */
			...
			case SweConst.SEFLG_SWIEPH:
				...
				for (i = istart; i <= 2; i++) {
					...
					retc = swemoon(t, iflag | SweConst.SEFLG_SPEED, SwephData.NO_SAVE, xpos[i], serr);/**/
					if (retc == SweConst.ERR) {
						return (SweConst.ERR);
					}
					...
					if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && retc >= SweConst.OK) {
						dt = SMath.sqrt(sl.square_sum(xpos[i])) * SweConst.AUNIT / SwephData.CLIGHT / 86400.0;
						retc = swemoon(t - dt, iflag | SweConst.SEFLG_SPEED, SwephData.NO_SAVE, xpos[i], serr);/**/
						if (retc == SweConst.ERR) {
							return (SweConst.ERR);
						}
					}
			...
			case SweConst.SEFLG_MOSEPH:
				/*
				 * with moshier moon, we need a greater speed_intv, because here
				 * the node and apogee oscillate wildly within small intervals
				 */
		...
		/*
		 * now we have the correct direction of the node, the intersection of
		 * the lunar plane and the ecliptic plane. the distance is the distance
		 * of the point where the tangent of the lunar motion penetrates the
		 * ecliptic plane. this can be very large, e.g. j2415080.37372. below, a
		 * new distance will be derived from the osculating ellipse.
		 */
		...
		/************************************************************
		 * apogee with speed * must be computed anyway to get the node's
		 * distance *
		 ************************************************************/
		ndap = swed.nddat[SwephData.SEI_OSCU_APOG];
		Gmsm = SwephData.GEOGCONST * (1 + 1 / SwephData.EARTH_MOON_MRAT) / SweConst.AUNIT / SweConst.AUNIT
				/ SweConst.AUNIT * 86400.0 * 86400.0;
		/* three apogees */
		for (i = istart; i <= 2; i++) {
			/* node */
			...
			sl.swi_polcart(xxa[i], xxa[i]);
			/*
			 * new distance of node from orbital ellipse: true anomaly of node:
			 */
		...
		/**********************************************************************
		 * precession and nutation have already been taken into account because
		 * the computation is on the basis of lunar positions that have gone
		 * through swi_plan_for_osc_elem. light-time is already contained in
		 * lunar positions. now compute polar and equatorial coordinates:
		 **********************************************************************/
		double[] x = new double[6];
		for (j = 0; j <= 1; j++) {
			if (j == 0) {
			...
			if ((iflag & SweConst.SEFLG_NONUT) == 0) {
				sl.swi_coortrf2(ndp.xreturn, 18, ndp.xreturn, 18, -swed.nut.snut, swed.nut.cnut);
				if ((iflag & SweConst.SEFLG_SPEED) != 0) {
					sl.swi_coortrf2(ndp.xreturn, 21, ndp.xreturn, 21, -swed.nut.snut, swed.nut.cnut);
				}
			}
			/* polar equatorial */
			...
			if ((iflag & SweConst.SEFLG_SIDEREAL) != 0) {
				/*
				 * node and apogee are referred to t; the ecliptic position must
				 * be transformed to t0
				 */
				/* rigorous algorithm */
				if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_ECL_T0) != 0
						|| (swed.sidd.sid_mode & SweConst.SE_SIDBIT_SSY_PLANE) != 0) {
					...
					sl.swi_cartpol_sp(ndp.xreturn, 18, ndp.xreturn, 12);
					/*
					 * traditional algorithm; this is a bit clumsy, but allows
					 * us to keep the sidereal code together
					 */
				} else {
					sl.swi_cartpol_sp(ndp.xreturn, 6, ndp.xreturn, 0);
	...
	private int intp_apsides(double tjd, int ipl, int iflag, StringBuffer serr) {
		...
		int speedf1, speedf2;
		// TM - temporary inclusion for version 2.00.00 to give an end date to
		// -pg / -pc //
		if (tjd < SwephData.MOSHLUEPH_START || tjd > SwephData.MOSHLUEPH_END) {
			if (serr != null) {
				String s = "jd " + tjd + " outside Moshier's Moon range " + SwephData.MOSHLUEPH_START + " .. "
						+ SwephData.MOSHLUEPH_END + " ";
				if (serr.length() + s.length() < SwissData.AS_MAXCH) {
					serr.append(s);
				}
		...
		ndp = swed.nddat[ipl];
		/*
		 * if same calculation was done before, return if speed flag has been
		 * turned on, recompute
		 */
		flg1 = iflag & ~SweConst.SEFLG_EQUATORIAL & ~SweConst.SEFLG_XYZ;
		flg2 = ndp.xflgs & ~SweConst.SEFLG_EQUATORIAL & ~SweConst.SEFLG_XYZ;
		...
		for (t = tjd - speed_intv, i = 0; i < 3; t += speed_intv, i++) {
			if (((iflag & SweConst.SEFLG_SPEED) == 0) && i != 1)
				continue;
		...
		if ((iflag & SweConst.SEFLG_SIDEREAL) != 0) {
			/*
			 * apogee is referred to t; the ecliptic position must be
			 * transformed to t0
			 */
			/* rigorous algorithm */
			if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_ECL_T0) != 0
			...
			/*
			 * node and apogee are referred to t; the ecliptic position must be
	...
	/*
	 * transforms the position of the moon in a way we can use it for
	 * calculation of osculating node and apogee: precession and nutation
	 * (attention to speed vector!) according to flags iflag flags tjd time for
	 * which the element is computed i.e. date of ecliptic xx array equatorial
	 * cartesian position and speed serr error string
	 */
	int swi_plan_for_osc_elem(int iflag, double tjd, double xx[]) {
		...
		/************************************************
		 * precession, equator 2000 -> equator of date * attention: speed vector
		 * has to be rotated, * but daily precession 0.137" may not be added!
		 */
		...
		/************************************************
		 * nutation * again: speed vector must be rotated, but not * added
		 * 'speed' of nutation *
		 ************************************************/
		if ((iflag & SweConst.SEFLG_NONUT) == 0) {
	...
	static final MeffEle eff_arr[] = {
			/*
			 * r , m_eff for photon passing the sun at min distance r (fraction
			 * of Rsun) the values where computed with sun_model.c, which is a
			 * classic treatment of a photon passing a gravity field, multiplied
			 * by 2. The sun mass distribution m(r) is from Michael Stix, The
			 * Sun, p. 47.
			 */
			new MeffEle(1.000, 1.000000), new MeffEle(0.990, 0.999979), new MeffEle(0.980, 0.999940),
			new MeffEle(0.970, 0.999881), new MeffEle(0.960, 0.999811), new MeffEle(0.950, 0.999724),
			new MeffEle(0.940, 0.999622), new MeffEle(0.930, 0.999497), new MeffEle(0.920, 0.999354),
			new MeffEle(0.910, 0.999192), new MeffEle(0.900, 0.999000), new MeffEle(0.890, 0.998786),
			new MeffEle(0.880, 0.998535), new MeffEle(0.870, 0.998242), new MeffEle(0.860, 0.997919),
			new MeffEle(0.850, 0.997571), new MeffEle(0.840, 0.997198), new MeffEle(0.830, 0.996792),
			new MeffEle(0.820, 0.996316), new MeffEle(0.810, 0.995791), new MeffEle(0.800, 0.995226),
			new MeffEle(0.790, 0.994625), new MeffEle(0.780, 0.993991), new MeffEle(0.770, 0.993326),
			new MeffEle(0.760, 0.992598), new MeffEle(0.750, 0.991770), new MeffEle(0.740, 0.990873),
			new MeffEle(0.730, 0.989919), new MeffEle(0.720, 0.988912), new MeffEle(0.710, 0.987856),
			new MeffEle(0.700, 0.986755), new MeffEle(0.690, 0.985610), new MeffEle(0.680, 0.984398),
			new MeffEle(0.670, 0.982986), new MeffEle(0.660, 0.981437), new MeffEle(0.650, 0.979779),
			new MeffEle(0.640, 0.978024), new MeffEle(0.630, 0.976182), new MeffEle(0.620, 0.974256),
			new MeffEle(0.610, 0.972253), new MeffEle(0.600, 0.970174), new MeffEle(0.590, 0.968024),
			new MeffEle(0.580, 0.965594), new MeffEle(0.570, 0.962797), new MeffEle(0.560, 0.959758),
			new MeffEle(0.550, 0.956515), new MeffEle(0.540, 0.953088), new MeffEle(0.530, 0.949495),
			new MeffEle(0.520, 0.945741), new MeffEle(0.510, 0.941838), new MeffEle(0.500, 0.937790),
			new MeffEle(0.490, 0.933563), new MeffEle(0.480, 0.928668), new MeffEle(0.470, 0.923288),
			new MeffEle(0.460, 0.917527), new MeffEle(0.450, 0.911432), new MeffEle(0.440, 0.905035),
			new MeffEle(0.430, 0.898353), new MeffEle(0.420, 0.891022), new MeffEle(0.410, 0.882940),
			new MeffEle(0.400, 0.874312), new MeffEle(0.390, 0.865206), new MeffEle(0.380, 0.855423),
			new MeffEle(0.370, 0.844619), new MeffEle(0.360, 0.833074), new MeffEle(0.350, 0.820876),
			new MeffEle(0.340, 0.808031), new MeffEle(0.330, 0.793962), new MeffEle(0.320, 0.778931),
			new MeffEle(0.310, 0.763021), new MeffEle(0.300, 0.745815), new MeffEle(0.290, 0.727557),
			new MeffEle(0.280, 0.708234), new MeffEle(0.270, 0.687583), new MeffEle(0.260, 0.665741),
			new MeffEle(0.250, 0.642597), new MeffEle(0.240, 0.618252), new MeffEle(0.230, 0.592586),
			new MeffEle(0.220, 0.565747), new MeffEle(0.210, 0.537697), new MeffEle(0.200, 0.508554),
			new MeffEle(0.190, 0.478420), new MeffEle(0.180, 0.447322), new MeffEle(0.170, 0.415454),
			new MeffEle(0.160, 0.382892), new MeffEle(0.150, 0.349955), new MeffEle(0.140, 0.316691),
			new MeffEle(0.130, 0.283565), new MeffEle(0.120, 0.250431), new MeffEle(0.110, 0.218327),
			new MeffEle(0.100, 0.186794), new MeffEle(0.090, 0.156287), new MeffEle(0.080, 0.128421),
			new MeffEle(0.070, 0.102237), new MeffEle(0.060, 0.077393), new MeffEle(0.050, 0.054833),
			new MeffEle(0.040, 0.036361), new MeffEle(0.030, 0.020953), new MeffEle(0.020, 0.009645),
			new MeffEle(0.010, 0.002767), new MeffEle(0.000, 0.000000) };
	...
	/*
	 * computes nutation, if it is wanted and has not yet been computed. if
	 * speed flag has been turned on since last computation, nutation is
	 * recomputed
	 */
	...
	void swi_check_nutation(double tjd, int iflag) {
		...
		speedf2 = iflag & SweConst.SEFLG_SPEED;
		if ((iflag & SweConst.SEFLG_NONUT) == 0
				&& (tjd != swed.nut.tnut || tjd == 0 || ((speedf1 == 0) && (speedf2 != 0)))) {
	...
	private int plaus_iflag(int iflag, int ipl, double tjd, StringBuffer serr) {
		...
		if (jplhor_model == 0)
			jplhor_model = SweConst.SEMOD_JPLHOR_DEFAULT;
		if (jplhora_model == 0)
			jplhora_model = SweConst.SEMOD_JPLHORA_DEFAULT;
		...
		/*
		 * if topocentric bit, turn helio- and barycentric bits off; also turn
		 * JPL Horizons mode off
		 */
		if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
		...
		/*
		 * if sidereal bit is set, set also no_nutation bit * also turn JPL
		 * Horizons mode off
		 */
		if ((iflag & SweConst.SEFLG_SIDEREAL) != 0) {
			iflag |= SweConst.SEFLG_NONUT;
		...
		if (ipl == SweConst.SE_OSCU_APOG || ipl == SweConst.SE_TRUE_NODE || ipl == SweConst.SE_MEAN_APOG
				|| ipl == SweConst.SE_MEAN_NODE || ipl == SweConst.SE_INTP_APOG || ipl == SweConst.SE_INTP_PERG)
			iflag = iflag & ~(SweConst.SEFLG_JPLHOR | SweConst.SEFLG_JPLHOR_APPROX);
		if (ipl >= SweConst.SE_FICT_OFFSET && ipl <= SweConst.SE_FICT_MAX)
			iflag = iflag & ~(SweConst.SEFLG_JPLHOR | SweConst.SEFLG_JPLHOR_APPROX);
		/*
		 * SEFLG_JPLHOR requires SEFLG_ICRS, if calculated with *
		 * precession/nutation IAU 1980 and corrections dpsi, deps
		 */
		if ((iflag & SweConst.SEFLG_JPLHOR) != 0) {
			if (swed.eop_dpsi_loaded <= 0 || ((tjd < swed.eop_tjd_beg || tjd > swed.eop_tjd_end)
		 ...
		 * !APPROXIMATE_HORIZONS_ASTRODIENST)
		 */
	...
	int swe_fixstar_found(StringBuffer serr, String s, StringBuffer star, int fline, double tjd, int iflag,
			int iflgsave, int epheflag, double[] xx) {
		double xpo[] = null;
		...
		ra_pm /= SMath.cos(
				de); /* catalogues give proper motion in RA as great circle */
		...
		/*
		 * FK5 to ICRF, if jpl ephemeris is referred to ICRF. With data that are
		 * already ICRF, epoch = 0
		 */
		if (epoch != 0) {
		...
		/* for parallax */
		if ((iflag & SweConst.SEFLG_HELCTR) != 0 && (iflag & SweConst.SEFLG_MOSEPH) != 0) {
		...
		for (i = 0; i <= 5; i++) {
			x[i] *= 10000; /*
							 * great distance, to allow algorithm used with
							 * planets
							 */
		}
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && (iflag & SweConst.SEFLG_NOGDEFL) == 0) {
		...
		/**********************************
		 * 'annual' aberration of light * speed is incorrect !!! *
		 **********************************/
		if ((iflag & SweConst.SEFLG_TRUEPOS) == 0 && (iflag & SweConst.SEFLG_NOABERR) == 0) {
			swi_aberr_light(x, xpo, iflag & SweConst.SEFLG_SPEED);
		}
		/* ICRS to J2000 */
		if ((iflag & SweConst.SEFLG_ICRS) == 0 && (swed.jpldenum >= 403 || (iflag & SweConst.SEFLG_BARYCTR) != 0)) {
		...
		if ((iflag & SweConst.SEFLG_SIDEREAL) != 0) {
			...
			if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_ECL_T0) != 0) {
				...
				if (swi_trop_ra2sid_lon_sosy(xxsv, x, xxsv, iflag, serr) != SweConst.OK) {
					return SweConst.ERR;
				}
				if ((iflag & SweConst.SEFLG_EQUATORIAL) != 0) {
					for (i = 0; i <= 5; i++) {
		 ...
		 * radians to degrees *
		 **********************/
		if ((iflag & SweConst.SEFLG_RADIANS) == 0 && (iflag & SweConst.SEFLG_XYZ) == 0) {
			for (i = 0; i < 2; i++) {
				x[i] *= SwissData.RADTODEG;
	...
	/**
	 * Returns the magnitude (brightness) of a fixstar. Use this version, if you
	 * just need the magnitude of the star, but not the name of the star on
	 * output.<br>
	...
	/**
	 * Returns the magnitude (brightness) of a fixstar. Use this version, if you
	 * also need the star name on output.<br>
	 * Corresponds to <code>swe_fixstar_mag(...)</code> method in the original.
	 * 
	 * @param star
	 *            (Both input and output parameter.) Name of star or line number
	 *            in star file (start from 1, don't count comment lines).
	 *            <p>
	 *            The name of the star is returned in the format trad_name,
	 *            nomeclat_name in this parameter.
	 * @return magnitude of the star.
	...
	/**********************************************************
	 * get fixstar magnitude parameters: star name of star or line number in
	 * star file (start from 1, don't count comment). If no error occurs, the
	 * name of the star is returned in the format trad_name, nomeclat_name
	 *
	 * mag pointer to a double, for star magnitude serr error return string
	 **********************************************************/
	/**
	 ...
	 *            in star file (start from 1, don't count comment lines).
	 *            <p>
	 *            If no error occurs, the name of the star is returned in the
	 *            format trad_name, nomeclat_name in this parameter.
	 * @param mag
	 *            (Output parameter.) The magnitude of the star. The parameter
	 *            has to be a double[1].
	 * @param serr
	 *            Buffer for error message on output
	 * @return SweConst.OK. All errors will throw a SwissephException.
	 */
	protected int swe_fixstar_mag(StringBuffer star, double[] mag, StringBuffer serr) throws SwissephException {
		...
		if (swed.fixfp == null) {
			...
			try {
				...
				if (swed.fixfp == null) {
					throw new SwissephException(0. / 0., swErrorType, SweConst.ERR, serr.toString());
					// return SweConst.ERR;
					// retc = ERR;
					// goto return_err;
				}
		...
		cmplen = sstar.length();
		if (cmplen == 0) {
			throw new SwissephException(0. / 0., SwissephException.UNSUPPORTED_OBJECT, retc,
		...
		try {
			while ((s = swed.fixfp.readLine()) != null) {
				...
				if (star_nr == line)
					...
					continue;
				if (s.indexOf(',') < 0) {
					throw new SwissephException(0. / 0., SwissephException.DAMAGED_FILE_ERROR, retc,
							"star file " + SweConst.SE_STARFILE + " damaged at line " + fline);
				}
				sp = s.substring(s.indexOf(','));
				if (isnomclat) {
					if (sp.substring(0, SMath.min(sp.length(), cmplen))
							.equals(sstar.substring(0, SMath.min(sstar.length(), cmplen))))
						break;
					else
						continue;
				}
				fstar = s.substring(0, SMath.min(s.length(), SweConst.SE_MAX_STNAME)).trim(); // Left
																								// trimming
																								// only
																								// in
																								// original
				...
				if (fstar.substring(0, SMath.min(fstar.length(), cmplen))
						.equals(sstar.substring(0, SMath.min(sstar.length(), cmplen))))
					break;
			}
		} catch (java.io.IOException ioe) {
		...
		if (s == null) {
			...
			mag[0] = 0;
			throw new SwissephException(0. / 0., SwissephException.UNSUPPORTED_OBJECT, retc, errmsg);
		}
		...
		if (i < 13) {
			...
			throw new SwissephException(0. / 0., SwissephException.DAMAGED_FILE_ERROR, retc, errmsg);
		}
		try {
	...
	int swi_get_observer(double tjd, int iflag, boolean do_save, double xobs[], StringBuffer serr) {
		...
		/*
		 * geocentric position of observer depends on sidereal time, which
		 * depends on UT. compute UT from ET. this UT will be slightly different
		...
		/*
		 * mean or apparent sidereal time, depending on whether or not
		 * SEFLG_NONUT is set
		 */
		sidt = sl.swe_sidtime0(tjd_ut, eps, nut);
		sidt *= 15; /* in degrees */
		/*
		 * length of position and speed vectors; the height above sea level must
		 * be taken into account. with the moon, an altitude of 3000 m makes a
		 * difference of about 2 arc seconds. height is referred to the average
		 * sea level. however, the spheroid (geoid), which is defined by the
		 * average sea level (or rather by all points of same gravitational
		 * potential), is of irregular shape and cannot easily be taken into
		 * account. therefore, we refer height to the surface of the ellipsoid.
		 * the resulting error is below 500 m, i.e. 0.2 - 0.3 arc seconds with
		 * the moon.
		 */
		...
		ss = (1 - f) * (1 - f) * cc;
		/*
		 * neglect polar motion (displacement of a few meters), as long as we
		 * use the earth ellipsoid
		 */
		/* ... */
		/* add sidereal time */
	 ...
	 * Equation of Time
	 *
	 * The function returns the difference between local apparent and local mean
	 * time in days. E = LAT - LMT Input variable tjd is UT.
	 */
	/**
	 ...
	 * 
	 * @param tjd_ut
	 *            input date in julian days (UT)
	 * @param E
	 *            double[1], output value: the difference between the times
	 * @param serr
	 *            buffer for error message on output
	...
	private int open_jpl_file(double[] ss, String fname, String fpath, StringBuffer serr) {
		...
		/*
		 * If we fail with default JPL ephemeris (DE431), we try the second
		 * default (DE406), but only if serr is not NULL and an warning message
		 * can be returned.
		 */
