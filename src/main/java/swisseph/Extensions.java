...
class Extensions implements java.io.Serializable {
	...
	// two days.
	double getTransit(TransitCalculator tc, double jdET, boolean back, double jdMax)
		...
		double degPrec = tc.getDegreePrecision(jdET) / 2.; // Divided by two to
															// have a range of
															// +-degPrec
		double timePrec = tc.getTimePrecision(degPrec);

		val = tc.calc(jdET);
		if (tc.checkIdenticalResult(offset, val)) { // If not 0.0 but "very
													// small", then
		...
		while (true) {
			...
			if (found) { // Return an interpolated value, but not prior to
				...
				if (tc.rollover) {
					if (tc.rollover && SMath.abs(val - lastVal) > 300.) { // How
																			// to
																			// do
																			// it
																			// formally
																			// correct???
						// Probably one value is about 359.99 and the other one
						// is in the area of 0.01
						if (val > lastVal) {
			...
			if ((back && jdET < jdMax) || (!back && jdET > jdMax)) {
				throw new SwissephException(jdET, SwissephException.BEYOND_USER_TIME_LIMIT, -1,
						"User time limit of " + jdMax + " has been reached.");
			}
		}
	}
	...
	protected double maxBaryDist[] = new double[] { 0.009570999, // 0 Sun == 0
																	// 1.017545559
