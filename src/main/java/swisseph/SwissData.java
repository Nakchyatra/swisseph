...
public class SwissData implements java.io.Serializable {
	 ...
	 * applications (and probably awt applications in Java 1.1 and below?) will
	 * require to differentiate between different characters, awt&nbsp;/ swing
	 * components of Java 1.2 and above will use the unicode encoding always!
	 */
	...
	public static final String ayanamsa_name[] = { "Fagan/Bradley", "Lahiri", "De Luce", "Raman", "Ushashashi",
			...
			"Babylonian/Kugler 3", "Babylonian/Huber", "Babylonian/Eta Piscium", "Babylonian/Aldebaran = 15 Tau",
			"Hipparchos", "Sassanian", "Galact. Center = 0 Sag", "J2000", "J1900", "B1950", "Suryasiddhanta",
			"Suryasiddhanta, mean Sun", "Aryabhata", "Aryabhata, mean Sun", "SS Revati", "SS Citra", "True Citra",
			"True Revati", "True Pushya", };
	...
	static final String BFILE_RW_ACCESS = "r+";// open binary file for writing
												// and reading
	static final String BFILE_W_CREATE = "w"; // create/open binary file for
												// write
	static final String BFILE_A_ACCESS = "a+"; // create/open binary file for
												// append
	static final String FILE_R_ACCESS = "r"; // open text file for reading
	static final String FILE_RW_ACCESS = "r+"; // open text file for writing and
												// reading
	static final String FILE_W_CREATE = "w"; // create/open text file for write
	static final String FILE_A_ACCESS = "a+"; // create/open text file for
												// append
	...
	public static final String DIR_GLUE = (System.getProperty("file.separator") == null ? "/"
			: System.getProperty("file.separator")); // glue string for
														// directory/file
	public static final String PATH_SEPARATOR = ";:"; // semicolon or colon may
														// be used
	...
	static final int pnoext2int[] = { SwephData.SEI_SUN, SwephData.SEI_MOON, SwephData.SEI_MERCURY, SwephData.SEI_VENUS,
			SwephData.SEI_MARS, SwephData.SEI_JUPITER, SwephData.SEI_SATURN, SwephData.SEI_URANUS,
			SwephData.SEI_NEPTUNE, SwephData.SEI_PLUTO, 0, 0, 0, 0, SwephData.SEI_EARTH, SwephData.SEI_CHIRON,
			SwephData.SEI_PHOLUS, SwephData.SEI_CERES, SwephData.SEI_PALLAS, SwephData.SEI_JUNO, SwephData.SEI_VESTA, };
	...
	String jplfnam = SweConst.SE_FNAME_DFT; /*
	...
	boolean is_old_starfile = false; /*
										 * is_old_starfile, fixstars.cat is used
										 * (default is sefstars.txt)
										 */
	...
	public SwissData() {
		...
		for (i = 0; i < SwephData.SEI_NEPHFILES; i++) {
			fidat[i] = new FileData();
		}
		gcdat = new GenConst();
		for (i = 0; i < SwephData.SEI_NPLANETS; i++) {
			pldat[i] = new PlanData();
		}
		for (i = 0; i < SwephData.SEI_NNODE_ETC; i++) {
			nddat[i] = new PlanData();
		}
		for (i = 0; i < SweConst.SE_NPLANETS + 1; i++) {
			savedat[i] = new SavePositions();
		}
		oec = new Epsilon();
		oec2000 = new Epsilon();
		nut = new Nut();
