...
public abstract class TransitCalculator implements java.io.Serializable {
	...
	/**
	 * @return Returns the value, which is identical to zero on the other end of
	 *         a linear scale.
	 * @see #rolloverVal
	 */
	 ...
	 * TransitCalculator. It may be the planet number or planet numbers, when
	 * calculating planets.
	 * 
	...
	// Rollover from 360 degrees to 0 degrees for planetary longitudinal
	// positions
	// or similar, or continuous and unlimited values:
	protected boolean rollover = false; // We need a rollover of 360 degrees
										// being
										// equal to 0 degrees for longitudinal
										// position transits only.
	protected double rolloverVal = 360.; // if rollover, we roll over from 360
											// to 0
											// as default. Other values than 0.0
											// for the
											// minimum values are not supported
											// for now.
	...
	protected boolean checkResult(double offset, double lastVal, double val, boolean above, boolean pxway) {
		...
		(above && val <= offset && !pxway) ||
				...
				(!above && val >= offset && pxway)) || (rollover && (
		...
		if (rollover) {
			...
			if (back) {
				if (jdPlus >= 0 && jdMinus >= 0) {
					throw new SwissephException(jdET, SwissephException.OUT_OF_TIME_RANGE, -1,
							"No transit in ephemeris time range."); // I mean:
																	// No
																	// transits
																	// possible...
				} else if (jdPlus >= 0) {
