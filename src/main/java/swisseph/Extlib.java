...
/**
 * Some supportive methods, mainly for internationalization. These methods are
 * not available in the original Swiss Ephemeris package.
 */
public class Extlib implements java.io.Serializable {
	...
	/**
	 * This class contains some additional method not contained in the original
	 * Swiss Ephemeris package. Currently, these methods deal with
		...
		for (int r = 0; r < locs.length; r++) {
			...
			if (locs[r].getCountry().length() > 0) {
				...
				if (locs[r].getVariant().length() > 0) {
					locStrings[r] += "_" + locs[r].getVariant(); // e.g.
																	// th_TH_TH
				}
			}
		}
	 ...
	 * 
	 * @param locString
	 *            A String describing the locale as a two letter language code,
	 *            a two letter language code plus a "_" plus a two letter
	 *            country code, or null or the empty string. Null or the empty
	 *            string will return the default locale, all others will return
	 ...
	 * 4 digit years and UTC timezone. You will format dates like this:
	 * <p>
	 * <code>&nbsp;&nbsp;&nbsp;
	 ...
	 * 
	 * @param locString
	 *            The input locale for which this date time format should be
	 *            created. See getLocale() for more infos.
	...
	public SimpleDateFormat createLocDateTimeFormatter(String locString, boolean force24h) {
		...
		SimpleDateFormat df = (SimpleDateFormat) DateFormat.getDateTimeInstance(java.text.DateFormat.SHORT,
	...
	public String getNormalizedDatePattern(String pattern, boolean force24h) {
		...
		// force year, month, day, hour, minutes and seconds to appear with two
		// digits:
		...
		if (pattern.indexOf("s") < 0) {
			...
			if (idx >= 0) { // If not, it not even has a minutes part???
				try {
					...
					// case with "mk" here ("d.M.yy HH:mm:" original,
					// "dd.MM.yyyy HH:mm:"
					// when changed):
					pattern = pattern.substring(0, idx + 3) + "ss" + pattern.substring(idx + 3);
				} catch (StringIndexOutOfBoundsException sb) {
					// In zh_SG, the format looks like dd/MM/yyyy a hh:mm (above
					// assumption fails)
					pattern = pattern.substring(0, idx + 2) + pattern.substring(idx - 1, idx) + "ss"
	 ...
	 * zh_HK the pattern is: yyyy'���'MM'���'dd'��?' ahh:mm:ss The index of 'ss'
	...
	public int getPatternLastIdx(String pattern, String what, SimpleDateFormat dof) {
		...
		if (pattern.indexOf("a") >= 0 && pattern.indexOf("a") < pattern.indexOf(what)) {
			int len = dof.getDateFormatSymbols().getAmPmStrings()[0].length(); // No
																				// input
																				// with
																				// fractions
																				// of
