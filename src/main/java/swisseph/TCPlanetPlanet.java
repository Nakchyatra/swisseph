...
/**
 * This class implements a TransitCalculator for two planets in relative
 * positions or speeds to each other.
 * <p>
 ...
 * </pre>
 * 
 * This would calculate the last (UT-) date, when Mercury and Venus had the same
...
public class TCPlanetPlanet extends TransitCalculator implements java.io.Serializable {
	 ...
	 * and speeds.
	 * <p>
	 * Calculation of partile transits is possible now, but it's still a rather
	 * slow calculation and not finally tested.
	 * <p>
	 * 
	 * @param sw
	 *            A SwissEph object, if you have one available. May be null.
	 * @param pl1
	 *            The planet number of the first planet.<br>
	 *            Planets from SweConst.SE_SUN up to SweConst.SE_INTP_PERG (with
	 *            the exception of SweConst.SE_EARTH) have their extreme speeds
	 *            saved, so these extreme speeds will be used on calculation.
	 *            <br>
	 *            Other objects calculate extreme speeds by randomly calculating
	 *            by default 200 speed values and multiply them by 1.4 as a
	 *            safety factor.<br>
	 *            ATTENTION: be sure to understand that you might be able to
	 *            miss some transit or you might get a rather bad transit time
	 *            in very rare circumstances.<br>
	 *            Use SweConst.SE_AST_OFFSET + asteroid number for planets with
	 *            a planet number not defined by SweConst.SEFLG_*.
	 * @param pl2
	 *            The second planet that will be transited by the first planet.
	 * @param flags
	 *            The calculation type flags (SweConst.SEFLG_TRANSIT_LONGITUDE,
	 *            SweConst.SEFLG_TRANSIT_LATITUDE or
	 *            SweConst.SEFLG_TRANSIT_DISTANCE in conjunction with
	 *            SweConst.SEFLG_TRANSIT_SPEED for transits over a speed value).
	 *            <br>
	 *            <br>
	 *            Also flags modifying the basic planet calculations, these are
	 *            SweConst.SEFLG_TOPOCTR, SweConst.SEFLG_EQUATORIAL,
	 *            SweConst.SEFLG_HELCTR, SweConst.SEFLG_TRUEPOS, and
	 *            SweConst.SEFLG_SIDEREAL, plus the ephemeris flags
	 *            SweConst.SEFLG_MOSEPH, SweConst.SEFLG_SWIEPH or
	 *            SweConst.SEFLG_JPLEPH optionally. <br>
	 *            For <i>right ascension</i> use
	 *            <code>SEFLG_TRANSIT_LONGITUDE | SEFLG_EQUATORIAL</code>, for
	 *            <i>declination</i>
	 *            <code>SEFLG_TRANSIT_LATITUDE | SEFLG_EQUATORIAL</code>.<br>
	 *            <br>
	 *            SEFLG_PARTILE_TRANSIT will calculate the next time, when the
	 *            planets will have a partile aspect to each other, if they
	 *            don't have a partile aspect now. If they do have a partile
	 *            aspect now, it calculates the next time, when this partile
	 *            aspect gets lost.<br>
	 * @param offset
	 *            This is the desired transit degree or distance in AU or
	 *            transit speed in deg/day.
	 * @see swisseph.TCPlanetPlanet#TCPlanetPlanet(SwissEph, int, int,
	 *      int, double, int, double)
	 * @see swisseph.TCPlanet#TCPlanet(SwissEph, int, int, double)
	 * @see swisseph.TCPlanet#TCPlanet(SwissEph, int, int, double, int,
	 *      double)
	 ...
	 *            SweConst.SEFLG_TRANSIT_DISTANCE in conjunction with
	 *            SweConst.SEFLG_TRANSIT_SPEED for transits over a speed value,
	 *            and SweConst.SEFLG_YOGA_TRANSIT to calculate for the SUM of
	 *            the two positions or speeds instead of the difference plus
	 *            SweConst.SEFLG_PARTILE_TRANSIT,
	 *            SweConst.SEFLG_PARTILE_TRANSIT_START or
	 *            SweConst.SEFLG_PARTILE_TRANSIT_END to calculate partile
	 *            aspects).<br>
	 *            <br>
	 *            Also flags modifying the basic planet calculations, these are
	 *            SweConst.SEFLG_TOPOCTR, SweConst.SEFLG_EQUATORIAL,
	 ...
	 *            E.g., when the offset is 180 for longitude calculations, you
	 *            will get the dates, when the two planets are opposite to each
	 *            other. Note: The offset is related to the FIRST planet, so an
	 *            offset value of 30 degree will find the transit points, when
	 *            the FIRST planet will be 30 degrees after the position of the
	 *            second planet.<br>
	 *            Specify the desired transit degree or distance (in AU) or
	 *            transit speed (in deg/day).
	 * @see swisseph.TCPlanetPlanet#TCPlanetPlanet(SwissEph, int, int,
	 *      int, double)
	 * @see swisseph.TCPlanet#TCPlanet(SwissEph, int, int, double)
	 ...
	 * @see swisseph.SweConst#SEFLG_JPLEPH
	 */
	public TCPlanetPlanet(SwissEph sw, int pl1, int pl2, int flags, double offset, int precalcCount,
		...
		this.tflags = flags;
		int vFlags = SweConst.SEFLG_EPHMASK | SweConst.SEFLG_TOPOCTR | SweConst.SEFLG_EQUATORIAL | SweConst.SEFLG_HELCTR
				| SweConst.SEFLG_NOABERR | SweConst.SEFLG_NOGDEFL | SweConst.SEFLG_SIDEREAL | SweConst.SEFLG_TRUEPOS
				| SweConst.SEFLG_PARTILE_TRANSIT | SweConst.SEFLG_PARTILE_TRANSIT_START
				| SweConst.SEFLG_PARTILE_TRANSIT_END | SweConst.SEFLG_YOGA_TRANSIT | SweConst.SEFLG_TRANSIT_LONGITUDE
				| SweConst.SEFLG_TRANSIT_LATITUDE | SweConst.SEFLG_TRANSIT_DISTANCE | SweConst.SEFLG_TRANSIT_SPEED;
		...
		int type = flags & (SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE
				| SweConst.SEFLG_TRANSIT_DISTANCE);
		if (type != SweConst.SEFLG_TRANSIT_LONGITUDE && type != SweConst.SEFLG_TRANSIT_LATITUDE
			...
			throw new IllegalArgumentException("Invalid flag combination '" + flags
					...
					+ "), SEFLG_TRANSIT_LATITUDE (" + SweConst.SEFLG_TRANSIT_LATITUDE + "), SEFLG_TRANSIT_DISTANCE ("
					+ SweConst.SEFLG_TRANSIT_DISTANCE + ").");
		}
		if ((flags & SweConst.SEFLG_HELCTR) != 0 && (pl1 == SweConst.SE_MEAN_APOG || pl1 == SweConst.SE_OSCU_APOG
				|| pl1 == SweConst.SE_MEAN_NODE || pl1 == SweConst.SE_TRUE_NODE)) {
			throw new IllegalArgumentException("Unsupported planet number " + pl1 + " (" + sw.swe_get_planet_name(pl1)
					+ ") for heliocentric " + "calculations");
		}
		if ((flags & SweConst.SEFLG_HELCTR) != 0 && (pl2 == SweConst.SE_MEAN_APOG || pl2 == SweConst.SE_OSCU_APOG
				|| pl2 == SweConst.SE_MEAN_NODE || pl2 == SweConst.SE_TRUE_NODE)) {
			throw new IllegalArgumentException("Unsupported planet number " + pl2 + " (" + sw.swe_get_planet_name(pl2)
					+ ") for heliocentric " + "calculations");
		}
		if (pl1 == pl2) {
		...
		calcPartile = ((flags & SweConst.SEFLG_PARTILE_TRANSIT_START) != 0); // Included
																				// in
																				// partileBoth
		calcNonPartile = ((flags & SweConst.SEFLG_PARTILE_TRANSIT_END) != 0); // Included
																				// in
																				// partileBoth
		...
		flags &= ~(SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE | SweConst.SEFLG_TRANSIT_DISTANCE
				| SweConst.SEFLG_PARTILE_TRANSIT | SweConst.SEFLG_PARTILE_TRANSIT_START
				| SweConst.SEFLG_PARTILE_TRANSIT_END | SweConst.SEFLG_YOGA_TRANSIT | SweConst.SEFLG_TRANSIT_SPEED);
		this.flags = flags;
		...
		if (calcPartile || calcNonPartile) {
			if ((offset % 30) != 0) {
				throw new IllegalArgumentException("Wrong offset (" + offset
						+ "). Calculation of partile aspect may have offsets of multiples of 30 degrees only.");
			}
			rollover = false;
		}
		...
		if (Double.isInfinite(maxSpeed1) || Double.isInfinite(minSpeed1)) {
			// Trying to find some reasonable min- and maxSpeed by randomly
			// testing some speed values.
		...
		if (Double.isInfinite(maxSpeed1) || Double.isInfinite(minSpeed1)) {
			throw new IllegalArgumentException(((flags & SweConst.SEFLG_TOPOCTR) != 0 ? "Topo"
					: ((flags & SweConst.SEFLG_HELCTR) != 0 ? "Helio" : "Geo"))
					+ "centric transit calculations with planet number " + pl1 + " (" + sw.swe_get_planet_name(pl1)
					+ ") not possible: extreme " + ((flags & SweConst.SEFLG_SPEED) != 0 ? "accelerations" : "speeds")
					+ " of the planet " + ((flags & SweConst.SEFLG_EQUATORIAL) != 0 ? "in equatorial system " : "")
					+ "not available.");
		}
		...
		if (Double.isInfinite(maxSpeed2) || Double.isInfinite(minSpeed2)) {
			throw new IllegalArgumentException(((flags & SweConst.SEFLG_TOPOCTR) != 0 ? "Topo"
					: ((flags & SweConst.SEFLG_HELCTR) != 0 ? "Helio" : "Geo"))
					+ "centric transit calculations with planet number " + pl2 + " (" + sw.swe_get_planet_name(pl2)
					+ ") not possible: extreme " + ((flags & SweConst.SEFLG_SPEED) != 0 ? "accelerations" : "speeds")
					+ " of the planet " + ((flags & SweConst.SEFLG_EQUATORIAL) != 0 ? "in equatorial system " : "")
					+ "not available.");
		}
	...
	// Looks for next time when the condition (partile / non-partile) is NOT
	// met,
		...
		if (calcPartile || calcNonPartile) {
			...
			if (!calcPartileBoth) {
				if (isPartile && calcPartile) {
					...
					TransitCalculator tc = new TCPlanetPlanet(sw, pl1, pl2, flgs, offset);
	...
	/**
	 * This sets the transit degree or other transit value for the difference or
	 * sum of the positions or speeds of both planets. It will be used on the
	 * next call to getTransit().
	 * 
	 * @param value
	 *            The offset value.
	 * @see #getOffset()
	...
	public boolean hasPartileAspect(double jdET, int p1, int p2, int flgs, double offset) {
		...
		flgs &= ~(SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_TRANSIT_LATITUDE | SweConst.SEFLG_TRANSIT_DISTANCE
				| SweConst.SEFLG_PARTILE_TRANSIT | SweConst.SEFLG_PARTILE_TRANSIT_START
				| SweConst.SEFLG_PARTILE_TRANSIT_END | SweConst.SEFLG_YOGA_TRANSIT | SweConst.SEFLG_TRANSIT_SPEED);
	...
	protected double calc(double jdET) {
		...
		if (calcPartile || calcNonPartile) {
			...
			if (lon2 > target1) {
				delta1 = SMath.abs(lon2 - target1 - 1 + 1E-9); // Might be
																// better to use
																// a different
																// comparison
																// operator
			...
			double diff = SMath.min(SMath.min(SMath.min(delta1, delta2), delta3), delta4) % 1; // Using
																								// '%1',
																								// as
																								// I
																								// don't
			...
			if (((int) xx1[idx] - (int) xx2[idx] + rolloverVal) % rolloverVal - offset == 0
				...
				return Double.POSITIVE_INFINITY; // Means: found, but don't
	...
	protected double getDegreePrecision(double jd) {
		...
		// based on the discussions on http://www.astro.com/swisseph, even
		// though
		...
		if (idx > 2) { // Speed
			...
			if (pl1 >= SweConst.SE_SUN && pl1 <= SweConst.SE_JUPITER) {
				...
				if (jd >= 1900 && jd < 1980) {
					degPrec = 0.08;
				} else if (jd < 1900 || jd > 2099) { // Unclear about true
														// nodes...
					degPrec = 1;
				}
			}
		...
		degPrec /= 3600.;
		degPrec *= 0.5; // We take the precision to be BETTER THAN ... as it is
						// stated somewhere
	...
	private double getSpeed(boolean min, int planet) {
		...
		try {
			...
			if (topo) {
				if (!sw.swed.geopos_is_set) {
					throw new IllegalArgumentException(
							"Geographic position is not set for " + "requested topocentric calculations.");
				}
				// if (sw.swed.topd.geoalt>50000.) {
				// throw new IllegalArgumentException("Topocentric transit
				// calculations "+
			...
			// Heliocentric speeds are very different to the geocentric speeds,
			// so
	...
	protected boolean checkIdenticalResult(double offset, double val) {
		if (calcPartile) {
			boolean res = (((lon1 % 1) < 1E-9 || (lon2 % 1) < 1E-9) && // leave
																		// other
																		// values
																		// to
																		// the
	...
	// This routine returns extreme speed values by randomly calculating the
	// speed
	...
	double[] getTestspeed(int planet, int idx, int precalcCount, double precalcSafetyfactor) {
		...
		if (planet > SweConst.SE_AST_OFFSET) {
			...
			// Unfortunately, the name from swi_gen_filename may be slightly
			// different,
			// so we have to test opening the filename and change the filename
			// if
			...
			// Now finally we have a filename for which we can get the time
			// range,
		...
		for (int f = 0; f < precalcCount; f++) {
			...
			if (ret < 0) {
				// throw new SwissephException(jdET,
				// SwissephException.UNDEFINED_ERROR,
				// "Calculation failed with return code
				// "+ret+":\n"+serr.toString());
				continue;
			}
			if (min > xx[idx + 3]) {
