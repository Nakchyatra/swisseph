 ...
 * RandomAccessFile class. It adds the ability to read (and search and seek) in
 * files using http://... access. This is needed to allow applets read access to
 * files, in this case the Swiss Ephemeris and JPL data files.
 */
public class FilePtr implements java.io.Serializable {
	 ...
	 */
	public FilePtr(RandomAccessFile fp, Socket sk, InputStream is, BufferedOutputStream os, String fnamp,
			long fileLength, int bufsize) throws IOException {
		this.fp = fp;
		this.sk = sk;
		this.is = is;
		this.os = os;
		...
		if (useHTTP && fp == null) { // RandomAccessFile, try file access via
										// http:
			try {
				URL u = new URL(fnamp);
				host = u.getHost();
	 ...
	 * @throws EOFException
	 *             if the end of file is reached <i>before</i> the byte could be
	 *             read.
	 */
	...
	public int readInt() throws IOException, EOFException {
		if (bigendian) {
			return (((int) readByte()) << 24) + (((int) readUnsignedByte()) << 16) + (((int) readUnsignedByte()) << 8)
					+ (int) readUnsignedByte();
		}
		return (int) readUnsignedByte() + (((int) readUnsignedByte()) << 8) + (((int) readUnsignedByte()) << 16)
				+ (((int) readByte()) << 24);
	}
	...
	public double readDouble() throws IOException, EOFException {
		long ldb = (bigendian
				? ((((long) readUnsignedByte()) << 56) + (((long) readUnsignedByte()) << 48)
						+ (((long) readUnsignedByte()) << 40) + (((long) readUnsignedByte()) << 32)
						+ (((long) readUnsignedByte()) << 24) + (((long) readUnsignedByte()) << 16)
	...
	public long length() throws IOException {
		...
		if (rc == 200 && sout.indexOf("Content-Length:") >= 0) {
			...
			throw new IOException("Can't determine length of (HTTP-)file '" + fnamp + "'. HTTP error code: " + rc);
		}
		return len;
	}
	...
	private void readToBuffer() throws IOException, EOFException {
		...
		if (fp != null) {
			...
			// operations???
			if (cnt == -1) {
				throw new EOFException("Filepointer position " + fpos + " exceeds file" + " length by "
		...
		String s = "GET " + fnamp + " HTTP/1.1\r\n" + "User-Agent: " + useragent + "\r\n" + "Host: " + host + ":" + port
				+ "\r\n" + "Range: bytes=" + fpos + "-" + SMath.min(length() - 1, fpos + BUFSIZE - 1) + "\r\n\r\n";
		String sout = "";
		int slen = 0;
		int failures = 0;
		while (true) {
			...
			if (rc < 0) { // What has happened?
				if (++failures >= MAX_FAILURES) { // Too many failures in a row,
													// abort:
					throw new IOException("Failed to read successfully from address\n'" + fnamp
							+ "'. The http reply from the server was " + sout.length()
			...
			slen = sout.length();
			if ((rc != 200 /* OK */ && rc != 206 /* Partial content */) || slen > BUFSIZE
					|| (slen < BUFSIZE && savedLength >= 0 && fpos + slen != savedLength)) {
				if (++failures >= MAX_FAILURES) {
					throw new IOException("HTTP read failed with HTTP response " + rc + ". Read " + slen
							+ " bytes, requested " + BUFSIZE + " bytes.");
				}
				continue;
			}
