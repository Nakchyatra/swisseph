package swisseph.examples;

import swisseph.*;

/**
 * This class calculates some nakshatra starting and ending dates for february
 * 1986 in India (time zone offset 5.5 hours).
 *
 * It calculates topocentric (for Bangalore) and geocentric dates, even though
 * geocentric seems to be normally used in India.
 */
public class NakshatraDates {

	final static double TZ_OFFSET_HOURS = 5.5; // IST

	final static String[] nakshatraNames = new String[] { "Ashvini", "Bharani", "Krittika", "Rohini", "Mrigashirsha",
			"Ardra", "Punarvasu", "Pushya", "Ashlesha", "Magha", "Purva Phalguni", "Uttar Phalguni", "Hasta", "Chitra",
			"Svati", "Vishakha", "Anuradha", "Jyeshtha", "Mula", "Purvashadha", "Uttarashadha", "Shravan",
			"Shravishtha", "Shatabhisha", "Purva Bhadrapada", "Uttara Bhadrapada", "Revati", };
	...
	public static void main(String[] p) {
		...
		for (int naksh = 0; naksh < 27; naksh++) { // Nakshatra number zero
													// based
			...
			System.out.println(String.format("topocentric %-20s: %s - %s", nakshatraNames[naksh],
					sj.toDateString(nakshStart, TZ_OFFSET_HOURS), sj.toDateString(nakshEnd, TZ_OFFSET_HOURS)));
	...
	double getNextNakshatraStart(double juld, int nakshatra, boolean topoctr) {
		...
		int flags = SweConst.SEFLG_SWIEPH | SweConst.SEFLG_TRANSIT_LONGITUDE | SweConst.SEFLG_SIDEREAL;
		if (topoctr) {
			flags |= SweConst.SEFLG_TOPOCTR;
		}

		TransitCalculator tc = new TCPlanet(sw, SweConst.SE_MOON, flags, nakshatra * (360. / 27.));
	...
	String toDateString(double d, double tzHours) {
		SweDate sd = new SweDate(
				d + tzHours / 24. + 0.5 / 24. / 3600. /* round to second */);
		double hour = sd.getHour();
		return String.format(java.util.Locale.US, "%4d/%02d/%02d, %2d:%02d:%02dh", sd.getYear(), sd.getMonth(),
				sd.getDay(), (int) hour, (int) ((hour - (int) hour) * 60),
