...
public class SweHel implements java.io.Serializable {
	...
	private static final int PLSV = 0; /*
										 * if Planet, Lunar and Stellar
										 * Visibility formula is needed PLSV=1
										 */
	...
	private static final int FormAstroRefrac = REFR_SINCLAIR; /*
																 * for
																 * Astronomical
																 * refraction
																 * can be
	...
	private static final int GravitySource = 2; /*
												 * 0=RGO, 1=Wikipedia,2=Exp.
												 * Suppl. 1992,3=van der Werf
												 */
	private static final int REarthSource = 1; /*
												 * 0=RGO (constant), 1=WGS84
												 * method
												 */
	...
	private static final int MAX_COUNT_SYNPER_MAX = 1000000; /*
																 ...
																 * count
																 */
	private static final double AvgRadiusMoon = (15.541
			/ 60); /* '[Deg] at 2007 CE or BCE */
	...
	private static final int staticAirmass = 0; /*
												 * use staticAirmass=1 instead
												 * depending on difference k's
												 */
	...
	/*
	 * 'Input ' Bback [nL] ' kX [-] ' Binocular [-] ' OpticMag [-] ' OpticDia
	 * [mm] ' OpticTrans [-] ' JDNDaysUT [JDN] ' Age [Year] ' SN [-] '
	 * ObjectName ' TypeFactor [0=itensity factor 1=background factor] 'Output '
	 * OpticFactor [-]
	 */
	private double OpticFactor(double Bback, double kX, double[] dobs, double JDNDaysUT,
			String ObjectName /* unused parameter */, int TypeFactor, int helflag) {
		...
		CIi = 0.5; /*
					 * Color index for white (from Ben Sugerman), should be
					 * function of ObjectName
					 */
		ObjectSize = 0;
		...
		if (Bback <= BNIGHT && (helflag & SweConst.SE_HELFLAG_VISLIM_PHOTOPIC) == 0) {
			...
			Fsc = SMath.min(1, SMath.pow((OpticDia / OpticMag / Pst), 2) * (1 - SMath.exp(-SMath.pow((Pst / 6.2), 2)))
					/ (1 - SMath.exp(-SMath.pow((OpticDia / OpticMag / 6.2), 2))));
			Fci = 1;
			Fcb = 1;
		}
		Ft = 1 / OpticTrans;
	 ...
	 * swe_fixstar()
	 */
	int call_swe_rise_trans(double tjd, int ipl, String star, int helflag, int eventtype, double[] dgeo, double atpress,
			double attemp, double[] tret, StringBuffer serr) {
		int retval;
	...
	/*
	 * Written by Dieter Koch: Fast function for risings and settings of
	 * planets, can be used instead of swe_rise_trans(), which is much slower.
	 * For circumpolar and near-circumpolar planets use swe_rise_trans(), or
	 * generally use it for geographical latitudes higher than 58N/S. For fixed
	 * stars, swe_rise_trans() is fast enough.
	 */
	// private int my_rise_trans(double tjd, int ipl, String starname, int
	// eventtype, int helflag, double *dgeo, double *datm, double *tret,
	// StringBuffer serr)
	// my_rise_trans:
	// retc = calc_rise_and_set(tjd, ipl, dgeo, datm, eventtype, helflag, tret,
	// serr);
	// dgeo: double*
	// ist in der Praxis ein Eingabeparameter "dgeo[3] geograph. longitude,
	// latitude, height above sea"
	// datm: double*
	// ist in der Praxis ein Eingabeparameter von "datm[] = new double[]
	// {atpress, attemp}"
	// trise: double* == R?ckgabe eines einzelnen double "double[0] trise"
	// RiseSet:
	// retval = my_rise_trans(JDNDaysUT, Planet, "", eventtype, helflag, dgeo,
	// datm, tret, serr);
	// retval = my_rise_trans(JDNDaysUT, -1, ObjectName, eventtype, helflag,
	// dgeo, datm, tret, serr);
	// dgeo: double*
	// datm: double*
	// heliacal_ut_arc_vis:
	// if ((retval = my_rise_trans(JDNDaysUTstep, SweConst.SE_SUN, "",
	// eventtype, helflag, dgeo, datm, &tret, serr)) == SweConst.ERR)
	// dgeo: double*
	// datm: double*
	// get_heliacal_day:
	// if ((retval = my_rise_trans(tday, SweConst.SE_SUN, "", is_rise_or_set,
	// helflag, dgeo, datm, &tret, serr)) == SweConst.ERR)
	// dgeo: double*
	// datm: double*
	// [get_acronychal_day_new:]
	// - if ((retval = my_rise_trans(tret, 0, ObjectName, SweConst.SE_CALC_RISE,
	// helflag, dgeo, datm, &tret, serr)) == SweConst.ERR)
	// - if ((retval = my_rise_trans(tret, 0, ObjectName, SweConst.SE_CALC_SET,
	// helflag, dgeo, datm, &tret, serr)) == SweConst.ERR)
	// dgeo: double*
	// datm: double*
	// get_acronychal_day:
	// retval = my_rise_trans(tret, SweConst.SE_SUN, "", is_rise_or_set,
	// helflag, dgeo, datm, &tret, serr);
	// dgeo: double*
	// datm: double*
	// moon_event_vis_lim:
	// if ((retval = my_rise_trans(tjd, SweConst.SE_SUN, "",
	// SweConst.SE_CALC_SET, helflag, dgeo, datm, &trise, serr)) ==
	// SweConst.ERR)
	// if ((retval = my_rise_trans(dret[1], SweConst.SE_SUN, "",
	// SweConst.SE_CALC_RISE, helflag, dgeo, datm, &trise, serr)) ==
	// SweConst.ERR)
	// dgeo: double*
	// datm: double*
	private int calc_rise_and_set(double tjd_start, int ipl, double[] dgeo, double[] datm, int eventflag, int helflag,
			double[] trise /* double[0] as return parameter */, StringBuffer serr) {
		int retc = SweConst.OK, i;
		double sda, xs[] = new double[6], xx[] = new double[6], xaz[] = new double[6], xaz2[] = new double[6],
				dfac = 1 / 365.25;
		double rdi, rh;
		...
		if ((eventflag & SweConst.SE_CALC_RISE) != 0) {
			...
			if (xaz[2] > 0) {
				...
				while (tjd0 - tjdnoon > 0.0) {
					/* printf("c"); */ tjdnoon += 1;
				}
				while (tjd0 - tjdnoon < -1.0) {
					/* printf("d"); */ tjdnoon -= 1;
				}
			}
		}
		 ...
		 * atmospheric temperature 10 deg C, atmospheric pressure 1013.25 is
		 * 34.5 arcmin
		 */
		rh = -(34.5 / 60.0 + rdi);
		/* semidiurnal arc of sun */
		sda = SMath.acos(-SMath.tan(dgeo[1] * SwissData.DEGTORAD) * SMath.tan(xx[1] * SwissData.DEGTORAD))
				* SwissData.RADTODEG;
		...
		/*
		 * now calculate more accurate rising and setting times. use vertical
		 * speed in order to determine crossing of the horizon refraction of 34'
		 * and solar disk diameter of 16' = 50' = 0.84 deg
		 */
	...
	private int my_rise_trans(double tjd, int ipl, String starname, int eventtype, int helflag, double[] dgeo,
			double[] datm, double[] tret, StringBuffer serr) {
		...
		/*
		 * if (!(helflag & SweConst.SE_HELFLAG_HIGH_PRECISION) && ipl != -1 &&
		 * SMath.abs(dgeo[1]) < 58) {
		 */
	...
	/*
	 * ################################################################### '
	 * JDNDaysUT [Days] ' dgeo [array: longitude, latitude, eye height above sea
	 * m] ' TempE [C] ' PresE [mbar] ' ObjectName (string) ' RSEvent (1=rise,
	 * 2=set,3=up transit,4=down transit) ' Rim [0=center,1=top] ' RiseSet [Day]
	 */
	private int RiseSet(double JDNDaysUT, double[] dgeo, double[] datm, String ObjectName, int RSEvent, int helflag,
			int Rim, double[] tret /* double[0] as output parameter */, StringBuffer serr) {
	...
	/*
	 * ################################################################### '
	 * JDNDaysUT [Days] ' actual [0= approximation, 1=actual] ' SunRA [deg]
	 */
	private double SunRA(double JDNDaysUT, int helflag, StringBuffer serr) {
		...
		if (JDNDaysUT == sunRA_tjdlast)
			return sunRA_ralast;
		// JAVA: swe_revjul(JDNDaysUT, calflag, &iyar, &imon, &iday, &dut); /*
		// this seems to be much faster than calling swe_revjul() ! Note: only
		// because SunRA is called 1000s of times */
		...
		/*
		 * sunRA_ralast = (DatefromJDut(JDNDaysUT, 2) + (DatefromJDut(JDNDaysUT,
		 * 3) - 1) / 30.4 - 3.69) * 30;
		 */
		return sunRA_ralast;
	}
	...
	/*
	 * ################################################################### '
	 * AppAlt [deg] ' TempE [C] ' PresE [mbar] ' TopoAltitudefromAppAlt [deg]
	 */
	...
	/*
	 * ################################################################### '
	 * TopoAlt [deg] ' TempE [C] ' PresE [mbar] ' AppAltfromTopoAlt [deg] ' call
	 * this instead of swe_azalt(), because it is faster (lower precision ' is
	 * required)
	 */
	private double AppAltfromTopoAlt(double TopoAlt, double TempE, double PresE, int helflag) {
		/*
		 * using methodology of Newtown derivatives (analogue to what Swiss
		 * Emphemeris uses)
		 */
		int i, nloop = 2;
		...
		for (i = 0; i <= nloop; i++) {
			...
			/*
			 * newTopoAlt = newAppAlt - swe_refrac(newAppAlt, PresE, TempE,
			 * SE_CALC_APP_TO_TRUE);
			 */
	...
	/*
	 * ################################################################### '
	 * TopoAlt [deg] ' TopoDecl [deg] ' Lat [deg] ' HourAngle [hour]
	 */
	...
	/*
	 * ################################################################### '
	 * JDNDays [Days] ' COD [msec/cy] ' DeltaTSE [Sec]
	 */
	...
	/*
	 * ################################################################### '
	 * JDNDays [Day] ' COD [msec/cy] ' DeltaTVR [Sec]
	 */
	private double DeltaTVR(double JDNDays, int COD) {
		...
		OffSetYear = (SweDate.getJulDay(StartYear, 1, 1, 0, gregflag) - JDNDays) / 365.25;
		if (COD == 0) {
			DeltaTVR = (OffSetYear * OffSetYear / 100.0 / 2.0 * Average
					+ Periodicy / 2.0 / SMath.PI * Amplitude * (SMath.cos((2 * SMath.PI * OffSetYear / Periodicy)) - 1))
					* Y2D;
		} else {
	...
	/*
	 * ################################################################### '
	 * JDNDays [Days] ' COD [msec/cy] ' DeltaT [Sec]
	 */
	 ...
	 * ################################################################### '
	 * JDNDaysUT [Days] ' dgeo [array: longitude, latitude, eye height above sea
	 * m] ' TempE [C] ' PresE [mbar] ' ObjectName [-] ' Angle (0 = TopoAlt, 1 =
	 * Azi, 2=Topo Declination, 3=Topo Rectascension, 4=AppAlt,5=Geo
	 * Declination, 6=Geo Rectascension) ' ObjectLoc [deg]
	 */
	// dret = single double output value
	private int ObjectLoc(double JDNDaysUT, double[] dgeo, double[] datm, String ObjectName, int Angle, int helflag,
			double[] dret, StringBuffer serr) {
		return ObjectLoc(JDNDaysUT, dgeo, datm, new StringBuffer(ObjectName), Angle, helflag, dret, serr);
	}

	private int ObjectLoc(double JDNDaysUT, double[] dgeo, double[] datm, StringBuffer ObjectName, int Angle,
			int helflag, double[] dret, StringBuffer serr) {
	 ...
	 * Declination, 6=Geo Rectascension) ' ObjectLoc [deg]
	 */
	private int azalt_cart(double JDNDaysUT, double[] dgeo, double[] datm, StringBuffer ObjectName, int helflag,
			double[] dret, StringBuffer serr) {
		return azalt_cart(JDNDaysUT, dgeo, datm, ObjectName, helflag, dret, 0, serr);
	}

	private int azalt_cart(double JDNDaysUT, double[] dgeo, double[] datm, StringBuffer ObjectName, int helflag,
			double[] dret, int dret_offset, StringBuffer serr) {
		double x[] = new double[6], xin[] = new double[3], xaz[] = new double[3], tjd_tt;
		int Planet;
		int epheflag;
		int iflag = SweConst.SEFLG_EQUATORIAL;
	...
	/*
	 * ################################################################### '
	 * LatA [rad] ' LongA [rad] ' LatB [rad] ' LongB [rad] ' DistanceAngle [rad]
	 */
	private double DistanceAngle(double LatA, double LongA, double LatB, double LongB) {
		...
		/*
		 * Haversine formula
		 * http://www.movable-type.co.uk/scripts/GIS-FAQ-5.1.html R.W. Sinnott,
		 * Virtues of the Haversine, Sky and Telescope, vol. 68, no. 2, 1984, p.
		 * 159
		 */
	...
	/*
	 * ################################################################### '
	 * JDNDaysUT [-] ' AltS [deg] ' lat [deg] ' kOZ [-]
	 */
	private double kOZ(double AltS, double sunra, double Lat) {
		...
		koz__alts_last = AltS;
		koz__sunra_last = sunra;
		OZ = 0.031;
		LT = Lat * SwissData.DEGTORAD;
		/* From Schaefer , Archaeoastronomy, XV, 2000, page 128 */
		kOZret = OZ * (3.0 + 0.4 * (LT * SMath.cos(sunra * SwissData.DEGTORAD) - SMath.cos(3 * LT))) / 3.0;
		/*
		 * depending on day/night vision (altitude of sun < start astronomical
		 * twilight), KO changes from 100% to 30% see extinction section of
		 * Vistas in Astronomy page 343
		 */
	...
	/*
	 * ################################################################### '
	 * AltS [deg] ' heighteye [m] ' kR [-]
	 */
	private double kR(double AltS, double HeightEye) {
		/*
		 * depending on day/night vision (altitude of sun < start astronomical
		 * twilight), lambda eye sensibility changes see extinction section of
		 * Vistas in Astronomy page 343
		 */
	...
	/*
	 * ################################################################### '
	 * JDNDaysUT [-] ' AltS [deg] ' lat [deg] ' heighteye [m] ' TempS [C] ' RH
	 * [%] ' VR [km] ' ka [-]
	 */
	private double ka(double AltS, double sunra, double Lat, double HeightEye, double TempS, double RH, double VR,
			StringBuffer serr) {
		...
		if (VR != 0) {
			if (VR >= 1) {
				 ...
				 * Narasimhan_CVPR03.pdf
				 * http://www.icao.int/anb/SG/AMOSSG/meetings/amossg3/wp/SN11Rev
				 * .pdf where MOR=2.995/ke factor 1.3 is the relation between
				 * "prevailing visibility" and meteorological range was derived
				 * by Koshmeider in the 1920's
				 */
				BetaVr = 3.912 / VR;
				Betaa = BetaVr - (kW(HeightEye, TempS, RH) / scaleHwater + kR(AltS, HeightEye) / scaleHrayleigh) * 1000
						* astr2tau;
			...
			if (RH >= 99.99999999)
				RH = 99.99999999;
			kaact = 0.1 * SMath.exp(-1 * HeightEye / scaleHaerosol) * SMath.pow(1 - 0.32 / SMath.log(RH / 100.0), 1.33)
					* (1 + 0.33 * SL * SMath.sin(sunra * SwissData.DEGTORAD));
	 ...
	 * ################################################################### '
	 * JDNDaysUT [-] ' AltS [deg] ' lat [deg] ' heighteye [m] ' TempS [C] ' RH
	 * [%] ' VR [km] ' ExtType [0=ka,1=kW,2=kR,3=kOZ,4=ktot] ' kt [-]
	 */
	private double kt(double AltS, double sunra, double Lat, double HeightEye, double TempS, double RH, double VR,
			int ExtType, StringBuffer serr) {
		double kRact = 0;
		double kWact = 0;
		double kOZact = 0;
		double kaact = 0;
	...
	/*
	 * ################################################################### '
	 * AppAlt0 [deg] ' PresS [mbar] ' Airmass [??]
	 */
	...
	/*
	 * ################################################################### '
	 * scaleH '[m] ' zend [rad] ' PresS [mbar] ' Xext [-]
	 */
	private double Xext(double scaleH, double zend, double Press) {
		return Press / 1013.0 / (SMath.cos(zend) + 0.01 * SMath.sqrt(scaleH / 1000.0)
				* SMath.exp(-30.0 / SMath.sqrt(scaleH / 1000.0) * SMath.cos(zend)));
	}

	/*
	 * ################################################################### '
	 * scaleH '[m] ' zend [rad] ' PresS [mbar] ' Xlay [-]
	 */
	private double Xlay(double scaleH, double zend, double Press) {
		/*
		 * return Press / 1013.0 / SMath.sqrt(1.0 - SMath.pow(SMath.sin(zend) /
		 * (1.0 + (scaleH / Ra)), 2));
		 */
	 ...
	 * '################################################################### '
	 * TempS [C] ' HeightEye [m] ' TempEfromTempS [C]
	 */
	...
	/*
	 * ################################################################### '
	 * TempS [C] ' PresS [mbar] ' HeightEye [m] ' PresEfromPresS [mbar]
	 */
	private double PresEfromPresS(double TempS, double Press, double HeightEye) {
		return Press
				* SMath.exp(-9.80665 * 0.0289644 / (Kelvin(TempS) + 3.25 * HeightEye / 1000) / 8.31441 * HeightEye);
	}
	...
	/*
	 * ################################################################### '
	 * AltO [deg] ' JDNDaysUT [-] ' AltS [deg] ' lat [deg] ' heighteye [m] '
	 * TempS [C] ' PresS [mbar] ' RH [%] ' VR [km] ' Deltam [-]
	 */
	// *datm = double[], see int swe_heliacal_ut(...);
	private double Deltam(double AltO, double AltS, double sunra, double Lat, double HeightEye, double[] datm,
			int helflag, StringBuffer serr) {
		...
		Deltam__alts_last = AltS;
		Deltam__alto_last = AltO;
		Deltam__sunra_last = sunra;
		if (staticAirmass == 0) {
			zend = (90 - AppAltO) * SwissData.DEGTORAD;
			if (zend > SMath.PI / 2)
				zend = SMath.PI / 2;
			...
			deltam = kR(AltS, HeightEye) * xR + kt(AltS, sunra, Lat, HeightEye, datm[1], datm[2], datm[3], 0, serr) * Xa
					+ kOZ(AltS, sunra, Lat) * XOZ + kW(HeightEye, datm[1], datm[2]) * XW;
		} else {
	 ...
	 * ################################################################### '
	 * AltO [deg] ' JDNDaysUT [-] ' AltS [deg] ' lat [deg] ' heighteye [m] '
	 * TempS [C] ' PresS [mbar] ' RH [%] ' VR [km] ' Bn [nL]
	 */
	private double Bn(double AltO, double JDNDayUT, double AltS, double sunra, double Lat, double HeightEye,
			double[] datm, int helflag, StringBuffer serr) {
		double PresE = PresEfromPresS(datm[1], datm[0], HeightEye);
		double TempE = TempEfromTempS(datm[1], HeightEye, LapseSA);
		double AppAltO = AppAltfromTopoAlt(AltO, TempE, PresE, helflag);
		...
		// int iyar, imon, iday;
		/*
		 * Below altitude of 10 degrees, the Bn stays the same (see page 343
		 * Vistas in Astronomy)
		 */
		if (AppAltO < 10)
			AppAltO = 10;
		zend = (90 - AppAltO) * SwissData.DEGTORAD;
		/*
		 * From Schaefer , Archaeoastronomy, XV, 2000, page 128 and adjusted for
		 * sunspot period
		 */
		/*
		 * YearB = DatefromJDut(JDNDayUT, 1); MonthB = DatefromJDut(JDNDayUT,
		 * 2); DayB = DatefromJDut(JDNDayUT, 3);
		 */
	...
	/*
	 * ################################################################### '
	 * JDNDaysUT [-] ' dgeo [array: longitude, latitude, eye height above sea m]
	 * ' TempE [C] ' PresE [mbar] ' ObjectName [-] ' Magnitude [-]
	 */
	private int Magnitude(double JDNDaysUT, double[] dgeo, StringBuffer ObjectName, int helflag, double[] dmag,
			int dmag_offset, StringBuffer serr) {
		...
		if (Planet != -1) {
			/**
			 * dmag = Phenomena(JDNDaysUT, Lat, Longitude, HeightEye, TempE,
			 * PresE, ObjectName, 4);
			 */
	...
	/*
	 * ################################################################### '
	 * dist [km] ' phasemoon [-] ' MoonsBrightness [-]
	 */
	private double MoonsBrightness(double dist, double phasemoon) {
		 ...
		 * http://hem.passagen.se/pausch/comp/ppcomp.html#15
		 */
		return -21.62 + 5 * SMath.log(dist / (Ra / 1000)) / log10 + 0.026 * SMath.abs(phasemoon)
				+ 0.000000004 * SMath.pow(phasemoon, 4);
	}

	/*
	 * ################################################################### '
	 * AltM [deg] ' AziM [deg] ' AziS [deg] ' MoonPhase [deg]
	 */
	private double MoonPhase(double AltM, double AziM, double AziS) {
		...
		return 180 - SMath.acos(SMath.cos(AziSi - AziMi) * SMath.cos(AltMi + 0.95 * SwissData.DEGTORAD))
				/ SwissData.DEGTORAD;
	}
	 ...
	 * Pressure [mbar]
	 */
	private double Bm(double AltO, double AziO, double AltM, double AziM, double AltS, double AziS, double sunra,
			double Lat, double HeightEye, double[] datm, int helflag, StringBuffer serr) {
		double M0 = -11.05;
		double Bm = 0;
		double RM, kXM, kX, C3, FM, phasemoon, MM;
		if (AltM > -0.26) {
			/*
			 * moon only adds light when (partly) above horizon From Schaefer ,
			 * Archaeoastronomy, XV, 2000, page 129
			 */
			RM = DistanceAngle(AltO * SwissData.DEGTORAD, AziO * SwissData.DEGTORAD, AltM * SwissData.DEGTORAD,
					AziM * SwissData.DEGTORAD) / SwissData.DEGTORAD;
			kXM = Deltam(AltM, AltS, sunra, Lat, HeightEye, datm, helflag, serr);
			kX = Deltam(AltO, AltS, sunra, Lat, HeightEye, datm, helflag, serr);
			C3 = SMath.pow(10, -0.4 * kXM);
			FM = (62000000.0) / RM / RM + SMath.pow(10, 6.15 - RM / 40)
					+ SMath.pow(10, 5.36) * (1.06 + SMath.pow(SMath.cos(RM * SwissData.DEGTORAD), 2));
	 ...
	 * Pressure [mbar]
	 */
	private double Btwi(double AltO, double AziO, double AltS, double AziS, double sunra, double Lat, double HeightEye,
			double[] datm, int helflag, StringBuffer serr) {
		double M0 = -11.05;
		double MS = -26.74;
		double PresE = PresEfromPresS(datm[1], datm[0], HeightEye);
		...
		double RS = DistanceAngle(AltO * SwissData.DEGTORAD, AziO * SwissData.DEGTORAD, AltS * SwissData.DEGTORAD,
				AziS * SwissData.DEGTORAD) / SwissData.DEGTORAD;
	 ...
	 * Pressure [mbar]
	 */
	private double Bday(double AltO, double AziO, double AltS, double AziS, double sunra, double Lat, double HeightEye,
			double[] datm, int helflag, StringBuffer serr) {
		double M0 = -11.05;
		double MS = -26.74;
		double RS = DistanceAngle(AltO * SwissData.DEGTORAD, AziO * SwissData.DEGTORAD, AltS * SwissData.DEGTORAD,
		...
		double FS = (62000000.0) / RS / RS + SMath.pow(10, (6.15 - RS / 40))
				+ SMath.pow(10, 5.36) * (1.06 + SMath.pow(SMath.cos(RS * SwissData.DEGTORAD), 2));
	 ...
	 * Pressure [mbar]
	 */
	private double Bsky(double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AltS, double AziS,
			double sunra, double Lat, double HeightEye, double[] datm, int helflag, StringBuffer serr) {
		double Bsky = 0;
		if (AltS < -3) {
			...
			if (AltS > 4) {
				...
				Bsky += SMath.min(Bday(AltO, AziO, AltS, AziS, sunra, Lat, HeightEye, datm, helflag, serr),
						Btwi(AltO, AziO, AltS, AziS, sunra, Lat, HeightEye, datm, helflag, serr));
			}
		}
	...
	/*
	 * default handling: 1. datm (atmospheric conditions): datm consists of [0]
	 * atmospheric pressure [1] temperature [2] relative humidity [3] extinction
	 * coefficient In order to get default values for [0..2], set datm[0] = 0.
	 * Default values for [1-2] are only provided if [0] == 0. [3] defaults
	 * outside this function, depending on [0-2].
	 * 
	 * 2. dobs (observer definition): [0] age (default 36) [1] Snellen ratio or
	 * visual acuity of observer (default 1)
	 */
	private void default_heliacal_parameters(double[] datm, double[] dgeo, double[] dobs, int helflag) {
		int i;
		if (datm[0] <= 0) {
			/*
			 * estimate atmospheric pressure, according to the International
			...
			/*
			 * relative humidity, independent of atmospheric pressure and
			 * altitude
			 */
			if (datm[2] == 0)
				datm[2] = 40;
		...
		if (dobs[3] == 0) {
			...
			/*
			 * dobs[4] and dobs[5] (OpticDia and OpticTrans) will be defaulted
			 * in OpticFactor()
			 */
		}
	}
	...
	/*
	 * ################################################################### ' age
	 * [Year] ' SN [-] ' AltO [deg] ' AziO [deg] ' AltM [deg] ' AziM [deg] '
	 * MoonDistance [km] ' JDNDaysUT [-] ' AltS [deg] ' AziS [deg] ' lat [deg] '
	 * heighteye [m] ' TempS [C] ' PresS [mbar] ' RH [%] ' VR [km] ' VisLimMagn
	 * [-]
	 */
	private double VisLimMagn(double[] dobs, double AltO, double AziO, double AltM, double AziM, double JDNDaysUT,
			double AltS, double AziS, double sunra, double Lat, double HeightEye, double[] datm, int helflag,
			int[] scotopic_flag, StringBuffer serr) {
		...
		if (Bsk < BNIGHT && (helflag & SweConst.SE_HELFLAG_VISLIM_PHOTOPIC) == 0) {
			C1 = 1.5848931924611e-10; /* SMath.pow(10, -9.8); */ /*
																	 * C1 = 10 ^
																	 * (-9.8);
																	 */
			C2 = 0.012589254117942; /* SMath.pow(10, -1.9); */ /*
																 * C2 = 10 ^
																 * (-1.9);
																 */
			...
			C1 = 4.4668359215096e-9; /* SMath.pow(10, -8.35); */ /*
																	 * C1 = 10 ^
																	 * (-8.35);
																	 */
			C2 = 1.2589254117942e-6; /* SMath.pow(10, -5.9); */ /*
																 * C2 = 10 ^
																 * (-5.9);
																 */
			if (scotopic_flag != null)
	 ...
	 *            "mercury", "jupiter", "saturn", "uranus", "neptune", "moon"
	 *            (case insensitive)
	 * @param helflag
	 *            calculation flag, bitmap<br>
	 *            helflag contains ephemeris flag, like iflag in swe_calc() etc.
	 *            In addition it can contain the following bits:
	 * 
	 *            <pre>
	 ...
	 *      dret[3] ... dret[49]: unused but required on input
	 *            </pre>
	 * 
	 * @param serr
	 *            error string
	 ...
	 */
	public int swe_vis_limit_mag(double tjdut, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName,
			int helflag, double[] dret, StringBuffer serr) {
		int retval = SweConst.OK, i;
		int scotopic_flag[] = new int[] { 0 };
		double AltO[] = new double[1], AziO[] = new double[1], AltM[] = new double[1], AziM[] = new double[1],
				AltS[] = new double[1], AziS[] = new double[1];
		double sunra;
		...
		if (ObjectName.toString().startsWith("moon") || ((helflag & SweConst.SE_HELFLAG_VISLIM_DARK) != 0)
		...
		dret[0] = VisLimMagn(dobs, AltO[0], AziO[0], AltM[0], AziM[0], tjdut, AltS[0], AziS[0], sunra, dgeo[1], dgeo[2],
		...
		/*
		 * if (*serr != '\0') * in VisLimMagn(), serr is only a warning * retval
		 * = SweConst.ERR;
		 */
		return retval;
	}
	...
	/*
	 * ################################################################### '
	 * Magn [-] ' age [Year] ' SN [-] ' AltO [deg] ' AziO [deg] ' AltM [deg] '
	 * AziM [deg] ' MoonDistance [km] ' JDNDaysUT [-] ' AziS [deg] ' lat [deg] '
	 * heighteye [m] ' Temperature [C] ' Pressure [mbar] ' RH [%] ' VR [km] '
	 * TopoArcVisionis [deg]
	 */
	private int TopoArcVisionis(double Magn, double[] dobs, double AltO, double AziO, double AltM, double AziM,
			double JDNDaysUT, double AziS, double sunra, double Lat, double HeightEye, double[] datm, int helflag,
			double[] dret, StringBuffer serr) {
		double Xm, Ym, AltSi, AziSi;
		double xR = 0;
		double Xl = 45;
		double Yl, Yr;
		Yl = Magn - VisLimMagn(dobs, AltO, AziO, AltM, AziM, JDNDaysUT, AltO - Xl, AziS, sunra, Lat, HeightEye, datm,
				helflag, null, serr);
		/* if (*serr != '\0') return SweConst.ERR; * serr is only a warning */
		Yr = Magn - VisLimMagn(dobs, AltO, AziO, AltM, AziM, JDNDaysUT, AltO - xR, AziS, sunra, Lat, HeightEye, datm,
				helflag, null, serr);
		/* if (*serr != '\0') return SweConst.ERR; * serr is only a warning */
		...
		if ((Yl * Yr) <= 0) {
			while (SMath.abs(xR - Xl) > epsilon) {
				...
				AltSi = AltO - Xm;
				AziSi = AziS;
				Ym = Magn - VisLimMagn(dobs, AltO, AziO, AltM, AziM, JDNDaysUT, AltSi, AziSi, sunra, Lat, HeightEye,
						datm, helflag, null, serr);
				/*
				 * if (*serr != '\0') return SweConst.ERR; * serr is only a
				 * warning
				 */
				if ((Yl * Ym) > 0) {
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_topo_arcus_visionis(double tjdut, double[] dgeo, double[] datm, double[] dobs, int helflag,
			double mag, double azi_obj, double alt_obj, double azi_sun, double azi_moon, double alt_moon, double[] dret,
			StringBuffer serr) {
		double sunra;
		SweDate.swi_set_tid_acc(tjdut, helflag, 0);
		sunra = SunRA(tjdut, helflag, serr);
		if (serr != null && serr.length() > 0)
			return SweConst.ERR;
		return TopoArcVisionis(mag, dobs, alt_obj, azi_obj, alt_moon, azi_moon, tjdut, azi_sun, sunra, dgeo[1], dgeo[2],
				datm, helflag, dret, serr);
	}

	/* ################################################################### */
	/*
	 * ' Magn [-] ' age [Year] ' SN Snellen factor of the visual aquity of the
	 * observer see: http://www.i-see.org/eyecharts.html#make-your-own ' AziO
	 * [deg] ' AltM [deg] ' AziM [deg] ' MoonDistance [km] ' JDNDaysUT [-] '
	 * AziS [deg] ' Lat [deg] ' HeightEye [m] ' Temperature [C] ' Pressure
	 * [mbar] ' RH [%] relative humidity ' VR [km] Meteorological Range, see
	 * http://www.iol.ie/~geniet/eng/atmoastroextinction.htm ' TypeAngle '
	 * [0=Object's altitude, ' 1=Arcus Visonis (Object's altitude - Sun's
	 * altitude), ' 2=Sun's altitude] ' HeliacalAngle [deg]
	 */
	private int HeliacalAngle(double Magn, double[] dobs, double AziO, double AltM, double AziM, double JDNDaysUT,
			double AziS, double[] dgeo, double[] datm, int helflag, double[] dangret, StringBuffer serr) {
		double x, minx, maxx, xmin, ymin, Xl, xR, Yr[] = new double[1], Yl[] = new double[1], Xm, Ym[] = new double[1],
				xmd, ymd[] = new double[1];
		...
		ymin = 10000;
		for (x = minx; x <= maxx; x++) {
			if (TopoArcVisionis(Magn, dobs, x, AziO, AltM, AziM, JDNDaysUT, AziS, sunra, Lat, HeightEye, datm, helflag,
					Arc, serr) == SweConst.ERR)
				return SweConst.ERR;
			if (Arc[0] < ymin) {
				ymin = Arc[0];
				xmin = x;
		...
		Xl = xmin - 1;
		xR = xmin + 1;
		if (TopoArcVisionis(Magn, dobs, xR, AziO, AltM, AziM, JDNDaysUT, AziS, sunra, Lat, HeightEye, datm, helflag, Yr,
				serr) == SweConst.ERR)
			return SweConst.ERR;
		if (TopoArcVisionis(Magn, dobs, Xl, AziO, AltM, AziM, JDNDaysUT, AziS, sunra, Lat, HeightEye, datm, helflag, Yl,
				serr) == SweConst.ERR)
			return SweConst.ERR;
		/* http://en.wikipedia.org/wiki/Bisection_method */
		while (SMath.abs(xR - Xl) > 0.1) {
			...
			DELTAx = 0.025;
			xmd = Xm + DELTAx;
			if (TopoArcVisionis(Magn, dobs, Xm, AziO, AltM, AziM, JDNDaysUT, AziS, sunra, Lat, HeightEye, datm, helflag,
					Ym, serr) == SweConst.ERR)
				return SweConst.ERR;
			if (TopoArcVisionis(Magn, dobs, xmd, AziO, AltM, AziM, JDNDaysUT, AziS, sunra, Lat, HeightEye, datm,
					helflag, ymd, serr) == SweConst.ERR)
				return SweConst.ERR;
			if (Ym[0] >= ymd[0]) {
				/* Throw away left half */
				Xl = Xm;
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_heliacal_angle(double tjdut, double[] dgeo, double[] datm, double[] dobs, int helflag, double mag,
			double azi_obj, double azi_sun, double azi_moon, double alt_moon, double[] dret, StringBuffer serr) {
		if (dgeo[2] < SwephData.SEI_ECL_GEOALT_MIN || dgeo[2] > SwephData.SEI_ECL_GEOALT_MAX) {
			if (serr != null) {
				serr.setLength(0);
				serr.append(String.format(Locale.US,
						"location for heliacal events must be between %.0f and %.0f m above sea",
						SwephData.SEI_ECL_GEOALT_MIN, SwephData.SEI_ECL_GEOALT_MAX));
			}
			return SweConst.ERR;
		}
		SweDate.swi_set_tid_acc(tjdut, helflag, 0);
	...
	/*
	 * ################################################################### '
	 * AltO [deg] ' AziO [deg] ' AltS [deg] ' AziS [deg] ' parallax [deg] '
	 * WidthMoon [deg]
	 */
	private double WidthMoon(double AltO, double AziO, double AltS, double AziS, double parallax) {
		...
		return 0.27245 * parallax
				* (1 + SMath.sin(GeoAltO * SwissData.DEGTORAD) * SMath.sin(parallax * SwissData.DEGTORAD))
				* (1 - SMath.cos((AltS - GeoAltO) * SwissData.DEGTORAD)
						* SMath.cos((AziS - AziO) * SwissData.DEGTORAD));
	}
	...
	private double LengthMoon(double W, double Diamoon) {
		 ...
		 * http://calendar.ut.ac.ir/Fa/Crescent/Data/Sultan2005.pdf
		 */
	...
	/* ################################################################### */
	private int DeterTAV(double[] dobs, double JDNDaysUT, double[] dgeo, double[] datm, StringBuffer ObjectName,
			int helflag, double[] dret, StringBuffer serr) {
		double[] Magn = new double[1], AltO = new double[1], AziS = new double[1], AziO = new double[1],
				AziM = new double[1], AltM = new double[1];
		double sunra = SunRA(JDNDaysUT, helflag, serr);
		...
		if (ObjectLoc(JDNDaysUT, dgeo, datm, "sun", 1, helflag, AziS, serr) == SweConst.ERR)
			return SweConst.ERR;
		if (TopoArcVisionis(Magn[0], dobs, AltO[0], AziO[0], AltM[0], AziM[0], JDNDaysUT, AziS[0], sunra, dgeo[1],
				dgeo[2], datm, helflag, dret, serr) == SweConst.ERR)
			return SweConst.ERR;
		return SweConst.OK;
	}
	...
	/*
	 * ################################################################### ' A
	 * y-value at x=1 ' B y-value at x=0 ' C y-value at x=-1 ' x2min minimum for
	...
	/*
	 * ################################################################### ' A
	 * y-value at x=1 ' B y-value at x=0 ' C y-value at x=-1 ' x ' y is y-value
	...
	/*
	 * ################################################################### '
	 * JDNDaysUT [JDN] ' HPheno '0=AltO [deg] topocentric altitude of object
	 * (unrefracted) '1=AppAltO [deg] apparent altitude of object (refracted)
	 * '2=GeoAltO [deg] geocentric altitude of object '3=AziO [deg] azimuth of
	 * object '4=AltS [deg] topocentric altitude of Sun '5=AziS [deg] azimuth of
	 * Sun '6=TAVact [deg] actual topocentric arcus visionis '7=ARCVact [deg]
	 * actual (geocentric) arcus visionis '8=DAZact [deg] actual difference
	 * between object's and sun's azimuth '9=ARCLact [deg] actual longitude
	 * difference between object and sun '10=kact [-] extinction coefficient
	 * '11=minTAV [deg] smallest topocentric arcus visionis '12=TfistVR [JDN]
	 * first time object is visible, according to VR '13=TbVR [JDN] optimum time
	 * the object is visible, according to VR '14=TlastVR [JDN] last time object
	 * is visible, according to VR '15=TbYallop[JDN] best time the object is
	 * visible, according to Yallop '16=WMoon [deg] cresent width of moon
	 * '17=qYal [-] q-test value of Yallop '18=qCrit [-] q-test criterion of
	 * Yallop '19=ParO [deg] parallax of object '20 Magn [-] magnitude of object
	 * '21=RiseO [JDN] rise/set time of object '22=RiseS [JDN] rise/set time of
	 * sun '23=Lag [JDN] rise/set time of object minus rise/set time of sun
	 * '24=TvisVR [JDN] visibility duration '25=LMoon [deg] cresent length of
	 * moon '26=CVAact [deg] '27=Illum [%] 'new '28=CVAact [deg] 'new '29=MSk
	 * [-]
	 */
	/**
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_heliacal_pheno_ut(double JDNDaysUT, double[] dgeo, double[] datm, double[] dobs,
			StringBuffer ObjectNameIn, int TypeEvent, int helflag, double[] darr, StringBuffer serr) {
		double[] AziS = new double[1], AltS = new double[1], AltS2 = new double[1], AziO = new double[1],
				AltO = new double[1], AltO2 = new double[1], GeoAltO = new double[1], MagnO = new double[1];
		...
		// Attention: MinTAV does not get initialized in C but assigned to
		// darr[] sometimes later
		...
		if (Magnitude(JDNDaysUT, dgeo, ObjectName, helflag, MagnO, 0, serr) == SweConst.ERR)
			return SweConst.ERR;
		ARCVact = TAVact + ParO;
		ARCLact = SMath.acos(SMath.cos(ARCVact * SwissData.DEGTORAD) * SMath.cos(DAZact * SwissData.DEGTORAD))
				/ SwissData.DEGTORAD;
		Planet = DeterObject(ObjectName);
		if (Planet == -1) {
			elong = ARCLact;
			...
			retval = sw.swe_pheno_ut(JDNDaysUT, Planet, iflag | (SweConst.SEFLG_TOPOCTR | SweConst.SEFLG_EQUATORIAL),
					attr, serr);
			if (retval == SweConst.ERR)
				return SweConst.ERR;
			elong = attr[2];
		...
		if ((TypeEvent == 3 || TypeEvent == 4) && (Planet == -1 || Planet >= SweConst.SE_MARS)) {
			...
			goto_output_heliacal_pheno(darr, AltO, AppAltO, GeoAltO, AziO, AltS, AziS, TAVact, ARCVact, DAZact, ARCLact,
					kact, MinTAV, TfirstVR, TbVR, TlastVR, TbYallop, WMoon, qYal, qCrit, ParO, MagnO, RiseSetO,
					RiseSetS, Lag, TvisVR, LMoon, elong, illum);
		}
		...
		do {
			...
			if (MinTAVoud < MinTAVact[0] && TbVR == 0) {
				/*
				 * determine if this is a local minimum with object still above
				 * horizon
				 */
		...
		} while (SMath.abs(TimePointer - RiseSetS[0]) <= MaxTryHours / 24.0 && Ta == 0
				&& !((TbVR != 0 && (TypeEvent == 3 || TypeEvent == 4)
						&& (!ObjectName.toString().startsWith("moon") && !ObjectName.toString().startsWith("venus")
								&& !ObjectName.toString().startsWith("mercury")))));
		if (RS == 2) {
			TfirstVR = Tc;
			TlastVR = Ta;
		} else {
		...
		if (TlastVR == 0)
			TlastVR = SweConst.TJD_INVALID; /* "#NA!" */
		if (TbVR == 0)
			TbVR = SweConst.TJD_INVALID; /* "#NA!" */
		if (TfirstVR == 0)
			TfirstVR = SweConst.TJD_INVALID; /* "#NA!" */

		goto_output_heliacal_pheno(darr, AltO, AppAltO, GeoAltO, AziO, AltS, AziS, TAVact, ARCVact, DAZact, ARCLact,
				kact, MinTAV, TfirstVR, TbVR, TlastVR, TbYallop, WMoon, qYal, qCrit, ParO, MagnO, RiseSetO, RiseSetS,
				Lag, TvisVR, LMoon, elong, illum);
		return SweConst.OK;
	}

	private void goto_output_heliacal_pheno(double[] darr, double[] AltO, double AppAltO, double[] GeoAltO,
			double[] AziO, double[] AltS, double[] AziS, double TAVact, double ARCVact, double DAZact, double ARCLact,
			double kact, double MinTAV, double TfirstVR, double TbVR, double TlastVR, double TbYallop, double WMoon,
			double qYal, double qCrit, double ParO, double[] MagnO, double[] RiseSetO, double[] RiseSetS, double Lag,
			double TvisVR, double LMoon, double elong, double illum) {
		/* End If */
		darr[0] = AltO[0];
		darr[1] = AppAltO;
		darr[2] = GeoAltO[0];
	...
	private double get_synodic_period(int Planet) {
		/*
		 * synodic periods from: Kelley/Milone/Aveni, "Exploring ancient Skies",
		 * p. 43.
		 */
		switch (Planet) {
	...
	/* ################################################################### */
	private int moon_event_arc_vis(double JDNDaysUTStart, double[] dgeo, double[] datm, double[] dobs, int TypeEvent,
			int helflag, double[] dret, StringBuffer serr) {
		...
		do {
			JDNDaysUT = JDNDaysUT + Daystep;
			if ((retval = RiseSet(JDNDaysUT, dgeo, datm, ObjectName.toString(), TypeEvent, helflag, 0, tjd_moonevent,
					serr)) != SweConst.OK)
				return retval;
			...
			do {
				...
				// private int ObjectLoc(double JDNDaysUT, double[] dgeo,
				// double[] datm, String ObjectName, int Angle, int helflag,
				// double[] dret, StringBuffer serr) {
				// private int ObjectLoc(double JDNDaysUT, double[] dgeo,
				// double[] datm, StringBuffer ObjectName, int Angle, int
				// helflag, double[] dret, StringBuffer serr) {
				 ...
				 */
				/*
				 * while (MinTAV <= MinTAVoud && SMath.abs(tjd_moonevent -
				 * tjd_moonevent_start) < 120.0 / 60.0 / 24.0);
				 */
			} while ((MinTAV[0] <= MinTAVoud || LocalminCheck[0] < MinTAV[0])
					&& SMath.abs(tjd_moonevent[0] - tjd_moonevent_start) < 120.0 / 60.0 / 24.0);
			/*
			 * while (DeltaAlt < MinTAVoud && SMath.abs(JDNDaysUT - JDNDaysUTi)
			 * < 15);
			 */
	...
	private int heliacal_ut_arc_vis(double JDNDaysUTStart, double[] dgeo, double[] datm, double[] dobs,
			StringBuffer ObjectName, int TypeEventIn, int helflag, double[] dret, StringBuffer serr_ret) {
		...
		double JDNDaysUT, JDNDaysUTfinal, JDNDaysUTstep, JDNDaysUTstepoud, JDNarcvisUT, tjd_tt, tret[] = new double[1],
				OudeDatum, JDNDaysUTinp = JDNDaysUTStart, JDNDaysUTtijd;
		double ArcusVis, ArcusVisDelta, ArcusVisPto, ArcusVisDeltaoud;
		double Trise, sunsangle, Theliacal, Tdelta, Angle;
		double TimeStep, TimePointer, OldestMinTAV[] = new double[1], MinTAVoud[] = new double[1],
				MinTAVact[] = new double[1], extrax, TbVR = 0;
		...
		if ((helflag & SweConst.SE_HELFLAG_HIGH_PRECISION) == 0)
			iflag |= SweConst.SEFLG_NONUT | SweConst.SEFLG_TRUEPOS;
		/*
		 * start values for search of heliacal rise maxlength = phase period in
		 * days, smaller than minimal synodic period
		 */
		...
		/* normalize the maxlength to the step size */
		{
			...
			JDNDaysUT = JDNDaysUTStart;
			/*
			 * make sure one can find an event on the just after the
			 * JDNDaysUTStart
			 */
			...
			do { /* this is a do {} while() loop */
				...
				do { /* this is a do {} while() loop */
					...
					if ((retval = my_rise_trans(JDNDaysUTstep, SweConst.SE_SUN, "", eventtype, helflag, dgeo, datm,
							tret, serr)) == SweConst.ERR)
						// goto swe_heliacal_err;
						return goto_swe_heliacal_err1(serr, serr_ret, retval);
					/*
					 * determine time compensation to get Sun's altitude at
					 * heliacal rise
					 */
					...
					if ((helflag & SweConst.SE_HELFLAG_AVKIND_MIN7) != 0)
						sunsangle = -7;
					if ((helflag & SweConst.SE_HELFLAG_AVKIND_MIN9) != 0)
						sunsangle = -9;
					...
					if (TypeEvent == 2 || TypeEvent == 3)
						Tdelta = -Tdelta;
					/*
					 * determine appr.time when sun is at the wanted Sun's
					 * altitude
					 */
					...
					if (Planet != -1) {
						...
						if ((retval = Magnitude(JDNarcvisUT, dgeo, ObjectName, helflag, objectmagn, 0,
								serr)) == SweConst.ERR)
							// goto swe_heliacal_err;
							return goto_swe_heliacal_err1(serr, serr_ret, retval);
					} else {
					...
					DeltaAlt = AltO - AltS;
					/*
					 * if ((retval = HeliacalAngle(objectmagn, dobs, AziO, AltM,
					 * AziM, JDNarcvisUT, AziS, dgeo, datm, helflag, dang,
					 * serr)) == SweConst.ERR)
					 */
					if ((retval = HeliacalAngle(objectmagn[0], dobs, AziO, -1, 0, JDNarcvisUT, AziS, dgeo, datm,
							helflag, dang, serr)) == SweConst.ERR)
						// goto swe_heliacal_err;
						return goto_swe_heliacal_err1(serr, serr_ret, retval);
					ArcusVis = dang[1];
					ArcusVisPto = dang[2];
					 ...
					 * ArcusVisDelta < 0) && (JDNDaysUTfinal - JDNDaysUTstep) *
					 * Sgn(DayStep) > 0);
					 */
				} while ((ArcusVisDeltaoud > 0 || ArcusVisDelta < 0)
						&& (JDNDaysUTfinal - JDNDaysUTstep) * Sgn(DayStep) > 0);
	...
	private int get_asc_obl(double tjd, int ipl, String star, int iflag, double[] dgeo, boolean desc_obl, double[] daop,
			StringBuffer serr) {
		return get_asc_obl(tjd, ipl, new StringBuffer(star), iflag, dgeo, desc_obl, daop, serr);
	}

	private int get_asc_obl(double tjd, int ipl, StringBuffer star, int iflag, double[] dgeo, boolean desc_obl,
			double[] daop, StringBuffer serr) {
		int retval;
	...
	private int get_asc_obl_diff(double tjd, int ipl, StringBuffer star, int iflag, double[] dgeo, boolean desc_obl,
			boolean is_acronychal, double[] dsunpl, StringBuffer serr) {
	...
	/*
	 * times of - superior and inferior conjunction (Mercury and Venus) -
	 * conjunction and opposition (ipl >= Mars)
	 */
	private double tcon[] = new double[] { 0, 0, 2451550, 2451550, /* Moon */
	...
	private int find_conjunct_sun(double tjd_start, int ipl, int helflag, int TypeEvent, double[] tjd,
			StringBuffer serr) {
		int epheflag = helflag & (SweConst.SEFLG_JPLEPH | SweConst.SEFLG_SWIEPH | SweConst.SEFLG_MOSEPH);
		int i;
		...
		// type cast to "floor"???
		// tjdcon = tjd0 + ((floor) ((tjd_start - tjd0) / dsynperiod) + 1) *
		// dsynperiod;
	...
	private int get_asc_obl_with_sun(double tjd_start, int ipl, StringBuffer star, int helflag, int evtyp,
			double dperiod, double[] dgeo, double[] tjdret, StringBuffer serr) {
		int i, retval;
		...
		while (dsunpl_save == -999999999 ||
				...
				SMath.abs(dsunpl[0]) + SMath.abs(dsunpl_save) > 180 || (retro && !(dsunpl_save < 0 && dsunpl[0] >= 0))
	...
	private int get_heliacal_day(double tjd, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName,
			int helflag, int TypeEvent, double[] thel, StringBuffer serr) {
		...
		retval_old = -2;
		for (tday = tjd; (direct_day > 0 && tday < tend)
				|| (direct_day < 0 && tday > tend); tday += daystep * direct_day) {
			vdelta = -100;
			if ((retval = my_rise_trans(tday, SweConst.SE_SUN, "", is_rise_or_set, helflag, dgeo, datm, tret,
					serr)) == SweConst.ERR) {
				return SweConst.ERR;
			}
			...
			if (retval_old == -2 && retval >= 0 && daystep > 1) {
				 ...
				 * the event. I don't know if this happens
				 */
	...
	private int time_optimum_visibility(double tjd, double[] dgeo, double[] datm, double[] dobs,
			StringBuffer ObjectName, int helflag, double[] tret, int tret_offset, StringBuffer serr) {
		...
		for (i = 0, d = 100.0 / 86400.0; i < 3; i++, d /= 10.0) {
			while ((retval = swe_vis_limit_mag(tjd - d, dgeo, datm, dobs, ObjectName, helflag, darr, serr)) >= 0
					&& darr[0] > darr[7] && darr[0] - darr[7] > vl) {
				tjd -= d;
				vl = darr[0] - darr[7];
				retval_sv = retval;
				phot_scot_opic_sv = retval & SweConst.SE_SCOTOPIC_FLAG;
		...
		if (retval >= 0) {
			/*
			 * search for optimum came to an end because change
			 * scotopic/photopic:
			 */
	...
	private int time_limit_invisible(double tjd, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName,
			int helflag, int direct, double[] tret, int tret_offset, StringBuffer serr) {
		...
		for (i = 0, d = d0; i < ncnt; i++, d /= 10.0) {
			while ((retval = swe_vis_limit_mag(tjd + d * direct, dgeo, datm, dobs, ObjectName, helflag, darr,
					serr)) >= 0 && darr[0] > darr[7]) {
				tjd += d * direct;
				retval_sv = retval;
				phot_scot_opic_sv = retval & SweConst.SE_SCOTOPIC_FLAG;
				/* printf("%d: %f\n", direct, darr[8]); */
		...
		/*
		 * printf("4: %f, %f/%f %f <-> %f\n", darr[8], darr[0], darr[7], tjd,
		 * phot_scot_opic_sv);
		 */
		tret[tret_offset] = tjd;
		/*
		 * if object disappears at setting, retval is -2, but we want it
		 * SweConst.OK, and also suppress the warning
	...
	private int get_acronychal_day(double tjd, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName,
			int helflag, int TypeEvent, double[] thel, StringBuffer serr) {
		...
		if (TypeEvent == 3 || TypeEvent == 5) {
			is_rise_or_set = SweConst.SE_CALC_RISE;
			/*
			 * tret = tjdc - 3; if (ipl >= SE_MARS) tret = tjdc - 3;
			 */
			direct = -1;
		} else {
			is_rise_or_set = SweConst.SE_CALC_SET;
		...
		while (SMath.abs(dtret) > 0.5 / 1440.0) {
			...
			if (direct < 0)
				tjd_intern[0] -= 1;
			retval = my_rise_trans(tjd_intern[0], ipl, ObjectName.toString(), is_rise_or_set, helflag, dgeo, datm,
					tjd_intern, serr);
			if (retval == SweConst.ERR)
				return SweConst.ERR;
			...
			retval = time_limit_invisible(tjd_intern[0], dgeo, datm, dobs, ObjectName,
					helflag | SweConst.SE_HELFLAG_VISLIM_DARK, direct, tret_dark, 0, serr);
			if (retval == SweConst.ERR)
				return SweConst.ERR;
			retval = time_limit_invisible(tjd_intern[0], dgeo, datm, dobs, ObjectName,
					helflag | SweConst.SE_HELFLAG_VISLIM_NOMOON, direct, tret, 0, serr);
			if (retval == SweConst.ERR)
				return SweConst.ERR;
	...
	private int get_heliacal_details(double tday, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName,
			int TypeEvent, int helflag, double[] dret, StringBuffer serr) {
		int i, retval, direct;
		...
		/*
		 * correct sequence of times: with event types 2 and 3 swap dret[0] and
		 * dret[2]
		 */
		if (TypeEvent == 2 || TypeEvent == 3) {
			tday = dret[2];
	...
	private int heliacal_ut_vis_lim(double tjd_start, double[] dgeo, double[] datm, double[] dobs,
			StringBuffer ObjectName, int TypeEventIn, int helflag, double[] dret, StringBuffer serr_ret) {
		int i;
		...
		if (ipl == SweConst.SE_MERCURY)
			...
			tjd[0] = tjd_start
					- 50; /*
							 * -50 makes sure, that no event is missed, but may
							 * return an event before start date
							 */
		helflag2 = helflag;
		...
		if (ipl == SweConst.SE_MERCURY || ipl == SweConst.SE_VENUS || TypeEvent <= 2) {
			if (ipl == -1) {
				...
				if (retval != SweConst.OK)
					// goto swe_heliacal_err; /* retval may be -2 or
					// SweConst.ERR */
			...
			if (true || ipl == -1) {
				/*
				 * retval = get_asc_obl_acronychal(tjd, ipl, ObjectName,
				 * helflag2, TypeEvent, dgeo, &tjd, serr);
				 */
				retval = get_asc_obl_with_sun(tjd[0], ipl, ObjectName, helflag, TypeEvent, 0, dgeo, tjd, serr);
				if (retval != SweConst.OK)
					// goto swe_heliacal_err;
		...
		if ((helflag & SweConst.SE_HELFLAG_NO_DETAILS) == 0) {
			/*
			 * more precise event times for - morning first, evening last -
			...
			if (ipl == SweConst.SE_MERCURY || ipl == SweConst.SE_VENUS || TypeEvent <= 2) {
				...
				if (TypeEvent == 4 || TypeEvent == 6)
					direct = -1;
				for (i = 0, d = 100.0 / 86400.0; i < 3; i++, d /= 10.0) {
					while ((retval = swe_vis_limit_mag(dret[0] + d * direct, dgeo, datm, dobs, ObjectName, helflag,
							darr, serr)) == -2 || (retval >= 0 && darr[0] < darr[7])) {
						dret[0] += d * direct;
					}
				}
	...
	/* ################################################################### */
	private int moon_event_vis_lim(double tjdstart, double[] dgeo, double[] datm, double[] dobs, int TypeEvent,
			int helflag, double[] dret, StringBuffer serr_ret) {
		...
		/* check Synodic/phase Period */
		tjd[0] = tjdstart - 30; /*
								 * -50 makes sure, that no event is missed, but
								 * may return an event before start date
								 */
		if ((retval = find_conjunct_sun(tjd[0], ipl, helflag, TypeEvent, tjd, serr)) == SweConst.ERR)
			return SweConst.ERR;
		/* find the day and minute on which the object becomes visible */
		 ...
		 */
		if (TypeEvent == 3) {
			if ((retval = my_rise_trans(tjd[0], SweConst.SE_SUN, "", SweConst.SE_CALC_SET, helflag, dgeo, datm, trise,
					serr)) == SweConst.ERR)
				return SweConst.ERR;
			if (trise[0] < dret[1]) {
				dret[0] = trise[0];
				...
				/*
				 * strcpy(serr,
				 * "start time given is sunset, but moon is observable before that"
				 * );
				 */
			}
			/*
			 * if the moon is visible after sunrise, we return sunrise as end
			 * time
			 */
		} else {
			if ((retval = my_rise_trans(dret[1], SweConst.SE_SUN, "", SweConst.SE_CALC_RISE, helflag, dgeo, datm, trise,
					serr)) == SweConst.ERR)
				return SweConst.ERR;
			if (dret[0] > trise[0]) {
				dret[0] = trise[0];
				...
				/*
				 * strcpy(serr,
				 * "end time given is sunrise, but moon is observable after that"
				 * );
				 */
			}
		}
	...
	private int MoonEventJDut(double JDNDaysUTStart, double[] dgeo, double[] datm, double[] dobs, int TypeEvent,
			int helflag, double[] dret, StringBuffer serr) {
		int avkind = helflag & SweConst.SE_HELFLAG_AVKIND;
		if (avkind != 0)
	...
	private int heliacal_ut(double JDNDaysUTStart, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName,
			int TypeEventIn, int helflag, double[] dret, StringBuffer serr_ret) {
		int avkind = helflag & SweConst.SE_HELFLAG_AVKIND;
		if (avkind != 0)
			return heliacal_ut_arc_vis(JDNDaysUTStart, dgeo, datm, dobs, ObjectName, TypeEventIn, helflag, dret,
					serr_ret);
		else
			return heliacal_ut_vis_lim(JDNDaysUTStart, dgeo, datm, dobs, ObjectName, TypeEventIn, helflag, dret,
					serr_ret);
	}

	/*
	 * ' Magn [-] ' tjd_ut start date (JD) for event search ' dgeo[3] geogr.
	 * longitude, latitude, eye height (m above sea level) ' datm[4] atm.
	 * pressure, temperature, RH, and VR ' - pressure atmospheric pressure
	 * (mbar, =hPa) default 1013.25hPa ' - temperature deg C, default 15 deg C
	 * (if at ' If both attemp and atpress are 0, a temperature and '
	 * atmospheric pressure are estimated from the above-mentioned ' default
	 * values and the height above sea level. ' - RH relative humidity in % ' -
	 * VR VR>=1: the Meteorological range: default 40 km ' 1>VR>0: the ktot (so
	 * the total atmospheric coefficient): ' a good default would be 0.25 '
	 * VR=-1: the ktot is calculated from the other atmospheric ' constants. '
	 * age [Year] default 36, experienced sky observer in ancient times '
	 * optimum age is 23 ' SN Snellen factor of the visual aquity of the
	 * observer ' default 1 ' see:
	 * http://www.i-see.org/eyecharts.html#make-your-own ' TypeEvent 1 morning
	 * first ' 2 evening last ' 3 evening first ' 4 morning last ' dret output:
	 * time (tjd_ut) of heliacal event ' dret[0]: beginning of visibility
	 * (Julian day number) ' dret[1]: optimum visibility (Julian day number; 0
	 * if SE_HELFLAG_AV) ' dret[2]: end of visibility (Julian day number; 0 if
	 * SE_HELFLAG_AV) ' see
	 * http://www.iol.ie/~geniet/eng/atmoastroextinction.htm
	 */
	/**
	 * <b>ATTENTION: This method possibly (re-)sets a global parameter used in
	 * calculation of delta T. See SweDate.setGlobalTidalAcc(double).</b>
	 ...
	 * @see SweDate#setGlobalTidalAcc(double)
	 */
	public int swe_heliacal_ut(double JDNDaysUTStart, double[] dgeo, double[] datm, double[] dobs,
			StringBuffer ObjectNameIn, int TypeEvent, int helflag, double[] dret, StringBuffer serr_ret) {
		...
		String sevent[] = new String[] { "", "morning first", "evening last", "evening first", "morning last",
				"acronychal rising", "acronychal setting" };
		if (dgeo[2] < SwephData.SEI_ECL_GEOALT_MIN || dgeo[2] > SwephData.SEI_ECL_GEOALT_MAX) {
			if (serr_ret != null) {
				serr_ret.setLength(0);
				serr_ret.append(String.format(Locale.US,
						"location for heliacal events must be between %.0f and %.0f m above sea",
						SwephData.SEI_ECL_GEOALT_MIN, SwephData.SEI_ECL_GEOALT_MAX));
			}
			return SweConst.ERR;
		...
		/*
		 * if (helflag & SE_HELFLAG_SEARCH_1_PERIOD) MaxCountSynodicPeriod = 1;
		 */
		serr.setLength(0);
		if (serr_ret != null)
			serr_ret.setLength(0);
		...
		if (Planet == SweConst.SE_MOON) {
			if (TypeEvent == 1 || TypeEvent == 2) {
				if (serr_ret != null) {
					serr_ret.setLength(0);
					serr_ret.append(
							sevent[TypeEvent] + " (event type " + TypeEvent + ") does not exist for the moon\n");
				}
				return SweConst.ERR;
			}
			tjd = tjd0;
			...
			if (Planet == -1 || Planet >= SweConst.SE_MARS) {
				if (TypeEvent == 3 || TypeEvent == 4) {
					if (serr_ret != null) {
						...
						serr_ret.setLength(0);
						serr_ret.append(
								sevent[TypeEvent] + " (event type " + TypeEvent + ") does not exist for " + s + "\n");
					}
					return SweConst.ERR;
				}
			}
		...
		if ((helflag & SweConst.SE_HELFLAG_AVKIND) != 0) {
			...
			if (TypeEvent == SweConst.SE_ACRONYCHAL_RISING || TypeEvent == SweConst.SE_ACRONYCHAL_SETTING) {
				if (serr_ret != null) {
					...
					serr_ret.setLength(0);
					serr_ret.append(
							sevent[TypeEvent] + " (event type " + TypeEvent + ") is not provided for " + s + "\n");
				}
				return SweConst.ERR;
			}
		}
		 ...
		 */
		if ((helflag & SweConst.SE_HELFLAG_SEARCH_1_PERIOD) != 0
