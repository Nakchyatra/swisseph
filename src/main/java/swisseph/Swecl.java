...
/**
 * This class initiates the calculation of solar and lunar eclipses, of sun
 * risetime and time of sunset, and lastly the calculation of the phenomena
 * phase angle, phase, elongation of planet, apparent diameter of disc and
 * apparent magnitude of the sun, moon, any planet or asteroid.<BR>
 *
 * <P>
...
class Swecl implements java.io.Serializable {
	...
	private double const_lapse_rate = SwephData.SE_LAPSE_RATE; /*
																 * for refraction
																 */
	...
	/**
	 * Creates a new instance of this object and uses all the given objects, as
	 * far as they are not null.
	 * 
	 * @param sw
	 *            A SwissEph object that might already be available at the time
	 *            of creation of this object.
	 * @param sl
	 *            A SwissLib object that might already be available at the time
	 *            of creation of this object.
	 * @param sm
	 *            A Swemmoon object that might already be available at the time
	 *            of creation of this object.
	 * @param swed
	 *            A SwissData object that might already be available at the time
	 *            of creation of this object.
	 */
	 ...
	 * iflag: to indicate ephemeris to be used (SEFLG_JPLEPH, SEFLG_SWIEPH,
	 * SEFLG_MOSEPH)
	 *
	 * Algorithms for the central line is taken from Montenbruck, pp. 179ff.,
	 * with the exception, that we consider refraction for the maxima of partial
	 * and noncentral eclipses. Geographical positions are referred to sea level
	 * / the mean ellipsoid.
	 *
	 * Errors: - from uncertainty of JPL-ephemerides (0.01 arcsec): about 40
	 * meters - from displacement of shadow points by atmospheric refraction: a
	 * few meters - from deviation of the geoid from the ellipsoid a few meters
	 * - from polar motion a few meters For geographical locations that are
	 * interesting for observation, the error is always < 100 m. However, if the
	 * sun is close to the horizon, all of these errors can grow up to a km or
	 * more.
	 *
	 * Function returns: -1 (ERR) on error (e.g. if swe_calc() for sun or moon
	 * fails) 0 if there is no solar eclipse at tjd SE_ECL_TOTAL SE_ECL_ANNULAR
	 * SE_ECL_TOTAL | SE_ECL_CENTRAL SE_ECL_TOTAL | SE_ECL_NONCENTRAL
	 * SE_ECL_ANNULAR | SE_ECL_CENTRAL SE_ECL_ANNULAR | SE_ECL_NONCENTRAL
	 * SE_ECL_PARTIAL
	 *
	 * geopos[0]: geographic longitude of central line geopos[1]: geographic
	 * latitude of central line
	 *
	 * not implemented so far:
	 *
	 * geopos[2]: geographic longitude of northern limit of umbra geopos[3]:
	 * geographic latitude of northern limit of umbra geopos[4]: geographic
	 * longitude of southern limit of umbra geopos[5]: geographic latitude of
	 * southern limit of umbra geopos[6]: geographic longitude of northern limit
	 * of penumbra geopos[7]: geographic latitude of northern limit of penumbra
	 * geopos[8]: geographic longitude of southern limit of penumbra geopos[9]:
	 * geographic latitude of southern limit of penumbra
	 *
	 * Attention: "northern" and "southern" limits of umbra do not necessarily
	 * correspond to the northernmost or southernmost geographic position, where
	 * the total, annular, or partial phase is visible at a given time. Imagine
	 * a situation in northern summer, when the sun illuminates the northern
	 * polar circle. The southernmost point of the core shadow may then touch
	 * the north pole, and therefore the northernmost point will be more in the
	 * south. Note also that with annular eclipses, the northern edge is usually
	 * geographically the southern one. With annular-total ones, the two lines
	 * cross, usually twice. The maximum is always total in such cases.
	 *
	 * attr[0] fraction of solar diameter covered by moon (magnitude) attr[1]
	 * ratio of lunar diameter to solar one attr[2] fraction of solar disc
	 * covered by moon (obscuration) attr[3] diameter of core shadow in km
	 * attr[4] azimuth of sun at tjd attr[5] true altitude of sun above horizon
	 * at tjd attr[6] apparent altitude of sun above horizon at tjd attr[7]
	 * angular distance of moon from sun in degrees declare as attr[20] at least
	 * !
	 */
	/**
	 ...
	 * </CODE>
	 * <P>
	 * </BLOCKQUOTE> <B>Attention: geopos must be a double[10], attr a
	 * double[20]!</B>
	 * 
	 * @param tjd_ut
	 *            The Julian Day number in UT
	 * @param ifl
	 *            To indicate, which ephemeris to be used (SEFLG_JPLEPH,
	 *            SEFLG_SWIEPH, SEFLG_MOSEPH)
	 * @param geopos
	 *            An array[10], on return containing the geographic positions.
	 * @param attr
	 *            An array[20], on return containing the attributes of the
	 *            eclipse as above.
	 * @param serr
	...
	int swe_sol_eclipse_where(double tjd_ut, int ifl, double[] geopos, double[] attr, StringBuffer serr) {
		...
		if ((retflag2 = eclipse_how(tjd_ut, SweConst.SE_SUN, null, ifl, geopos[0], geopos[1], 0, attr,
				serr)) == SweConst.ERR) {
			return retflag2;
		}
		attr[3] = dcore[0];
	...
	int swe_lun_occult_where(double tjd_ut, int ipl, StringBuffer starname, int ifl, double[] geopos, double[] attr,
			StringBuffer serr) {
		int retflag, retflag2;
		double dcore[] = new double[10];
		if (ipl < 0)
	...
	/*
	 * Used by several swe_sol_eclipse_ functions. Like swe_sol_eclipse_where(),
	 * but instead of attr[0], it returns:
	 *
	 * dcore[0]: core shadow width in km dcore[2]: distance of shadow axis from
	 * geocenter r0 dcore[3]: diameter of core shadow on fundamental plane d0
	 * dcore[4]: diameter of half-shadow on fundamental plane D0
	 */
	private int eclipse_where(double tjd_ut, int ipl, StringBuffer starname, int ifl, double[] geopos, double[] dcore,
			StringBuffer serr) {
		int i;
		int retc = 0, niter = 0;
		double e[] = new double[6], et[] = new double[6], rm[] = new double[6], rs[] = new double[6],
				rmt[] = new double[6], rst[] = new double[6], xs[] = new double[6], xst[] = new double[6];
		...
		int iflag, iflag2;
		/*
		 * double ecce = SMath.sqrt(2 * SwephData.EARTH_OBLATENESS -
		 * SwephData.EARTH_OBLATENESS * SwephData.EARTH_OBLATENESS);
		 */
		...
		for (i = 0; i < 10; i++)
			dcore[i] = 0;
		/*
		 * nutation need not be in lunar and solar positions, if mean sidereal
		 * time will be used
		 */
		...
		if ((iflag & SweConst.SEFLG_NONUT) != 0) {
			sidt = sl.swe_sidtime0(tjd_ut, oe.eps * SwissData.RADTODEG, 0) * 15 * SwissData.DEGTORAD;
		} else {
		...
		if (starname != null && starname.length() > 0)
			...
			drad = swed.ast_diam / 2 * 1000
					/ SweConst.AUNIT; /* km -> m -> AU */
		else
			drad = 0;
		// iter_where:
		while (true) {
			...
			/*
			 * Account for oblateness of earth: Instead of flattening the earth,
			 * we apply the correction to the z coordinate of the moon and the
			 * sun. This makes the calculation easier.
			 */
			for (i = 0; i <= 2; i++)
				lx[i] = lm[i];
			...
			s = s0 - d;
			/*
			 * next: geographic position of eclipse center. if shadow axis does
			 * not touch the earth, place on earth with maximum occultation is
			 * computed.
			 */
			...
			geopos[1] = xs[1];
			/*
			 * diameter of core shadow: first, distance moon - place of eclipse
			 * on earth
			 */
			for (i = 0; i <= 2; i++)
				x[i] = rmt[i] - xst[i];
	...
	private int calc_planet_star(double tjd_et, int ipl, StringBuffer starname, int iflag, double[] x,
			StringBuffer serr) {
		int i;
		...
		if (starname == null || starname.length() == 0) {
			...
			if ((retc = sw.swe_fixstar(starname, tjd_et, iflag, x, serr)) == SweConst.OK) {
				/*
				 * fixstars have the standard distance 1. in the occultation
				 * routines, this might lead to errors if interpreted as AU
				 * distance. To avoid this, we make it very high.
				 */
	 ...
	 * SE_ECL_NONCENTRAL if 0, no eclipse is visible at geogr. position.
	 *
	 * attr[0] fraction of solar diameter covered by moon; with total/annular
	 * eclipses, it results in magnitude acc. to IMCCE. attr[1] ratio of lunar
	 * diameter to solar one attr[2] fraction of solar disc covered by moon
	 * (obscuration) attr[3] diameter of core shadow in km attr[4] azimuth of
	 * sun at tjd attr[5] true altitude of sun above horizon at tjd attr[6]
	 * apparent altitude of sun above horizon at tjd attr[7] elongation of moon
	 * in degrees declare as attr[20] at least ! attr[8] magnitude acc. to NASA;
	 * = attr[0] for partial and attr[1] for annular and total eclipses attr[9]
	 * saros series number attr[10] saros series member number declare as
	 * attr[20] at least !
	 *
	 */
	/**
	...
	*         declare as attr[20] at least !<BR>
	...
	int swe_sol_eclipse_how(double tjd_ut, int ifl, double[] geopos, double[] attr, StringBuffer serr) {
		...
		if (geopos[2] < SwephData.SEI_ECL_GEOALT_MIN || geopos[2] > SwephData.SEI_ECL_GEOALT_MAX) {
			if (serr != null) {
				serr.setLength(0);
				serr.append("location for eclipses must be between " + ((int) SwephData.SEI_ECL_GEOALT_MIN) + " and "
						+ ((int) SwephData.SEI_ECL_GEOALT_MAX) + " m above sea");
			}
			return SweConst.ERR;
		}
		ifl &= SweConst.SEFLG_EPHMASK;
		SweDate.swi_set_tid_acc(tjd_ut, ifl, 0);
		if ((retflag = eclipse_how(tjd_ut, SweConst.SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr,
				serr)) == SweConst.ERR) {
			return retflag;
		}
		if ((retflag2 = eclipse_where(tjd_ut, SweConst.SE_SUN, null, ifl, geopos2, dcore, serr)) == SweConst.ERR) {
			return retflag2;
		}
		if (retflag != 0) {
		...
		if (sw.swe_calc_ut(tjd_ut, SweConst.SE_SUN, ifl | SweConst.SEFLG_TOPOCTR | SweConst.SEFLG_EQUATORIAL, ls,
				serr) == SweConst.ERR)
			return SweConst.ERR;
	...
	private int eclipse_how(double tjd_ut, int ipl, StringBuffer starname, int ifl, double geolon, double geolat,
			double geohgt, double[] attr, StringBuffer serr) {
		int i, j, k;
		int retc = 0;
		double te, d;
		double xs[] = new double[6], xm[] = new double[6], ls[] = new double[6], lm[] = new double[6],
				x1[] = new double[6], x2[] = new double[6];
		 ...
		 */
		swe_azalt(tjd_ut, SweConst.SE_EQU2HOR, geopos, 0, 10, ls,
				xh); /* azimuth from south, clockwise, via west */
		/* eclipse description */
		...
		/*
		 * if (retc == SE_ECL_ANNULAR || retc == SE_ECL_TOTAL) attr[0] =
		 * attr[1];
		 */
		/*
		...
		attr[7] = dctr;
		/*
		 * approximate minimum height for visibility, considering refraction and
		 * dip 34.4556': refraction at horizon, from Bennets formulae 1.75' /
		 * SMath.sqrt(geohgt): dip of horizon 0.37' / SMath.sqrt(geohgt):
		 * refraction between horizon and observer
		 */
	 ...
	 * input parameters:
	 *
	 * tjd_start start time for search (UT) ifl ephemeris to be used
	 * (SEFLG_SWIEPH, etc.) ifltype eclipse type to be searched (SE_ECL_TOTAL,
	 * etc.) 0, if any type of eclipse is required
	 *
	 * return values:
	 *
	 ...
	 * </CODE>
	 * <P>
	 * <B>Attention: tret must be a double[10]!</B>
	 * 
	 * @param tjd_start
	 ...
	 * @param tret
	 *            An array[10], on return containing the times of different
	 *            occasions of the eclipse as above
	 * @param backward
	 *            1, if search should be done backwards
	 * @param serr
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails.
	 * @return -1 (SweConst.ERR), if the calculation failed<BR>
	 ...
	 */
	int swe_sol_eclipse_when_glob(double tjd_start, int ifl, int ifltype, double tret[], int backward,
			StringBuffer serr) {
		...
		while (true) {
			...
			T3 = T2 * T;
			T4 = T3 * T;
			Ff = sl.swe_degnorm(160.7108 + 390.67050274 * K - 0.0016341 * T2 - 0.00000227 * T3 + 0.000000011 * T4);
			if (Ff > 180) {
				Ff -= 180;
			}
			...
			/*
			 * approximate time of geocentric maximum eclipse formula from
			 * Meeus, German, p. 381
			 */
			tjd = 2451550.09765 + 29.530588853 * K + 0.0001337 * T2 - 0.000000150 * T3 + 0.00000000073 * T4;
			M = sl.swe_degnorm(2.5534 + 29.10535669 * K - 0.0000218 * T2 - 0.00000011 * T3);
			Mm = sl.swe_degnorm(201.5643 + 385.81693528 * K + 0.1017438 * T2 + 0.00001239 * T3 + 0.000000058 * T4);
			E = 1 - 0.002516 * T - 0.0000074 * T2;
			M *= SwissData.DEGTORAD;
			Mm *= SwissData.DEGTORAD;
			tjd = tjd - 0.4075 * SMath.sin(Mm) + 0.1721 * E * SMath.sin(M);
			/*
			 * time of maximum eclipse (if eclipse) = minimum geocentric angle
			 * between sun and moon edges. After this time has been determined,
			 * check whether or not an eclipse is taking place with the
			 * functions eclipse_where() and _how().
			 */
			dtstart = 1;
			...
			if ((retflag = eclipse_where(tjd, SweConst.SE_SUN, null, ifl, geopos, dcore, serr)) == SweConst.ERR) {
				return retflag;
			}
			retflag2 = retflag;
			/*
			...
			tret[0] = tjd;
			if ((backward != 0 && tret[0] >= tjd_start - 0.0001) || (backward == 0 && tret[0] <= tjd_start + 0.0001)) {
				K += direction;
				continue;
			}
			/*
			 * eclipse type, SE_ECL_TOTAL, _ANNULAR, etc. SE_ECL_ANNULAR_TOTAL
			...
			/*
			 * n = 0: times of eclipse begin and end n = 1: times of totality
			 * begin and end n = 2: times of center line begin and end
			 */
			...
			for (n = 0; n <= o; n++) {
				...
				for (i = 0, t = tjd - dta; i <= 2; i += 1, t += dta) {
					if ((retflag2 = eclipse_where(t, SweConst.SE_SUN, null, ifl, geopos, dcore,
							serr)) == SweConst.ERR) {
						return retflag2;
					}
					if (n == 0) {
			...
			if ((retflag & SweConst.SE_ECL_TOTAL) != 0) {
				if ((retflag2 = eclipse_where(tret[0], SweConst.SE_SUN, null, ifl, geopos, dcore,
						serr)) == SweConst.ERR) {
					return retflag2;
				}
				dc[0] = dcore[0];
				if ((retflag2 = eclipse_where(tret[4], SweConst.SE_SUN, null, ifl, geopos, dcore,
						serr)) == SweConst.ERR) {
					return retflag2;
				}
				dc[1] = dcore[0];
				if ((retflag2 = eclipse_where(tret[5], SweConst.SE_SUN, null, ifl, geopos, dcore,
						serr)) == SweConst.ERR) {
					return retflag2;
				}
				dc[2] = dcore[0];
				/*
				 * the maximum is always total, and there is either one or to
				 * times before and after, when the core shadow becomes zero and
				 * totality changes into annularity or vice versa.
				 */
			 ...
			 */
			/*
			 * first, find out, if there is a solar transit between begin and
			 * end of eclipse
			 */
			k = 2;
			for (i = 0; i < 2; i++) {
		...
		return retflag;
		/*
		 * the time of maximum occultation is practically identical with the
		 * time of maximum core shadow diameter.
		 *
		 * the time, when duration of totality is maximal, is not an interesting
		 * computation either. Near the maximum occulation, the time of totality
		 * can be the same by a second for hundreds of kilometers (for 10
		 * minutes or more).
		 *
		 * for annular eclipses the maximum duration is close to the beginning
		 * and the end of the center lines, where is also the minimum of core
		 * shadow diameter.
		 */
	}
	 ...
	 *
	 * If you want to have only one conjunction of the moon with the body
	 * tested, add the following flag: backward |= SE_ECL_ONE_TRY. If this flag
	 * is not set, the function will search for an occultation until it finds
	 * one. For bodies with ecliptical latitudes > 5, the function may search
	 * successlessly until it reaches the end of the ephemeris. (Note: we do not
	 * add SE_ECL_ONE_TRY to ifl, because ifl may contain SEFLG_TOPOCTR
	 * (=SE_ECL_ONE_TRY) from the parameter iflag of swe_calc() etc. Although
	 * the topocentric flag is irrelevant here, it might cause confusion.)
	 *
	 * return values:
	 *
	 * retflag SE_ECL_TOTAL or SE_ECL_ANNULAR or SE_ECL_PARTIAL or
	 ...
	 *
	 */
	int swe_lun_occult_when_glob(double tjd_start, int ipl, StringBuffer starname, int ifl, int ifltype, double[] tret,
		...
		// boolean one_try = (backward & SweConst.SE_ECL_ONE_TRY) != 0;
		/*
		 * if ((backward & SEI_OCC_FAST) != 0) dont_times = true;
		 */
		/*
		 * function calls for Pluto with asteroid number 134340 are treated as
		 * calls for Pluto as main body SE_PLUTO
		...
		while (true) {
			...
			if (calc_planet_star(t, ipl, starname, ifl, ls, serr) == SweConst.ERR)
				return SweConst.ERR;
			/*
			 * fixed stars with an ecliptic latitude > 7 or < -7 cannot have an
			 * occultation. Even lunar parallax andd proper motion of star will
			 * never allow it.
			 */
			...
			if (drad > 2) {
				if (one_try) {
					tret[0] = t
							+ direction; /*
											 * return a date suitable for next try
											 */
					return 0;
				}
				t += direction * 20;
			 ...
			 */
			/*
			 * if ((retflag2 = eclipse_how(tjd, ipl, starname, ifl, geopos[0],
			 * geopos[1], 0, attr, serr)) == SweConst.ERR) return retflag2;
			 */
			if (retflag2 == 0) {
				/* only one try! */
				/*
				 * if (one_try && ((direction == 1 && tjd > tjd_start) ||
				 * (direction == -1 && tjd < tjd_start))) {
				 */
				if (one_try) {
					tret[0] = tjd;
					return 0;
		 ...
		 * for annular eclipses the maximum duration is close to the beginning
		 * and the end of the center lines, where is also the minimum of core
		 * shadow diameter. beginning and the end of the center lines, where is
		 * also the minimum of core shadow diameter.
		 */
	}

	/*
	 * When is the next solar eclipse at a given geographical position? Note the
	 * uncertainty of Delta T for the remote past and for the future.
	 *
	 * retflag SE_ECL_TOTAL or SE_ECL_ANNULAR or SE_ECL_PARTIAL SE_ECL_VISIBLE,
	 * SE_ECL_MAX_VISIBLE, SE_ECL_1ST_VISIBLE, SE_ECL_2ND_VISIBLE
	 * SE_ECL_3ST_VISIBLE, SE_ECL_4ND_VISIBLE
	 *
	 * tret[0] time of maximum eclipse tret[1] time of first contact tret[2]
	 * time of second contact tret[3] time of third contact tret[4] time of
	 * forth contact tret[5] time of sun rise between first and forth contact
	 * (not implemented so far) tret[6] time of sun set beween first and forth
	 * contact (not implemented so far)
	 *
	 * attr[0] fraction of solar diameter covered by moon (magnitude) attr[1]
	 * ratio of lunar diameter to solar one attr[2] fraction of solar disc
	 * covered by moon (obscuration) attr[3] diameter of core shadow in km
	 * attr[4] azimuth of sun at tjd attr[5] true altitude of sun above horizon
	 * at tjd attr[6] apparent altitude of sun above horizon at tjd attr[7]
	 * elongation of moon in degrees declare as attr[20] at least !
	 */
	/**
	 ...
	 * </CODE>
	 * <P>
	 * <B>Attention: attr must be a double[20] at least!</B>
	 * 
	 * @param tjd_start
	 *            The Julian Day number in UT, from when to start searching
	 * @param ifl
	 ...
	 *            SEFLG_SWIEPH or SEFLG_MOSEPH)
	 * @param geopos
	 *            An array double[3] containing the longitude, latitude and
	 *            height of the geographic position
	 * @param tret
	 *            An array[7], on return containing the times of different
	 *            occasions of the eclipse as specified above
	 * @param attr
	 *            An array[20], on return containing different attributes of the
	 *            eclipse. See above.
	 * @param backward
	 ...
	 */
	int swe_sol_eclipse_when_loc(double tjd_start, int ifl, double[] geopos, double[] tret, double[] attr, int backward,
			StringBuffer serr) {
		int retflag = 0, retflag2 = 0;
		 ...
		 */
		if ((retflag2 = eclipse_where(tret[0], SweConst.SE_SUN, null, ifl, geopos2, dcore, serr)) == SweConst.ERR) {
			return retflag2;
		}
	 ...
	 * retflag SE_ECL_TOTAL or SE_ECL_ANNULAR or SE_ECL_PARTIAL SE_ECL_VISIBLE,
	 * SE_ECL_MAX_VISIBLE, SE_ECL_1ST_VISIBLE, SE_ECL_2ND_VISIBLE
	 * SE_ECL_3ST_VISIBLE, SE_ECL_4ND_VISIBLE SE_ECL_OCC_BEG_DAYLIGHT,
	 * SE_ECL_OCC_END_DAYLIGHT The latter two indicate that the beginning or end
	 * of the occultation takes place during the day. If Venus is occulted, it
	 * may be observable with the naked eye; if other objects, it may be
	 * observable with telescopes.
	 *
	 * int32 ipl planet number of occulted body char* starname name of occulted
	 * star. Must be NULL or "", if a planetary occultation is to be calculated.
	 * For the use of this field, also see swe_fixstar(). int32 ifl ephemeris
	 * flag. If you want to have only one conjunction of the moon with the body
	 * tested, add the following flag: backward |= SE_ECL_ONE_TRY. If this flag
	 * is not set, the function will search for an occultation until it finds
	 * one. For bodies with ecliptical latitudes > 5, the function may search
	 * unsuccessfully until it reaches the end of the ephemeris.
	 *
	 * for all other parameters, see function swe_sol_eclipse_when_loc().
	 */
	int swe_lun_occult_when_loc(double tjd_start, int ipl, StringBuffer starname, int ifl, double[] geopos,
		...
		if (geopos[2] < SwephData.SEI_ECL_GEOALT_MIN || geopos[2] > SwephData.SEI_ECL_GEOALT_MAX) {
			if (serr != null) {
				serr.setLength(0);
				serr.append("location for occultations must be between " + ((int) SwephData.SEI_ECL_GEOALT_MIN)
						+ " and " + ((int) SwephData.SEI_ECL_GEOALT_MAX) + " m above sea");
			}
			return SweConst.ERR;
		}
		if (ipl < 0)
	...
	private int eclipse_when_loc(double tjd_start, int ifl, double[] geopos, double[] tret, double[] attr, int backward,
			StringBuffer serr) {
		int i, j, k, m;
		int retflag = 0, retc;
		...
		while (true) {
			...
			T3 = T2 * T;
			T4 = T3 * T;
			Ff = F = sl.swe_degnorm(160.7108 + 390.67050274 * K - 0.0016341 * T2 - 0.00000227 * T3 + 0.000000011 * T4);
			if (Ff > 180) {
				Ff -= 180;
			}
			if (Ff > 21 && Ff < 159) { /* no eclipse possible */
			...
			M = sl.swe_degnorm(2.5534 + 29.10535669 * K - 0.0000218 * T2 - 0.00000011 * T3);
			Mm = sl.swe_degnorm(201.5643 + 385.81693528 * K + 0.1017438 * T2 + 0.00001239 * T3 + 0.000000058 * T4);
			Om = sl.swe_degnorm(124.7746 - 1.56375580 * K + 0.0020691 * T2 + 0.00000215 * T3);
			E = 1 - 0.002516 * T - 0.0000074 * T2;
			...
			for (dt = dtstart; dt > 0.00001; dt /= dtdiv) {
				...
				for (i = 0, t = tjd - dt; i <= 2; i++, t += dt) {
					/*
					 * this takes some time, but is necessary to avoid missing
					 * an eclipse
					 */
					if (sw.swe_calc(t, SweConst.SE_SUN, iflagcart, xs, serr) == SweConst.ERR) {
						return SweConst.ERR;
			...
			if (sw.swe_calc(tjd, SweConst.SE_SUN, iflagcart, xs, serr) == SweConst.ERR) {
				return SweConst.ERR;
			}
			...
			/* contacts 2 and 3 */
			if (dctr.val > SMath
					.abs(rsminusrm)) {/* partial, no 2nd and 3rd contact */
				tret[2] = tret[3] = 0;
			} else {
				...
				for (i = 0, t = tjd - twomin; i <= 2; i += 2, t = tjd + twomin) {
					...
					rmoon *= 0.99916; /*
										 * gives better accuracy for 2nd/3rd
										 * contacts
										 */
				...
				for (m = 0, dt = tensec; m < 2; m++, dt /= 10) {
					for (j = 2; j <= 3; j++) {
						if (sw.swe_calc(tret[j], SweConst.SE_SUN, iflagcart | SweConst.SEFLG_SPEED, xs,
								serr) == SweConst.ERR) {
							return SweConst.ERR;
						}
						if (sw.swe_calc(tret[j], SweConst.SE_MOON, iflagcart | SweConst.SEFLG_SPEED, xm,
								serr) == SweConst.ERR) {
							return SweConst.ERR;
						}
						for (i = 0; i < 2; i++) {
			...
			for (i = 4; i >= 0; i--) { /* attr for i = 0 must be kept !!! */
				...
				if (eclipse_how(tret[i], SweConst.SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr,
						serr) == SweConst.ERR) {
					return SweConst.ERR;
				}
				/*
				 * if (retflag2 & SweConst.SE_ECL_VISIBLE) {} could be wrong for
				 * 1st/4th contact
				 */
				if (attr[6] > 0) { /*
									 * this is save, sun above horizon, using
									 * app. alt.
									 */
					retflag |= SweConst.SE_ECL_VISIBLE;
					switch (i) {
					case 0:
						retflag |= SweConst.SE_ECL_MAX_VISIBLE;
						break;
					case 1:
						retflag |= SweConst.SE_ECL_1ST_VISIBLE;
						break;
					case 2:
						retflag |= SweConst.SE_ECL_2ND_VISIBLE;
						break;
					case 3:
						retflag |= SweConst.SE_ECL_3RD_VISIBLE;
						break;
					case 4:
						retflag |= SweConst.SE_ECL_4TH_VISIBLE;
						break;
					default:
						break;
					}
			...
			if (swe_rise_trans(tret[1] - 0.001, SweConst.SE_SUN, null, iflag,
					SweConst.SE_CALC_RISE | SweConst.SE_BIT_DISC_BOTTOM, geopos, 0, 0, tjdr, serr) == SweConst.ERR)
				return SweConst.ERR;
			if (swe_rise_trans(tret[1] - 0.001, SweConst.SE_SUN, null, iflag,
					SweConst.SE_CALC_SET | SweConst.SE_BIT_DISC_BOTTOM, geopos, 0, 0, tjds, serr) == SweConst.ERR)
				return SweConst.ERR;
		...
		if (tjdr.val > tret[1] && tjdr.val < tret[4]) {
			...
			if ((retflag & SweConst.SE_ECL_MAX_VISIBLE) == 0) {
				tret[0] = tjdr.val;
				if ((retc = eclipse_how(tret[5], SweConst.SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr,
						serr)) == SweConst.ERR)
					return SweConst.ERR;
		...
		if (tjds.val > tret[1] && tjds.val < tret[4]) {
			...
			if ((retflag & SweConst.SE_ECL_MAX_VISIBLE) == 0) {
				tret[0] = tjds.val;
				if ((retc = eclipse_how(tret[6], SweConst.SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr,
						serr)) == SweConst.ERR)
					return SweConst.ERR;
				retflag &= ~(SweConst.SE_ECL_TOTAL | SweConst.SE_ECL_ANNULAR | SweConst.SE_ECL_PARTIAL);
				retflag |= (retc & (SweConst.SE_ECL_TOTAL | SweConst.SE_ECL_ANNULAR | SweConst.SE_ECL_PARTIAL));
	...
	private int occult_when_loc(double tjd_start, int ipl, StringBuffer starname, int ifl, double[] geopos,
			double[] tret, double[] attr, int backward, StringBuffer serr) {
		int i, j, k, m;
		int retflag = 0;
		double t, tjd, dt;
		...
		while (true) {
			...
			if (dctr.val > SMath
				...
				for (m = 0, dt = tensec; m < 2; m++, dt /= 10) {
					for (j = 2; j <= 3; j++) {
						if (calc_planet_star(tret[j], ipl, starname, iflagcart | SweConst.SEFLG_SPEED, xs,
								serr) == SweConst.ERR)
							return SweConst.ERR;
						if (sw.swe_calc(tret[j], SweConst.SE_MOON, iflagcart | SweConst.SEFLG_SPEED, xm,
								serr) == SweConst.ERR)
			...
			for (i = 4; i >= 0; i--) { /* attr for i = 0 must be kept !!! */
				if (tret[i] == 0)
					continue;
				if (eclipse_how(tret[i], ipl, starname, ifl, geopos[0], geopos[1], geopos[2], attr,
						serr) == SweConst.ERR)
					return SweConst.ERR;
				/*
				 * if (retflag2 & SweConst.SE_ECL_VISIBLE) { could be wrong for
		...
		if (swe_rise_trans(tret[1] - 0.1, ipl, starname, iflag, SweConst.SE_CALC_RISE | SweConst.SE_BIT_DISC_BOTTOM,
				geopos, 0, 0, tjdr, serr) == SweConst.ERR)
			return SweConst.ERR;
		if (swe_rise_trans(tret[1] - 0.1, ipl, starname, iflag, SweConst.SE_CALC_SET | SweConst.SE_BIT_DISC_BOTTOM,
				geopos, 0, 0, tjds, serr) == SweConst.ERR)
			return SweConst.ERR;
		if (tjdr.val > tret[1] && tjdr.val < tret[4])
			tret[5] = tjdr.val;
		if (tjds.val > tret[1] && tjds.val < tret[4])
			tret[6] = tjds.val;
		if (swe_rise_trans(tret[2], SweConst.SE_SUN, null, iflag, SweConst.SE_CALC_RISE, geopos, 0, 0, tjdr,
				serr) == SweConst.ERR)
			return SweConst.ERR;
		if (swe_rise_trans(tret[2], SweConst.SE_SUN, null, iflag, SweConst.SE_CALC_SET, geopos, 0, 0, tjds,
				serr) == SweConst.ERR)
			return SweConst.ERR;
		if (tjds.val < tjdr.val)
			retflag |= SweConst.SE_ECL_OCC_BEG_DAYLIGHT;
		if (swe_rise_trans(tret[3], SweConst.SE_SUN, null, iflag, SweConst.SE_CALC_RISE, geopos, 0, 0, tjdr,
				serr) == SweConst.ERR)
			return SweConst.ERR;
		if (swe_rise_trans(tret[3], SweConst.SE_SUN, null, iflag, SweConst.SE_CALC_SET, geopos, 0, 0, tjds,
				serr) == SweConst.ERR)
			return SweConst.ERR;
		if (tjds.val < tjdr.val)
	...
	/*
	 * swe_azalt() Computes azimut and height, from either ecliptic or
	 * equatorial coordinates
	 *
	 * input: tjd_ut iflag either SE_ECL2HOR or SE_EQU2HOR geopos[3] geograph.
	 * longitude, latitude, height above sea atpress atmospheric pressure at
	 * geopos in millibars (hPa) attemp atmospheric temperature in degrees C
	 * xin[2] input coordinates polar, in degrees
	 *
	 * Horizontal coordinates are returned in xaz[3] xaz[0] = azimuth xaz[1] =
	 * true altitude xaz[2] = apparent altitude
	 *
	 * If atpress is not given (= 0), the programm assumes 1013.25 mbar; if a
	 * non-zero height above sea is given, atpress is estimated. geohgt height
	 * of observer above sea (optional)
	 */
	/**
	 ...
	 *            above.
	 */
	void swe_azalt(double tjd_ut, int calc_flag, double[] geopos, double atpress, double attemp, double[] xin,
			double[] xaz) {
		int i;
		...
		/*
		 * xaz[2] = swe_refrac_extended(xaz[2], geopos[2], atpress, attemp,
		 * const_lapse_rate, SweConst.SE_APP_TO_TRUE, null);
		 */

	}

	/*
	 * swe_azalt_rev() computes either ecliptical or equatorial coordinates from
	 * azimuth and true altitude in degrees. For conversion between true and
	 * apparent altitude, there is the function swe_refrac().
	 *
	 * input: tjd_ut iflag either SE_HOR2ECL or SE_HOR2EQU xin[2] azimut and
	...
	void swe_azalt_rev(double tjd_ut, int calc_flag, double[] geopos, double[] xin, double[] xout) {
		...
		xaz[2] = 1;
		/*
		 * azimuth is from south, clockwise. we need it from east, counterclock
		 */
		xaz[0] = 360 - xaz[0];
	...
	/*
	 * swe_refrac() Transforms apparent to true altitude and vice-versa. These
	 * formulae do not handle the case when the sun is visible below the
	 * geometrical horizon (from a mountain top or an air plane) input: double
	 * inalt; * altitude of object in degrees * double atpress; * millibars
	 * (hectopascal) * double attemp; * degrees C * int32 calc_flag; * either
	 * SE_CALC_APP_TO_TRUE or * SE_CALC_TRUE_TO_APP
	 */
	/**
	...
	double swe_refrac(double inalt, double atpress, double attemp, int calc_flag) {
		...
		if (calc_flag == SweConst.SE_TRUE_TO_APP) {
			...
			if (trualt > 15) {
				...
				/*
				 * the following tan is not defined for a value of trualt near
				 * -5.00158 and 89.89158
				 */
			...
			appalt = inalt;
			/*
			 * the following tan is not defined for a value of inalt near
			 * -4.3285 and 89.9225
			 */
	 ...
	 * archaeoastronomer Victor Reijs. It is more correct and more skilled than
	 * the old function swe_refrac(): - it allows correct calculation of
	 * refraction for altitudes above sea > 0, where the ideal horizon and
	 * planets that are visible may have a negative height. (for swe_refrac(),
	 * negative apparent heights do not exist!) - it allows to manipulate the
	 * refraction constant
	 *
	 * Transforms apparent to true altitude and vice-versa. input: double inalt;
	 * * altitude of object above geometric horizon in degrees* * geometric
	 * horizon = plane perpendicular to gravity * double geoalt; * altitude of
	 * observer above sea level in meters * double atpress; * millibars
	 * (hectopascal) * double lapse_rate; * (dT/dh) [deg K/m] double attemp; *
	 * degrees C * int32 calc_flag; * either SE_CALC_APP_TO_TRUE or *
	 * SE_CALC_TRUE_TO_APP
	 *
	 * function returns: case 1, conversion from true altitude to apparent
	 * altitude - apparent altitude, if body appears above is observable above
	 * ideal horizon - true altitude (the input value), otherwise
	 * "ideal horizon" is the horizon as seen above an ideal sphere (as seen
	 * from a plane over the ocean with a clear sky) case 2, conversion from
	 * apparent altitude to true altitude - the true altitude resulting from the
	 * input apparent altitude, if this value is a plausible apparent altitude,
	 * i.e. if it is a position above the ideal horizon - the input altitude
	 * otherwise
	 *
	 * in addition the array dret[] is given the following values - dret[0] true
	 * altitude, if possible; otherwise input value - dret[1] apparent altitude,
	 * if possible; otherwise input value - dret[2] refraction - dret[3] dip of
	 * the horizon
	 *
	 * The body is above the horizon if the dret[0] != dret[1]
	 */
	double swe_refrac_extended(double inalt, double geoalt, double atpress, double attemp, double lapse_rate,
		...
		if (calc_flag == SweConst.SE_TRUE_TO_APP) {
			...
			for (i = 0; i < 5; i++) {
				...
				if (N != 0.0 && yy0 != 0.0) /* sic !!! code by Moshier */
					N = y - N * (inalt + D - y)
							/ yy0; /*
									 * Newton iteration with numerically
	...
	/*
	 * calculate the astronomical refraction input parameters: double inalt *
	 * apparent altitude of object double atpress * atmospheric pressure
	 * millibars (hectopascal) * double attemp * atmospheric temperature degrees
	 * C * returns double r in degrees
	 */
	private double calc_astronomical_refr(double inalt, double atpress, double attemp) {
		...
		if (inalt > 17.904104638432) { /*
										 * for continuous function, instead of
										 * '>15'
										 */
	...
	/*
	 * calculate dip of the horizon input parameters: double geoalt * altitude
	 * of observer above sea level in meters * double atpress * atmospheric
	 * pressure millibars (hectopascal) * double attemp * atmospheric
	 * temperature degrees C * double lapse_rate * (dT/dh) [deg K/m] returns dip
	 * in degrees
	 */
	private double calc_dip(double geoalt, double atpress, double attemp, double lapse_rate) {
		/*
		 * below formula is based on A. Thom, Megalithic lunar observations,
		 * 1973 (page 32). conversion to metric has been done by V. Reijs, 2000,
	 ...
	 *
	 * retflag SE_ECL_TOTAL or SE_ECL_PARTIAL SE_ECL_PENUMBRAL if 0, there is no
	 * eclipse
	 *
	 ...
	 * </CODE>
	 * <P>
	 * </BLOCKQUOTE><B>Attention: attr must be a double[20]!</B>
	 * 
	 * @param tjd_ut
	 *            The Julian Day number in UT
	 * @param ifl
	...
	int swe_lun_eclipse_how(double tjd_ut, int ifl, double[] geopos, double[] attr, StringBuffer serr) {
		 ...
		 */
		sw.swe_set_topo(geopos[0], geopos[1], geopos[2]);
		if (sw.swe_calc_ut(tjd_ut, SweConst.SE_MOON, ifl | SweConst.SEFLG_TOPOCTR | SweConst.SEFLG_EQUATORIAL, lm,
				serr) == SweConst.ERR)
			return SweConst.ERR;
	 ...
	 *
	 * dcore[0]: distance of shadow axis from geocenter r0 dcore[1]: diameter of
	 * core shadow on fundamental plane d0 dcore[2]: diameter of half-shadow on
	 * fundamental plane D0
	 */
	...
	int swe_lun_eclipse_when(double tjd_start, int ifl, int ifltype, double[] tret, int backward, StringBuffer serr) {
		...
		iflagcart = iflag | SweConst.SEFLG_XYZ;
		if (ifltype == 0) {
			ifltype = SweConst.SE_ECL_TOTAL | SweConst.SE_ECL_PENUMBRAL | SweConst.SE_ECL_PARTIAL;
		}
		if (backward != 0) {
			direction = -1;
		}
		...
		while (true) {
			...
			T3 = T2 * T;
			T4 = T3 * T;
			Ff = F = sl.swe_degnorm(160.7108 + 390.67050274 * kk - 0.0016341 * T2 - 0.00000227 * T3 + 0.000000011 * T4);
			if (Ff > 180) {
				Ff -= 180;
			}
			if (Ff > 21 && Ff < 159) { /* no eclipse possible */
			...
			M = sl.swe_degnorm(2.5534 + 29.10535669 * kk - 0.0000218 * T2 - 0.00000011 * T3);
			Mm = sl.swe_degnorm(201.5643 + 385.81693528 * kk + 0.1017438 * T2 + 0.00001239 * T3 + 0.000000058 * T4);
			Om = sl.swe_degnorm(124.7746 - 1.56375580 * kk + 0.0020691 * T2 + 0.00000215 * T3);
			E = 1 - 0.002516 * T - 0.0000074 * T2;
			...
			A1 *= SwissData.DEGTORAD;
			tjd = tjd - 0.4075 * SMath.sin(Mm) + 0.1721 * E * SMath.sin(M) + 0.0161 * SMath.sin(2 * Mm)
					- 0.0097 * SMath.sin(2 * F1) + 0.0073 * E * SMath.sin(Mm - M) - 0.0050 * E * SMath.sin(Mm + M)
					- 0.0023 * SMath.sin(Mm - 2 * F1) + 0.0021 * E * SMath.sin(2 * M) + 0.0012 * SMath.sin(Mm + 2 * F1)
					+ 0.0006 * E * SMath.sin(2 * Mm + M) - 0.0004 * SMath.sin(3 * Mm)
					- 0.0003 * E * SMath.sin(M + 2 * F1) + 0.0003 * SMath.sin(A1) - 0.0002 * E * SMath.sin(M - 2 * F1)
					- 0.0002 * E * SMath.sin(2 * Mm - M) - 0.0002 * SMath.sin(Om);
			/*
			 * precise computation: time of maximum eclipse (if eclipse) =
			 * minimum selenocentric angle between sun and earth edges. After
			 * this time has been determined, check whether or not an eclipse is
			 * taking place with the function lun_eclipse_how().
			 */
			dtstart = 0.1;
			if (tjd < 2000000 || tjd > 2500000) {
				dtstart = 5;
			...
			if ((retflag = swe_lun_eclipse_how(tjd, ifl, null, attr, serr)) == SweConst.ERR) {
				return retflag;
			}
			if (retflag == 0) {
				K += direction;
			...
			if ((ifltype & SweConst.SE_ECL_PENUMBRAL) == 0 && (retflag & SweConst.SE_ECL_PENUMBRAL) != 0) {
				K += direction;
				continue;
			}
			/* non partial eclipse is wanted: */
			...
			for (n = 0; n <= o; n++) {
				...
				for (i = 0, t = tjd - dta; i <= 2; i += 1, t += dta) {
					if ((retflag2 = lun_eclipse_how(t, ifl, attr, dcore, serr)) == SweConst.ERR) {
						return retflag2;
					}
					if (n == 0) {
	 ...
	 * declare as attr[20] at least !
	 */
	int swe_lun_eclipse_when_loc(double tjd_start, int ifl, double geopos[], double tret[], double attr[], int backward,
			StringBuffer serr) {
		int retflag = 0, retflag2 = 0;
		DblObj tjdr = new DblObj(), tjds = new DblObj();
		double tjd_max = 0;
		...
		while (true) {
			...
			for (i = 7; i >= 0; i--) {
				...
				if (attr[6] > 0) { /* moon above horizon, using app. alt. */
					...
					case 0:
						...
						break;
					case 2:
						retflag |= SweConst.SE_ECL_PARTBEG_VISIBLE;
						break;
					case 3:
						retflag |= SweConst.SE_ECL_PARTEND_VISIBLE;
						break;
					case 4:
						retflag |= SweConst.SE_ECL_TOTBEG_VISIBLE;
						break;
					case 5:
						retflag |= SweConst.SE_ECL_TOTEND_VISIBLE;
						break;
					case 6:
						retflag |= SweConst.SE_ECL_PENUMBBEG_VISIBLE;
						break;
					case 7:
						retflag |= SweConst.SE_ECL_PENUMBEND_VISIBLE;
						break;
					default:
						break;
					}
			...
			if (swe_rise_trans(tret[6] - 0.001, SweConst.SE_MOON, null, ifl,
					SweConst.SE_CALC_RISE | SweConst.SE_BIT_DISC_BOTTOM, geopos, 0, 0, tjdr, serr) == SweConst.ERR)
				return SweConst.ERR;
			if (swe_rise_trans(tret[6] - 0.001, SweConst.SE_MOON, null, ifl,
					SweConst.SE_CALC_SET | SweConst.SE_BIT_DISC_BOTTOM, geopos, 0, 0, tjds, serr) == SweConst.ERR)
	 ...
	 *
	 * attr[0] = phase angle (earth-planet-sun) attr[1] = phase (illumined
	 * fraction of disc) attr[2] = elongation of planet attr[3] = apparent
	 * diameter of disc attr[4] = apparent magnitude attr[5] = geocentric
	 * horizontal parallax (Moon) declare as attr[20] at least !
	 *
	 * Note: the lunar magnitude is quite a complicated thing, but our algorithm
	 * is very simple. The phase of the moon, its distance from the earth and
	...
	private static final double mag_elem[][] = {
			...
			{ -26.86, 0, 0, 0 }, { -12.55, 0, 0, 0 },
			/* IAU 1986 */
			{ -0.42, 3.80, -2.73, 2.00 }, { -4.40, 0.09, 2.39, -0.65 }, { -1.52, 1.60, 0, 0 }, /* Mars */
			...
			{ 99, 0, 0, 0 }, { 99, 0, 0, 0 }, { 99, 0, 0, 0 }, { 99, 0, 0, 0 }, /* Earth */
	 ...
	 * <P>
	 * See <A HREF=
	 * "Swecl.html#swe_pheno_ut(double, int, int, double[], java.lang.StringBuffer)"
	 * >swe_pheno_ut(...)</A> for missing information.
	 */
	int swe_pheno(double tjd, int ipl, int iflag, double[] attr, StringBuffer serr) {
		int i;
		double xx[] = new double[6], xx2[] = new double[6], xxs[] = new double[6], lbr[] = new double[6],
				lbr2[] = new double[6], dt = 0, dd;
		double fac;
		...
		iflag = iflag & (SweConst.SEFLG_EPHMASK | SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_J2000 | SweConst.SEFLG_NONUT
				| SweConst.SEFLG_NOGDEFL | SweConst.SEFLG_NOABERR | SweConst.SEFLG_TOPOCTR);
		iflagp = iflag & (SweConst.SEFLG_EPHMASK | SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_J2000 | SweConst.SEFLG_NONUT
		 ...
		 * geocentric planet
		 */
		if (sw.swe_calc(tjd, ipl, iflag | SweConst.SEFLG_XYZ, xx, serr) == SweConst.ERR) {
			return SweConst.ERR;
		}
		...
		if (ipl == SweConst.SE_MOON) {
			if (sw.swe_calc(tjd, SweConst.SE_SUN, iflag | SweConst.SEFLG_XYZ, xxs, serr) == SweConst.ERR) {
				return SweConst.ERR;
			}
		}
		if (ipl != SweConst.SE_SUN && ipl != SweConst.SE_EARTH && ipl != SweConst.SE_MEAN_NODE
				&& ipl != SweConst.SE_TRUE_NODE && ipl != SweConst.SE_MEAN_APOG && ipl != SweConst.SE_OSCU_APOG) {
			/*
			 ...
			 */
			if (sw.swe_calc(tjd - dt, ipl, iflagp | SweConst.SEFLG_XYZ, xx2, serr) == SweConst.ERR) {
				return SweConst.ERR;
			}
		 ...
		 * apparent magnitude
		 */
		if (ipl > SweConst.SE_AST_OFFSET || (ipl < NMAG_ELEM && mag_elem[ipl][0] < 99)) {
			if (ipl == SweConst.SE_SUN) {
				/* ratio apparent diameter : average diameter */
				fac = attr[3] / (SMath.asin(SwephData.pla_diam[SweConst.SE_SUN] / 2.0 / SweConst.AUNIT) * 2
						* SwissData.RADTODEG);
				fac *= fac;
				attr[4] = mag_elem[ipl][0] - 2.5 * log10(fac);
			} else if (ipl == SweConst.SE_MOON) {
				/*
				 * attr[4] = -21.62 + 5 * log10(384410497.8 / EARTH_RADIUS) /
				 * log10(10) + 0.026 * fabs(attr[0]) + 0.000000004 *
				 * pow(attr[0], 4);
				 */
				attr[4] = -21.62 + 5 * log10(lbr[2] * SwephData.AUNIT / SwephData.EARTH_RADIUS) / log10(10)
						+ 0.026 * SMath.abs(attr[0]) + 0.000000004 * SMath.pow(attr[0], 4);
				/* printf("1 = %f, 2 = %f\n", mag, mag2); */
				...
				u1 = SMath.atan2(
						SMath.sin(in) * SMath.tan(lbr2[1] * SwissData.DEGTORAD)
								+ SMath.cos(in) * SMath.sin(lbr2[0] * SwissData.DEGTORAD - om),
						SMath.cos(lbr2[0] * SwissData.DEGTORAD - om)) * SwissData.RADTODEG;
				u2 = SMath.atan2(
						SMath.sin(in) * SMath.tan(lbr[1] * SwissData.DEGTORAD)
								+ SMath.cos(in) * SMath.sin(lbr[0] * SwissData.DEGTORAD - om),
						SMath.cos(lbr[0] * SwissData.DEGTORAD - om)) * SwissData.RADTODEG;
				du = sl.swe_degnorm(u1 - u2);
				if (du > 10) {
					du = 360 - du;
				}
				attr[4] = 5 * log10(lbr2[2] * lbr[2]) + mag_elem[ipl][1] * sinB + mag_elem[ipl][2] * sinB * sinB
						+ mag_elem[ipl][3] * du + mag_elem[ipl][0];
			} else if (ipl < SweConst.SE_CHIRON) {
				attr[4] = 5 * log10(lbr2[2] * lbr[2]) + mag_elem[ipl][1] * attr[0] / 100.0
						+ mag_elem[ipl][2] * attr[0] * attr[0] / 10000.0
						+ mag_elem[ipl][3] * attr[0] * attr[0] * attr[0] / 1000000.0 + mag_elem[ipl][0];
			} else if (ipl < NMAG_ELEM || ipl > SweConst.SE_AST_OFFSET) {/* asteroids */
				ph1 = SMath.pow(EULER, -3.33 * SMath.pow(SMath.tan(attr[0] * SwissData.DEGTORAD / 2), 0.63));
				ph2 = SMath.pow(EULER, -1.87 * SMath.pow(SMath.tan(attr[0] * SwissData.DEGTORAD / 2), 1.22));
				...
				attr[4] = 5 * log10(lbr2[2] * lbr[2]) + me[0] - 2.5 * log10((1 - me[1]) * ph1 + me[1] * ph2);
		...
		if (ipl != SweConst.SE_SUN && ipl != SweConst.SE_EARTH) {
			 ...
			 */
			if (sw.swe_calc(tjd, SweConst.SE_SUN, iflag | SweConst.SEFLG_XYZ, xx2, serr) == SweConst.ERR) {
				return SweConst.ERR;
			}
		...
		if (ipl == SweConst.SE_MOON) {
			...
			if (sw.swe_calc(tjd, (int) ipl,
					epheflag | SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_EQUATORIAL | SweConst.SEFLG_RADIANS, xm,
					serr) == SweConst.ERR)
				/*
				 * int cast can be removed when swe_calc() gets int32 ipl
				 * definition
				 */
				return SweConst.ERR;
			...
			if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
				if (sw.swe_calc(tjd, (int) ipl, epheflag | SweConst.SEFLG_XYZ | SweConst.SEFLG_TOPOCTR, xm,
						serr) == SweConst.ERR)
					return SweConst.ERR;
	 ...
	 * </CODE>
	 *            <P>
	 *            <B>Attention: attr must be a double[20]!</B>
	 * @param iflag
	 *            Which ephemeris is to be used (SEFLG_JPLEPH, SEFLG_SWIEPH,
	 *            SEFLG_MOSEPH). Additonally useable flags: SEFLG_TRUEPOS,
	 *            SEFLG_HELCTR.
	 * @param attr
	 ...
	 *
	 * tjd_ut universal time from when on search ought to start ipl planet
	 * number, neglected, if Starname is given starname pointer to string. if a
	 * planet, not a star, is wanted, starname must be null or "" epheflag used
	 * for ephemeris only rsmi SE_CALC_RISE, SE_CALC_SET, SE_CALC_MTRANSIT,
	 * SE_CALC_ITRANSIT | SE_BIT_DISC_CENTER for rises of disc center of body |
	 * SE_BIT_DISC_BOTTOM for rises of disc bottom of body |
	 * SE_BIT_NO_REFRACTION to neglect refraction | SE_BIT_FIXED_DISC_SIZE
	 * neglect the effect of distance on disc size geopos array of doubles for
	 * geogr. long., lat. and height above sea atpress atmospheric pressure
	 * attemp atmospheric temperature
	 *
	 * return variables: tret time of rise, set, meridian transits serr[256]
	 * error string function return value -2 means that the body does not rise
	 * or set
	 */
	/**
	 ...
	 *            SE_CALC_ITRANSIT) plus optionally SE_BIT_DISC_CENTER, when the
	 *            rise time of the disc center of the body is requested,
	 *            SE_BIT_DISC_BOTTOM for rises of disc bottom of body and or
	 *            SE_BIT_NO_REFRACTION for calculation without refraction
	 *            effects together with SE_BIT_FIXED_DISC_SIZE to neglect the
	 *            effect of distance on disc size. The calculation method
	 *            defaults to SE_CALC_RISE.
	 * @param geopos
	 *            An array double[3] containing the longitude, latitude and
	 ...
	 *            A StringBuffer containing a warning or error message, if
	 *            something fails
	 * @return SweConst.OK (0) or SweConst.ERR (-1) or -2 if that body does not
	 *         rise or set
	 * @see swisseph.SweConst#OK
	 * @see swisseph.SweConst#ERR
	 * @see swisseph.SweConst#SEFLG_JPLEPH
	 ...
	 * @see swisseph.DblObj
	 */
	int swe_rise_trans(double tjd_ut, int ipl, StringBuffer starname, int epheflag, int rsmi, double[] geopos,
	 ...
	 */
	int swe_rise_trans_true_hor(double tjd_ut, int ipl, StringBuffer starname, int epheflag, int rsmi, double[] geopos,
			double atpress, double attemp, double horhgt, DblObj tret, StringBuffer serr) {
		int i, j, k, ii, calc_culm, nculm = -1;
		double tjd_et = tjd_ut + SweDate.getDeltaT(tjd_ut);
		double xc[] = new double[6], xh[][] = new double[20][6], ah[] = new double[6], aha;
		double tculm[] = new double[4], tcu, tc[] = new double[20], h[] = new double[20], t2[] = new double[6],
				dc[] = new double[6];
		DblObj dtint = new DblObj();
		...
		if (geopos[2] < SwephData.SEI_ECL_GEOALT_MIN || geopos[2] > SwephData.SEI_ECL_GEOALT_MAX) {
			if (serr != null) {
				serr.setLength(0);
				serr.append("location for swe_rise_trans() must be between " + ((int) SwephData.SEI_ECL_GEOALT_MIN)
						+ " and " + ((int) SwephData.SEI_ECL_GEOALT_MAX) + " m above sea");
			}
			return SweConst.ERR;
		}
		...
		if ((rsmi & (SweConst.SE_CALC_MTRANSIT | SweConst.SE_CALC_ITRANSIT)) != 0) {
			return calc_mer_trans(tjd_ut, ipl, epheflag, rsmi, geopos, starname, tret, serr);
		}
		if ((rsmi & (SweConst.SE_CALC_RISE | SweConst.SE_CALC_SET)) == 0) {
			rsmi |= SweConst.SE_CALC_RISE;
		}
		/* twilight calculation */
		if (ipl == SweConst.SE_SUN && ((rsmi & (SweConst.SE_BIT_CIVIL_TWILIGHT | SweConst.SE_BIT_NAUTIC_TWILIGHT
				| SweConst.SE_BIT_ASTRO_TWILIGHT)) != 0)) {
		 ...
		 * culminations are required in case there are maxima or minima in
		 * height slightly above or below the horizon. we do not use meridian
		 * transits, because in polar regions the culmination points may
		 * considerably deviate from transits. also, there are cases where the
		 * moon rises in the western half of the sky for a short time.
		 */
		if (do_fixstar) {
		...
		for (ii = 0, t = tjd_ut - twohrs; ii <= jmax; ii++, t += twohrs) {
			...
			if (ii == 0) {
				if (do_fixstar) {
					dd = 0;
				} else if ((rsmi & SweConst.SE_BIT_DISC_CENTER) != 0) {
					dd = 0;
				} else if (ipl < SwephData.NDIAM && ipl >= 0) { // added for
		...
		/*
		 * note: there can be a rise or set on the poles, even if there is no
		 * culmination. So, we must not leave here in any case.
		 */
		...
		tret.val = 0;
		/*
		 * find points with zero height. binary search
		 */
		for (ii = 1; ii <= jmax; ii++) {
			if (h[ii - 1] * h[ii] >= 0) {
				continue;
	...
	private int calc_mer_trans(double tjd_ut, int ipl, int epheflag, int rsmi, double[] geopos, StringBuffer starname,
	 ...
	 * 
	 * Planetary nodes can be defined in three different ways: a) They can be
	 * understood as a direction or as an axis defined by the intersection line
	 * of two orbital planes. E.g., the nodes of Mars are defined by the
	 * intersection line of Mars' orbital plane with the ecliptic (= the Earths
	 * orbit heliocentrically or the solar orbit geocentrically). However, as
	 * Michael Erlewine points out in his elaborate web page on this topic
	 * (http://thenewage.com/resources/articles/interface.html), planetary nodes
	 * can be defined for any couple of planets. E.g. there is also an
	 * intersection line for the two orbital planes of Mars and Saturn. Because
	 * such lines are, in principle, infinite, the heliocentric and the
	 ...
	 * charts. The ascending and the descending node will, in this case, be in
	 * precise opposition.
	 * 
	 * b) The planetary nodes can also be understood in a different way, not as
	 * an axis, but as the two points on a planetary orbit that are located
	 * precisely on the intersection line of the two planes. This second
	 * definition makes no difference for the moon or for heliocentric positions
	 * of planets, but it does so for geocentric positions. There are two
	 * possibilities for geocentric planetary nodes based on this definition. 1)
	 * The common solution is that the points on the planets orbit are
	 * transformed to the geocenter. The two points will not be in opposition
	 * anymore, or they will only roughly be so with the outer planets. The
	 * advantage of these nodes is that when a planet is in conjunction with its
	 * node, then its ecliptic latitude will be zero. This is not true when a
	 * planet is in geocentric conjunction with its heliocentric node. (And
	 * neither is it always true for the inner planets, i.e. Mercury and Venus.)
	 * 2) The second possibility that nobody seems to have thought of so far:
	 * One may compute the points of the earth's orbit that lie exactly on
	 * another planet's orbital plane and transform it to the geocenter. The two
	 * points will always be in an approximate square.
	 * 
	 * c) Third, the planetary nodes could be defined as the intersection points
	 * of the plane defined by their momentary geocentric position and motion
	 * with the plane of the ecliptic. Such points would move very fast around
	 * the planetary stations. Here again, as in b)1), the planet would cross
	 * the ecliptic and its ecliptic latitude would be 0 exactly when it were in
	 ...
	 * 
	 * a) The planetary apsides can be defined as the perihelion and aphelion
	 * points on a planetary orbit. For a geocentric chart, these points could
	 * be transformed from the heliocenter to the geocenter. b) However, one
	 * might consider these points as astrologically relevant axes rather than
	 * as points on a planetary orbit. Again, this would allow heliocentric
	 * positions in a geocentric chart.
	 * 
	 * Note: For the "Dark Moon" or "Lilith", which I usually define as the
	 * lunar apogee, some astrologers give a different definition. They
	 * understand it as the second focal point of the moon's orbital ellipse.
	 * This definition does not make a difference for geocentric positions,
	 * because the apogee and the second focus are in exactly the same
	 * geocentric direction. However, it makes a difference with topocentric
	 * positions, because the two points do not have same distance. Analogous
	 * "black planets" have been proposed: they would be the second focal points
	 * of the planets' orbital ellipses. The heliocentric positions of these
	 * "black planets" are identical with the heliocentric positions of the
	 * aphelia, but geocentric positions are not identical, because the focal
	 * points are much closer to the sun than the aphelia.
	 * 
	 ...
	 * VSOP87. Mean points can not be calculated for Pluto and the asteroids,
	 * because there is no planetary theory for them.
	 * 
	 * Osculating nodes and apsides
	 * 
	 * Nodes and apsides can also be derived from the osculating orbital
	 * elements of a body, the paramaters that define an ideal unperturbed
	 * elliptic (two-body) orbit. For astrology, note that this is a
	 * simplification and idealization. Problem with Neptune: Neptune's orbit
	 * around the sun does not have much in common with an ellipse. There are
	 * often two perihelia and two aphelia within one revolution. As a result,
	 * there is a wild oscillation of the osculating perihelion (and aphelion).
	 * In actuality, Neptune's orbit is not heliocentric orbit at all. The
	 * twofold perihelia and aphelia are an effect of the motion of the sun
	 * about the solar system barycenter. This motion is much faster than the
	 * motion of Neptune, and Neptune cannot react on such fast displacements of
	 * the Sun. As a result, Neptune seems to move around the barycenter (or a
	 * mean sun) rather than around the true sun. In fact, Neptune's orbit
	 * around the barycenter is therefore closer to an ellipse than the his
	 * orbit around the sun. The same statement is also true for Saturn, Uranus
	 ...
	 * 
	 * Two solutions can be thought of for this problem: 1) The one would be to
	 * interpolate between actual passages of the planets through their nodes
	 * and apsides. However, this works only well with Mercury. With all other
	 * planets, the supporting points are too far apart as to make an accurate
	 * interpolation possible. This solution is not implemented, here. 2) The
	 * other solution is to compute the apsides of the orbit around the
	 * barycenter rather than around the sun. This procedure makes sense for
	 * planets beyond Jupiter, it comes closer to the mean apsides and nodes for
	 * planets that have such points defined. For all other transsaturnian
	 * planets and asteroids, this solution yields a kind of "mean" nodes and
	 * apsides. On the other hand, the barycentric ellipse does not make any
	 * sense for inner planets and Jupiter.
	 * 
	 * The Swiss Ephemeris supports solution 2) for planets and asteroids beyond
	 * Jupiter.
	 * 
	 * Anyway, neither the heliocentric nor the barycentric ellipse is a perfect
	 * representation of the nature of a planetary orbit, and it will not yield
	 * the degree of precision that today's astrology is used to. The best
	 * choice of method will probably be: - For Mercury - Neptune: mean nodes
	 * and apsides - For asteroids that belong to the inner asteroid belt:
	 * osculating nodes/apsides from a heliocentric ellipse - For Pluto and
	 * outer asteroids: osculating nodes/apsides from a barycentric ellipse
	 * 
	 * The Moon is a special case: A "lunar true node" makes more sense, because
	 * it can be defined without the idea of an ellipse, e.g. as the
	 * intersection axis of the momentary lunar orbital plane with the ecliptic.
	 * Or it can be said that the momentary motion of the moon points to one of
	 * the two ecliptic points that are called the "true nodes". So, these
	 * points make sense. With planetary nodes, the situation is somewhat
	 * different, at least if we make a difference between heliocentric and
	 * geocentric positions. If so, the planetary nodes are points on a
	 * heliocentric orbital ellipse, which are transformed to the geocenter. An
	 * ellipse is required here, because a solar distance is required. In
	 * contrast to the planetary nodes, the lunar node does not require a
	 * distance, therefore manages without the idea of an ellipse and does not
	 * share its weaknesses. On the other hand, the lunar apsides DO require the
	 * idea of an ellipse. And because the lunar ellipse is actually extremely
	 * distorted, even more than any other celestial ellipse, the "true Lilith"
	 * (apogee), for which printed ephemerides are available, does not make any
	 * sense at all. (See the chapter on the lunar node and apogee.)
	 * 
	 * Special case: the Earth
	 * 
	 * The Earth is another special case. Instead of the motion of the Earth
	 * herself, the heliocentric motion of the Earth- Moon-Barycenter (EMB) is
	 * used to determine the osculating perihelion. There is no node of the
	 * earth orbit itself. However, there is an axis around which the earth's
	 * orbital plane slowly rotates due to planetary precession. The position
	 * points of this axis are not calculated by the Swiss Ephemeris.
	 * 
	 * Special case: the Sun
	 * 
	 * In addition to the Earth (EMB) apsides, the function computes so-to-say
	 * "apsides" of the sun, i.e. points on the orbit of the Sun where it is
	 * closest to and where it is farthest from the Earth. These points form an
	 * opposition and are used by some astrologers, e.g. by the Dutch astrologer
	 * George Bode or the Swiss astrologer Liduina Schmed. The perigee, located
	 * at about 13 Capricorn, is called the "Black Sun", the other one, in
	 * Cancer, the "Diamond". So, for a complete set of apsides, one ought to
	 * calculate them for the Sun and the Earth and all other planets.
	 * 
	 * The modes of the Swiss Ephemeris function swe_nod_aps()
	 * 
	 * The function swe_nod_aps() can be run in the following modes: 1) Mean
	 * positions are given for nodes and apsides of Sun, Moon, Earth, and the up
	 * to Neptune. Osculating positions are given with Pluto and all asteroids.
	 * This is the default mode. 2) Osculating positions are returned for nodes
	 * and apsides of all planets. 3) Same as 2), but for planets and asteroids
	 * beyond Jupiter, a barycentric ellipse is used. 4) Same as 1), but for
	 * Pluto and asteroids beyond Jupiter, a barycentric ellipse is used.
	 * 
	 * In all of these modes, the second focal point of the ellipse can be
	 * computed instead of the aphelion. Like the planetary function swe_calc(),
	 * swe_nod_aps() is able to return geocentric, topocentric, heliocentric, or
	 * barycentric position.
	 *
	 * tjd_ut julian day, ephemeris time ipl planet number iflag as usual,
	 * SEFLG_HELCTR, etc. xnasc an array of 6 doubles: ascending node xndsc an
	 * array of 6 doubles: ascending node xperi an array of 6 doubles:
	 * perihelion xaphe an array of 6 doubles: aphelion method see below serr
	 * error message
	 *
	 * method can have the following values: - 0 or SE_NODBIT_MEAN. MEAN
	 * positions are given for nodes and apsides of Sun, Moon, Earth, and the
	 * planets up to Neptune. Osculating positions are given with Pluto and all
	 * asteroids. - SE_NODBIT_OSCU. Osculating positions are given for all nodes
	 * and apsides. - SE_NODBIT_OSCU_BAR. Osculating nodes and apsides are
	 * computed from barycentric ellipses, for planets beyond Jupiter, but from
	 * heliocentric ones for ones for Jupiter and inner planets. -
	 * SE_NODBIT_MEAN and SE_NODBIT_OSCU_BAR can be combined. The program
	 * behaves the same way as with simple SE_NODBIT_MEAN, but uses barycentric
	 * ellipses for planets beyond Neptune and asteroids beyond Jupiter. -
	 * SE_NODBIT_FOCAL can be combined with any of the other bits. The second
	 * focal points of the ellipses will be returned instead of the aphelia.
	 */
	...
	private static final double el_sema[][] = new double[][] { { 0.387098310, 0.0, 0.0, 0.0, }, /* Mercury */
	...
	private static final double plmass[] = new double[] { 6023600, /* Mercury */
			408523.5, /* Venus */
	 ...
	 * <P>
	 * See <A HREF=
	 * "Swecl.html#swe_nod_aps_ut(double, int, int, int, double[], double[], double[], double[], java.lang.StringBuffer)"
	 * >swe_nod_aps_ut(...)</A> for missing information.
	 */
	int swe_nod_aps(double tjd_et, int ipl, int iflag, int method, double[] xnasc, double[] xndsc, double[] xperi,
			double[] xaphe, StringBuffer serr) {
		int ij, i, j;
		int iplx;
		int ipli;
		int istart, iend;
		...
		double x[] = new double[6], xx[] = new double[24], xp[], xobs[] = new double[6], x2000[] = new double[6];
		int xpOffs = 0;
		...
		boolean do_aberr = (iflag & (SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_NOABERR)) == 0;
		boolean do_defl = (iflag & SweConst.SEFLG_TRUEPOS) == 0 && (iflag & SweConst.SEFLG_NOGDEFL) == 0;
		...
		iflg0 = (iflag & (SweConst.SEFLG_EPHMASK | SweConst.SEFLG_NONUT)) | SweConst.SEFLG_SPEED
				| SweConst.SEFLG_TRUEPOS;
		if (ipli != SweConst.SE_MOON) {
			iflg0 |= SweConst.SEFLG_HELCTR;
		}
		if (ipl == SweConst.SE_MEAN_NODE || ipl == SweConst.SE_TRUE_NODE || ipl == SweConst.SE_MEAN_APOG
				|| ipl == SweConst.SE_OSCU_APOG || ipl < 0
		...
		if ((method == 0 || (method & SweConst.SE_NODBIT_MEAN) != 0)
				&& ((ipl >= SweConst.SE_SUN && ipl <= SweConst.SE_NEPTUNE) || ipl == SweConst.SE_EARTH)) {
			if (ipl == SweConst.SE_MOON) {
				// sm.swi_mean_lunar_elements(tjd_et, &xna[0], &xna[3], &xpe[0],
				// &xpe[3]);
				...
				xpe0.val = xpe[0 + xpeOffs];
				DblObj xpe3 = new DblObj();
				xpe3.val = xpe[3 + xpeOffs];
			...
			ea = SMath.atan(SMath.tan(-parg * SwissData.DEGTORAD / 2) * SMath.sqrt((1 - ecce) / (1 + ecce))) * 2;
			eax = SMath.atan(
					SMath.tan(-pargx * SwissData.DEGTORAD / 2) * SMath.sqrt((1 - ecce - vecce) / (1 + ecce + vecce)))
					* 2;
			xna[2] = sema * (SMath.cos(ea) - ecce) / SMath.cos(parg * SwissData.DEGTORAD);
			xna[5] = (sema + vsema) * (SMath.cos(eax) - ecce - vecce) / SMath.cos(pargx * SwissData.DEGTORAD);
			xna[5] -= xna[2];
			ea = SMath.atan(SMath.tan((180 - parg) * SwissData.DEGTORAD / 2) * SMath.sqrt((1 - ecce) / (1 + ecce))) * 2;
			...
			iflJ2000 = (iflag & SweConst.SEFLG_EPHMASK) | SweConst.SEFLG_J2000 | SweConst.SEFLG_EQUATORIAL
					| SweConst.SEFLG_XYZ | SweConst.SEFLG_TRUEPOS | SweConst.SEFLG_NONUT | SweConst.SEFLG_SPEED;
			...
			/*
			 * we need three positions and three speeds for three nodes/apsides.
			 * from the three node positions, the speed of the node will be
			 * computed.
			 */
			if (ipli == SweConst.SE_MOON) {
				...
				if ((ipli >= SweConst.SE_MERCURY && ipli <= SweConst.SE_PLUTO) || ipli == SweConst.SE_EARTH) {
				...
				Gmsm = SwephData.HELGRAVCONST * (1 + plm) / SweConst.AUNIT / SweConst.AUNIT / SweConst.AUNIT * 86400.0
						* 86400.0;
			}
			...
			for (i = istart, t = tjd_et - dt; i <= iend; i++, t += dt) {
				...
				if (ipli == SweConst.SE_EARTH) {
					if (sw.swe_calc(t, SweConst.SE_MOON, iflJ2000 & ~(SweConst.SEFLG_BARYCTR | SweConst.SEFLG_HELCTR),
			...
			for (i = istart; i <= iend; i++) {
				...
				sinE = 1 / ecce / SMath.sqrt(sema * Gmsm) * sw.dot_prod(xpos[i], xpos[i], 3);
				/* true anomaly */
				...
				/*
				 * new distance of node from orbital ellipse: true anomaly of
				 * node:
				 */
				...
				cosE2 = SMath.cos(2 * SMath.atan(SMath.tan(ny2 / 2) / SMath.sqrt((1 + ecce) / (1 - ecce))));
				/* new distance */
				rn = sema * (1 - ecce * cosE);
		...
		/*
		 * to set the variables required in the save area, i.e. ecliptic,
		 * nutation, barycentric sun, earth we compute the planet
		 */
		if (ipli == SweConst.SE_MOON && (iflag & (SweConst.SEFLG_HELCTR | SweConst.SEFLG_BARYCTR)) != 0) {
			sw.swi_force_app_pos_etc();
			if (sw.swe_calc(tjd_et, SweConst.SE_SUN, iflg0, x, serr) == SweConst.ERR) {
				return SweConst.ERR;
			}
		} else {
			if (sw.swe_calc(tjd_et, ipli, iflg0 | (iflag & SweConst.SEFLG_TOPOCTR), x, serr) == SweConst.ERR) {
				return SweConst.ERR;
			}
		}
		/***********************
		...
		if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
			...
			/*
			 * for (i = 0; i <= 5; i++) xobs[i] = swed.topd.xobs[i];
			 */
		} else {
			for (i = 0; i <= 5; i++)
				xobs[i] = 0;
		...
		for (ij = 0, xp = xx, xpOffs = 0; ij < 4; ij++, xpOffs += 6) {
			...
			if (ipl == SweConst.SE_SUN && (iflag & (SweConst.SEFLG_HELCTR | SweConst.SEFLG_BARYCTR)) == 0) {
				for (j = 0; j <= 5; j++)
			...
			if (do_aberr) {
				 ...
				 * of the earth between t and t-dt. Neglecting this would result
				 * in an error of several 0.1"
				 */
				if ((iflag & SweConst.SEFLG_SPEED) != 0) {
					/* get barycentric sun and earth for t-dt into save area */
					if (sw.swe_calc(tjd_et - dt, ipli, iflg0 | (iflag & SweConst.SEFLG_TOPOCTR), x2,
							serr) == SweConst.ERR) {
						return SweConst.ERR;
					}
					if ((iflag & SweConst.SEFLG_TOPOCTR) != 0) {
						/* geocentric position of observer */
						/*
						 * if (sw.swi_get_observer(tjd_et - dt, iflag, false,
						 * xobs, serr) != SweConst.OK) return SweConst.ERR;
						 */
						for (i = 0; i <= 5; i++)
					 ...
					 * the save area (i.e. bary sun, earth nutation matrix!). to
					 * restore it:
					 */
					if (sw.swe_calc(tjd_et, SweConst.SE_SUN, iflg0 | (iflag & SweConst.SEFLG_TOPOCTR), x2,
							serr) == SweConst.ERR) {
						return SweConst.ERR;
					}
				}
			...
			if ((iflag & SweConst.SEFLG_SIDEREAL) != 0) {
				...
				if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_ECL_T0) != 0) {
					if (sw.swi_trop_ra2sid_lon(x2000, pldat.xreturn, 6, pldat.xreturn, 18, iflag,
							serr) != SweConst.OK) {
						return SweConst.ERR;
					}
					/* project onto solar system equator */
				} else if ((swed.sidd.sid_mode & SweConst.SE_SIDBIT_SSY_PLANE) != 0) {
					if (sw.swi_trop_ra2sid_lon_sosy(x2000, pldat.xreturn, 6, pldat.xreturn, 18, iflag,
							serr) != SweConst.OK) {
						return SweConst.ERR;
					}
				} else {
			...
			if ((iflag & SweConst.SEFLG_XYZ) != 0 && (iflag & SweConst.SEFLG_EQUATORIAL) != 0) {
				for (j = 0; j <= 5; j++)
	 ...
	 * points of the orbital ellipses). This method is identical to
	 * swe_nod_aps_ut() with the one exception that the time has to be given in
	 * UT (Universal Time) and not in ET (Ephemeris Time or Dynamical Time).
	 * 
	 * @param tjd_ut
	 *            The time in UT
	 * @param ipl
	 ...
	 *            something fails.
	 * @return SweConst.OK (0) or SweConst.ERR (-1)
	 * @see Swecl.html#swe_nod_aps(double, int, int, int, double[], double[],
	 ...
	 */
	int swe_nod_aps_ut(double tjd_ut, int ipl, int iflag, int method, double[] xnasc, double[] xndsc, double[] xperi,
			double[] xaphe, StringBuffer serr) {
		SweDate.swi_set_tid_acc(tjd_ut, iflag, 0);
		return swe_nod_aps(tjd_ut + SweDate.getDeltaT(tjd_ut), ipl, iflag, method, xnasc, xndsc, xperi, xaphe, serr);
	}
	 ...
	 * SEFLG_SWIEPH, SEFLG_JPLEPH, SEFLG_MOSEPH, SEFLG_TOPOCTR.
	 *
	 * imeth defines method: imeth = 0 use Placidus house position imeth = 1 use
	 * Placidus house posiition (with planetary lat = 0) imeth = 2 use rise and
	 * set of body's disc center imeth = 3 use rise and set of body's disc
	 * center with refraction rise and set are defined as appearance and
	 ...
	 */
	int swe_gauquelin_sector(double t_ut, int ipl, StringBuffer starname, int iflag, int imeth, double[] geopos,
		...
		dtmp.val = tret[0];
		retval = swe_rise_trans(t_ut, ipl, starname, epheflag, SweConst.SE_CALC_RISE | risemeth, geopos, atpress,
				attemp, dtmp, serr);
		tret[0] = dtmp.val;
		if (retval == SweConst.ERR) {
			...
			/*
			 * actually, we could return ERR here. However, we keep this
			 * variable, in case we implement an algorithm for Gauquelin sector
			 * positions of circumpolar bodies. As with the Ludwig Otto
			 * procedure with Placidus, one could replace missing rises or sets
			 * by meridian transits, although there are cases where even this is
			 * not possible. Sometimes a body both appears and disappears on the
			 * western part of the horizon. Using true culminations rather than
			 * meridan transits would not help in any case either, because there
			 * are cases where a body does not have a culmination within days,
		...
		dtmp.val = tret[1];
		retval = swe_rise_trans(t_ut, ipl, starname, epheflag, SweConst.SE_CALC_SET | risemeth, geopos, atpress, attemp,
				dtmp, serr);
		tret[1] = dtmp.val;
		if (retval == SweConst.ERR) {
			return SweConst.ERR;
		...
		if (tret[0] < tret[1] && rise_found == true) {
			...
			set_found = true;
			dtmp.val = tret[1];
			retval = swe_rise_trans(t, ipl, starname, epheflag, SweConst.SE_CALC_SET | risemeth, geopos, atpress,
					attemp, dtmp, serr);
			tret[1] = dtmp.val;
			if (retval == SweConst.ERR) {
				return SweConst.ERR;
			...
			rise_found = true;
			dtmp.val = tret[0];
			retval = swe_rise_trans(t, ipl, starname, epheflag, SweConst.SE_CALC_RISE | risemeth, geopos, atpress,
					attemp, dtmp, serr);
			tret[0] = dtmp.val;
			if (retval == SweConst.ERR) {
				return SweConst.ERR;
