...
/**
 * This class does all the calculations that are related to astrological houses.
 * <P>
...
class SweHouse implements java.io.Serializable {
	 ...
	 * 
	 * @param sl
	 *            A SwissLib object that might be already available.
	 * @param sw
	 *            A SwissEph object that might be already available.
	 * @param swed
	 *            A SwissData object that might be already available.
	 */
	...
	/*
	 * housasp.c cusps are returned in double cusp[13], or cusp[37] with house
	 * system 'G': cusp[1...12] houses 1 - 12 additional points are returned in
	 * ascmc[10]. ascmc[0] = ascendant ascmc[1] = mc ascmc[2] = armc ascmc[3] =
	 * vertex ascmc[4] = equasc * "equatorial ascendant" * ascmc[5] = coasc1 *
	 * "co-ascendant" (W. Koch) * ascmc[6] = coasc2 * "co-ascendant" (M.
	 * Munkasey) * ascmc[7] = polasc * "polar ascendant" (M. Munkasey) *
	 */
	/**
	 * /** Calculates the house positions and other vital points. The possible
	 * house systems (parameter &quot;hsys&quot;) are:
	 * <P>
	 * <CODE><BLOCKQUOTE>
	 ...
	 * done in that case by swapping MC/IC so that MC is always before AC in the
	 * zodiac. Then the quadrants are divided into 3 equal parts.
	 * <P>
	 * Calculates the house positions and other vital points.
	 * <p>
	 * <b>Note:</b>This routine is identical to the routine swe_houses_ex() in
	 * the original C code, and <b>not</b> to swe_houses()!
	 * <p>
	 * 
	 * @param tjd_ut
	 *            The Julian Day number in UT
	 * @param iflag
	 *            An additional flag for calculation, e.g. 0 or SEFLG_SIDEREAL
	 ...
	 *            house 1 to 12. It has to be a double[13].
	 * @param ascmc
	 *            The special points like ascendant etc. are returned here. It
	 *            has to be a double[10].
	...
	/*
	 * houses to sidereal ------------------ there are two methods: a) the
	 * traditional one houses are computed tropically, then nutation and the
	 * ayanamsa are subtracted. b) the projection on the ecliptic of t0 The
	 * house computation is then as follows:
	 *
	 * Be t the birth date and t0 the epoch at which ayanamsa = 0. 1. Compute
	 * the angle between the mean ecliptic at t0 and the true equator at t. The
	 * intersection point of these two circles we call the
	 * "auxiliary vernal point", and the angle between them the
	 * "auxiliary obliquity". 2. Compute the distance of the auxiliary vernal
	 * point from the vernal point at t. (this is a section on the equator) 3.
	 * subtract this value from the armc of t = aux. armc. 4. Compute the axes
	 * and houses for this aux. armc and aux. obliquity. 5. Compute the distance
	 * between the auxiliary vernal point and the vernal point at t0 (this is
	 * the ayanamsa at t, measured on the ecliptic of t0) 6. subtract this
	 * distance from all house cusps. 7. subtract ayanamsa_t0 from all house
	 * cusps.
	 */
	private int sidereal_houses_ecl_t0(double tjde, double armc, double eps, double[] nutlo, double lat, int hsys,
			double[] cusp, double[] ascmc, int aOffs) {
		int i, j, retc = SweConst.OK;
		double x[] = new double[6], xvpx[] = new double[6], x2[] = new double[6], epst0, xnorm[] = new double[6];
		...
		/*
		 * cartesian coordinates of an imaginary moving body on the the mean
		 * ecliptic of t0; we take the vernal point:
		 */
		x[0] = x[4] = 1;
		x[1] = x[2] = x[3] = x[5] = 0;
		/* to equator */
		...
		/*
		 * now, we have the moving point precessed to tjd_et. next, we compute
		 * the auxiliary epsilon:
		 */
		...
		/*
		 * distance of the auxiliary vernal point from the zero point at tjd_et
		 * (a section on the equator):
		 */
		...
		/*
		 * distance between auxiliary vernal point and vernal point of t0 (a
		 * section on the sidereal plane)
		 */
	...
	/*
	 * Be t the birth date and t0 the epoch at which ayanamsa = 0. 1. Compute
	 * the angle between the solar system rotation plane and the true equator at
	 * t. The intersection point of these two circles we call the
	 * "auxiliary vernal point", and the angle between them the
	 * "auxiliary obliquity". 2. Compute the distance of the auxiliary vernal
	 * point from the zero point at t. (this is a section on the equator) 3.
	 * subtract this value from the armc of t = aux. armc. 4. Compute the axes
	 * and houses for this aux. armc and aux. obliquity. 5. Compute the distance
	 * between the auxiliary vernal point at t and the zero point of the solar
	 * system plane J2000 (a section measured on the solar system plane) 6.
	 * subtract this distance from all house cusps. 7. compute the ayanamsa of
	 * J2000 on the solar system plane, referred to t0 8. subtract ayanamsa_t0
	 * from all house cusps. 9. subtract ayanamsa_2000 from all house cusps.
	 */
	private int sidereal_houses_ssypl(double tjde, double armc, double eps, double[] nutlo, double lat, int hsys,
			double[] cusp, double[] ascmc, int aOffs) {
		int i, j, retc = SweConst.OK;
		double x[] = new double[6], x0[] = new double[6], xvpx[] = new double[6], x2[] = new double[6],
				xnorm[] = new double[6];
		...
		/*
		 * cartesian coordinates of the zero point on the the solar system
		 * rotation plane
		 */
		x[0] = x[4] = 1;
		x[1] = x[2] = x[3] = x[5] = 0;
		...
		for (j = 0; j <= 2; j++)
			xvpx[j] = (x[j] - fac * x[j + 3]) * sgn; /* 1b */
		/*
		 * distance of the auxiliary vernal point from mean vernal point at
		 * tjd_et (a section on the equator):
		 */
		sl.swi_cartpol(xvpx, x2);
		dvpx = x2[0] * SwissData.RADTODEG; /* 2 */
		/* auxiliary armc */
		...
		retc = swe_houses_armc(armcx, lat, epsx, hsys, cusp, ascmc, aOffs); /* 4 */
		/*
		 * distance between the auxiliary vernal point at t and the sidereal
		 * zero point of 2000 at t (a section on the sidereal plane).
		 */
		dvpxe = SMath.acos(sl.swi_dot_prod_unit(x, xvpx)) * SwissData.RADTODEG; /* 5 */
	...
	private int sidereal_houses_trad(double tjde, double armc, double eps, double nutl, double lat, int hsys,
			double[] cusp, double[] ascmc, int aOffs) {
		int i, retc = SweConst.OK;
		double ay;
		int ito;
		char ihs = Character.toUpperCase((char) hsys);
		char ihs2 = ihs;
	...
	/*
	 * this function is required for very special computations where no date is
	 * given for house calculation, e.g. for composite charts or progressive
	 * charts. cusps are returned in double cusp[13], or cusp[37] with house
	 * system 'G'. cusp[1...12] houses 1 - 12 additional points are returned in
	 * ascmc[10]. ascmc[0] = ascendant ascmc[1] = mc ascmc[2] = armc ascmc[3] =
	 * vertex ascmc[4] = equasc * "equatorial ascendant" * ascmc[5] = coasc1 *
	 * "co-ascendant" (Walter Koch) * ascmc[6] = coasc2 * "co-ascendant"
	 * (Michael Munkasey) * ascmc[7] = polasc * "polar ascendant" (Michael
	 * Munkasey) *
	 */
	/**
	 ...
	 *            house 1 to 12.
	 * @param ascmc
	 *            The special points like ascendant etc. are returned here.
	...
	/* for APC houses */
	/*
	 * n number of house ph geographic latitude e ecliptic obliquity az armc
	 */
	private double apc_sector(int n, double ph, double e, double az) {
		...
		kv = SMath.atan(
				SMath.tan(ph) * SMath.tan(e) * SMath.cos(az) / (1 + SMath.tan(ph) * SMath.tan(e) * SMath.sin(az)));
		/* declination of the ascendant */
		dasc = SMath.atan(SMath.sin(kv) / SMath.tan(ph));
		/*
		 * note, at polar circles, when the mc sinks below the horizon, kv and
		 * dasc change sign in the above formulae. this is what we need, because
		 * the ascendand jumps by 180 deg
		 */
		...
		/*
		 * az + PI/2 + kv = armc + 90 + asc. diff. = right ascension of
		 * ascendant PI/2 +- kv = semi-diurnal or seminocturnal arc of ascendant
		...
		dret = SMath.atan2(SMath.tan(dasc) * SMath.tan(ph) * SMath.sin(az) + SMath.sin(a),
				SMath.cos(e) * (SMath.tan(dasc) * SMath.tan(ph) * SMath.cos(az) + SMath.cos(a))
						+ SMath.sin(e) * SMath.tan(ph) * SMath.sin(az - a));
	...
	/*
	 * ********************************************************* Arguments: th =
	 * sidereal time (angle 0..360 degrees hsy = letter code for house system; A
	 * equal E equal B Alcabitius C Campanus G 36 Gauquelin sectors H horizon /
	 * azimut K Koch M Morinus O Porphyry P Placidus R Regiomontanus T
	 * Polich/Page ("topocentric") U Krusinski-Pisa-Goelzer V equal Vehlow W
	 * equal, whole sign X axial rotation system/ Meridian houses Y APC houses
	 * fi = geographic latitude ekl = obliquity of the ecliptic iteration_count
	 * = number of iterations in Placidus calculation; can be 1 or 2.
	 * ********************************************************* Koch and
	 * Placidus don't work in the polar circle. We swap MC/IC so that MC is
	 * always before AC in the zodiac We than divide the quadrants into 3 equal
	 * parts. ********************************************************* All
	 * angles are expressed in degrees. Special trigonometric functions sind,
	 * cosd etc. are implemented for arguments in degrees.
	 ***********************************************************/
	{
		...
		tanfi = tand(fi);
		/* mc */
		if (SMath.abs(th - 90) > VERY_SMALL && SMath.abs(th - 270) > VERY_SMALL) {
			tant = tand(th);
		...
		case 'H':
			...
			/*
			 * within polar circle, when mc sinks below horizon and ascendant
			 * changes to western hemisphere, all cusps must be added 180
			 * degrees. houses will be in clockwise direction
			 */
		...
		case 'X': {
			/*
			 * Meridian or axial rotation system: ecliptic points whose
			 * rectascensions are armc + n * 30
			 */
			int j;
			double a2 = th;
			for (i = 1; i <= 12; i++) {
				...
				if (SMath.abs(a2 - 90) > VERY_SMALL && SMath.abs(a2 - 270) > VERY_SMALL) {
					tant = tand(a2);
		...
		case 'M': {
			/*
			 * Morinus points of the equator (armc + n * 30) are transformed
		...
		case 'B': { /* Alcabitius */
			 ...
			 * electrical library. The code reproduces the example! See
			 * http://www.astro.com/cgi/adict.cgi query: alcabitius in the
			 * resuotl page, see program code example. I think the Alcabitius
			 * code in Walter Pullen's Astrolog 5.40 is wrong, because he
			 * remains in RA and forgets the transform to the ecliptic.
			 */
			 ...
			 */
			sda = SMath.acos(r) * SwissData.RADTODEG; /*
														 * semidiurnal arc,
														 * measured on equator
														 */
		...
		case 'U': /* Krusinski-Pisa */
			 ...
			 *
			 * Definition: "Krusinski - house system based on the great circle
			 * passing through ascendant and zenith. This circle is divided into
			 * 12 equal parts (1st cusp is ascendant, 10th cusp is zenith), then
			 * the resulting points are projected onto the ecliptic through
			 * meridian circles. The house cusps in space are half-circles
			 * perpendicular to the equator and running from the north to the
			 * south celestial pole through the resulting cusp points on the
			 * house circle. The points where they cross the ecliptic mark the
			 * ecliptic house cusps."
			 *
			 * Description of the algorithm: Transform into great circle running
			 * through Asc and zenit (where arc between Asc and zenith is always
			 * 90 deg), and then return with house cusps into ecliptic. Eg.
			 * solve trigonometrical triangle with three transformations and two
			 * rotations starting from ecliptic. House cusps in space are
			 * meridian circles.
			 *
			 * Notes: 1. In this definition we assume MC on ecliptic as point
			 * where half-meridian (from north to south pole) cuts ecliptic, so
			 * MC may be below horizon in arctic regions. 2. Houses could be
			 * calculated in all latitudes except the poles themselves (-90,90)
			 * and points on arctic circle in cases where ecliptic is equal to
			 * horizon and then ascendant is undefined. But ascendant when
			 * 'horizon=ecliptic' could be deduced as limes from both sides of
			 * that point and houses with that provision can be computed also
			 * there.
			 *
			 * Starting values for calculations: - Asc ecliptic longitude -
			 * right ascension of MC (RAMC) - geographic latitude.
			 */
			/*
			 * within polar circle we swap AC/DC if AC is on wrong side
			 */
			...
			x[2] = 1.0; /*
						 * Radius to test validity of subsequent
						 * transformations.
						 */
			sl.swe_cotrans(x, x,
					-ekl); /* A1. Transform into equatorial coords */
			x[0] = x[0] - (th - 90); /* A2. Rotate */
			sl.swe_cotrans(x, x,
					-(90 - fi)); /* A3. Transform into horizontal coords */
			krHorizonLon = x[0]; /*
									 * ...save asc lon on horizon to get back later
									 * with house cusp
									 */
			x[0] = x[0] - x[0]; /* A4. Rotate */
			sl.swe_cotrans(x, x, -90); /*
										 * A5. Transform into this house system
										 * great circle (asc-zenith)
										 */
			/* As it is house circle now, simple add 30 deg increments... */
			for (i = 0; i < 6; i++) {
				/*
				 * B0. Set 'n-th' house cusp. Note that IC/MC are also
				 * calculated here to check if really this is the asc-zenith
				 * great circle.
				 */
				x[0] = 30.0 * i;
				x[1] = 0.0;
				sl.swe_cotrans(x, x,
						90); /* B1. Transform back into horizontal coords */
				x[0] = x[0] + krHorizonLon; /* B2. Rotate back. */
				sl.swe_cotrans(x, x, 90
						- fi); /* B3. Transform back into equatorial coords */
				x[0] = sl.swe_degnorm(
						x[0] + (th - 90)); /*
											 * B4. Rotate back -> RA of house
											 * cusp as result.
											 */
				/* B5. Where's this house cusp on ecliptic? */
				/*
				 * ... so last but not least - get ecliptic longitude of house
				 * cusp:
				 */
		...
		/*
		 * with tropical latitudes, the vertex behaves strange, in a similar way
		 * as the ascendant within the polar circle. we keep it always on the
		 * western hemisphere.
		 */
		if (SMath.abs(fi) <= ekl) {
		...
		if (SMath.abs(th2 - 90) > VERY_SMALL && SMath.abs(th2 - 270) > VERY_SMALL) {
			tant = tand(th2);
	...
	/*
	 * Computes the house position of a planet or another point, in degrees: 0 -
	 * 30 = 1st house, 30 - 60 = 2nd house, etc. armc sidereal time in degrees
	 * geolat geographic latitude eps true ecliptic obliquity hsys house system
	 * character xpin array of 6 doubles: only the first two of them are used:
	 * ecl. long., lat. serr error message area
	 *
	 * House position is returned by function.
	 *
	 * IMPORTANT: This function should NOT be used for sidereal astrology. If
	 * you cannot avoid doing so, please note: - The input longitudes (xpin)
	 * MUST always be tropical, even if you are a siderealist. - Sidereal and
	 * tropical house positions are identical for most house systems, if a
	 * traditional definition of the sidereal zodiac is used (sid = trop -
	 * ayanamsa). - The function does NOT provide correct positions for Whole
	 * Sign houses. - The function does NOT provide correct positions, if you
	 * use a non-traditional sidereal method (where the sidereal plane is not
	 * identical to the ecliptic of date) with a house system whose definition
	 * is dependent on the ecliptic, such as: equal, Porphyry, Alcabitius, Koch,
	 * Krusinski (all others should work). The Swiss Ephemeris currently does
	 * not handle these cases.
	 */
	/**
	 * The function returns a value between 1.0 and 12.999999, indicating in
	 * which house a planet is and how far from its cusp it is. It computes the
	 * house position of a planet or another point in degrees: 0 - 30 = 1st
	 * house, 30 - 60 = 2nd house, etc.
	 * 
	 * @param armc
	 *            The ARMC (sidereal time in degrees)
	 * @param geolat
	 *            The geographic latitude
	 * @param eps
	 *            The true ecliptic obliquity (e.g. xx[0] of swe_calc(...))
	 * @param hsys
	 *            The house system. See swe_houses(...) for a description of the
	 *            possible houses.
	 * @param xpin
	 *            A double[6] containing the ecliptic longitude and latitude of
	 *            the planet in degrees in xpin[0] and xpin[1]. The other xpin[]
	 *            values are not used, but the array has to be this size! The
	 *            values must describe tropical positions.
	 * @param serr
	 ...
	 *         planet is and how far from its cusp it is. #swe_houses
	 */
	double swe_house_pos(double armc, double geolat, double eps, int hsys, double xpin[], StringBuffer serr) {
		...
		double x[] = new double[3], xasc[] = new double[3], raep, raaz, oblaz,
				xtemp; /* BK 21.02.2006 */
		...
		case 'W':
			...
			/*
			 * to make sure that a call with a house cusp position returns a
			 * value within the house, 0.001" is added
			 */
		...
		case 'B': /* Alcabitius */
			...
			demc = atand(sind(armc) * tand(eps));
			/* mc */
			if (SMath.abs(armc - 90) > VERY_SMALL && SMath.abs(armc - 270) > VERY_SMALL) {
				tant = tand(armc);
			...
			/*
			 * while MC is always south, Asc must always be in eastern
			 * hemisphere
			 */
			if (geolat >= 0 && 90 - geolat + demc < 0) {
				asc = sl.swe_degnorm(asc + 180);
			...
			if (hsys == 'O') {
				...
				/*
				 * must treat the case of abs(r) > 1; probably does not happen
				 * because dek becomes smaller when fi is large, as ac is close
				 * to zero Aries/Libra in that case.
				 */
				sda = SMath.acos(r)
						* SwissData.RADTODEG; /*
												 * semidiurnal arc, measured on
												 * equator
												 */
				sna = 180 - sda; /* complement, seminocturnal arc */
				if (mdd > 0) {
		...
		case 'M': { /* Morinus */
			double am = xpin[0];
			if (SMath.abs(am - 90) > VERY_SMALL && SMath.abs(am - 270) > VERY_SMALL) {
				tant = tand(am);
			...
			break;
		/*
		 * version of Koch method: do calculations within circumpolar circle, if
		 * possible; make sure house positions 4 - 9 only appear on western
		 * hemisphere
		 */
		case 'K':
			demc = atand(sind(armc) * tand(eps));
			...
			if (SMath.abs(samc) > 0) {
				if (mdd >= 0) { /* east */
					...
					if (dfac > 2 || dfac < 0)
						is_invalid = true; /*
											 * if this is omitted, funny things
											 * happen
											 */
				} else {
		...
		case 'U': /* Krusinski-Pisa-Goelzer */
			/*
			 * Purpose: find point where planet's house circle (meridian) cuts
			 * house plane, giving exact planet's house position. Input data:
			 * ramc, geolat, asc.
			 */
			asc = Asc1(sl.swe_degnorm(armc + 90), geolat, sine, cose);
			demc = atand(sind(armc) * tand(eps));
			 ...
			 * with equator and at what angle, and then simple find arc from asc
			 * on that plane to planet's meridian intersection with this plane.
			 */
			/*
			 * I. find plane of 'asc-zenith' great circle relative to equator:
			 * solve spherical triangle 'EP-asc-intersection of house circle
			 * with equator'
			 */
			/* Ia. Find intersection of house plane with equator: */
			x[0] = asc;
			x[1] = 0.0;
			x[2] = 1.0; /* 1. Start with ascendant on ecliptic */
			sl.swe_cotrans(x, x,
					-eps); /* 2. Transform asc into equatorial coords */
			raep = sl.swe_degnorm(armc + 90); /* 3. RA of east point */
			x[0] = sl.swe_degnorm(
					raep - x[0]); /* 4. Rotation - found arc raas-raep */
			sl.swe_cotrans(x, x,
					-(90 - geolat)); /*
										 * 5. Transform into horizontal coords -
										 * arc EP-asc on horizon
										 */
			xtemp = atand(tand(x[0])
					/ cosd((90 - geolat))); /*
											 * 6. Rotation from horizon on
											 * circle perpendicular to equator
											 */
			if (x[0] > 90 && x[0] <= 270)
			...
			raaz = sl.swe_degnorm(
					raep - x[0]); /*
									 * result: RA of intersection 'asc-zenith'
									 * great circle with equator
									 */
			/* Ib. Find obliquity to equator of 'asc-zenith' house plane: */
			x[0] = raaz;
			x[1] = 0.0;
			x[0] = sl.swe_degnorm(
					raep - x[0]); /* 1. Rotate start point relative to EP */
			sl.swe_cotrans(x, x,
					-(90 - geolat)); /* 2. Transform into horizontal coords */
			x[1] = x[1] + 90; /*
								 * 3. Add 90 deg do decl - so get the point on
								 * house plane most distant from equ.
								 */
			sl.swe_cotrans(x, x, 90 - geolat); /* 4. Rotate back to equator */
			oblaz = x[1]; /* 5. Obliquity of house plane to equator */
			/*
			 * II. Next find asc and planet position on house plane, so to find
			 * relative distance of planet from coords beginning.
			 */
			/*
			 * IIa. Asc on house plane relative to intersection of equator with
			 * 'asc-zenith' plane.
			 */
			xasc[0] = asc;
			xasc[1] = 0.0;
			...
			/*
			 * IIb. Planet on house plane relative to intersection of equator
			 * with 'asc-zenith' plane
			 */
			xp[0] = sl.swe_degnorm(xeq[0] - raaz); /* Rotate on equator */
			xtemp = atand(tand(xp[0])
					/ cosd(oblaz)); /* Find arc on house plane from equator */
			if (xp[0] > 90 && xp[0] <= 270)
				xtemp = sl.swe_degnorm(xtemp + 180);
			xp[0] = sl.swe_degnorm(xtemp);
			xp[0] = sl.swe_degnorm(
					xp[0] - xasc[0]); /*
										 * find arc between asc and planet, and
			...
			xp[1] = xeq[1] - x[1]; /*
									 * How many degrees is the point on
									 * declination circle from house circle
									 */
			/*
			 * to make sure that a call with a house cusp position returns a
			 * value within the house, 0.001" is added
		...
		case 'T':
			...
			/*
			 * mirror everything below the horizon to the opposite point above
			 * the horizon
			 */
			if (!is_above_hor) {
