...
*/
class SwephData implements java.io.Serializable {

	static final String SE_VERSION = "2.01.00";
	static final String SE_JAVA_VERSION = "2.01.00_01";

	// Aus sweph.h:
	static final int[] PNOINT2JPL = new int[] { SwephJPL.J_EARTH, SwephJPL.J_MOON, SwephJPL.J_MERCURY, SwephJPL.J_VENUS,
			SwephJPL.J_MARS, SwephJPL.J_JUPITER, SwephJPL.J_SATURN, SwephJPL.J_URANUS, SwephJPL.J_NEPTUNE,
			SwephJPL.J_PLUTO, SwephJPL.J_SUN };
	...
	/*
	 * Ayanamsas For each ayanamsa, there are two values: t0 epoch of ayanamsa,
	 * TDT (ET) ayan_t0 ayanamsa value at epoch
	 */
	static final AyaInit[] ayanamsa = new AyaInit[] {
			new AyaInit(2433282.5,
			...
			new AyaInit(2435553.5, 23.250182778
					- 0.004660222), /*
									 * 1: Lahiri (derived from: Indian
									 * Astronomical Ephemeris 1989, p. 556; the
									 * subtracted value is nutation)
									 */
			...
			new AyaInit(1673941,
					-5.079167), /*
								 * 13: Babylonian, Mercier; eta Piscium
								 * culminates with zero point
								 */
			new AyaInit(1684532.5,
					-4.44088389), /*
			...
			new AyaInit(1903396.8128654,
					0), /*
						 * 21: Suryasiddhanta, assuming ingress of mean Sun into
						 * Aries at point of mean equinox of date on 21.3.499,
						 * noon, Ujjain (75.7684565 E) = 7:30:31.57 UT
						 */
			new AyaInit(1903396.8128654,
					-0.21463395), /*
									 * 22: Suryasiddhanta, assuming ingress of
									 * mean Sun into Aries at true position of
									 * mean Sun at same epoch
									 */
			new AyaInit(1903396.7895321, 0), /*
												 * 23: Aryabhata, same date, but
												 * UT 6:56:55.57 analogous 21
												 */
			new AyaInit(1903396.7895321,
					-0.23763238), /* 24: Aryabhata, analogous 22 */
			new AyaInit(1903396.8128654,
					-0.79167046), /*
	...
	/*
	 * Chiron's orbit becomes chaotic before 720 AD and after 4606 AD, because
	 * of close encounters with Saturn. Accepting a maximum error of 5 degrees,
	 * the ephemeris is good between the following dates:
	 */
	/*
	 * static final double CHIRON_START = 1958470.5; * 1.1.650 old limit until
	 * v. 2.00
	 */
	...
	/*
	 * Pholus's orbit is unstable as well, because he sometimes approaches
	 * Saturn. Accepting a maximum error of 5 degrees, the ephemeris is good
	 * after the following date:
	 */
	/*
	 * static final double PHOLUS_START = 314845.5; * 1.1.-3850 old limit until
	 * v. 2.00
	 */
	...
	/*
	 * static final double MOSHNDEPH_START = -254900.5; // 14 Feb -5410 00:00 ET
	 * jul.cal.
	 */
	/*
	 * static final double MOSHNDEPH_END = 3697000.5; // 11 Dec 5409 00:00 ET,
	 * greg. cal.
	 */
	static final double MOSHNDEPH_START = -3100015.5; // 15 Aug -13200 00:00 ET
														// jul.cal.*/
	static final double MOSHNDEPH_END = 8000016.5; // 15 Mar 17191 00:00 ET,
													// greg. cal */
	/*
	*/
	...
	static final double SUN_EARTH_MRAT = 332946.050895; // Su / (Ea only) AA
														// 2006 K7
	...
	static final double HELGRAVCONST = 1.32712440017987e+20; // G * M(sun),
																// m^3/sec^2, AA
																// 2006 K6
	static final double GEOGCONST = 3.98600448e+14; // G * M(earth) m^3/sec^2,
													// AA 1996 K6
	static final double KGAUSS = 0.01720209895; // Gaussian gravitational
												// constant K6
	static final double KGAUSS_GEO = 0.0000298122353216; // Earth only
	// static final double KGAUSS_GEO = 0.0000299502129737 // Earth + Moon
	static final double SUN_RADIUS = 959.63 / 3600 * SwissData.DEGTORAD; // Meeus
																			// germ.
																			// p
																			// 391
	static final double EARTH_RADIUS = 6378136.6; // AA 2006 K6
	/*
	 * static final double EARTH_OBLATENESS = (1.0/ 298.257223563); * AA 1998
	 * K13
	 */
	static final double EARTH_OBLATENESS = (1.0 / 298.25642); // AA 2006 K6
	static final double EARTH_ROT_SPEED = 7.2921151467e-5 * 86400; // in
																	// rad/day,
																	// expl.
																	// suppl., p
																	// 162

	static final double LIGHTTIME_AUNIT = (499.0047838061 / 3600 / 24); // 8.3167
																		// minutes
																		// (days),
																		// AA
																		// 2006
	...
	/*
	 * stuff exported from swemplan.c and swemmoon.c and constants used inside
	 * these functions.
	 ************************************************************/

	static final double STR = 4.8481368110953599359e-6; // radians per arc
														// second
	...
	// static final int pnoext2int[] = {SEI_SUN, SEI_MOON, SEI_MERCURY,
	// SEI_VENUS,
	...
	// java Swetest -head -bj-3027215.5 -s... -n... -ejplde431.eph -edir./ephe
	// -fPJadss -p0
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > sun-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > sun-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > sun-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > sun-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > sun-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > sun-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > sun-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > sun-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > sun-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p0 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > sun-topo--179-0-50000 &
	...
	// SEFLG_EQUATORIAL refers to the earth equator ALWAYS, so we skip
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p1
	// -s0.7064 -fPJadss -n15511619 > moon_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > moon-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > moon-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > moon-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > moon-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > moon-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > moon-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > moon-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > moon-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > moon-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p1 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > moon-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p2
	// -s0.7064 -fPJadss -n15511619 > mercury_jpl
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p2
	// -s0.7064 -fPJadss -n15511619 -topo0,0,50000 > mercury-topo-0-0-50000
	// sort -n -k+6 mercury-topo-0-0-50000 | tail -3; sort -n -k+6
	// mercury-topo-0-0-50000 | head -6
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p3
	// -s0.7064 -fPJadss -n15511619 > venus_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > venus-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > venus-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > venus-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > venus-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > venus-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > venus-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > venus-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > venus-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > venus-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p3 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > venus-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p4
	// -s0.7064 -fPJadss -n15511619 > mars_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > mars-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > mars-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > mars-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > mars-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > mars-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > mars-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > mars-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > mars-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > mars-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p4 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > mars-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p5
	// -s0.7064 -fPJadss -n15511619 > jupiter_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > jupiter-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > jupiter-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > jupiter-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > jupiter-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > jupiter-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > jupiter-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > jupiter-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > jupiter-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > jupiter-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p5 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > jupiter-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p6
	// -s0.7064 -fPJadss -n15511619 > saturn_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > saturn-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > saturn-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > saturn-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > saturn-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > saturn-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > saturn-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > saturn-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > saturn-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > saturn-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p6 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > saturn-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p7
	// -s0.7064 -fPJadss -n15511619 > uranus_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > uranus-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > uranus-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > uranus-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > uranus-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > uranus-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > uranus-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > uranus-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > uranus-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > uranus-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p7 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > uranus-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p8
	// -s0.7064 -fPJadss -n15511619 > neptun_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > neptun-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > neptun-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > neptun-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > neptun-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > neptun-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > neptun-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > neptun-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > neptun-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > neptun-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p8 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > neptun-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -p9
	// -s0.7064 -fPJadss -n15511619 > pluto_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > pluto-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > pluto-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > pluto-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > pluto-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > pluto-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > pluto-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > pluto-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > pluto-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > pluto-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -p9 -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > pluto-topo--179-0-50000 &
	...
	/**
	 * There is no meaning in calculating topocentric positions of mean
	 * elements, so it more an academic kind of exercise...
	 */
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pm
	// -s0.7064 -fPJadss -n15511619 > meannode_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > meannode-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > meannode-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > meannode-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > meannode-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > meannode-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > meannode-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > meannode-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > meannode-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > meannode-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pm -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > meannode-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pt
	// -s0.7064 -fPJadss -n15511619 > truenode_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > truenode-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > truenode-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > truenode-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > truenode-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > truenode-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > truenode-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > truenode-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > truenode-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > truenode-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pt -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > truenode-topo--179-0-50000 &
	...
	/**
	 * There is no meaning in calculating topocentric positions of mean
	 * elements, so it's more an academic kind of exercise...
	 */
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pA
	// -s0.7064 -fPJadss -n15511619 > meanapogee_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > meanapogee-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > meanapogee-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > meanapogee-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > meanapogee-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > meanapogee-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > meanapogee-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > meanapogee-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > meanapogee-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > meanapogee-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pA -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > meanapogee-topo--179-0-50000 &
	// ... -topo0,0,50000
	// ... -topo11,0,0
	// ... -topo11,0,50000
	// ... -topo11,0,-6300000
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pB
	// -s0.7064 -fPJadss -n15511619 > oscapogee_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > oscapogee-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > oscapogee-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > oscapogee-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > oscapogee-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > oscapogee-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > oscapogee-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > oscapogee-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > oscapogee-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > oscapogee-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pB -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > oscapogee-topo--179-0-50000 &
	// ... -topo0,0,50000
	// ... -topo11,0,0
	// ... -topo11,0,50000
	// ... -topo11,0,-6300000
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pD
	// -s0.7064 -fPJadss -n15511619 > chiron_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > chiron-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > chiron-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > chiron-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > chiron-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > chiron-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > chiron-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > chiron-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > chiron-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > chiron-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pD -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > chiron-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pE
	// -s0.7064 -fPJadss -n15511619 > pholus_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > pholus-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > pholus-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > pholus-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > pholus-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > pholus-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > pholus-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > pholus-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > pholus-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > pholus-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pE -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > pholus-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pF
	// -s0.7064 -fPJadss -n15511619 > ceres_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > ceres-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > ceres-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > ceres-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > ceres-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > ceres-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > ceres-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > ceres-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > ceres-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > ceres-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pF -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > ceres-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pG
	// -s0.7064 -fPJadss -n15511619 > pallas_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > pallas-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > pallas-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > pallas-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > pallas-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > pallas-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > pallas-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > pallas-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > pallas-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > pallas-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pG -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > pallas-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pH
	// -s0.7064 -fPJadss -n15511619 > juno_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > juno-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > juno-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > juno-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > juno-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > juno-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > juno-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > juno-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > juno-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > juno-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pH -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > juno-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pI
	// -s0.7064 -fPJadss -n15511619 > vesta_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > vesta-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > vesta-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > vesta-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > vesta-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > vesta-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > vesta-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > vesta-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > vesta-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > vesta-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pI -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > vesta-topo--179-0-50000 &
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pc
	// -s0.7064 -fPJadss -n15511619 > intpapogee_jpl
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > intpapogee-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > intpapogee-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > intpapogee-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > intpapogee-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > intpapogee-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > intpapogee-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > intpapogee-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > intpapogee-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > intpapogee-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pc -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > intpapogee-topo--179-0-50000 &
	// ... -topo0,0,50000
	// ... -topo11,0,0
	// ... -topo11,0,50000
	// ... -topo11,0,-6300000
	...
	// time ./swetest -head -bj-3027215.5 -ejplde431.eph -edir./ephe -pg
	// -s0.7064 -fPJadss -n15511619 > intpperigee_jpl
	// -2.6484532 -> 0.6663555
	...
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo0,0,50000 > intpperigee-topo-0-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,0,0 > intpperigee-topo-11-0-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > intpperigee-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,0,-6300000 > intpperigee-topo-11-0--6300000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,0,50000 > intpperigee-topo-11-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,89,0 > intpperigee-topo-11-89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,-89,0 > intpperigee-topo-11--89-0 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,89,50000 > intpperigee-topo-11-89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo11,-89,50000 > intpperigee-topo-11--89-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo179,0,50000 > intpperigee-topo-179-0-50000 &
	// time ./swetest -bj-3027215.5 -ejplde431.eph -edir./ephe -pg -s0.7064
	// -fPJadss -n15511619 -topo-179,0,50000 > intpperigee-topo--179-0-50000 &
	// ... -topo0,0,50000
	// ... -topo11,0,0
	// ... -topo11,0,-6300000
	// ... -topo11,89,0
	...
	static final double[] maxLonSpeed = new double[] { SURYA_MAX_LON_SPEED, CHANDRA_MAX_LON_SPEED, BUDHA_MAX_LON_SPEED,
			SHUKRA_MAX_LON_SPEED, MANGALA_MAX_LON_SPEED, GURU_MAX_LON_SPEED, SHANI_MAX_LON_SPEED, URANUS_MAX_LON_SPEED,
			NEPTUNE_MAX_LON_SPEED, PLUTO_MAX_LON_SPEED, MNODE_MAX_LON_SPEED, TNODE_MAX_LON_SPEED, MAPOGEE_MAX_LON_SPEED,
			OAPOGEE_MAX_LON_SPEED, 1. / 0, CHIRON_MAX_LON_SPEED, PHOLUS_MAX_LON_SPEED, CERES_MAX_LON_SPEED,
			PALLAS_MAX_LON_SPEED, JUNO_MAX_LON_SPEED, VESTA_MAX_LON_SPEED, INTPAPOG_MAX_LON_SPEED,
			INTPPERG_MAX_LON_SPEED };
	// Minimum geocentric speeds in longitudinal direction:
	static final double[] minLonSpeed = new double[] { SURYA_MIN_LON_SPEED, CHANDRA_MIN_LON_SPEED, BUDHA_MIN_LON_SPEED,
			SHUKRA_MIN_LON_SPEED, MANGALA_MIN_LON_SPEED, GURU_MIN_LON_SPEED, SHANI_MIN_LON_SPEED, URANUS_MIN_LON_SPEED,
			NEPTUNE_MIN_LON_SPEED, PLUTO_MIN_LON_SPEED, MNODE_MIN_LON_SPEED, TNODE_MIN_LON_SPEED, MAPOGEE_MIN_LON_SPEED,
			OAPOGEE_MIN_LON_SPEED, 1. / 0, CHIRON_MIN_LON_SPEED, PHOLUS_MIN_LON_SPEED, CERES_MIN_LON_SPEED,
			PALLAS_MIN_LON_SPEED, JUNO_MIN_LON_SPEED, VESTA_MIN_LON_SPEED, INTPAPOG_MIN_LON_SPEED,
			INTPPERG_MIN_LON_SPEED };

	// Maximum topocentric speeds in longitudinal direction, up to 50000m
	// altitude:
	static final double[] maxTopoLonSpeed = new double[] { SURYA_MAX_TOPO_LON_SPEED, CHANDRA_MAX_TOPO_LON_SPEED,
			BUDHA_MAX_TOPO_LON_SPEED, SHUKRA_MAX_TOPO_LON_SPEED, MANGALA_MAX_TOPO_LON_SPEED, GURU_MAX_TOPO_LON_SPEED,
			SHANI_MAX_TOPO_LON_SPEED, URANUS_MAX_TOPO_LON_SPEED, NEPTUNE_MAX_TOPO_LON_SPEED, PLUTO_MAX_TOPO_LON_SPEED,
			MNODE_MAX_TOPO_LON_SPEED, TNODE_MAX_TOPO_LON_SPEED, MAPOGEE_MAX_TOPO_LON_SPEED, OAPOGEE_MAX_TOPO_LON_SPEED,
			1. / 0, CHIRON_MAX_TOPO_LON_SPEED, PHOLUS_MAX_TOPO_LON_SPEED, CERES_MAX_TOPO_LON_SPEED,
			PALLAS_MAX_TOPO_LON_SPEED, JUNO_MAX_TOPO_LON_SPEED, VESTA_MAX_TOPO_LON_SPEED, INTPAPOG_MAX_TOPO_LON_SPEED,
			INTPPERG_MAX_TOPO_LON_SPEED };
	// Minimum topocentric speeds in longitudinal direction, up to 50000m
	// altitude:
	static final double[] minTopoLonSpeed = new double[] { SURYA_MIN_TOPO_LON_SPEED, CHANDRA_MIN_TOPO_LON_SPEED,
			BUDHA_MIN_TOPO_LON_SPEED, SHUKRA_MIN_TOPO_LON_SPEED, MANGALA_MIN_TOPO_LON_SPEED, GURU_MIN_TOPO_LON_SPEED,
			SHANI_MIN_TOPO_LON_SPEED, URANUS_MIN_TOPO_LON_SPEED, NEPTUNE_MIN_TOPO_LON_SPEED, PLUTO_MIN_TOPO_LON_SPEED,
			MNODE_MIN_TOPO_LON_SPEED, TNODE_MIN_TOPO_LON_SPEED, MAPOGEE_MIN_TOPO_LON_SPEED, OAPOGEE_MIN_TOPO_LON_SPEED,
			1. / 0, CHIRON_MIN_TOPO_LON_SPEED, PHOLUS_MIN_TOPO_LON_SPEED, CERES_MIN_TOPO_LON_SPEED,
	...
	static final double[] maxHelioLonSpeed = new double[] { SURYA_MAX_HELIO_LON_SPEED, CHANDRA_MAX_HELIO_LON_SPEED,
			BUDHA_MAX_HELIO_LON_SPEED, SHUKRA_MAX_HELIO_LON_SPEED, MANGALA_MAX_HELIO_LON_SPEED,
			GURU_MAX_HELIO_LON_SPEED, SHANI_MAX_HELIO_LON_SPEED, URANUS_MAX_HELIO_LON_SPEED,
			NEPTUNE_MAX_HELIO_LON_SPEED, PLUTO_MAX_HELIO_LON_SPEED, MNODE_MAX_HELIO_LON_SPEED,
			TNODE_MAX_HELIO_LON_SPEED, MAPOGEE_MAX_HELIO_LON_SPEED, OAPOGEE_MAX_HELIO_LON_SPEED, 1. / 0,
			CHIRON_MAX_HELIO_LON_SPEED, PHOLUS_MAX_HELIO_LON_SPEED, CERES_MAX_HELIO_LON_SPEED,
			PALLAS_MAX_HELIO_LON_SPEED, JUNO_MAX_HELIO_LON_SPEED, VESTA_MAX_HELIO_LON_SPEED,
			INTPAPOG_MAX_HELIO_LON_SPEED, INTPPERG_MAX_HELIO_LON_SPEED };
	// Minimum heliocentric speeds in longitudinal direction:
	static final double[] minHelioLonSpeed = new double[] { SURYA_MIN_HELIO_LON_SPEED, CHANDRA_MIN_HELIO_LON_SPEED,
			BUDHA_MIN_HELIO_LON_SPEED, SHUKRA_MIN_HELIO_LON_SPEED, MANGALA_MIN_HELIO_LON_SPEED,
			GURU_MIN_HELIO_LON_SPEED, SHANI_MIN_HELIO_LON_SPEED, URANUS_MIN_HELIO_LON_SPEED,
			NEPTUNE_MIN_HELIO_LON_SPEED, PLUTO_MIN_HELIO_LON_SPEED, MNODE_MIN_HELIO_LON_SPEED,
			TNODE_MIN_HELIO_LON_SPEED, MAPOGEE_MIN_HELIO_LON_SPEED, OAPOGEE_MIN_HELIO_LON_SPEED, 1. / 0,
			CHIRON_MIN_HELIO_LON_SPEED, PHOLUS_MIN_HELIO_LON_SPEED, CERES_MIN_HELIO_LON_SPEED,
			PALLAS_MIN_HELIO_LON_SPEED, JUNO_MIN_HELIO_LON_SPEED, VESTA_MIN_HELIO_LON_SPEED,
			INTPAPOG_MIN_HELIO_LON_SPEED, INTPPERG_MIN_HELIO_LON_SPEED };

	// Maximum geocentric accelerations in longitudinal direction:
	static final double[] maxLonAccel = new double[] { SURYA_MAX_LON_ACCEL, CHANDRA_MAX_LON_ACCEL, BUDHA_MAX_LON_ACCEL,
			SHUKRA_MAX_LON_ACCEL, MANGALA_MAX_LON_ACCEL, GURU_MAX_LON_ACCEL, SHANI_MAX_LON_ACCEL, URANUS_MAX_LON_ACCEL,
			NEPTUNE_MAX_LON_ACCEL, PLUTO_MAX_LON_ACCEL, MNODE_MAX_LON_ACCEL, TNODE_MAX_LON_ACCEL, MAPOGEE_MAX_LON_ACCEL,
			OAPOGEE_MAX_LON_ACCEL, 1. / 0, CHIRON_MAX_LON_ACCEL, PHOLUS_MAX_LON_ACCEL, CERES_MAX_LON_ACCEL,
			PALLAS_MAX_LON_ACCEL, JUNO_MAX_LON_ACCEL, VESTA_MAX_LON_ACCEL, INTPAPOG_MAX_LON_ACCEL,
			INTPPERG_MAX_LON_ACCEL };
	// Minimum geocentric accelerations in longitudinal direction:
	static final double[] minLonAccel = new double[] { SURYA_MIN_LON_ACCEL, CHANDRA_MIN_LON_ACCEL, BUDHA_MIN_LON_ACCEL,
			SHUKRA_MIN_LON_ACCEL, MANGALA_MIN_LON_ACCEL, GURU_MIN_LON_ACCEL, SHANI_MIN_LON_ACCEL, URANUS_MIN_LON_ACCEL,
			NEPTUNE_MIN_LON_ACCEL, PLUTO_MIN_LON_ACCEL, MNODE_MIN_LON_ACCEL, TNODE_MIN_LON_ACCEL, MAPOGEE_MIN_LON_ACCEL,
			OAPOGEE_MIN_LON_ACCEL, 1. / 0, CHIRON_MIN_LON_ACCEL, PHOLUS_MIN_LON_ACCEL, CERES_MIN_LON_ACCEL,
			PALLAS_MIN_LON_ACCEL, JUNO_MIN_LON_ACCEL, VESTA_MIN_LON_ACCEL, INTPAPOG_MIN_LON_ACCEL,
			INTPPERG_MIN_LON_ACCEL };

	// Maximum topocentric accelerations in longitudinal direction, up to
	// 50000m altitude:
	static final double[] maxTopoLonAccel = new double[] { SURYA_MAX_TOPO_LON_ACCEL, CHANDRA_MAX_TOPO_LON_ACCEL,
			BUDHA_MAX_TOPO_LON_ACCEL, SHUKRA_MAX_TOPO_LON_ACCEL, MANGALA_MAX_TOPO_LON_ACCEL, GURU_MAX_TOPO_LON_ACCEL,
			SHANI_MAX_TOPO_LON_ACCEL, URANUS_MAX_TOPO_LON_ACCEL, NEPTUNE_MAX_TOPO_LON_ACCEL, PLUTO_MAX_TOPO_LON_ACCEL,
			MNODE_MAX_TOPO_LON_ACCEL, TNODE_MAX_TOPO_LON_ACCEL, MAPOGEE_MAX_TOPO_LON_ACCEL, OAPOGEE_MAX_TOPO_LON_ACCEL,
			1. / 0, CHIRON_MAX_TOPO_LON_ACCEL, PHOLUS_MAX_TOPO_LON_ACCEL, CERES_MAX_TOPO_LON_ACCEL,
			PALLAS_MAX_TOPO_LON_ACCEL, JUNO_MAX_TOPO_LON_ACCEL, VESTA_MAX_TOPO_LON_ACCEL, INTPAPOG_MAX_TOPO_LON_ACCEL,
			INTPPERG_MAX_TOPO_LON_ACCEL };
	// Minimum topocentric accelerations in longitudinal direction, up to
	// 50000m altitude:
	static final double[] minTopoLonAccel = new double[] { SURYA_MIN_TOPO_LON_ACCEL, CHANDRA_MIN_TOPO_LON_ACCEL,
			BUDHA_MIN_TOPO_LON_ACCEL, SHUKRA_MIN_TOPO_LON_ACCEL, MANGALA_MIN_TOPO_LON_ACCEL, GURU_MIN_TOPO_LON_ACCEL,
			SHANI_MIN_TOPO_LON_ACCEL, URANUS_MIN_TOPO_LON_ACCEL, NEPTUNE_MIN_TOPO_LON_ACCEL, PLUTO_MIN_TOPO_LON_ACCEL,
			MNODE_MIN_TOPO_LON_ACCEL, TNODE_MIN_TOPO_LON_ACCEL, MAPOGEE_MIN_TOPO_LON_ACCEL, OAPOGEE_MIN_TOPO_LON_ACCEL,
			1. / 0, CHIRON_MIN_TOPO_LON_ACCEL, PHOLUS_MIN_TOPO_LON_ACCEL, CERES_MIN_TOPO_LON_ACCEL,
			PALLAS_MIN_TOPO_LON_ACCEL, JUNO_MIN_TOPO_LON_ACCEL, VESTA_MIN_TOPO_LON_ACCEL, INTPAPOG_MIN_TOPO_LON_ACCEL,
			INTPPERG_MIN_TOPO_LON_ACCEL };

	// Maximum heliocentric accelerations in longitudinal direction:
	static final double[] maxHelioLonAccel = new double[] { SURYA_MAX_HELIO_LON_ACCEL, CHANDRA_MAX_HELIO_LON_ACCEL,
			BUDHA_MAX_HELIO_LON_ACCEL, SHUKRA_MAX_HELIO_LON_ACCEL, MANGALA_MAX_HELIO_LON_ACCEL,
			GURU_MAX_HELIO_LON_ACCEL, SHANI_MAX_HELIO_LON_ACCEL, URANUS_MAX_HELIO_LON_ACCEL,
			NEPTUNE_MAX_HELIO_LON_ACCEL, PLUTO_MAX_HELIO_LON_ACCEL, MNODE_MAX_HELIO_LON_ACCEL,
			TNODE_MAX_HELIO_LON_ACCEL, MAPOGEE_MAX_HELIO_LON_ACCEL, OAPOGEE_MAX_HELIO_LON_ACCEL, 1. / 0,
			CHIRON_MAX_HELIO_LON_ACCEL, PHOLUS_MAX_HELIO_LON_ACCEL, CERES_MAX_HELIO_LON_ACCEL,
			PALLAS_MAX_HELIO_LON_ACCEL, JUNO_MAX_HELIO_LON_ACCEL, VESTA_MAX_HELIO_LON_ACCEL,
			INTPAPOG_MAX_HELIO_LON_ACCEL, INTPPERG_MAX_HELIO_LON_ACCEL };
	// Minimum heliocentric accelerations in longitudinal direction:
	static final double[] minHelioLonAccel = new double[] { SURYA_MIN_HELIO_LON_ACCEL, CHANDRA_MIN_HELIO_LON_ACCEL,
			BUDHA_MIN_HELIO_LON_ACCEL, SHUKRA_MIN_HELIO_LON_ACCEL, MANGALA_MIN_HELIO_LON_ACCEL,
			GURU_MIN_HELIO_LON_ACCEL, SHANI_MIN_HELIO_LON_ACCEL, URANUS_MIN_HELIO_LON_ACCEL,
			NEPTUNE_MIN_HELIO_LON_ACCEL, PLUTO_MIN_HELIO_LON_ACCEL, MNODE_MIN_HELIO_LON_ACCEL,
			TNODE_MIN_HELIO_LON_ACCEL, MAPOGEE_MIN_HELIO_LON_ACCEL, OAPOGEE_MIN_HELIO_LON_ACCEL, 1. / 0,
			CHIRON_MIN_HELIO_LON_ACCEL, PHOLUS_MIN_HELIO_LON_ACCEL, CERES_MIN_HELIO_LON_ACCEL,
			PALLAS_MIN_HELIO_LON_ACCEL, JUNO_MIN_HELIO_LON_ACCEL, VESTA_MIN_HELIO_LON_ACCEL,
			INTPAPOG_MIN_HELIO_LON_ACCEL, INTPPERG_MIN_HELIO_LON_ACCEL };

	// Speeds and accelerations in latitudinal direction:
	// Maximum geocentric speeds in latitudinal direction:
	static final double[] maxLatSpeed = new double[] { SURYA_MAX_LAT_SPEED, CHANDRA_MAX_LAT_SPEED, BUDHA_MAX_LAT_SPEED,
			SHUKRA_MAX_LAT_SPEED, MANGALA_MAX_LAT_SPEED, GURU_MAX_LAT_SPEED, SHANI_MAX_LAT_SPEED, URANUS_MAX_LAT_SPEED,
			NEPTUNE_MAX_LAT_SPEED, PLUTO_MAX_LAT_SPEED, MNODE_MAX_LAT_SPEED, TNODE_MAX_LAT_SPEED, MAPOGEE_MAX_LAT_SPEED,
			OAPOGEE_MAX_LAT_SPEED, 1. / 0, CHIRON_MAX_LAT_SPEED, PHOLUS_MAX_LAT_SPEED, CERES_MAX_LAT_SPEED,
			PALLAS_MAX_LAT_SPEED, JUNO_MAX_LAT_SPEED, VESTA_MAX_LAT_SPEED, INTPAPOG_MAX_LAT_SPEED,
			INTPPERG_MAX_LAT_SPEED };
	// Minimum geocentric speeds in latitudinal direction:
	static final double[] minLatSpeed = new double[] { SURYA_MIN_LAT_SPEED, CHANDRA_MIN_LAT_SPEED, BUDHA_MIN_LAT_SPEED,
			SHUKRA_MIN_LAT_SPEED, MANGALA_MIN_LAT_SPEED, GURU_MIN_LAT_SPEED, SHANI_MIN_LAT_SPEED, URANUS_MIN_LAT_SPEED,
			NEPTUNE_MIN_LAT_SPEED, PLUTO_MIN_LAT_SPEED, MNODE_MIN_LAT_SPEED, TNODE_MIN_LAT_SPEED, MAPOGEE_MIN_LAT_SPEED,
			OAPOGEE_MIN_LAT_SPEED, 1. / 0, CHIRON_MIN_LAT_SPEED, PHOLUS_MIN_LAT_SPEED, CERES_MIN_LAT_SPEED,
			PALLAS_MIN_LAT_SPEED, JUNO_MIN_LAT_SPEED, VESTA_MIN_LAT_SPEED, INTPAPOG_MIN_LAT_SPEED,
			INTPPERG_MIN_LAT_SPEED };

	// Maximum topocentric speeds in latitudinal direction, up to 50000m
	// altitude:
	static final double[] maxTopoLatSpeed = new double[] { SURYA_MAX_TOPO_LAT_SPEED, CHANDRA_MAX_TOPO_LAT_SPEED,
			BUDHA_MAX_TOPO_LAT_SPEED, SHUKRA_MAX_TOPO_LAT_SPEED, MANGALA_MAX_TOPO_LAT_SPEED, GURU_MAX_TOPO_LAT_SPEED,
			SHANI_MAX_TOPO_LAT_SPEED, URANUS_MAX_TOPO_LAT_SPEED, NEPTUNE_MAX_TOPO_LAT_SPEED, PLUTO_MAX_TOPO_LAT_SPEED,
			MNODE_MAX_TOPO_LAT_SPEED, TNODE_MAX_TOPO_LAT_SPEED, MAPOGEE_MAX_TOPO_LAT_SPEED, OAPOGEE_MAX_TOPO_LAT_SPEED,
			1. / 0, CHIRON_MAX_TOPO_LAT_SPEED, PHOLUS_MAX_TOPO_LAT_SPEED, CERES_MAX_TOPO_LAT_SPEED,
			PALLAS_MAX_TOPO_LAT_SPEED, JUNO_MAX_TOPO_LAT_SPEED, VESTA_MAX_TOPO_LAT_SPEED, INTPAPOG_MAX_TOPO_LAT_SPEED,
			INTPPERG_MAX_TOPO_LAT_SPEED };
	// Minimum topocentric speeds in latitudinal direction, up to 50000m
	// altitude:
	static final double[] minTopoLatSpeed = new double[] { SURYA_MIN_TOPO_LAT_SPEED, CHANDRA_MIN_TOPO_LAT_SPEED,
			BUDHA_MIN_TOPO_LAT_SPEED, SHUKRA_MIN_TOPO_LAT_SPEED, MANGALA_MIN_TOPO_LAT_SPEED, GURU_MIN_TOPO_LAT_SPEED,
			SHANI_MIN_TOPO_LAT_SPEED, URANUS_MIN_TOPO_LAT_SPEED, NEPTUNE_MIN_TOPO_LAT_SPEED, PLUTO_MIN_TOPO_LAT_SPEED,
			MNODE_MIN_TOPO_LAT_SPEED, TNODE_MIN_TOPO_LAT_SPEED, MAPOGEE_MIN_TOPO_LAT_SPEED, OAPOGEE_MIN_TOPO_LAT_SPEED,
			1. / 0, CHIRON_MIN_TOPO_LAT_SPEED, PHOLUS_MIN_TOPO_LAT_SPEED, CERES_MIN_TOPO_LAT_SPEED,
			PALLAS_MIN_TOPO_LAT_SPEED, JUNO_MIN_TOPO_LAT_SPEED, VESTA_MIN_TOPO_LAT_SPEED, INTPAPOG_MIN_TOPO_LAT_SPEED,
			INTPPERG_MIN_TOPO_LAT_SPEED };

	// Maximum heliocentric speeds in latitudinal direction:
	static final double[] maxHelioLatSpeed = new double[] { SURYA_MAX_HELIO_LAT_SPEED, CHANDRA_MAX_HELIO_LAT_SPEED,
			BUDHA_MAX_HELIO_LAT_SPEED, SHUKRA_MAX_HELIO_LAT_SPEED, MANGALA_MAX_HELIO_LAT_SPEED,
			GURU_MAX_HELIO_LAT_SPEED, SHANI_MAX_HELIO_LAT_SPEED, URANUS_MAX_HELIO_LAT_SPEED,
			NEPTUNE_MAX_HELIO_LAT_SPEED, PLUTO_MAX_HELIO_LAT_SPEED, MNODE_MAX_HELIO_LAT_SPEED,
			TNODE_MAX_HELIO_LAT_SPEED, MAPOGEE_MAX_HELIO_LAT_SPEED, OAPOGEE_MAX_HELIO_LAT_SPEED, 1. / 0,
			CHIRON_MAX_HELIO_LAT_SPEED, PHOLUS_MAX_HELIO_LAT_SPEED, CERES_MAX_HELIO_LAT_SPEED,
			PALLAS_MAX_HELIO_LAT_SPEED, JUNO_MAX_HELIO_LAT_SPEED, VESTA_MAX_HELIO_LAT_SPEED,
			INTPAPOG_MAX_HELIO_LAT_SPEED, INTPPERG_MAX_HELIO_LAT_SPEED };
	// Minimum heliocentric speeds in latitudinal direction:
	static final double[] minHelioLatSpeed = new double[] { SURYA_MIN_HELIO_LAT_SPEED, CHANDRA_MIN_HELIO_LAT_SPEED,
			BUDHA_MIN_HELIO_LAT_SPEED, SHUKRA_MIN_HELIO_LAT_SPEED, MANGALA_MIN_HELIO_LAT_SPEED,
			GURU_MIN_HELIO_LAT_SPEED, SHANI_MIN_HELIO_LAT_SPEED, URANUS_MIN_HELIO_LAT_SPEED,
			NEPTUNE_MIN_HELIO_LAT_SPEED, PLUTO_MIN_HELIO_LAT_SPEED, MNODE_MIN_HELIO_LAT_SPEED,
			TNODE_MIN_HELIO_LAT_SPEED, MAPOGEE_MIN_HELIO_LAT_SPEED, OAPOGEE_MIN_HELIO_LAT_SPEED, 1. / 0,
			CHIRON_MIN_HELIO_LAT_SPEED, PHOLUS_MIN_HELIO_LAT_SPEED, CERES_MIN_HELIO_LAT_SPEED,
			PALLAS_MIN_HELIO_LAT_SPEED, JUNO_MIN_HELIO_LAT_SPEED, VESTA_MIN_HELIO_LAT_SPEED,
			INTPAPOG_MAX_HELIO_LAT_SPEED, INTPPERG_MIN_HELIO_LAT_SPEED };

	// Maximum geocentric accelerations in latitudinal direction:
	static final double[] maxLatAccel = new double[] { SURYA_MAX_LAT_ACCEL, CHANDRA_MAX_LAT_ACCEL, BUDHA_MAX_LAT_ACCEL,
			SHUKRA_MAX_LAT_ACCEL, MANGALA_MAX_LAT_ACCEL, GURU_MAX_LAT_ACCEL, SHANI_MAX_LAT_ACCEL, URANUS_MAX_LAT_ACCEL,
			NEPTUNE_MAX_LAT_ACCEL, PLUTO_MAX_LAT_ACCEL, MNODE_MAX_LAT_ACCEL, TNODE_MAX_LAT_ACCEL, MAPOGEE_MAX_LAT_ACCEL,
			OAPOGEE_MAX_LAT_ACCEL, 1. / 0, CHIRON_MAX_LAT_ACCEL, PHOLUS_MAX_LAT_ACCEL, CERES_MAX_LAT_ACCEL,
			PALLAS_MAX_LAT_ACCEL, JUNO_MAX_LAT_ACCEL, VESTA_MAX_LAT_ACCEL, INTPAPOG_MAX_LAT_ACCEL,
			INTPPERG_MAX_LAT_ACCEL };
	// Minimum geocentric accelerations in latitudinal direction:
	static final double[] minLatAccel = new double[] { SURYA_MIN_LAT_ACCEL, CHANDRA_MIN_LAT_ACCEL, BUDHA_MIN_LAT_ACCEL,
			SHUKRA_MIN_LAT_ACCEL, MANGALA_MIN_LAT_ACCEL, GURU_MIN_LAT_ACCEL, SHANI_MIN_LAT_ACCEL, URANUS_MIN_LAT_ACCEL,
			NEPTUNE_MIN_LAT_ACCEL, PLUTO_MIN_LAT_ACCEL, MNODE_MIN_LAT_ACCEL, TNODE_MIN_LAT_ACCEL, MAPOGEE_MIN_LAT_ACCEL,
			OAPOGEE_MIN_LAT_ACCEL, 1. / 0, CHIRON_MIN_LAT_ACCEL, PHOLUS_MIN_LAT_ACCEL, CERES_MIN_LAT_ACCEL,
			PALLAS_MIN_LAT_ACCEL, JUNO_MIN_LAT_ACCEL, VESTA_MIN_LAT_ACCEL, INTPAPOG_MIN_LAT_ACCEL,
			INTPPERG_MIN_LAT_ACCEL };

	// Maximum topocentric accelerations in latitudinal direction, up to
	// 50000m altitude:
	static final double[] maxTopoLatAccel = new double[] { SURYA_MAX_TOPO_LAT_ACCEL, CHANDRA_MAX_TOPO_LAT_ACCEL,
			BUDHA_MAX_TOPO_LAT_ACCEL, SHUKRA_MAX_TOPO_LAT_ACCEL, MANGALA_MAX_TOPO_LAT_ACCEL, GURU_MAX_TOPO_LAT_ACCEL,
			SHANI_MAX_TOPO_LAT_ACCEL, URANUS_MAX_TOPO_LAT_ACCEL, NEPTUNE_MAX_TOPO_LAT_ACCEL, PLUTO_MAX_TOPO_LAT_ACCEL,
			MNODE_MAX_TOPO_LAT_ACCEL, TNODE_MAX_TOPO_LAT_ACCEL, MAPOGEE_MAX_TOPO_LAT_ACCEL, OAPOGEE_MAX_TOPO_LAT_ACCEL,
			1. / 0, CHIRON_MAX_TOPO_LAT_ACCEL, PHOLUS_MAX_TOPO_LAT_ACCEL, CERES_MAX_TOPO_LAT_ACCEL,
			PALLAS_MAX_TOPO_LAT_ACCEL, JUNO_MAX_TOPO_LAT_ACCEL, VESTA_MAX_TOPO_LAT_ACCEL, INTPAPOG_MAX_TOPO_LAT_ACCEL,
			INTPPERG_MAX_TOPO_LAT_ACCEL };
	// Minimum topocentric accelerations in latitudinal direction, up to
	// 50000m altitude:
	static final double[] minTopoLatAccel = new double[] { SURYA_MIN_TOPO_LAT_ACCEL, CHANDRA_MIN_TOPO_LAT_ACCEL,
			BUDHA_MIN_TOPO_LAT_ACCEL, SHUKRA_MIN_TOPO_LAT_ACCEL, MANGALA_MIN_TOPO_LAT_ACCEL, GURU_MIN_TOPO_LAT_ACCEL,
			SHANI_MIN_TOPO_LAT_ACCEL, URANUS_MIN_TOPO_LAT_ACCEL, NEPTUNE_MIN_TOPO_LAT_ACCEL, PLUTO_MIN_TOPO_LAT_ACCEL,
			MNODE_MIN_TOPO_LAT_ACCEL, TNODE_MIN_TOPO_LAT_ACCEL, MAPOGEE_MIN_TOPO_LAT_ACCEL, OAPOGEE_MIN_TOPO_LAT_ACCEL,
			1. / 0, CHIRON_MIN_TOPO_LAT_ACCEL, PHOLUS_MIN_TOPO_LAT_ACCEL, CERES_MIN_TOPO_LAT_ACCEL,
			PALLAS_MIN_TOPO_LAT_ACCEL, JUNO_MIN_TOPO_LAT_ACCEL, VESTA_MIN_TOPO_LAT_ACCEL, INTPAPOG_MIN_TOPO_LAT_ACCEL,
			INTPPERG_MIN_TOPO_LAT_ACCEL };

	// Maximum heliocentric accelerations in latitudinal direction:
	static final double[] maxHelioLatAccel = new double[] { SURYA_MAX_HELIO_LAT_ACCEL, CHANDRA_MAX_HELIO_LAT_ACCEL,
			BUDHA_MAX_HELIO_LAT_ACCEL, SHUKRA_MAX_HELIO_LAT_ACCEL, MANGALA_MAX_HELIO_LAT_ACCEL,
			GURU_MAX_HELIO_LAT_ACCEL, SHANI_MAX_HELIO_LAT_ACCEL, URANUS_MAX_HELIO_LAT_ACCEL,
			NEPTUNE_MAX_HELIO_LAT_ACCEL, PLUTO_MAX_HELIO_LAT_ACCEL, MNODE_MAX_HELIO_LAT_ACCEL,
			TNODE_MAX_HELIO_LAT_ACCEL, MAPOGEE_MAX_HELIO_LAT_ACCEL, OAPOGEE_MAX_HELIO_LAT_ACCEL, 1. / 0,
			CHIRON_MAX_HELIO_LAT_ACCEL, PHOLUS_MAX_HELIO_LAT_ACCEL, CERES_MAX_HELIO_LAT_ACCEL,
			PALLAS_MAX_HELIO_LAT_ACCEL, JUNO_MAX_HELIO_LAT_ACCEL, VESTA_MAX_HELIO_LAT_ACCEL,
			INTPAPOG_MAX_HELIO_LAT_ACCEL, INTPPERG_MAX_HELIO_LAT_ACCEL };
	// Minimum heliocentric accelerations in latitudinal direction:
	static final double[] minHelioLatAccel = new double[] { SURYA_MIN_HELIO_LAT_ACCEL, CHANDRA_MIN_HELIO_LAT_ACCEL,
			BUDHA_MIN_HELIO_LAT_ACCEL, SHUKRA_MIN_HELIO_LAT_ACCEL, MANGALA_MIN_HELIO_LAT_ACCEL,
			GURU_MIN_HELIO_LAT_ACCEL, SHANI_MIN_HELIO_LAT_ACCEL, URANUS_MIN_HELIO_LAT_ACCEL,
			NEPTUNE_MIN_HELIO_LAT_ACCEL, PLUTO_MIN_HELIO_LAT_ACCEL, MNODE_MIN_HELIO_LAT_ACCEL,
			TNODE_MIN_HELIO_LAT_ACCEL, MAPOGEE_MIN_HELIO_LAT_ACCEL, OAPOGEE_MIN_HELIO_LAT_ACCEL, 1. / 0,
			CHIRON_MIN_HELIO_LAT_ACCEL, PHOLUS_MIN_HELIO_LAT_ACCEL, CERES_MIN_HELIO_LAT_ACCEL,
			PALLAS_MIN_HELIO_LAT_ACCEL, JUNO_MIN_HELIO_LAT_ACCEL, VESTA_MIN_HELIO_LAT_ACCEL,
			INTPAPOG_MIN_HELIO_LAT_ACCEL, INTPPERG_MIN_HELIO_LAT_ACCEL };

	// Speeds and accelerations in the distance to the earth:
	// Maximum geocentric speeds in the distance to the earth:
	static final double[] maxDistSpeed = new double[] { SURYA_MAX_DIST_SPEED, CHANDRA_MAX_DIST_SPEED,
			BUDHA_MAX_DIST_SPEED, SHUKRA_MAX_DIST_SPEED, MANGALA_MAX_DIST_SPEED, GURU_MAX_DIST_SPEED,
			SHANI_MAX_DIST_SPEED, URANUS_MAX_DIST_SPEED, NEPTUNE_MAX_DIST_SPEED, PLUTO_MAX_DIST_SPEED,
			MNODE_MAX_DIST_SPEED, TNODE_MAX_DIST_SPEED, MAPOGEE_MAX_DIST_SPEED, OAPOGEE_MAX_DIST_SPEED, 1. / 0.,
			CHIRON_MAX_DIST_SPEED, PHOLUS_MAX_DIST_SPEED, CERES_MAX_DIST_SPEED, PALLAS_MAX_DIST_SPEED,
			JUNO_MAX_DIST_SPEED, VESTA_MAX_DIST_SPEED, INTPAPOG_MAX_DIST_SPEED, INTPPERG_MAX_DIST_SPEED };
	// Minimum geocentric speeds in the direction of the distance to the earth:
	static final double[] minDistSpeed = new double[] { SURYA_MIN_DIST_SPEED, CHANDRA_MIN_DIST_SPEED,
			BUDHA_MIN_DIST_SPEED, SHUKRA_MIN_DIST_SPEED, MANGALA_MIN_DIST_SPEED, GURU_MIN_DIST_SPEED,
			SHANI_MIN_DIST_SPEED, URANUS_MIN_DIST_SPEED, NEPTUNE_MIN_DIST_SPEED, PLUTO_MIN_DIST_SPEED,
			MNODE_MIN_DIST_SPEED, TNODE_MIN_DIST_SPEED, MAPOGEE_MIN_DIST_SPEED, OAPOGEE_MIN_DIST_SPEED, 1. / 0.,
			CHIRON_MIN_DIST_SPEED, PHOLUS_MIN_DIST_SPEED, CERES_MIN_DIST_SPEED, PALLAS_MIN_DIST_SPEED,
			JUNO_MIN_DIST_SPEED, VESTA_MIN_DIST_SPEED, INTPAPOG_MIN_DIST_SPEED, INTPPERG_MIN_DIST_SPEED };

	// Maximum topocentric speeds in the distance to the earth:
	static final double[] maxTopoDistSpeed = new double[] { SURYA_MAX_TOPO_DIST_SPEED, CHANDRA_MAX_TOPO_DIST_SPEED,
			BUDHA_MAX_TOPO_DIST_SPEED, SHUKRA_MAX_TOPO_DIST_SPEED, MANGALA_MAX_TOPO_DIST_SPEED,
			GURU_MAX_TOPO_DIST_SPEED, SHANI_MAX_TOPO_DIST_SPEED, URANUS_MAX_TOPO_DIST_SPEED,
			NEPTUNE_MAX_TOPO_DIST_SPEED, PLUTO_MAX_TOPO_DIST_SPEED, MNODE_MAX_TOPO_DIST_SPEED,
			TNODE_MAX_TOPO_DIST_SPEED, MAPOGEE_MAX_TOPO_DIST_SPEED, OAPOGEE_MAX_TOPO_DIST_SPEED, 1. / 0.,
			CHIRON_MAX_TOPO_DIST_SPEED, PHOLUS_MAX_TOPO_DIST_SPEED, CERES_MAX_TOPO_DIST_SPEED,
			PALLAS_MAX_TOPO_DIST_SPEED, JUNO_MAX_TOPO_DIST_SPEED, VESTA_MAX_TOPO_DIST_SPEED,
			INTPAPOG_MAX_TOPO_DIST_SPEED, INTPPERG_MAX_TOPO_DIST_SPEED };
	// Minimum topocentric speeds in the distance to the earth:
	static final double[] minTopoDistSpeed = new double[] { SURYA_MIN_TOPO_DIST_SPEED, CHANDRA_MIN_TOPO_DIST_SPEED,
			BUDHA_MIN_TOPO_DIST_SPEED, SHUKRA_MIN_TOPO_DIST_SPEED, MANGALA_MIN_TOPO_DIST_SPEED,
			GURU_MIN_TOPO_DIST_SPEED, SHANI_MIN_TOPO_DIST_SPEED, URANUS_MIN_TOPO_DIST_SPEED,
			NEPTUNE_MIN_TOPO_DIST_SPEED, PLUTO_MIN_TOPO_DIST_SPEED, MNODE_MIN_TOPO_DIST_SPEED,
			TNODE_MIN_TOPO_DIST_SPEED, MAPOGEE_MIN_TOPO_DIST_SPEED, OAPOGEE_MIN_TOPO_DIST_SPEED, 1. / 0.,
			CHIRON_MIN_TOPO_DIST_SPEED, PHOLUS_MIN_TOPO_DIST_SPEED, CERES_MIN_TOPO_DIST_SPEED,
			PALLAS_MIN_TOPO_DIST_SPEED, JUNO_MIN_TOPO_DIST_SPEED, VESTA_MIN_TOPO_DIST_SPEED,
			INTPAPOG_MIN_TOPO_DIST_SPEED, INTPPERG_MIN_TOPO_DIST_SPEED };

	// Maximum speeds in the distance to the sun:
	static final double[] maxHelioDistSpeed = new double[] { SURYA_MAX_HELIO_DIST_SPEED, CHANDRA_MAX_HELIO_DIST_SPEED,
			BUDHA_MAX_HELIO_DIST_SPEED, SHUKRA_MAX_HELIO_DIST_SPEED, MANGALA_MAX_HELIO_DIST_SPEED,
			GURU_MAX_HELIO_DIST_SPEED, SHANI_MAX_HELIO_DIST_SPEED, URANUS_MAX_HELIO_DIST_SPEED,
			NEPTUNE_MAX_HELIO_DIST_SPEED, PLUTO_MAX_HELIO_DIST_SPEED, MNODE_MAX_HELIO_DIST_SPEED,
			TNODE_MAX_HELIO_DIST_SPEED, MAPOGEE_MAX_HELIO_DIST_SPEED, OAPOGEE_MAX_HELIO_DIST_SPEED, 1. / 0.,
			CHIRON_MAX_HELIO_DIST_SPEED, PHOLUS_MAX_HELIO_DIST_SPEED, CERES_MAX_HELIO_DIST_SPEED,
			PALLAS_MAX_HELIO_DIST_SPEED, JUNO_MAX_HELIO_DIST_SPEED, VESTA_MAX_HELIO_DIST_SPEED,
			INTPAPOG_MAX_HELIO_DIST_SPEED, INTPPERG_MAX_HELIO_DIST_SPEED };
	// Minimum speeds in the distance to the sun:
	static final double[] minHelioDistSpeed = new double[] { SURYA_MIN_HELIO_DIST_SPEED, CHANDRA_MIN_HELIO_DIST_SPEED,
			BUDHA_MIN_HELIO_DIST_SPEED, SHUKRA_MIN_HELIO_DIST_SPEED, MANGALA_MIN_HELIO_DIST_SPEED,
			GURU_MIN_HELIO_DIST_SPEED, SHANI_MIN_HELIO_DIST_SPEED, URANUS_MIN_HELIO_DIST_SPEED,
			NEPTUNE_MIN_HELIO_DIST_SPEED, PLUTO_MIN_HELIO_DIST_SPEED, MNODE_MIN_HELIO_DIST_SPEED,
			TNODE_MIN_HELIO_DIST_SPEED, MAPOGEE_MIN_HELIO_DIST_SPEED, OAPOGEE_MIN_HELIO_DIST_SPEED, 1. / 0.,
			CHIRON_MIN_HELIO_DIST_SPEED, PHOLUS_MIN_HELIO_DIST_SPEED, CERES_MIN_HELIO_DIST_SPEED,
			PALLAS_MIN_HELIO_DIST_SPEED, JUNO_MIN_HELIO_DIST_SPEED, VESTA_MIN_HELIO_DIST_SPEED,
			INTPAPOG_MIN_HELIO_DIST_SPEED, INTPPERG_MIN_HELIO_DIST_SPEED };

	// Maximum geocentric accelerations in the distance to the earth:
	static final double[] maxDistAccel = new double[] { SURYA_MAX_DIST_ACCEL, CHANDRA_MAX_DIST_ACCEL,
			BUDHA_MAX_DIST_ACCEL, SHUKRA_MAX_DIST_ACCEL, MANGALA_MAX_DIST_ACCEL, GURU_MAX_DIST_ACCEL,
			SHANI_MAX_DIST_ACCEL, URANUS_MAX_DIST_ACCEL, NEPTUNE_MAX_DIST_ACCEL, PLUTO_MAX_DIST_ACCEL,
			MNODE_MAX_DIST_ACCEL, TNODE_MAX_DIST_ACCEL, MAPOGEE_MAX_DIST_ACCEL, OAPOGEE_MAX_DIST_ACCEL, 1. / 0.,
			CHIRON_MAX_DIST_ACCEL, PHOLUS_MAX_DIST_ACCEL, CERES_MAX_DIST_ACCEL, PALLAS_MAX_DIST_ACCEL,
			JUNO_MAX_DIST_ACCEL, VESTA_MAX_DIST_ACCEL, INTPAPOG_MAX_DIST_ACCEL, INTPPERG_MAX_DIST_ACCEL };
	// Minimum geocentric accelerations in the distance to the earth:
	static final double[] minDistAccel = new double[] { SURYA_MIN_DIST_ACCEL, CHANDRA_MIN_DIST_ACCEL,
			BUDHA_MIN_DIST_ACCEL, SHUKRA_MIN_DIST_ACCEL, MANGALA_MIN_DIST_ACCEL, GURU_MIN_DIST_ACCEL,
			SHANI_MIN_DIST_ACCEL, URANUS_MIN_DIST_ACCEL, NEPTUNE_MIN_DIST_ACCEL, PLUTO_MIN_DIST_ACCEL,
			MNODE_MIN_DIST_ACCEL, TNODE_MIN_DIST_ACCEL, MAPOGEE_MIN_DIST_ACCEL, OAPOGEE_MIN_DIST_ACCEL, 1. / 0.,
			CHIRON_MIN_DIST_ACCEL, PHOLUS_MIN_DIST_ACCEL, CERES_MIN_DIST_ACCEL, PALLAS_MIN_DIST_ACCEL,
			JUNO_MIN_DIST_ACCEL, VESTA_MIN_DIST_ACCEL, INTPAPOG_MIN_DIST_ACCEL, INTPPERG_MIN_DIST_ACCEL };

	// Maximum topocentric accelerations in the distance to the earth:
	static final double[] maxTopoDistAccel = new double[] { SURYA_MAX_TOPO_DIST_ACCEL, CHANDRA_MAX_TOPO_DIST_ACCEL,
			BUDHA_MAX_TOPO_DIST_ACCEL, SHUKRA_MAX_TOPO_DIST_ACCEL, MANGALA_MAX_TOPO_DIST_ACCEL,
			GURU_MAX_TOPO_DIST_ACCEL, SHANI_MAX_TOPO_DIST_ACCEL, URANUS_MAX_TOPO_DIST_ACCEL,
			NEPTUNE_MAX_TOPO_DIST_ACCEL, PLUTO_MAX_TOPO_DIST_ACCEL, MNODE_MAX_TOPO_DIST_ACCEL,
			TNODE_MAX_TOPO_DIST_ACCEL, MAPOGEE_MAX_TOPO_DIST_ACCEL, OAPOGEE_MAX_TOPO_DIST_ACCEL, 1. / 0.,
			CHIRON_MAX_TOPO_DIST_ACCEL, PHOLUS_MAX_TOPO_DIST_ACCEL, CERES_MAX_TOPO_DIST_ACCEL,
			PALLAS_MAX_TOPO_DIST_ACCEL, JUNO_MAX_TOPO_DIST_ACCEL, VESTA_MAX_TOPO_DIST_ACCEL,
			INTPAPOG_MAX_TOPO_DIST_ACCEL, INTPPERG_MAX_TOPO_DIST_ACCEL };
	// Minimum topocentric accelerations in the distance to the earth:
	static final double[] minTopoDistAccel = new double[] { SURYA_MIN_TOPO_DIST_ACCEL, CHANDRA_MIN_TOPO_DIST_ACCEL,
			BUDHA_MIN_TOPO_DIST_ACCEL, SHUKRA_MIN_TOPO_DIST_ACCEL, MANGALA_MIN_TOPO_DIST_ACCEL,
			GURU_MIN_TOPO_DIST_ACCEL, SHANI_MIN_TOPO_DIST_ACCEL, URANUS_MIN_TOPO_DIST_ACCEL,
			NEPTUNE_MIN_TOPO_DIST_ACCEL, PLUTO_MIN_TOPO_DIST_ACCEL, MNODE_MIN_TOPO_DIST_ACCEL,
			TNODE_MIN_TOPO_DIST_ACCEL, MAPOGEE_MIN_TOPO_DIST_ACCEL, OAPOGEE_MIN_TOPO_DIST_ACCEL, 1. / 0.,
			CHIRON_MIN_TOPO_DIST_ACCEL, PHOLUS_MIN_TOPO_DIST_ACCEL, CERES_MIN_TOPO_DIST_ACCEL,
			PALLAS_MIN_TOPO_DIST_ACCEL, JUNO_MIN_TOPO_DIST_ACCEL, VESTA_MIN_TOPO_DIST_ACCEL,
			INTPAPOG_MIN_TOPO_DIST_ACCEL, INTPPERG_MIN_TOPO_DIST_ACCEL };

	// Maximum accelerations in the distance to the sun:
	static final double[] maxHelioDistAccel = new double[] { SURYA_MAX_HELIO_DIST_ACCEL, CHANDRA_MAX_HELIO_DIST_ACCEL,
			BUDHA_MAX_HELIO_DIST_ACCEL, SHUKRA_MAX_HELIO_DIST_ACCEL, MANGALA_MAX_HELIO_DIST_ACCEL,
			GURU_MAX_HELIO_DIST_ACCEL, SHANI_MAX_HELIO_DIST_ACCEL, URANUS_MAX_HELIO_DIST_ACCEL,
			NEPTUNE_MAX_HELIO_DIST_ACCEL, PLUTO_MAX_HELIO_DIST_ACCEL, MNODE_MAX_HELIO_DIST_ACCEL,
			TNODE_MAX_HELIO_DIST_ACCEL, MAPOGEE_MAX_HELIO_DIST_ACCEL, OAPOGEE_MAX_HELIO_DIST_ACCEL, 1. / 0.,
			CHIRON_MAX_HELIO_DIST_ACCEL, PHOLUS_MAX_HELIO_DIST_ACCEL, CERES_MAX_HELIO_DIST_ACCEL,
			PALLAS_MAX_HELIO_DIST_ACCEL, JUNO_MAX_HELIO_DIST_ACCEL, VESTA_MAX_HELIO_DIST_ACCEL,
			INTPAPOG_MAX_HELIO_DIST_ACCEL, INTPPERG_MAX_HELIO_DIST_ACCEL };
	// Minimum accelerations in the distance to the sun:
	static final double[] minHelioDistAccel = new double[] { SURYA_MIN_HELIO_DIST_ACCEL, CHANDRA_MIN_HELIO_DIST_ACCEL,
			BUDHA_MIN_HELIO_DIST_ACCEL, SHUKRA_MIN_HELIO_DIST_ACCEL, MANGALA_MIN_HELIO_DIST_ACCEL,
			GURU_MIN_HELIO_DIST_ACCEL, SHANI_MIN_HELIO_DIST_ACCEL, URANUS_MIN_HELIO_DIST_ACCEL,
			NEPTUNE_MIN_HELIO_DIST_ACCEL, PLUTO_MIN_HELIO_DIST_ACCEL, MNODE_MIN_HELIO_DIST_ACCEL,
			TNODE_MIN_HELIO_DIST_ACCEL, MAPOGEE_MIN_HELIO_DIST_ACCEL, OAPOGEE_MIN_HELIO_DIST_ACCEL, 1. / 0.,
			CHIRON_MIN_HELIO_DIST_ACCEL, PHOLUS_MIN_HELIO_DIST_ACCEL, CERES_MIN_HELIO_DIST_ACCEL,
			PALLAS_MIN_HELIO_DIST_ACCEL, JUNO_MIN_HELIO_DIST_ACCEL, VESTA_MIN_HELIO_DIST_ACCEL,
			INTPAPOG_MIN_HELIO_DIST_ACCEL, INTPPERG_MIN_HELIO_DIST_ACCEL };

	// Speeds and accelerations in the rectascension direction
	// Maximum geocentric speeds in the rectascension direction
	static final double[] maxRectSpeed = new double[] { SURYA_MAX_RECT_SPEED, CHANDRA_MAX_RECT_SPEED,
			BUDHA_MAX_RECT_SPEED, SHUKRA_MAX_RECT_SPEED, MANGALA_MAX_RECT_SPEED, GURU_MAX_RECT_SPEED,
			SHANI_MAX_RECT_SPEED, URANUS_MAX_RECT_SPEED, NEPTUNE_MAX_RECT_SPEED, PLUTO_MAX_RECT_SPEED,
			MNODE_MAX_RECT_SPEED, TNODE_MAX_RECT_SPEED, MAPOGEE_MAX_RECT_SPEED, OAPOGEE_MAX_RECT_SPEED, 1. / 0.,
			CHIRON_MAX_RECT_SPEED, PHOLUS_MAX_RECT_SPEED, CERES_MAX_RECT_SPEED, PALLAS_MAX_RECT_SPEED,
			JUNO_MAX_RECT_SPEED, VESTA_MAX_RECT_SPEED, INTPAPOG_MAX_RECT_SPEED, INTPPERG_MAX_RECT_SPEED };
	// Minimum geocentric speeds in the rectascension direction
	static final double[] minRectSpeed = new double[] { SURYA_MIN_RECT_SPEED, CHANDRA_MIN_RECT_SPEED,
			BUDHA_MIN_RECT_SPEED, SHUKRA_MIN_RECT_SPEED, MANGALA_MIN_RECT_SPEED, GURU_MIN_RECT_SPEED,
			SHANI_MIN_RECT_SPEED, URANUS_MIN_RECT_SPEED, NEPTUNE_MIN_RECT_SPEED, PLUTO_MIN_RECT_SPEED,
			MNODE_MIN_RECT_SPEED, TNODE_MIN_RECT_SPEED, MAPOGEE_MIN_RECT_SPEED, OAPOGEE_MIN_RECT_SPEED, 1. / 0.,
			CHIRON_MIN_RECT_SPEED, PHOLUS_MIN_RECT_SPEED, CERES_MIN_RECT_SPEED, PALLAS_MIN_RECT_SPEED,
			JUNO_MIN_RECT_SPEED, VESTA_MIN_RECT_SPEED, INTPAPOG_MIN_RECT_SPEED, INTPPERG_MIN_RECT_SPEED };

	// Maximum topocentric speeds in the rectascension direction
	static final double[] maxTopoRectSpeed = new double[] { SURYA_MAX_TOPO_RECT_SPEED, CHANDRA_MAX_TOPO_RECT_SPEED,
			BUDHA_MAX_TOPO_RECT_SPEED, SHUKRA_MAX_TOPO_RECT_SPEED, MANGALA_MAX_TOPO_RECT_SPEED,
			GURU_MAX_TOPO_RECT_SPEED, SHANI_MAX_TOPO_RECT_SPEED, URANUS_MAX_TOPO_RECT_SPEED,
			NEPTUNE_MAX_TOPO_RECT_SPEED, PLUTO_MAX_TOPO_RECT_SPEED, MNODE_MAX_TOPO_RECT_SPEED,
			TNODE_MAX_TOPO_RECT_SPEED, MAPOGEE_MAX_TOPO_RECT_SPEED, OAPOGEE_MAX_TOPO_RECT_SPEED, 1. / 0.,
			CHIRON_MAX_TOPO_RECT_SPEED, PHOLUS_MAX_TOPO_RECT_SPEED, CERES_MAX_TOPO_RECT_SPEED,
			PALLAS_MAX_TOPO_RECT_SPEED, JUNO_MAX_TOPO_RECT_SPEED, VESTA_MAX_TOPO_RECT_SPEED,
			INTPAPOG_MAX_TOPO_RECT_SPEED, INTPPERG_MAX_TOPO_RECT_SPEED };
	// Minimum topocentric speeds in the rectascension direction
	static final double[] minTopoRectSpeed = new double[] { SURYA_MIN_TOPO_RECT_SPEED, CHANDRA_MIN_TOPO_RECT_SPEED,
			BUDHA_MIN_TOPO_RECT_SPEED, SHUKRA_MIN_TOPO_RECT_SPEED, MANGALA_MIN_TOPO_RECT_SPEED,
			GURU_MIN_TOPO_RECT_SPEED, SHANI_MIN_TOPO_RECT_SPEED, URANUS_MIN_TOPO_RECT_SPEED,
			NEPTUNE_MIN_TOPO_RECT_SPEED, PLUTO_MIN_TOPO_RECT_SPEED, MNODE_MIN_TOPO_RECT_SPEED,
			TNODE_MIN_TOPO_RECT_SPEED, MAPOGEE_MIN_TOPO_RECT_SPEED, OAPOGEE_MIN_TOPO_RECT_SPEED, 1. / 0.,
			CHIRON_MIN_TOPO_RECT_SPEED, PHOLUS_MIN_TOPO_RECT_SPEED, CERES_MIN_TOPO_RECT_SPEED,
			PALLAS_MIN_TOPO_RECT_SPEED, JUNO_MIN_TOPO_RECT_SPEED, VESTA_MIN_TOPO_RECT_SPEED,
			INTPAPOG_MIN_TOPO_RECT_SPEED, INTPPERG_MIN_TOPO_RECT_SPEED };

	// Maximum speeds in the rectascension direction
	static final double[] maxHelioRectSpeed = new double[] { SURYA_MAX_HELIO_RECT_SPEED, CHANDRA_MAX_HELIO_RECT_SPEED,
			BUDHA_MAX_HELIO_RECT_SPEED, SHUKRA_MAX_HELIO_RECT_SPEED, MANGALA_MAX_HELIO_RECT_SPEED,
			GURU_MAX_HELIO_RECT_SPEED, SHANI_MAX_HELIO_RECT_SPEED, URANUS_MAX_HELIO_RECT_SPEED,
			NEPTUNE_MAX_HELIO_RECT_SPEED, PLUTO_MAX_HELIO_RECT_SPEED, MNODE_MAX_HELIO_RECT_SPEED,
			TNODE_MAX_HELIO_RECT_SPEED, MAPOGEE_MAX_HELIO_RECT_SPEED, OAPOGEE_MAX_HELIO_RECT_SPEED, 1. / 0.,
			CHIRON_MAX_HELIO_RECT_SPEED, PHOLUS_MAX_HELIO_RECT_SPEED, CERES_MAX_HELIO_RECT_SPEED,
			PALLAS_MAX_HELIO_RECT_SPEED, JUNO_MAX_HELIO_RECT_SPEED, VESTA_MAX_HELIO_RECT_SPEED,
			INTPAPOG_MAX_HELIO_RECT_SPEED, INTPPERG_MAX_HELIO_RECT_SPEED };
	// Minimum speeds in the rectascension direction
	static final double[] minHelioRectSpeed = new double[] { SURYA_MIN_HELIO_RECT_SPEED, CHANDRA_MIN_HELIO_RECT_SPEED,
			BUDHA_MIN_HELIO_RECT_SPEED, SHUKRA_MIN_HELIO_RECT_SPEED, MANGALA_MIN_HELIO_RECT_SPEED,
			GURU_MIN_HELIO_RECT_SPEED, SHANI_MIN_HELIO_RECT_SPEED, URANUS_MIN_HELIO_RECT_SPEED,
			NEPTUNE_MIN_HELIO_RECT_SPEED, PLUTO_MIN_HELIO_RECT_SPEED, MNODE_MIN_HELIO_RECT_SPEED,
			TNODE_MIN_HELIO_RECT_SPEED, MAPOGEE_MIN_HELIO_RECT_SPEED, OAPOGEE_MIN_HELIO_RECT_SPEED, 1. / 0.,
			CHIRON_MIN_HELIO_RECT_SPEED, PHOLUS_MIN_HELIO_RECT_SPEED, CERES_MIN_HELIO_RECT_SPEED,
			PALLAS_MIN_HELIO_RECT_SPEED, JUNO_MIN_HELIO_RECT_SPEED, VESTA_MIN_HELIO_RECT_SPEED,
			INTPAPOG_MIN_HELIO_RECT_SPEED, INTPPERG_MIN_HELIO_RECT_SPEED };

	// Maximum geocentric accelerations in the rectascension direction
	static final double[] maxRectAccel = new double[] { SURYA_MAX_RECT_ACCEL, CHANDRA_MAX_RECT_ACCEL,
			BUDHA_MAX_RECT_ACCEL, SHUKRA_MAX_RECT_ACCEL, MANGALA_MAX_RECT_ACCEL, GURU_MAX_RECT_ACCEL,
			SHANI_MAX_RECT_ACCEL, URANUS_MAX_RECT_ACCEL, NEPTUNE_MAX_RECT_ACCEL, PLUTO_MAX_RECT_ACCEL,
			MNODE_MAX_RECT_ACCEL, TNODE_MAX_RECT_ACCEL, MAPOGEE_MAX_RECT_ACCEL, OAPOGEE_MAX_RECT_ACCEL, 1. / 0.,
			CHIRON_MAX_RECT_ACCEL, PHOLUS_MAX_RECT_ACCEL, CERES_MAX_RECT_ACCEL, PALLAS_MAX_RECT_ACCEL,
			JUNO_MAX_RECT_ACCEL, VESTA_MAX_RECT_ACCEL, INTPAPOG_MAX_RECT_ACCEL, INTPPERG_MAX_RECT_ACCEL };
	// Minimum geocentric accelerations in the rectascension direction
	static final double[] minRectAccel = new double[] { SURYA_MIN_RECT_ACCEL, CHANDRA_MIN_RECT_ACCEL,
			BUDHA_MIN_RECT_ACCEL, SHUKRA_MIN_RECT_ACCEL, MANGALA_MIN_RECT_ACCEL, GURU_MIN_RECT_ACCEL,
			SHANI_MIN_RECT_ACCEL, URANUS_MIN_RECT_ACCEL, NEPTUNE_MIN_RECT_ACCEL, PLUTO_MIN_RECT_ACCEL,
			MNODE_MIN_RECT_ACCEL, TNODE_MIN_RECT_ACCEL, MAPOGEE_MIN_RECT_ACCEL, OAPOGEE_MIN_RECT_ACCEL, 1. / 0.,
			CHIRON_MIN_RECT_ACCEL, PHOLUS_MIN_RECT_ACCEL, CERES_MIN_RECT_ACCEL, PALLAS_MIN_RECT_ACCEL,
			JUNO_MIN_RECT_ACCEL, VESTA_MIN_RECT_ACCEL, INTPAPOG_MIN_RECT_ACCEL, INTPPERG_MIN_RECT_ACCEL };

	// Maximum topocentric accelerations in the rectascension direction
	static final double[] maxTopoRectAccel = new double[] { SURYA_MAX_TOPO_RECT_ACCEL, CHANDRA_MAX_TOPO_RECT_ACCEL,
			BUDHA_MAX_TOPO_RECT_ACCEL, SHUKRA_MAX_TOPO_RECT_ACCEL, MANGALA_MAX_TOPO_RECT_ACCEL,
			GURU_MAX_TOPO_RECT_ACCEL, SHANI_MAX_TOPO_RECT_ACCEL, URANUS_MAX_TOPO_RECT_ACCEL,
			NEPTUNE_MAX_TOPO_RECT_ACCEL, PLUTO_MAX_TOPO_RECT_ACCEL, MNODE_MAX_TOPO_RECT_ACCEL,
			TNODE_MAX_TOPO_RECT_ACCEL, MAPOGEE_MAX_TOPO_RECT_ACCEL, OAPOGEE_MAX_TOPO_RECT_ACCEL, 1. / 0.,
			CHIRON_MAX_TOPO_RECT_ACCEL, PHOLUS_MAX_TOPO_RECT_ACCEL, CERES_MAX_TOPO_RECT_ACCEL,
			PALLAS_MAX_TOPO_RECT_ACCEL, JUNO_MAX_TOPO_RECT_ACCEL, VESTA_MAX_TOPO_RECT_ACCEL,
			INTPAPOG_MAX_TOPO_RECT_ACCEL, INTPPERG_MAX_TOPO_RECT_ACCEL };
	// Minimum topocentric accelerations in the rectascension direction
	static final double[] minTopoRectAccel = new double[] { SURYA_MIN_TOPO_RECT_ACCEL, CHANDRA_MIN_TOPO_RECT_ACCEL,
			BUDHA_MIN_TOPO_RECT_ACCEL, SHUKRA_MIN_TOPO_RECT_ACCEL, MANGALA_MIN_TOPO_RECT_ACCEL,
			GURU_MIN_TOPO_RECT_ACCEL, SHANI_MIN_TOPO_RECT_ACCEL, URANUS_MIN_TOPO_RECT_ACCEL,
			NEPTUNE_MIN_TOPO_RECT_ACCEL, PLUTO_MIN_TOPO_RECT_ACCEL, MNODE_MIN_TOPO_RECT_ACCEL,
			TNODE_MIN_TOPO_RECT_ACCEL, MAPOGEE_MIN_TOPO_RECT_ACCEL, OAPOGEE_MIN_TOPO_RECT_ACCEL, 1. / 0.,
			CHIRON_MIN_TOPO_RECT_ACCEL, PHOLUS_MIN_TOPO_RECT_ACCEL, CERES_MIN_TOPO_RECT_ACCEL,
			PALLAS_MIN_TOPO_RECT_ACCEL, JUNO_MIN_TOPO_RECT_ACCEL, VESTA_MIN_TOPO_RECT_ACCEL,
			INTPAPOG_MIN_TOPO_RECT_ACCEL, INTPPERG_MIN_TOPO_RECT_ACCEL };

	// Maximum heliocentric accelerations in the rectascension direction
	static final double[] maxHelioRectAccel = new double[] { SURYA_MAX_HELIO_RECT_ACCEL, CHANDRA_MAX_HELIO_RECT_ACCEL,
			BUDHA_MAX_HELIO_RECT_ACCEL, SHUKRA_MAX_HELIO_RECT_ACCEL, MANGALA_MAX_HELIO_RECT_ACCEL,
			GURU_MAX_HELIO_RECT_ACCEL, SHANI_MAX_HELIO_RECT_ACCEL, URANUS_MAX_HELIO_RECT_ACCEL,
			NEPTUNE_MAX_HELIO_RECT_ACCEL, PLUTO_MAX_HELIO_RECT_ACCEL, MNODE_MAX_HELIO_RECT_ACCEL,
			TNODE_MAX_HELIO_RECT_ACCEL, MAPOGEE_MAX_HELIO_RECT_ACCEL, OAPOGEE_MAX_HELIO_RECT_ACCEL, 1. / 0.,
			CHIRON_MAX_HELIO_RECT_ACCEL, PHOLUS_MAX_HELIO_RECT_ACCEL, CERES_MAX_HELIO_RECT_ACCEL,
			PALLAS_MAX_HELIO_RECT_ACCEL, JUNO_MAX_HELIO_RECT_ACCEL, VESTA_MAX_HELIO_RECT_ACCEL,
			INTPAPOG_MAX_HELIO_RECT_ACCEL, INTPPERG_MAX_HELIO_RECT_ACCEL };
	// Minimum heliocentric accelerations in the rectascension direction
	static final double[] minHelioRectAccel = new double[] { SURYA_MIN_HELIO_RECT_ACCEL, CHANDRA_MIN_HELIO_RECT_ACCEL,
			BUDHA_MIN_HELIO_RECT_ACCEL, SHUKRA_MIN_HELIO_RECT_ACCEL, MANGALA_MIN_HELIO_RECT_ACCEL,
			GURU_MIN_HELIO_RECT_ACCEL, SHANI_MIN_HELIO_RECT_ACCEL, URANUS_MIN_HELIO_RECT_ACCEL,
			NEPTUNE_MIN_HELIO_RECT_ACCEL, PLUTO_MIN_HELIO_RECT_ACCEL, MNODE_MIN_HELIO_RECT_ACCEL,
			TNODE_MIN_HELIO_RECT_ACCEL, MAPOGEE_MIN_HELIO_RECT_ACCEL, OAPOGEE_MIN_HELIO_RECT_ACCEL, 1. / 0.,
			CHIRON_MIN_HELIO_RECT_ACCEL, PHOLUS_MIN_HELIO_RECT_ACCEL, CERES_MIN_HELIO_RECT_ACCEL,
			PALLAS_MIN_HELIO_RECT_ACCEL, JUNO_MIN_HELIO_RECT_ACCEL, VESTA_MIN_HELIO_RECT_ACCEL,
			INTPAPOG_MIN_HELIO_RECT_ACCEL, INTPPERG_MIN_HELIO_RECT_ACCEL };

	// Speeds and accelerations in the declination direction
	// Maximum geocentric speeds in the declination direction
	static final double[] maxDeclSpeed = new double[] { SURYA_MAX_DECL_SPEED, CHANDRA_MAX_DECL_SPEED,
			BUDHA_MAX_DECL_SPEED, SHUKRA_MAX_DECL_SPEED, MANGALA_MAX_DECL_SPEED, GURU_MAX_DECL_SPEED,
			SHANI_MAX_DECL_SPEED, URANUS_MAX_DECL_SPEED, NEPTUNE_MAX_DECL_SPEED, PLUTO_MAX_DECL_SPEED,
			MNODE_MAX_DECL_SPEED, TNODE_MAX_DECL_SPEED, MAPOGEE_MAX_DECL_SPEED, OAPOGEE_MAX_DECL_SPEED, 1. / 0.,
			CHIRON_MAX_DECL_SPEED, PHOLUS_MAX_DECL_SPEED, CERES_MAX_DECL_SPEED, PALLAS_MAX_DECL_SPEED,
			JUNO_MAX_DECL_SPEED, VESTA_MAX_DECL_SPEED, INTPAPOG_MAX_DECL_SPEED, INTPPERG_MAX_DECL_SPEED };
	// Minimum geocentric speeds in the declination direction
	static final double[] minDeclSpeed = new double[] { SURYA_MIN_DECL_SPEED, CHANDRA_MIN_DECL_SPEED,
			BUDHA_MIN_DECL_SPEED, SHUKRA_MIN_DECL_SPEED, MANGALA_MIN_DECL_SPEED, GURU_MIN_DECL_SPEED,
			SHANI_MIN_DECL_SPEED, URANUS_MIN_DECL_SPEED, NEPTUNE_MIN_DECL_SPEED, PLUTO_MIN_DECL_SPEED,
			MNODE_MIN_DECL_SPEED, TNODE_MIN_DECL_SPEED, MAPOGEE_MIN_DECL_SPEED, OAPOGEE_MIN_DECL_SPEED, 1. / 0.,
			CHIRON_MIN_DECL_SPEED, PHOLUS_MIN_DECL_SPEED, CERES_MIN_DECL_SPEED, PALLAS_MIN_DECL_SPEED,
			JUNO_MIN_DECL_SPEED, VESTA_MIN_DECL_SPEED, INTPAPOG_MIN_DECL_SPEED, INTPPERG_MIN_DECL_SPEED };

	// Maximum topocentric speeds in the declination direction
	static final double[] maxTopoDeclSpeed = new double[] { SURYA_MAX_TOPO_DECL_SPEED, CHANDRA_MAX_TOPO_DECL_SPEED,
			BUDHA_MAX_TOPO_DECL_SPEED, SHUKRA_MAX_TOPO_DECL_SPEED, MANGALA_MAX_TOPO_DECL_SPEED,
			GURU_MAX_TOPO_DECL_SPEED, SHANI_MAX_TOPO_DECL_SPEED, URANUS_MAX_TOPO_DECL_SPEED,
			NEPTUNE_MAX_TOPO_DECL_SPEED, PLUTO_MAX_TOPO_DECL_SPEED, MNODE_MAX_TOPO_DECL_SPEED,
			TNODE_MAX_TOPO_DECL_SPEED, MAPOGEE_MAX_TOPO_DECL_SPEED, OAPOGEE_MAX_TOPO_DECL_SPEED, 1. / 0.,
			CHIRON_MAX_TOPO_DECL_SPEED, PHOLUS_MAX_TOPO_DECL_SPEED, CERES_MAX_TOPO_DECL_SPEED,
			PALLAS_MAX_TOPO_DECL_SPEED, JUNO_MAX_TOPO_DECL_SPEED, VESTA_MAX_TOPO_DECL_SPEED,
			INTPAPOG_MAX_TOPO_DECL_SPEED, INTPPERG_MAX_TOPO_DECL_SPEED };
	// Minimum topocentric speeds in the declination direction
	static final double[] minTopoDeclSpeed = new double[] { SURYA_MIN_TOPO_DECL_SPEED, CHANDRA_MIN_TOPO_DECL_SPEED,
			BUDHA_MIN_TOPO_DECL_SPEED, SHUKRA_MIN_TOPO_DECL_SPEED, MANGALA_MIN_TOPO_DECL_SPEED,
			GURU_MIN_TOPO_DECL_SPEED, SHANI_MIN_TOPO_DECL_SPEED, URANUS_MIN_TOPO_DECL_SPEED,
			NEPTUNE_MIN_TOPO_DECL_SPEED, PLUTO_MIN_TOPO_DECL_SPEED, MNODE_MIN_TOPO_DECL_SPEED,
			TNODE_MIN_TOPO_DECL_SPEED, MAPOGEE_MIN_TOPO_DECL_SPEED, OAPOGEE_MIN_TOPO_DECL_SPEED, 1. / 0.,
			CHIRON_MIN_TOPO_DECL_SPEED, PHOLUS_MIN_TOPO_DECL_SPEED, CERES_MIN_TOPO_DECL_SPEED,
			PALLAS_MIN_TOPO_DECL_SPEED, JUNO_MIN_TOPO_DECL_SPEED, VESTA_MIN_TOPO_DECL_SPEED,
			INTPAPOG_MIN_TOPO_DECL_SPEED, INTPPERG_MIN_TOPO_DECL_SPEED };

	// Maximum speeds in the declination direction
	static final double[] maxHelioDeclSpeed = new double[] { SURYA_MAX_HELIO_DECL_SPEED, CHANDRA_MAX_HELIO_DECL_SPEED,
			BUDHA_MAX_HELIO_DECL_SPEED, SHUKRA_MAX_HELIO_DECL_SPEED, MANGALA_MAX_HELIO_DECL_SPEED,
			GURU_MAX_HELIO_DECL_SPEED, SHANI_MAX_HELIO_DECL_SPEED, URANUS_MAX_HELIO_DECL_SPEED,
			NEPTUNE_MAX_HELIO_DECL_SPEED, PLUTO_MAX_HELIO_DECL_SPEED, MNODE_MAX_HELIO_DECL_SPEED,
			TNODE_MAX_HELIO_DECL_SPEED, MAPOGEE_MAX_HELIO_DECL_SPEED, OAPOGEE_MAX_HELIO_DECL_SPEED, 1. / 0.,
			CHIRON_MAX_HELIO_DECL_SPEED, PHOLUS_MAX_HELIO_DECL_SPEED, CERES_MAX_HELIO_DECL_SPEED,
			PALLAS_MAX_HELIO_DECL_SPEED, JUNO_MAX_HELIO_DECL_SPEED, VESTA_MAX_HELIO_DECL_SPEED,
			INTPAPOG_MAX_HELIO_DECL_SPEED, INTPPERG_MAX_HELIO_DECL_SPEED };
	// Minimum speeds in the declination direction
	static final double[] minHelioDeclSpeed = new double[] { SURYA_MIN_HELIO_DECL_SPEED, CHANDRA_MIN_HELIO_DECL_SPEED,
			BUDHA_MIN_HELIO_DECL_SPEED, SHUKRA_MIN_HELIO_DECL_SPEED, MANGALA_MIN_HELIO_DECL_SPEED,
			GURU_MIN_HELIO_DECL_SPEED, SHANI_MIN_HELIO_DECL_SPEED, URANUS_MIN_HELIO_DECL_SPEED,
			NEPTUNE_MIN_HELIO_DECL_SPEED, PLUTO_MIN_HELIO_DECL_SPEED, MNODE_MIN_HELIO_DECL_SPEED,
			TNODE_MIN_HELIO_DECL_SPEED, MAPOGEE_MIN_HELIO_DECL_SPEED, OAPOGEE_MIN_HELIO_DECL_SPEED, 1. / 0.,
			CHIRON_MIN_HELIO_DECL_SPEED, PHOLUS_MIN_HELIO_DECL_SPEED, CERES_MIN_HELIO_DECL_SPEED,
			PALLAS_MIN_HELIO_DECL_SPEED, JUNO_MIN_HELIO_DECL_SPEED, VESTA_MIN_HELIO_DECL_SPEED,
			INTPAPOG_MIN_HELIO_DECL_SPEED, INTPPERG_MIN_HELIO_DECL_SPEED };

	// Maximum geocentric accelerations in the declination direction
	static final double[] maxDeclAccel = new double[] { SURYA_MAX_DECL_ACCEL, CHANDRA_MAX_DECL_ACCEL,
			BUDHA_MAX_DECL_ACCEL, SHUKRA_MAX_DECL_ACCEL, MANGALA_MAX_DECL_ACCEL, GURU_MAX_DECL_ACCEL,
			SHANI_MAX_DECL_ACCEL, URANUS_MAX_DECL_ACCEL, NEPTUNE_MAX_DECL_ACCEL, PLUTO_MAX_DECL_ACCEL,
			MNODE_MAX_DECL_ACCEL, TNODE_MAX_DECL_ACCEL, MAPOGEE_MAX_DECL_ACCEL, OAPOGEE_MAX_DECL_ACCEL, 1. / 0.,
			CHIRON_MAX_DECL_ACCEL, PHOLUS_MAX_DECL_ACCEL, CERES_MAX_DECL_ACCEL, PALLAS_MAX_DECL_ACCEL,
			JUNO_MAX_DECL_ACCEL, VESTA_MAX_DECL_ACCEL, INTPAPOG_MAX_DECL_ACCEL, INTPPERG_MAX_DECL_ACCEL };
	// Minimum geocentric accelerations in the declination direction
	static final double[] minDeclAccel = new double[] { SURYA_MIN_DECL_ACCEL, CHANDRA_MIN_DECL_ACCEL,
			BUDHA_MIN_DECL_ACCEL, SHUKRA_MIN_DECL_ACCEL, MANGALA_MIN_DECL_ACCEL, GURU_MIN_DECL_ACCEL,
			SHANI_MIN_DECL_ACCEL, URANUS_MIN_DECL_ACCEL, NEPTUNE_MIN_DECL_ACCEL, PLUTO_MIN_DECL_ACCEL,
			MNODE_MIN_DECL_ACCEL, TNODE_MIN_DECL_ACCEL, MAPOGEE_MIN_DECL_ACCEL, OAPOGEE_MIN_DECL_ACCEL, 1. / 0.,
			CHIRON_MIN_DECL_ACCEL, PHOLUS_MIN_DECL_ACCEL, CERES_MIN_DECL_ACCEL, PALLAS_MIN_DECL_ACCEL,
			JUNO_MIN_DECL_ACCEL, VESTA_MIN_DECL_ACCEL, INTPAPOG_MIN_DECL_ACCEL, INTPPERG_MIN_DECL_ACCEL };

	// Maximum topocentric accelerations in the declination direction
	static final double[] maxTopoDeclAccel = new double[] { SURYA_MAX_TOPO_DECL_ACCEL, CHANDRA_MAX_TOPO_DECL_ACCEL,
			BUDHA_MAX_TOPO_DECL_ACCEL, SHUKRA_MAX_TOPO_DECL_ACCEL, MANGALA_MAX_TOPO_DECL_ACCEL,
			GURU_MAX_TOPO_DECL_ACCEL, SHANI_MAX_TOPO_DECL_ACCEL, URANUS_MAX_TOPO_DECL_ACCEL,
			NEPTUNE_MAX_TOPO_DECL_ACCEL, PLUTO_MAX_TOPO_DECL_ACCEL, MNODE_MAX_TOPO_DECL_ACCEL,
			TNODE_MAX_TOPO_DECL_ACCEL, MAPOGEE_MAX_TOPO_DECL_ACCEL, OAPOGEE_MAX_TOPO_DECL_ACCEL, 1. / 0.,
			CHIRON_MAX_TOPO_DECL_ACCEL, PHOLUS_MAX_TOPO_DECL_ACCEL, CERES_MAX_TOPO_DECL_ACCEL,
			PALLAS_MAX_TOPO_DECL_ACCEL, JUNO_MAX_TOPO_DECL_ACCEL, VESTA_MAX_TOPO_DECL_ACCEL,
			INTPAPOG_MAX_TOPO_DECL_ACCEL, INTPPERG_MAX_TOPO_DECL_ACCEL };
	// Minimum topocentric accelerations in the declination direction
	static final double[] minTopoDeclAccel = new double[] { SURYA_MIN_TOPO_DECL_ACCEL, CHANDRA_MIN_TOPO_DECL_ACCEL,
			BUDHA_MIN_TOPO_DECL_ACCEL, SHUKRA_MIN_TOPO_DECL_ACCEL, MANGALA_MIN_TOPO_DECL_ACCEL,
			GURU_MIN_TOPO_DECL_ACCEL, SHANI_MIN_TOPO_DECL_ACCEL, URANUS_MIN_TOPO_DECL_ACCEL,
			NEPTUNE_MIN_TOPO_DECL_ACCEL, PLUTO_MIN_TOPO_DECL_ACCEL, MNODE_MIN_TOPO_DECL_ACCEL,
			TNODE_MIN_TOPO_DECL_ACCEL, MAPOGEE_MIN_TOPO_DECL_ACCEL, OAPOGEE_MIN_TOPO_DECL_ACCEL, 1. / 0.,
			CHIRON_MIN_TOPO_DECL_ACCEL, PHOLUS_MIN_TOPO_DECL_ACCEL, CERES_MIN_TOPO_DECL_ACCEL,
			PALLAS_MIN_TOPO_DECL_ACCEL, JUNO_MIN_TOPO_DECL_ACCEL, VESTA_MIN_TOPO_DECL_ACCEL,
			INTPAPOG_MIN_TOPO_DECL_ACCEL, INTPPERG_MIN_TOPO_DECL_ACCEL };

	// Maximum heliocentric accelerations in the declination direction
	static final double[] maxHelioDeclAccel = new double[] { SURYA_MAX_HELIO_DECL_ACCEL, CHANDRA_MAX_HELIO_DECL_ACCEL,
			BUDHA_MAX_HELIO_DECL_ACCEL, SHUKRA_MAX_HELIO_DECL_ACCEL, MANGALA_MAX_HELIO_DECL_ACCEL,
			GURU_MAX_HELIO_DECL_ACCEL, SHANI_MAX_HELIO_DECL_ACCEL, URANUS_MAX_HELIO_DECL_ACCEL,
			NEPTUNE_MAX_HELIO_DECL_ACCEL, PLUTO_MAX_HELIO_DECL_ACCEL, MNODE_MAX_HELIO_DECL_ACCEL,
			TNODE_MAX_HELIO_DECL_ACCEL, MAPOGEE_MAX_HELIO_DECL_ACCEL, OAPOGEE_MAX_HELIO_DECL_ACCEL, 1. / 0.,
			CHIRON_MAX_HELIO_DECL_ACCEL, PHOLUS_MAX_HELIO_DECL_ACCEL, CERES_MAX_HELIO_DECL_ACCEL,
			PALLAS_MAX_HELIO_DECL_ACCEL, JUNO_MAX_HELIO_DECL_ACCEL, VESTA_MAX_HELIO_DECL_ACCEL,
			INTPAPOG_MAX_HELIO_DECL_ACCEL, INTPPERG_MAX_HELIO_DECL_ACCEL };
	// Minimum heliocentric accelerations in the declination direction
	static final double[] minHelioDeclAccel = new double[] { SURYA_MIN_HELIO_DECL_ACCEL, CHANDRA_MIN_HELIO_DECL_ACCEL,
			BUDHA_MIN_HELIO_DECL_ACCEL, SHUKRA_MIN_HELIO_DECL_ACCEL, MANGALA_MIN_HELIO_DECL_ACCEL,
			GURU_MIN_HELIO_DECL_ACCEL, SHANI_MIN_HELIO_DECL_ACCEL, URANUS_MIN_HELIO_DECL_ACCEL,
			NEPTUNE_MIN_HELIO_DECL_ACCEL, PLUTO_MIN_HELIO_DECL_ACCEL, MNODE_MIN_HELIO_DECL_ACCEL,
			TNODE_MIN_HELIO_DECL_ACCEL, MAPOGEE_MIN_HELIO_DECL_ACCEL, OAPOGEE_MIN_HELIO_DECL_ACCEL, 1. / 0.,
			CHIRON_MIN_HELIO_DECL_ACCEL, PHOLUS_MIN_HELIO_DECL_ACCEL, CERES_MIN_HELIO_DECL_ACCEL,
			PALLAS_MIN_HELIO_DECL_ACCEL, JUNO_MIN_HELIO_DECL_ACCEL, VESTA_MIN_HELIO_DECL_ACCEL,
			INTPAPOG_MIN_HELIO_DECL_ACCEL, INTPPERG_MIN_HELIO_DECL_ACCEL };
