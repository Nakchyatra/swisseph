...
class FileData implements java.io.Serializable {
	...
	// The error handling and error strings are different from the original C
	// version.
	...
	int sweph_denum; /*
						 * DE number of JPL ephemeris, which this file is
						 * derived from.
						 */
	...
	int read_const(int ifno, StringBuffer serr, SwissData swed) {
		...
		try {
			...
			if (ifno == SwephData.SEI_FILE_ANY_AST) {
				...
				/*
				 * MPC number and name; will be analyzed below: search
				 * "asteroid name"
				 */
				String sp = s;
				...
				if (swed.ast_diam == 0) {
					/*
					 * estimate the diameter from magnitude; assume albedo =
					 * 0.15
					 */
			...
			if (testendian == SwephData.SEI_FILE_TEST_ENDIAN) {
				...
				lng = ((testendian & 0x000000ff) << 24) + ((testendian & 0x0000ff00) << 8)
						+ ((testendian & 0x00ff0000) >> 8) + ((testendian & 0xff000000) >> 24);
				if (lng != SwephData.SEI_FILE_TEST_ENDIAN) {
					throw new SwissephException(tfstart, SwissephException.DAMAGED_FILE_ERROR, SweConst.ERR,
							"File is damaged: byte ordering info not correct.");
				}
			}
			/*
			 * is file bigendian or littlendian? test first byte of test
			 * integer, which is highest if bigendian
			 */
			if (SwephData.SEI_FILE_TEST_ENDIAN / 16777216 == (testendian & 0x000000ff)) {
			...
			npl = nplan;
			/* which ones? */
			// retc = do_fread((void *) fdp->ipl, nbytes_ipl, (int) nplan,
			// sizeof(int), fp, SEI_CURR_FPOS,...
			...
			if (ifno == SwephData.SEI_FILE_ANY_AST) {
				...
				int j;
				/*
				 * name of asteroid is taken from orbital elements record read
				 * above
				 */
				j = 4; /* old astorb.dat had only 4 characters for MPC# */
				while (sastnam.charAt(j) != ' '
						&& j < 10) { /* new astorb.dat has 5 */
					j++;
				}
				...
				if (i == ipl[0] - SweConst.SE_AST_OFFSET) {
					...
					/*
					 * older elements record structure: the name is taken from
					 * old name field
					 */
					astnam = "";
					for (i = 0; i < 30; i++) {
			 ...
			 *************************************/
			/*
			 * clight, aunit, helgravconst, ratme, sunradius these constants are
	 ...
	 *
	 * The CRCs this code generates agree with the vendor-supplied Verilog
	 * models of several of the popular FDDI "MAC" chips.
	 */
	/* unsigned long [...] */
	...
	private short read2(FilePtr fp, long fpos, int freord, int fendian) throws Exception { // IOException,
		...
		if (freord != 0) {
			val = (short) (((val << 8) | ((val >>> 8) & 0xff)) & 0xffff);
		}
		return val;
	}

	private int read3(FilePtr fp, long fpos, int freord, int fendian) throws Exception { // IOException,
																							// BufferUnderflowException
		if (fpos >= 0) {
			fp.seek(fpos);
		}
		int val = (int) fp.readUnsignedByte();
		int i2 = (int) fp.readShort();
		if (i2 < 0) {
			i2 &= 0xffff;
		}
		val = (val << 16) + i2;
	...
	private int read4(FilePtr fp, long fpos, boolean unsigned, int freord, int fendian) throws Exception { // IOException,
																											// BufferUnderflowException
		if (fpos >= 0) {
			fp.seek(fpos);
		}
	...
	private double read8(FilePtr fp, long fpos, int freord, int fendian) throws Exception { // IOException,
																							// BufferUnderflowException
		if (fpos >= 0) {
			fp.seek(fpos);
		}
	...
	/*
	 * fetch chebyshew coefficients from sweph file for tjd time ipli planet
	 * number ifno file number serr error string
	 */
	int get_new_segment(SwissData swed, double tjd, int ipli, int ifno, StringBuffer serr) {
		...
		/* unsigned long */ long longs[] = new long[SwephData.MAXORD + 1]; // unsigned
																			// ist
																			// wichtig?
		...
		/*
		 * if (tjd - pdp->tfstart < 0) return(NOT_AVAILABLE);
		 */
		...
		try {
			...
			for (icoord = 0; icoord < 3; icoord++) {
				...
				/*
				 * there may not be more coefficients than interpolation order +
				 * 1
				 */
				if (nco > pdp.ncoe) {
					if (serr != null) {
						...
						if (serr.length() + fdp.fnam.length() < SwissData.AS_MAXCH - 1) {
							serr.setLength(0);
							serr.append("error in ephemeris file " + fdp.fnam + ": " + nco + " coefficients instead of "
									+ pdp.ncoe + ". ");
						}
					}
					pdp.segp = null;
					...
					if (i < 4) {
						...
						for (kCnt = 0; kCnt < k; kCnt++) {
							...
							case 1:
								...
								break;
							case 2:
								longs[kCnt] = (int) read2(fptr, SwephData.SEI_CURR_FPOS, freord, fendian);
								if (longs[kCnt] < 0) {
									longs[kCnt] &= 0xffff;
								}
								break;
							case 3:
								longs[kCnt] = read3(fptr, SwephData.SEI_CURR_FPOS, freord, fendian);
								if (longs[kCnt] < 0) {
									longs[kCnt] &= 0xffffff;
								}
								break;
							case 4:
								longs[kCnt] = read4(fptr, SwephData.SEI_CURR_FPOS, false, freord, fendian);
								if (longs[kCnt] < 0) {
									longs[kCnt] &= 0xffffffff;
								}
								break;
							}
						}
						...
						for (m = 0, j = 0; m < k && j < nsize[i]; m++) {
							for (n = 0, o = 16; n < 2 && j < nsize[i]; n++, j++, idbl++, longs[m] %= o, o /= 16) {
								if ((longs[m] & o) != 0) {
						...
						for (m = 0, j = 0; m < k && j < nsize[i]; m++) {
							for (n = 0, o = 64; n < 4 && j < nsize[i]; n++, j++, idbl++, longs[m] %= o, o /= 4) {
								if ((longs[m] & o) != 0) {
									pdp.segp[idbl] = -(((longs[m] + o) / o / 2) * pdp.rmax / 2 / 1e+9);
								} else {
									pdp.segp[idbl] = (longs[m] / o / 2) * pdp.rmax / 2 / 1e+9;
	...
	double[] getDatafileTimerange(SwissEph sw, String fname, String ephepath, boolean isAsteroidfile)
