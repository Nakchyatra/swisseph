...
class SwephMosh implements java.io.Serializable {
	...
	/*
	 * Moshier ephemeris. computes heliocentric cartesian equatorial coordinates
	 * of equinox 2000 for earth and a planet tjd julian day ipli internal SWEPH
	 * planet number xp array of 6 doubles for planet's position and speed xe
	 * earth's serr error string
	 */
	int swi_moshplan(double tjd, int ipli, boolean do_save, double[] xpret, double[] xeret, StringBuffer serr) {
		int i;
		...
		if (tjd < SwephData.MOSHPLEPH_START - 0.3 || tjd > SwephData.MOSHPLEPH_END + 0.3) {
			if (serr != null) {
				serr.setLength(0);
				s = "jd " + tjd + " outside Moshier planet range " + SwephData.MOSHPLEPH_START + " .. "
		...
		if (do_earth) {
			if (tjd == pedp.teval && pedp.iephe == SweConst.SEFLG_MOSEPH) {
				...
				/* emb */
				swi_moshplan2(tjd, pnoint2msh[SwephData.SEI_EMB],
						xe); /* emb hel. ecl. 2000 polar */
				sl.swi_polcart(xe, xe); /* to cartesian */
				sl.swi_coortrf2(xe, xe, -seps2000,
						ceps2000);/* and equator 2000 */
		...
		if (ipli == SwephData.SEI_EARTH) {
			...
			if (tjd == pdp.teval && pdp.iephe == SweConst.SEFLG_MOSEPH) {
				...
				/*
				 * one more position for speed. the following dt gives good
				 * speed for light-time correction
				 */
	 ...
	 *
	 * J = Julian day number xemb = rectangular equatorial coordinates of Earth
	 */
	private void embofs_mosh(double tjd, double xemb[]) {
		...
		L = L + 6.288750 * smp + 1.274018 * sx + 0.658309 * s2d + 0.213616 * s2mp
				- 0.185596 * SMath.sin(SwissData.DEGTORAD * M) - 0.114336 * s2f;
	 ...
	 * epoch equinox mean anomaly, semi axis, eccentricity, argument of
	 * perihelion, ascending node inclination
	 */
	/*
	 * use James Neely's revised elements of Uranian planets
	 */
	static final String plan_fict_nam[] = { "Cupido", "Hades", "Zeus", "Kronos", "Apollon", "Admetos", "Vulkanus",
			"Poseidon", "Isis-Transpluto", "Nibiru", "Harrington", "Leverrier", "Adams", "Lowell", "Pickering", };
	...
	String swi_get_fict_name(int ipl, String snam) {
		...
		if (read_elements_file(ipl, 0, null, null, null, null, null, null, null, null, sbnam, null,
				null) == SweConst.ERR) {
	...
	private static final double plan_oscu_elem[][] = new double[][] {
			{ SwephData.J1900, SwephData.J1900, 163.7409, 40.99837, 0.00460, 171.4333, 129.8325,
					1.0833 }, /* Cupido Neely */
			{ SwephData.J1900, SwephData.J1900, 27.6496, 50.66744, 0.00245, 148.1796, 161.3339,
					1.0500 }, /* Hades Neely */
			{ SwephData.J1900, SwephData.J1900, 165.1232, 59.21436, 0.00120, 299.0440, 0.0000,
					0.0000 }, /* Zeus Neely */
			{ SwephData.J1900, SwephData.J1900, 169.0193, 64.81960, 0.00305, 208.8801, 0.0000,
					0.0000 }, /* Kronos Neely */
			{ SwephData.J1900, SwephData.J1900, 138.0533, 70.29949, 0.00000, 0.0000, 0.0000,
					0.0000 }, /* Apollon Neely */
			{ SwephData.J1900, SwephData.J1900, 351.3350, 73.62765, 0.00000, 0.0000, 0.0000,
					0.0000 }, /* Admetos Neely */
			{ SwephData.J1900, SwephData.J1900, 55.8983, 77.25568, 0.00000, 0.0000, 0.0000,
					0.0000 }, /* Vulcanus Neely */
			{ SwephData.J1900, SwephData.J1900, 165.5163, 83.66907, 0.00000, 0.0000, 0.0000,
					0.0000 }, /* Poseidon Neely */
			/*
			 * Isis-Transpluto; elements from "Die Sterne" 3/1952, p. 70ff.
			 * Strubell does not give an equinox. 1945 is taken to best
			...
			/*
			 * Leverrier's Neptune, according to W.G. Hoyt,
	...
	/*
	 * computes a planet from osculating elements * tjd julian day ipl body
	 * number ipli body number in planetary data structure iflag flags
	 */
	int swi_osc_el_plan(double tjd, double xp[], int ipl, int ipli, double[] xearth, double[] xsun, StringBuffer serr) {
		...
		int i;
		/*
		 * orbital elements, either from file or, if file not found, from above
		 * built-in set
		 */
		if (read_elements_file(ipl, tjd, tjd0, tequ, mano, sema, ecce, parg, node, incl, null, fict_ifl,
				serr) == SweConst.ERR) {
			return SweConst.ERR;
		}
		...
		/* Kepler problem */
		E = M = sl.swi_mod2PI(
				mano.val + (tjd - tjd0.val) * dmot); /* mean anomaly of date */
		...
		if (ecce.val > 0.975) {
			...
			if (M2 < 30) {
				...
				E = Msgn * (M2 + ecce.val * (3 * sigma - 4 * sigma * sigma * sigma)) + M_180_or_0;
			}
		}
	...
	private int read_elements_file(int ipl, double tjd, DblObj tjd0, DblObj tequ, DblObj mano, DblObj sema, DblObj ecce,
			DblObj parg, DblObj node, DblObj incl, StringBuffer pname, IntObj fict_ifl, StringBuffer serr) {
		...
		try {
			...
			if (mano != null) {
				mano.val = plan_oscu_elem[ipl][2]
						* SwissData.DEGTORAD; /* mean anomaly */
			}
			if (sema != null) {
			...
			if (parg != null) {
				parg.val = plan_oscu_elem[ipl][5]
						* SwissData.DEGTORAD; /* arg. of peri. */
			}
			if (node != null) {
				node.val = plan_oscu_elem[ipl][6]
						* SwissData.DEGTORAD; /* asc. node */
			}
			if (incl != null) {
		...
		try {
			...
			while ((s = fp.readLine()) != null) {
				...
				serri = "error in file " + SweConst.SE_FICTFILE + ", line " + iline + ":";
				if (ncpos < 9) {
					if (serr != null) {
						serr.setLength(0);
				...
				if (tjd0 != null) {
					...
					// for (i = 0; i < 5; i++)
					// sp[i] = tolower(sp[i]);
					sp = sp.length() <= 5 ? sp.toLowerCase() : sp.substring(0, 5).toLowerCase() + sp.substring(5);
				...
				if (mano != null) {
					...
					/*
					 * if mean anomaly has t terms (which happens with
					 * fictitious planet Vulcan), we set epoch = tjd, so that no
					 * motion will be added anymore equinox = tjd
					 */
					if (retc == 1) {
						tjd0.val = tjd;
					}
				...
				if (ecce != null) {
					...
					if (ecce.val >= 1 || ecce.val < 0 || retc == SweConst.ERR) {
						if (serr != null) {
							serr.setLength(0);
							serr.append(serri)
									.append(" eccentricity invalid (no parabolic or hyperbolic or bits allowed)");
						}
						// goto return_err;
						fp.close();
						return SweConst.ERR;
	...
	private int check_t_terms(double t, String sinp, DblObj doutp) {
		...
		z = 0;
		while (true) {
			while (spidx < sinp.length() && (sinp.charAt(spidx) == ' ' || sinp.charAt(spidx) == '\t')) {
				spidx++;
			}
			if (spidx == sinp.length() || sinp.charAt(spidx) == '+' || sinp.charAt(spidx) == '-') {
				...
				while (spidx < sinp.length()
						&& (sinp.charAt(spidx) == '*' || sinp.charAt(spidx) == ' ' || sinp.charAt(spidx) == '\t')) {
					spidx++;
				}
	...
	private Plantbl planets[] = { SwemptabMer.mer404, SwemptabVen.ven404, SwemptabEar.ear404, SwemptabMar.mar404,
			SwemptabJup.jup404, SwemptabSat.sat404, SwemptabUra.ura404, SwemptabNep.nep404, SwemptabPlu.plu404, };
}
